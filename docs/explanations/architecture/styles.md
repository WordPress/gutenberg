...
- **Theme**. Historically, the themes enqueued their own stylesheets, whose id would be the theme name, as in `twentytwentytwo-style-css`. In addition to having its own styesheets they can now declare a theme.json file whose styles will be part of the global stylesheet.
