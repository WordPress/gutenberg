...
- **Cross-parent moves on the front end**: a pending-move block saves at its *proposed* position and `gutenberg_restore_pending_move_order` puts it back before render, but only within one sibling list. Client IDs do not survive to the server, so a move between two different nested parents cannot be identified as cross-parent and is restored inside its current parent; a move that crossed the root boundary is detectable from `fromParentClientId` and is left where it sits rather than misplaced.
