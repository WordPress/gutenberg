...
| `wordpress-7.1/block-suggestions.php`             | `gutenberg_strip_pending_structural_suggestions` — the type-aware `render_block` strip for pending structural suggestion state (`pending-insert` blocks dropped, `pending-remove`/`pending-move` blocks kept). |
...
- **Pending moves on the front end**: a pending-move block saves at its *proposed* position (the marker records the origin), and per-block `render_block` filters cannot reorder siblings — so on the published front end a pending move already renders in the proposed order until it is accepted or rejected. Restoring baseline order at render time would need a parse-level filter and is a possible follow-up.
