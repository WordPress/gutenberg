...
**Note:** Custom SVG icons are automatically wrapped in the [`wp.components.SVG` component](/packages/primitives/src/svg/) to add accessibility attributes (`aria-hidden`, `role`, and `focusable`).
