...
| fontVariations | Variable font axes that users can set, by font family slug. Only axes listed here are offered, within the range the font declares and, when given, the narrower range set here. `wght`, `wdth`, `slnt` and `ital` are set through font weight, width and style instead. |  |  |
