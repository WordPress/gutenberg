...
> The view object is the same structure the [DataViews component](/packages/dataviews/README.md#view-object) consumes; see its documentation for the complete list of view properties and filter operators. There are two properties from the DataViews API that cannot be configured via the server filter: `search` and `page`. These are URL-managed properties that are set by the editor when the user searches or paginates.
...
| -------- | ----------- | ---- |
| type | The layout type, one of `table`, `grid`, `list`, `activity`, `pickerGrid`, or `pickerTable`. | `string` |
| layout | Configuration specific to the selected layout type. Accepts the `layout` options of any layout type; see [`default_layouts`](#default_layouts). | `{ styles, density, enableMoving, badgeFields, previewSize }` |
| filters | Filters applied to the dataset. A filter with `isLocked` set cannot be changed or removed by the user. | `[ { field, operator, value, isLocked } ]` |
...
| groupBy | The grouping configuration: the field to group by, the direction (`asc` or `desc`), and whether to show the field label in each group header (`showLabel`, defaults to `true`). | `{ field, direction, showLabel }` |
...
| density | The density of the layout: `compact`, `balanced`, or `comfortable`. | `string` |
...
| density | The density of the grid layout: `compact`, `balanced`, or `comfortable`. | `string` |
...
| alignment | Vertical alignment of the fields in the row: `start`, `center`, or `end`. | `string` |
| styles | The styles for the fields in the row, keyed by field id. Each style accepts a `flex` value controlling how the field grows or shrinks. | `{ [ field ]: { flex } }` |
