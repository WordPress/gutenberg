...
For information on creating and managing packages in Gutenberg, see the [packages README](../contributors/code/managing-packages.md). For details on the build system and package configuration, see the [@wordpress/build README](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-wp-build).
