...
Note that `state.url` does not change on a navigation to the page you are already on, such as the `navigate( window.location.href, { force: true } )` pattern used to [show fresh content after a form submission](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#overriding-routers-internal-in-memory-cached-pages), so neither example above runs in that case. Code that must run on every navigation that publishes a lifecycle cycle should watch the router's `state.navigating` property instead. Every `navigate()` call that proceeds client-side—including a refresh of the current URL—and every cache-served back/forward traversal whose entry resolves truthy publishes that cycle. On the normal path, a back/forward traversal to a destination that is not in the cache, or whose cached entry resolves falsy, reloads the document without publishing a lifecycle cycle; from idle, a `state.navigating` watcher has nothing to observe before that reload. If a synchronous consumer throws while a cached-falsy traversal clears a retained identity, the traversal is aborted before its reload, so the document stays and the reading is left idle; it still publishes no lifecycle cycle of its own. See [Which navigations update the keys](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#which-navigations-update-the-keys) for this boundary, [Reacting to the navigation lifecycle](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#reacting-to-the-navigation-lifecycle) in the Client-Side Navigation guide for the lifecycle properties, and [Region-scoped focus after navigation](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#region-scoped-focus-after-navigation) for a callback that runs exactly once at the end of a navigation that published an end. The focus recipe is gated on the region's own initiator and deliberately refrains from traversals; it does not cover every navigation.

Also note that the two examples above do not see the same DOM. The `data-wp-watch` callback is deferred by a frame and, for a navigation that reaches its content commit, runs after the new content has been rendered, while the `watch()` callback runs synchronously as soon as `state.url` changes, before the regions have re-rendered, so it still sees the previous page. This makes no difference to page-view tracking, but any callback that reads or measures the new content should run at the end of the navigation instead. For a navigation that reaches its content commit, the end is published after the destination content is in the DOM, so a callback keyed on it can touch that content. An end reached without that commit — an exceptional termination before or during the commit, or a mid-flight fallback whose release returns the reading to idle — promises no destination content, so a callback that re-initializes on it may run against old, partially updated, or absent content. Look elements up inside the callback and check that they exist. See [Choosing where to read the keys](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#choosing-where-to-read-the-keys) and [When a navigation ends without new content](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#when-a-navigation-ends-without-new-content).
...
-   If the block has code that must run on every navigation that publishes a lifecycle cycle, including a refresh of the current URL, it watches `state.navigating` rather than `state.url`. On the normal path, a back/forward traversal to a destination that is not in the cache, or whose cached entry resolves falsy, reloads without a lifecycle cycle, so this watcher has nothing to observe before that reload. If a synchronous consumer throws while a cached-falsy traversal clears a retained identity, the traversal is aborted before its reload; the document stays and the reading is left idle, with no lifecycle cycle of its own. See [Which navigations update the keys](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#which-navigations-update-the-keys) for this boundary.
