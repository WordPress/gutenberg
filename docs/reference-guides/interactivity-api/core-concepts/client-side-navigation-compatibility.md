...
-   If the block has code that must run on every navigation that publishes a lifecycle cycle, including a refresh of the current URL, it watches `state.navigating` rather than `state.url`. A back/forward traversal to a destination that is not in the cache, or whose cached entry resolves falsy, reloads without a lifecycle cycle, so this watcher does not observe a lifecycle cycle for that traversal.
