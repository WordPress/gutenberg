...
Neither example is affected, because both read store state and nothing else. It starts to matter as soon as a callback measures an element, reads its text, or attaches something to it — written with `watch()` and keyed on `state.url`, that callback is inspecting the outgoing page. Re-initialization that touches the newly rendered content belongs on the lifecycle end transition rather than on `state.url`. For a navigation that reaches its content commit, the end is published after the new content is in the DOM, so a callback keyed on it can re-initialize that content. An end reached without a commit — after an exceptional termination or after a mid-flight fallback whose reading is released at the router's bound — means that no destination content was committed: `state.url` may still read the source URL, and the DOM may hold the source or a partially updated state. A callback that re-initializes on that end may therefore run without new content. The lifecycle end remains the signal for work that must cover every covered navigation; on a commitless path, it means only that no navigation is in flight, not that a separate failure state was published. [Choosing where to read the lifecycle](/docs/reference-guides/interactivity-api/core-concepts/client-side-navigation.md#choosing-where-to-read-the-lifecycle) compares the primitives on this point.
