...
-	**Name:** core/cover
-	**Category:** media
-	**Supports:** align, allowedBlocks, anchor, backgroundVideoFromURL, color (heading, text, ~~background~~, ~~enableContrastChecker~~), dimensions (aspectRatio), filter (duotone), interactivity (clientNavigation), layout (~~allowJustification~~), shadow, spacing (blockGap, margin, padding), typography (fontSize, lineHeight), ~~html~~
