...
When an external media item (one without a WordPress `id`) is inserted, it is uploaded to the media library, and the item's `title`, `alt`, `caption`, `description`, and `filename` are saved on the created attachment.
