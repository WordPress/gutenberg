...
-   `area` (optional): The template part area the pattern stands in for, such as `header` or `footer`. A Pattern block referencing it renders wrapped in that area's element (`<header>`, `<footer>`) unless the block sets its own `tagName`, shows the area's icon, and offers to be replaced by other patterns of the same area. In a theme's `patterns/` file, use the `Area: header` header.
