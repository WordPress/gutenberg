...
The main point is to let the machines work at what they are good at, and optimize for the user and the document. The analogy with the printing press can be taken further in that what matters is the printed page, not the arrangement of metal types that originated it. As a matter of fact, the arrangement of metal types is a pretty bad storage mechanism. The page is both the result _and_ the proper way to store the data. The metal types are just an instrument for publication and editing, but more ephemeral in nature. Exactly as our use of JSON in the editor. We have the ability to rebuild the JSON from the printed page, as if we printed invisible ink marks that allows a machine to know which types to assemble to recreate the page.
