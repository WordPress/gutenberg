...
CSS Modules (`*.module.css`, `*.module.scss`) hash local class names at build time, so they use short kebab-case names rather than the `package-directory` BEM prefix above.

TypeScript types for each module are generated by [`typed-css-modules`](https://github.com/Quramy/typed-css-modules) into a sibling `*.module.css.d.ts` or `*.module.scss.d.ts`. After adding, renaming, or removing a class, run `npm run css-modules:generate` and commit the updated declaration. `npm run css-modules:check` (CI and lint-staged) fails when a declaration is missing, stale, or orphaned.

Referencing a class that does not exist is a TypeScript error (`styles.doesNotExist`). Keep class names as written in CSS: `styles.button` or `styles['box-sizing']`. The build exports the original local names, not camelCase aliases.

Editors using the workspace JavaScript TypeScript language service also get autocomplete from [`typescript-plugin-css-modules`](https://github.com/mrmckeb/typescript-plugin-css-modules). Native TypeScript (`tsgo`) does not load that plugin; it type-checks the committed `.d.ts` files instead. See [`tools/css-modules/README.md`](/tools/css-modules/README.md).
