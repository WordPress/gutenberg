...
Behind the scenes, all steps are automated via `./tools/release/cli.js npm-latest` command. For the record, the manual process would look very close to the following steps:
...
Behind the scenes, the rest of the process is automated with `./tools/release/cli.js npm-bugfix` command. For the record, the manual process would look very close to the following steps:
...
Behind the scenes, the release process is fully automated via `./tools/release/cli.js npm-next` command. It ensures
