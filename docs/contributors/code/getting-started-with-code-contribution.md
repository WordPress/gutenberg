...
There are two ways to build your code. While developing, you probably will want to use `npm run dev` to run continuous builds automatically as source files change. The dev build also includes additional warnings and errors to help troubleshoot while developing. `npm run dev` also enables [Fast Refresh](/docs/contributors/code/fast-refresh.md), so editing a component updates the editor in place without a full page reload. Once you are happy with your changes, you can run `npm run build` to create optimized production build.
