...
CSS Modules are typed through sibling `*.module.css.d.ts` / `*.module.scss.d.ts` files generated by `npm run css-modules:generate`. `npm run typecheck` treats `styles.doesNotExist` as an error. `typescript-plugin-css-modules` is registered in `tsconfig.base.json` for live autocomplete, but it only loads when the editor uses the workspace **JavaScript** TypeScript language service — Native TypeScript Preview (`tsgo`, enabled in the settings example above) ignores language-service plugins and relies on those committed `.d.ts` files. See [`tools/css-modules/README.md`](/tools/css-modules/README.md).
