...
### Creating Gutenberg Plugin Point releases

Occasionally it's necessary to create a point release (ie: the `Z` in `X.Y.Z`) of the Gutenberg Plugin.

The method for doing this is nearly identical to the main Plugin release process (see above). In the `Run workflow` control simply specify `stable` and, if the previous version was stable (`x.y.z`), the workflow will release a point release, with z incremented (`x.y.(z+1)`).

Please note however, that if the previous release was an _RC_ (e.g. `x.y.0-rc.1`), the workflow will try to release the next _stable_ version (x.y.0). Therefore, should you wish to release a point release for a stable version of the Plugin (e.g. `10.1.0`)but there is an _RC already published_ for the _next_ version of the Plugin (e.g. `10.2.0-rc.1`) you will need to manually select the stable version's branch when creating the release.
