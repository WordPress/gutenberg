...
It is possible to create a minor release for any release branches even after a more recent stable release has been published. This can be done for _any_ previous release branches, allowing more flexibility in delivering updates to users.

The process is identical to the one documented above when an RC is already out: just choose a previous release branch, type `stable`, and click "Run workflow".

The release will be published in the GitHub releases page for Gutenberg and to the WordPress core repository SVN as a `tag` under http://plugins.svn.wordpress.org/gutenberg/tags/. The `trunk` directory will not be touched.
