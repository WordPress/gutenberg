...
The workflow is quite resilient, but it is possible that a stage may sometimes fail leading to a piece of the release process not completing correctly.

In this scenario it is usually easiest to simply revert and start over. To do this:

1. Revert any version bump commits on `trunk` and then push the revert commit directly to `trunk`. This will require `admin` level privillages on the repo, so you may need to ask a member of the Core team for assistance.
2. Delete the `release` _branch_ on the remote - `git push origin --delete release/X.Y.Z`
3. At this point you should be back with a clean slate. You can now trigger the release again.
