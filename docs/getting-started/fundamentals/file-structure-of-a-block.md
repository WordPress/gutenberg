...
The [build process](/docs/block-editor/getting-started/fundamentals/javascript-in-the-block-editor.md#javascript-with-a-build-process) provided by `wp-scripts` will then take the files from this folder and generate the production-ready files in the project's `build` folder.
