...
async function getActivationTemplates( activeView: string | undefined ) {
	const [ site, activeTheme, staticRecords, userRecords ] = await Promise.all(
		[
	...
	const defaultTemplateTypes = activeTheme?.default_template_types ?? [];
	...
	if ( activeTemplatesOption ) {
		for ( const activeSlug in activeTemplatesOption ) {
			...
			const template = (
				userRecords as TemplateRecord[] | undefined
			 )?.find(
				( userRecord ) =>
	...
	return activeTemplates.filter(
		( template ) => ! isCustomTemplate( template, defaultTemplateTypes )
	);
}
