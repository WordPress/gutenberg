...
type NavigationLinkUIProps = {
	insertedBlockClientId?: string | null;
	setInsertedBlockClientId: ( clientId: string | null ) => void;
	showBlockInserter?: boolean;
	onComplete?: (
		clientId: string | null | undefined,
		attributes: Record< string, unknown >
	) => void;
	onCancel?: ( clientId: string | null | undefined ) => void;
};
...
type NavigationTreeAdditionalContentProps = NavigationLinkUIProps & {
	NavigationLinkUI?: ComponentType< NavigationLinkUIProps >;
	onInsertedSubmenuComplete: ( clientId: string | null | undefined ) => void;
};
type PopoverAnchor =
	| Element
	| {
		...
		<>
			{ NavigationLinkUI && (
				<NavigationLinkUI
					...
					onComplete={ ( completedClientId ) =>
						onInsertedSubmenuComplete( completedClientId )
					}
				/>
			) }
		</>
...
export default function NavigationMenuContent( {
	...
	const selectInsertedSubmenu = useCallback(
		( clientId: string | null | undefined ) => {
			if (
				! clientId ||
				registry.select( blockEditorStore ).getBlockName( clientId ) !==
					'core/navigation-submenu'
			) {
				return;
			}
	...
	const addExistingPage = useCallback(
		(
			...
			setEditingBlockClientId( null );
			setInsertedBlockClientId( ( block as Block ).clientId );
		},
