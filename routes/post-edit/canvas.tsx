...
function PostEditCanvas() {
	const { type: postType, id: postId } = useParams( {
		from: '/types/$type/edit/$id',
	} );
	...
	const collectionTitle =
		...
		( postTypeObject?.labels?.name
			...
			: __( 'All content' ) );
	const frameTitle =
		postTypeObject?.labels?.edit_item ||
		postTypeObject?.labels?.singular_name ||
		__( 'Edit item' );

	return (
		<div className="routes-post-edit-admin-bridge">
			<div className="routes-post-edit-admin-bridge__toolbar">
				<Button
					icon={ chevronLeft }
					label={ __( 'Back' ) }
				...
				/>
				<h1 className="routes-post-edit-admin-bridge__title">
					{ collectionTitle }
				</h1>
			</div>
			<iframe
				className="routes-post-edit-admin-bridge__frame"
				onLoad={ ( event ) =>
					preserveAdminBridgeContext( event.currentTarget )
				}
				src={ getAdminPostEditUrl( postId ) }
				title={ frameTitle }
			/>
		</div>
	);
...
export const canvas = PostEditCanvas;
