...
import { chevronDown, closeSmall, moreVertical } from '@wordpress/icons';
...
export default function ExplorationC() {
	...
	// Auto-scan: trigger generation once data is loaded and guidelines are not empty.
	useEffect( () => {
		if ( pageLoading || pageError || scanTriggered ) {
			return;
		}
		if ( isEmpty ) {
			// Empty guidelines — don't auto-scan, show banner instead.
			return;
		}
	...
	}, [ pageLoading, pageError, isEmpty, scanTriggered ] );
	...
	const showBanner = ! bannerDismissed && isEmpty && ! pageLoading;
