 ...
 * live in `suggestion-mode.spec.ts`; this spec picks up where those leave
...
test.describe( 'Suggestion mode review flows', () => {
	...
	test( 'an inline decision leaves a co-resident attribute suggestion pending', async ( {
		...
		const attributeNoteStatuses = await page.evaluate( async () => {
			...
			return comments
				...
				.map( ( comment: any ) => comment.status );
		} );
	 ...
	 */
	async function transformParagraphToQuote( page: any, editor: any ) {
		await editor.canvas
			.getByRole( 'document', { name: 'Block: Paragraph' } )
			.first()
			.click();
	 ...
	 */
	async function suggestionStatuses( page: any ) {
		return page.evaluate( async () => {
			...
			return notes
				.filter( ( note: any ) => !! note.meta?._wp_suggestion )
				.map(
					( note: any ) =>
