...
test.describe( 'Block Notes', () => {
	...
	test.describe( 'Attachment persistence', () => {
		...
		test( 'refuses a dirty target after a same-cardinality reorder', async ( {
			editor,
			page,
			blockNoteUtils,
		} ) => {
			...
			await page.evaluate( () => {
				...
				dispatch.moveBlockToPosition( blocks[ 0 ].clientId, '', '', 1 );
				dispatch.updateBlockAttributes( blocks[ 0 ].clientId, {
					content: 'Dirty target',
				} );
				dispatch.selectBlock( blocks[ 0 ].clientId );
			} );

			await blockNoteUtils.addNote( 'Do not attach after reorder' );
			await page.reload();

			const blocks = await editor.getBlocks();
			expect(
				blocks.map( ( block ) => block.attributes.content )
			).toEqual( [ 'Saved target', 'Saved sibling' ] );
			expect(
				blocks.some( ( block ) => block.attributes.metadata?.noteId )
			).toBe( false );
		} );
		...
		test( 'does not fall back to a full post save when a clean post cannot be repaired', async ( {
			...
			await expect(
				page.getByText( 'Note added.', { exact: true } )
			).toHaveCount( 0 );
		} );
