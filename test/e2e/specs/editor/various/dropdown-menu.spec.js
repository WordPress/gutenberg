...
test.describe( 'Dropdown Menu', () => {
	...
	test( 'allows navigation through each item using arrow keys', async ( {
		page,
		pageUtils,
	} ) => {
		await page
			...
			.click();
		const menuItems = page.locator( '.components-menu-item__button' );
	...
	test( 'loops to the beginning and end when navigating past the boundaries of the menu', async ( {
		page,
	} ) => {
		await page
			.getByRole( 'region', { name: 'Editor top bar' } )
	...
	test( 'ignores arrow key navigation that is orthogonal to the orientation of the menu, but stays open', async ( {
		page,
		pageUtils,
	} ) => {
		await page
