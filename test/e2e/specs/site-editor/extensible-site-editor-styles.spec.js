...
async function visitStylesVariationsViaUi( admin, page ) {
	...
	await clickSidebarPath( page, [ 'Design', 'Styles' ] );
...
async function ensureRootSidebar( page ) {
	const sidebar = page.locator( '.boot-layout__sidebar' );
	await expect( sidebar ).toBeVisible( { timeout: 15000 } );

	const backButton = sidebar.getByRole( 'button', { name: 'Back' } );
	if ( await backButton.isVisible().catch( () => false ) ) {
		await backButton.click();
		await expect(
			sidebar.getByText( 'Homepage', { exact: true } )
		).toBeVisible();
	}
}

async function clickSidebarPath( page, labels ) {
	const labelPath = Array.isArray( labels ) ? labels : [ labels ];
	const sidebar = page.locator( '.boot-layout__sidebar' );

	await ensureRootSidebar( page );
...
async function navigateSidebar(
	...
	await clickSidebarPath( page, labelPath );
