			...
			if ( isDeprecated && console.info.mock.calls.length > 0 ) {
				...
				expect( console ).toHaveInformedWith(
					...
					expect.objectContaining( {
						name: expect.stringMatching( /^core\// ),
					} ),
					expect.any( String ),
					expect.any( String )
				);
			}
			// Some old fixtures use the deprecated children attribute source. Its
			// deprecation warning is emitted only once per file.
			// eslint-disable-next-line no-console
			if ( isDeprecated && console.warn.mock.calls.length > 0 ) {
