...
describe('Blocks raw handling', () => {
	...
	describe('pasteHandler', () => {
		[
			...
			it(type, () => {
				const HTML = readFile(
					path.join(__dirname, `fixtures/documents/${type}-in.html`)
				);
				const plainText = readFile(
					path.join(__dirname, `fixtures/documents/${type}-in.txt`)
				);
				const output = readFile(
					path.join(__dirname, `fixtures/documents/${type}-out.html`)
				);
...
describe('rawHandler', () => {
	...
	it('should convert a list with attributes', () => {
		const HTML = readFile(
			path.join(__dirname, 'fixtures/documents/list-with-attributes.html')
		);
		expect(serialize(rawHandler({ HTML }))).toMatchSnapshot();
	});
