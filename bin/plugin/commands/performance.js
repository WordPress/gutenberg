...
async function setUpGitBranch(branch, environmentDirectory) {
	...
	await runShellScript('npm install && npm run build', environmentDirectory);
}
			...
			const medians = mapValues(
				{
					...
					firstPaint: rawResults.map((r) => r[branch].firstPaint),
					...
					firstBlock: rawResults.map((r) => r[branch].firstBlock),
					...
					listViewOpen: rawResults.map((r) => r[branch].listViewOpen),
	...
	for (const testSuite of testSuites) {
		...
		Object.entries(results[testSuite]).reduce((acc, [key, val]) => {
			for (const entry of Object.keys(val)) {
				// @ts-ignore
				if (!acc[entry] && isFinite(val[entry])) acc[entry] = {};
				// @ts-ignore
