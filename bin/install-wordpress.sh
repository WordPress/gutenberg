...
docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm -u 33 $CLI core install --title="$SITE_TITLE" --admin_user=admin --admin_password=password --admin_email=test@test.com --skip-email --url=$PROTOCOL://localhost:$HOST_PORT >/dev/null
...
if [ "$CURRENT_URL" != "$PROTOCOL://localhost:$HOST_PORT" ]; then
	docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm $CLI option update home "$PROTOCOL://localhost:$HOST_PORT" >/dev/null
	docker-compose $DOCKER_COMPOSE_FILE_OPTIONS run --rm $CLI option update siteurl "$PROTOCOL://localhost:$HOST_PORT" >/dev/null
fi
