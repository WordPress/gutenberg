<?php
/**
 * Gutenberg-specific HTTP polling sync server.
 *
 * WordPress core may define {@see WP_HTTP_Polling_Sync_Server} first; this copy allows the
 * plugin to ship behavioral fixes ahead of core and to replace the REST route via
 * {@see register_rest_route()} override (see {@see self::register_routes()}).
 *
 * @package gutenberg
 */

/**
 * HTTP server used for collaborative editing (Gutenberg plugin implementation).
 *
 * @since 7.0.0
 * @access private
 */
	...
	private function process_sync_update( string $room, int $client_id, int $cursor, array $update ) {
		...
		switch ( $type ) {
			case self::UPDATE_TYPE_COMPACTION:
				...
				// A newer compaction already exists after this cursor; discard silently.
				return true;
