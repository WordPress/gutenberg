...
/**
 * Handles sync presence detection via the Heartbeat API.
 *
 * When a client sends 'wp-sync-presence' in a heartbeat request, this filter
 * tracks which clients are editing which rooms and returns the IDs of other
 * clients in the same rooms. This is provider-independent — it works regardless
 * of which sync transport is used (HTTP polling, WebSocket, etc.).
 *
 * Each client's presence is stored in a transient with a 120-second TTL.
 * Entries older than 60 seconds are considered stale and excluded from
 * the response.
 *
 ...
 */
function gutenberg_sync_heartbeat_presence( $response, $data ) {
	if ( ! isset( $data['wp-sync-presence'] ) || ! get_option( 'wp_collaboration_enabled' ) ) {
		return $response;
	}

	$presence_data = $data['wp-sync-presence'];
	if ( ! isset( $presence_data['rooms'] ) || ! is_array( $presence_data['rooms'] ) ) {
		return $response;
	}
	...
	foreach ( $presence_data['rooms'] as $room_data ) {
		$room      = isset( $room_data['room'] ) ? sanitize_text_field( $room_data['room'] ) : '';
		$client_id = isset( $room_data['client_id'] ) ? absint( $room_data['client_id'] ) : 0;

		if ( empty( $room ) || empty( $client_id ) ) {
			continue;
		}

		// Check the user has permission to edit this entity.
		// Room format is "objectType:objectId", e.g. "postType/post:123".
		$parts     = explode( ':', $room, 2 );
		$object_id = isset( $parts[1] ) ? absint( $parts[1] ) : 0;
		...
		$presence = array_filter(
			...
			function ( $entry ) use ( $now ) {
				return $entry['timestamp'] > $now - 60;
			}
		);

		set_transient( $transient_key, $presence, 120 );

		// Collect IDs of other clients in this room.
		$other_clients = array();
		foreach ( $presence as $id => $entry ) {
			if ( absint( $id ) !== $client_id ) {
				$other_clients[] = absint( $id );
			}
		}

		$room_responses[] = array(
			'room'           => $room,
			'other_clients'  => array_values( $other_clients ),
		);
	}

	$response['wp-sync-presence'] = array(
		'rooms' => $room_responses,
	);

	return $response;
}
add_filter( 'heartbeat_received', 'gutenberg_sync_heartbeat_presence', 10, 2 );
