...
( () => {
	...
	window.addEventListener( 'DOMContentLoaded', () => {
		...
		if ( data ) {
			// Tells the editor which boxes exist; the block editor page no
			// longer renders them so it cannot report them itself.
			const metaBoxesPerLocation = {};
			for ( const location of isSide
				? [ 'side' ]
				: [ 'normal', 'advanced' ] ) {
				metaBoxesPerLocation[ location ] = Array.from(
					document.querySelectorAll(
						`#${ location }-sortables .postbox`
					),
					( box ) => ( {
						id: box.id,
						title:
							box
								.querySelector( '.postbox-header .hndle' )
								?.textContent.trim() || box.id,
					} )
				);
			}
		...
		// Nothing inside an iframe can render outside of it, so while a
		// meta box shows a modal, the frame grows to cover the whole
		// editor. A modal is recognized by its backdrop: a fixed element
		// covering the viewport.
		if ( window.frameElement ) {
			let scanScheduled = false;
			const scanForModal = () => {
				scanScheduled = false;
				// The stack of elements rendered at the center of the
				// viewport, at any depth, including covered ones.
				const hasModal = document
					.elementsFromPoint(
						window.innerWidth / 2,
				...
				window.frameElement.classList.toggle(
					'has-open-modal',
					hasModal
				);
			};
			new window.MutationObserver( () => {
				if ( ! scanScheduled ) {
					scanScheduled = true;
					window.requestAnimationFrame( scanForModal );
				}
