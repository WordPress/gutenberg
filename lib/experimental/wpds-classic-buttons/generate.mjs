...
/**
 * Remove a state pseudo-class from a subject compound, if present at the top
 * level.
 *
 * Only a top-level occurrence counts: in `:hover:not(:disabled)` the `:hover`
 * is the subject's own state, while the `:disabled` inside `:not(...)` is a
 * guard that must survive untouched. Paren depth tells them apart.
 *
 * @param {string} mods   The subject's pseudos and attributes, classes stripped.
 * @param {string} pseudo The pseudo-class to remove, e.g. `:hover`.
 * @return {string|null} `mods` without the pseudo, or null if it is not present
 *                       at the top level.
 */
function stripTopLevelPseudo( mods, pseudo ) {
	let depth = 0;
	for ( let i = 0; i < mods.length; i++ ) {
		...
		if ( c === '(' ) {
			...
			depth--;
		} else if ( depth === 0 && mods.startsWith( pseudo, i ) ) {
			// Guard against matching a prefix of a longer pseudo
			// (`:focus` inside `:focus-visible`) or a functional form.
			const next = mods[ i + pseudo.length ];
			if ( next === undefined || ! /[\w-(]/.test( next ) ) {
				return mods.slice( 0, i ) + mods.slice( i + pseudo.length );
			}
		}
	}
	return null;
 ...
 * @return {string[]} The rewritten selector(s) — usually one, plus a class-form
 *                    variant for each state pseudo. Empty to drop the selector.
 */
function mapSelector( sel ) {
	...
	// Button groups map wholesale to the legacy group + button classes. React's
	// variant classes are compounded onto the button (`.components-button.is-primary`),
	// so collapse them to the legacy variant class rather than leaving an
	// `is-*` class that classic markup can never carry.
	if ( sel.startsWith( '.components-button-group' ) ) {
		const grouped = sel
			.replace( /\.components-button-group/g, `${ SCOPE } .button-group` )
			.replace( /\.components-button\.is-primary/g, '.button-primary' )
			.replace(
				/\.components-button\.is-secondary/g,
				'.button-secondary'
			)
			.replace( /\.components-button/g, '.button' );
		// Any surviving `is-*` has no legacy equivalent; drop the selector
		// rather than emitting a rule that can never match.
		return /\.is-[\w-]+/.test( grouped ) ? [] : [ grouped ];
	}
	...
	const selectors = [ `${ SCOPE } ${ legacy }${ nonClass }${ tail }` ];

	// Core drives button state from CLASSES as well as pseudo-classes, because
	// legacy admin JS toggles `.hover` / `.focus` / `.disabled` directly. React
	// has no such concept, so emit the class form alongside the pseudo form from
	// the same declarations — that is what makes the class-driven states render
	// identically rather than merely exist. See stateClassMap in the key.
	for ( const [ pseudo, className ] of Object.entries(
		key.stateClassMap ?? {}
	) ) {
		const stripped = stripTopLevelPseudo( nonClass, pseudo );
		if ( stripped === null ) {
			continue;
		}
		selectors.push(
			`${ SCOPE } ${ legacy }${ className }${ stripped }${ tail }`
		);

		// Some state classes exist only for certain variants. Core writes them
		// as bare selectors (`.wp-core-ui .button-disabled`) but always applies
		// them to an element that ALSO carries `.button`, so append rather than
		// replace: the bare form would lose to our re-scoped base rule, whose
		// `:is()`/`:not()` construction is more specific than Core's original.
		// `.button-disabled` / `.button-primary-disabled` are classic classes for
		// regular buttons; there is no link equivalent, so emitting one for the
		// link variant would leak link styling onto every disabled button.
		const aliases = has( 'is-link' )
			? null
			: key.stateClassVariantMap?.[ pseudo ];
		if ( aliases ) {
			const alias = has( 'is-primary' )
				? aliases[ 'is-primary' ]
				: aliases[ '(other)' ];
			if ( alias ) {
				selectors.push(
					`${ SCOPE } ${ legacy }${ alias }${ stripped }${ tail }`
				);
			}
		}
	}
...
function processRule( rule ) {
	...
	const mapped = rule.selectors.flatMap( mapSelector );
	if ( ! mapped.length ) {
		return null;
	}
