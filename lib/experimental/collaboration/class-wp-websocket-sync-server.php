...
if ( ! class_exists( 'WP_WebSocket_Sync_Server' ) ) {
	...
	class WP_WebSocket_Sync_Server {
		 ...
		 * @since 7.4.0
		 */
		private WP_Sync_Server_Core $core;
		 ...
		 * @since 7.4.0
		 *
		 * @param WP_Sync_Server_Core $core Sync server core.
		 * @param string              $host Host to bind.
		 * @param int                 $port Port to bind.
		 */
		public function __construct( WP_Sync_Server_Core $core, string $host = '127.0.0.1', int $port = 8787 ) {
			$this->core = $core;
			$this->host = $host;
			$this->port = $port;
		}
		...
		private function handle_message( int $key, string $payload ): void {
			...
			foreach ( $rooms as $room_request ) {
				...
				if ( ! isset( $this->clients[ $key ]['rooms'][ $room ] ) ) {
					...
					$permission = $this->core->check_rooms_permissions( array( $validated ) );

					if ( is_wp_error( $permission ) ) {
						$error_data = $permission->get_error_data();
						if ( ! is_array( $error_data ) || empty( $error_data['rooms'] ) ) {
							$error_data          = is_array( $error_data ) ? $error_data : array();
							$error_data['rooms'] = array( $room );
						}
						$this->send_error( $key, new WP_Error( $permission->get_error_code(), $permission->get_error_message(), $error_data ) );
						continue;
					}
				...
				$room_response = $this->core->process_room_request( $validated );
		...
		private function validate_room_request( $room_request ) {
			...
			$valid_types = array(
				WP_Sync_Server_Core::UPDATE_TYPE_COMPACTION,
				WP_Sync_Server_Core::UPDATE_TYPE_SYNC_STEP1,
				WP_Sync_Server_Core::UPDATE_TYPE_SYNC_STEP2,
				WP_Sync_Server_Core::UPDATE_TYPE_UPDATE,
			);
		...
		private function broadcast_room( string $room, ?int $exclude_key = null ): void {
			$awareness_map = $this->core->get_current_awareness_map( $room );
			...
			foreach ( $this->clients as $other_key => $other ) {
				...
				$room_response              = $this->core->get_updates( $room, $client_id, $cursor, false );
		...
		private function sweep_awareness(): void {
			...
			foreach ( $connected_clients_by_room as $room => $connected_client_ids ) {
				$entries      = $this->core->get_storage()->get_awareness_state( $room );
				$current_time = time();
				$kept         = array();
				$changed      = false;
				$removed_any  = false;
				...
				foreach ( $entries as $entry ) {
					...
					$is_expired   = $current_time - $entry['updated_at'] >= WP_Sync_Server_Core::AWARENESS_TIMEOUT;
				...
				if ( $changed ) {
					$this->core->get_storage()->set_awareness_state( $room, $kept );
				}
		...
		private function disconnect( int $key ): void {
			...
			foreach ( $rooms as $room ) {
				...
				$this->core->process_awareness_update( $room, $client['rooms'][ $room ]['client_id'], null );
				$this->broadcast_room( $room );
			}
		}
