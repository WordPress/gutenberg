...
class WP_HTML_Character_Reference_Transcoder {
		...
		while ( $at < $end && $budget-- > 0 ) {
			...
			if ( '#' === $input[ $at ] ) {
				...
				if ( $code_point >= 0x80 && $code_point <= 0x9F ) {
					$offset = 4 * ( $code_point - 0x80 );

					if ( '0' !== self::$c1_replacements[ $offset ] ) {
						$at += $digit_count;
						if ( $at < $end && ';' === $input[ $at ] ) {
							$at ++;
						}
						$length  = intval( self::$c1_replacements[ $offset ], 10 );
						$buffer .= substr( self::$c1_replacements, $offset + 1, $length );
					} else {
						$at -= 3;
					}
	 ...
	 * @var string[]
	 */
	static $c1_replacements = '3€0___3‚2ƒ_3„3…3†3‡2ˆ_3‰2Š_3‹2Œ_0___2Ž_0___0___3‘3’3“3”3•3–3—2˜_3™2š_3›2œ_0___2ž_2Ÿ_';
