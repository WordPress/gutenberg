...
class Gutenberg_REST_Attachments_Controller extends WP_REST_Attachments_Controller {
	...
	private function validate_image_dimensions( int $width, int $height, $image_size, int $attachment_id ) {
		 ...
		 * fire).
		 */
		if ( self::IMAGE_SIZE_ANIMATED_VIDEO === $image_size || 'optimized-video' === $image_size ) {
			return true;
		}
	...
	public function sideload_item( WP_REST_Request $request ) {
		...
		// falsely report the upload as "corrupted or unsupported". Source-format
		// originals ('source_original', e.g. the HEIC kept next to its JPEG
		// derivative) are exempt for the same reason: their dimensions are
		// neither validated nor recorded, and wp_getimagesize() may not be able
		// to read the source format at all on servers without HEIC/HEIF support.
		// Skip the read for these cases; validate_image_dimensions() also
		// short-circuits them below.
		$skip_dimension_read =
			self::IMAGE_SIZE_ANIMATED_VIDEO === $image_size ||
			'optimized-video' === $image_size ||
