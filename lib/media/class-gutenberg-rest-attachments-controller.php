...
class Gutenberg_REST_Attachments_Controller extends WP_REST_Attachments_Controller {
	...
	public function finalize_item( WP_REST_Request $request ) {
		...
		foreach ( $sub_sizes as $sub_size ) {
			...
			} elseif ( self::IMAGE_SIZE_SOURCE_ORIGINAL === $image_size || 'original-jxl' === $image_size ) {
				// Source-format original (HEIC or JXL): stored under its own meta
				// key so the scaled-sideload flow (which writes 'original_image')
	...
	private function validate_image_dimensions( int $width, int $height, $image_size, int $attachment_id ) {
		...
		// Source-format original (HEIC or JXL) companion and the
		// 'animated_video_poster' companion (a static poster image for the
		// converted video): real files that are not registered sub-sizes, so
		// they carry no dimension constraint.
		if (
			self::IMAGE_SIZE_SOURCE_ORIGINAL === $image_size ||
			'original-jxl' === $image_size ||
			self::IMAGE_SIZE_ANIMATED_VIDEO_POSTER === $image_size
		) {
			return true;
		}
	...
	public function sideload_item( WP_REST_Request $request ) {
		...
		// and the JXL companion original ('original-jxl') are exempt for the
		// same reason: their dimensions are neither validated nor recorded, and
		// wp_getimagesize() may not be able to read the source format at all on
		// servers without HEIC/HEIF/JXL support. Skip the read for these cases;
		// validate_image_dimensions() also short-circuits them below.
		$skip_dimension_read =
		...
		if ( is_array( $image_size ) ) {
			...
			// Source-format original (HEIC or JXL). finalize_item() writes the
			// filename to $metadata[ self::META_KEY_SOURCE_IMAGE ] (separate from
