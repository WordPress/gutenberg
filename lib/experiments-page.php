 ...
 *
 * This file integrates with the new @wordpress/boot routing system.
 * The page UI is rendered via routes/experiments/ using the boot framework.
 *
 * @package gutenberg
 */

/**
 * Include the generated boot page PHP if it exists.
 * This is generated by the build process from wpPlugin.pages configuration.
 */
$gutenberg_experiments_page_php = gutenberg_dir_path() . 'build/pages/gutenberg-experiments/page.php';
if ( file_exists( $gutenberg_experiments_page_php ) ) {
	require_once $gutenberg_experiments_page_php;
}
...
if ( ! function_exists( 'the_gutenberg_experiments' ) ) {
	 ...
	 *
	 * Uses the new boot-based routing system if available,
	 * otherwise falls back to the legacy React UI.
	 *
	 * @since 6.3.0
	 */
	function the_gutenberg_experiments() {
		// Check if the boot-based page render function exists (generated by build).
		if ( function_exists( 'gutenberg_experiments_render_page' ) ) {
			gutenberg_experiments_render_page();
			return;
		}

		// Fallback to legacy implementation if boot page isn't built yet.
