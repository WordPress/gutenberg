	...
	foreach ( glob( gutenberg_dir_path() . 'build/*/index.js' ) as $path ) {
		...
		switch ( $handle ) {
			case 'wp-block-library':
				array_push( $dependencies, 'editor', 'wp-block-library-reset' );
				break;
...
function is_css_reset_enabled() {
	...
	$raw = $base->get_raw_data();
	return $raw["global"]["settings"]["reset"];
}
...
function gutenberg_register_packages_styles( $styles ) {
	...
	gutenberg_override_style(
		...
		array( 'wp-components', 'wp-block-editor', 'wp-editor', 'wp-edit-blocks', 'wp-block-library', 'wp-block-library-reset', 'wp-nux' ),
	...
	if ( is_css_reset_enabled() ) {
		gutenberg_override_style(
			$styles,
			'wp-block-library-reset',
			gutenberg_url( 'build/block-library/reset.css' ),
			array(),
			filemtime( gutenberg_dir_path() . 'build/block-library/reset.css' )
		);
		$styles->add_data( 'wp-block-library-reset', 'rtl', 'replace' );
	}
