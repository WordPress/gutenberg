...
function gutenberg_apply_behaviors_support($block_type, $block_attributes)
{
    $attributes = array();
    ...
    if (wp_should_skip_block_supports_serialization($block_type, 'behaviors')) {
        return $attributes;
    }
    ...
    $has_lightbox_support = block_has_support($block_type, array('behaviors', 'lightbox'), false);
    if ($has_lightbox_support) {
        $attributes['behaviors']['lightbox'] = true;
    }
...
function gutenberg_blocks_supports_add_lightbox($block_content, $block)
{
    ...
    if (!$lightbox || $link_destination !== 'none' || empty($experiments['gutenberg-interactivity-api-core-blocks'])) {
...
WP_Block_Supports::get_instance()->register(
    ...
    array(
        ...
        'apply'              => 'gutenberg_apply_behaviors_support',
    )
);
