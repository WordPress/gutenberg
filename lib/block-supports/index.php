 ...
 */
function foo_inject_class( $input, $value ) {
	$close_token    = '>';
	$class_token    = 'class="';
 ...
 */
function foo_inject_style( $input, $value ) {
	$close_token    = '>';
	$style_token    = 'style="';
	$close_position = strpos( $input, $close_token );
 ...
 * @param  array  $block block object.
 */
function foo_inject_stuff( $block_content, $block ) {
	$classes = gutenberg_block_supports_classes();
	if ( ! empty( $classes ) ) {
		return foo_inject_class( $block_content, $classes );
	}
	return $block_content;
}
add_filter( 'render_block', 'foo_inject_stuff', 10, 2 );
