/*
 * PEG.js grammar to strip code fragments and parse PEG grammars as a readable
 * set of rules. A rule consists of an object with properties `displayName`,
 * `name` and `expression`.
 *
 * Based on [pegjs-strip][1], itself based on the official PEG.js [grammar][2].
 * This grammar should be identical to pegjs-strip's after rules `Grammar`,
 * `Initializer`, `Rule`.
 *
 * [1]: https://github.com/okaxaki/pegjs-strip
 * [2]: https://github.com/pegjs/pegjs/blob/master/src/parser.pegjs
 */
{
  var __options = {};
...
Rule
  = name:IdentifierName __
    ...
    return {
      displayName: stripQuotes( toString( displayName ) ),
      name: toString( name ),
