...
(async function run() {
	...
	const flakyTests = await downloadReportFromArtifact(octokit, artifactName);
...
async function downloadReportFromArtifact(octokit, artifactName) {
	...
	const { files } = await unzipper.Open.buffer(Buffer.from(download.data));
	const fileBuffers = await Promise.all(files.map((file) => file.buffer()));
