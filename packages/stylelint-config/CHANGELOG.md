...
-   Enable `custom-property-pattern` to disallow `--_gcd-*` and `--_wp-*` custom properties, which are private to `@wordpress/ui`. Stylelint does not merge this rule, so projects that already set `custom-property-pattern` will not gain the new ban and need to include `(?!_(?:gcd|wp)-)` in their own pattern if they want it. ([#83025](https://github.com/WordPress/gutenberg/pull/83025))
