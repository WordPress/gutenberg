...
-   `getEntityRecord` and `getEntityRecords` now infer the record type from their `kind` and `name` arguments, so `getEntityRecord( 'postType', 'post', id ).title` type checks without naming `Post` by hand. A plugin opts into the inference by merging a new kind into `EntityRecordTypes`, or a name under an existing kind into `RootEntityRecordTypes`, `PostTypeEntityRecordTypes` or `TaxonomyEntityRecordTypes`. Pairs the map does not cover, including entities registered at runtime, keep resolving to the previous union type.
-   `getEntityRecord` and `getEntityRecords` now resolve the record's context from the query, so a `context: 'view'` request is no longer typed with the edit-context fields. A query that does not pin the context down to a literal -- one held in a variable, where the property widens to `string`, an optional `context?: Context`, or an untyped query object -- resolves to every context, leaving only the fields all of them serialise readable rather than assuming `edit`.
...
-   Remove the type assertions that stood in for the missing return types at `getEntityRecord` and `getEntityRecords` call sites across `boot`, `fields`, `media-fields`, `global-styles-ui`, `lazy-editor`, the dashboard widgets and the routes. Two of them were hiding real defects: a site's `home` URL and a draft's `date` are both nullable, and the assertions claimed otherwise.
