...
-   `getEntityRecord`, `getEntityRecords`: Add a cache-busting argument to a request repeated after its resolution already received a response, so a host that caches REST responses by their full URL — even authenticated ones WordPress marks `no-cache` — cannot serve an invalidation-triggered refetch the very response it is trying to replace. First requests are untouched and stay cacheable ([#81904](https://github.com/WordPress/gutenberg/pull/81904)).
