...
-   `__experimentalFetchLinkSuggestions`: rank pages first, then other content, then categories, then other taxonomies, with attachments and post formats last, so a media library no longer crowds out what a link search is looking for. A caller passing `preferTypes` leads with the types it names — either a whole search type or one subtype of it — and everything else keeps its usual place behind them, which covers custom post types and taxonomies without naming them. A title containing what was typed ranks above one that does not, and a title beginning with it above one containing it further in. No longer divides by title length, so a long title is not marked down for being long and a repeated word earns nothing. An unscoped search returns every answer it found rather than cutting the merged results to `perPage`, which discarded results no later request could reach, and drops results that matched on a body or excerpt rather than a title; a search narrowed by `type` is unchanged ([#83293](https://github.com/WordPress/gutenberg/pull/83293)).
