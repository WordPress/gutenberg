...
-   `__experimentalFetchLinkSuggestions`: rank content above taxonomies, and both above attachments and post formats, so a media library no longer crowds out the pages and posts a link search is looking for. Weighted by `kind`, so custom post types and taxonomies are covered without being named. A title containing what was typed ranks above one that does not, and a title beginning with it above one containing it further in. No longer divides by title length, so a long title is not marked down for being long and a repeated word earns nothing. An unscoped search returns everything it found rather than cutting the merged results to `perPage`, which discarded results no later request could reach; a search narrowed by `type` is unchanged ([#83293](https://github.com/WordPress/gutenberg/pull/83293)).
