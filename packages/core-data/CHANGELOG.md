...
-   `__experimentalFetchLinkSuggestions`: rank results by type (content, taxonomy, post format, attachment). An unscoped search returns every title holding each word typed instead of cutting the merged results to `perPage` so no matching results are lost ([#83407](https://github.com/WordPress/gutenberg/pull/83407)).
