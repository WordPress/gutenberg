...
-   `Plugin.author` is now a string; `PostStatusObject` and `Taxonomy` omit edit-only fields in other contexts; `Theme.tags.raw` is a string array and `theme_supports` is optional; and `NavMenu` no longer exposes the unrelated menu-item `object_id`. ([#81863](https://github.com/WordPress/gutenberg/pull/81863))
