...
-   `__experimentalFetchLinkSuggestions`: rank content above taxonomies, and both above attachments and post formats, so a media library no longer crowds out what a link search is looking for. Ranked by search type, so custom post types count as content and custom taxonomies as taxonomies without being named. A title with every word that was typed ranks above one without them, then the type decides, then a title beginning with what was typed ranks above one containing it further in. No longer divides by title length, so a long title is not marked down for being long and a repeated word earns nothing. An unscoped search returns every answer it found rather than cutting the merged results to `perPage`, which discarded results no later request could reach, and drops results that matched on a body or excerpt rather than a title; a search narrowed by `type`, and the suggestions shown before anything is typed, are unchanged ([#83407](https://github.com/WordPress/gutenberg/pull/83407)).
