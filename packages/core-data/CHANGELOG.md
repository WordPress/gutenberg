...
-   `__experimentalFetchLinkSuggestions`: add an `exclude` option, so a caller that cannot use a type leaves it out of an otherwise unscoped search instead of paying for it in the results ([#83299](https://github.com/WordPress/gutenberg/pull/83299)).
-   `__experimentalFetchLinkSuggestions`: rank a title containing every word that was typed above every title that does not, and order the rest by type — pages, then categories, then posts and tags, with attachments and post formats last — so a media library does not crowd out what a link search is looking for. A caller naming `priorityTypes` leads with those types instead, in the order it names them ([#83293](https://github.com/WordPress/gutenberg/pull/83293)).
