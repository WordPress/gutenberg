...
-   `__experimentalFetchLinkSuggestions`: Score each result separately when a post, term or attachment share an id, so an unmatched result can no longer inherit another's relevance and sort above a real match ([#83093](https://github.com/WordPress/gutenberg/pull/83113)).
