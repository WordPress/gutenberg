...
-   `__experimentalFetchLinkSuggestions`: rank content above taxonomies, and both above attachments and post formats, so a media library no longer crowds out what a link search is looking for. An unscoped search returns everything it found instead of cutting the merged results to `perPage` ([#83407](https://github.com/WordPress/gutenberg/pull/83407)).
