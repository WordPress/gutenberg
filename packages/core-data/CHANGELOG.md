...
-   `__experimentalFetchLinkSuggestions`: order results by type before comparing their titles — pages, then categories, then posts and tags, with attachments and post formats last — so a media library does not crowd out what a link search is looking for. A caller naming `priorityTypes` leads with those types instead, in the order it names them ([#83293](https://github.com/WordPress/gutenberg/pull/83293)).
