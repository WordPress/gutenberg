...
-   `__experimentalFetchLinkSuggestions`: add a `preferTypes` option, ranking the types it names above the usual order and leaving everything else in place behind them. An entry is a search type, covering all of it, or a search type with one subtype, covering only that subtype, so a caller can lead with categories without listing every other type ([#BBBBB](https://github.com/WordPress/gutenberg/pull/BBBBB)).
-   `__experimentalFetchLinkSuggestions`: rank content above taxonomies, and both above attachments and post formats, so a media library no longer crowds out what a link search is looking for. Ranked by search type, so custom post types count as content and custom taxonomies as taxonomies without being named. A title containing what was typed ranks above one that does not, and a title beginning with it above one containing it further in. No longer divides by title length, so a long title is not marked down for being long and a repeated word earns nothing. An unscoped search returns every answer it found rather than cutting the merged results to `perPage`, which discarded results no later request could reach, and drops results that matched on a body or excerpt rather than a title; a search narrowed by `type` is unchanged ([#83407](https://github.com/WordPress/gutenberg/pull/83407)).
