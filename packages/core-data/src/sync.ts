...
export function getSyncProvider(): SyncProvider {
	...
	// Ensure returned sync provider is an instance of SyncProvider.
	if ( ! ( syncProvider instanceof SyncProvider ) ) {
		syncProvider = null;
	}
	...
	// If no sync provider is set, use a fallback no-op sync provider.
	if ( ! syncProvider ) {
