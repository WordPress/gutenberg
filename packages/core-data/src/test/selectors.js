...
describe.each([[getEntityRecord], [__experimentalGetEntityRecordNoResolver]])(
	'%p',
	(selector) => {
...
describe('isPreviewEmbedFallback()', () => {
	it('returns true if the preview html is just a single link', () => {
		...
		expect(isPreviewEmbedFallback(state, 'http://example.com/')).toEqual(
			true
		);
	});
});
...
describe('canUserEditEntityRecord', () => {
	it('returns false by default', () => {
		...
		expect(canUserEditEntityRecord(state, 'postType', 'post')).toBe(false);
	});
	...
	it('returns whether the user can edit', () => {
		...
		expect(canUserEditEntityRecord(state, 'postType', 'post', '1')).toBe(
			true
		);
	});
});
