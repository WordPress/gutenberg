...
export default function useEntityBlockEditor( kind, name, { id: _id } = {} ) {
	...
	const blocks = useMemo( () => {
		...
		// Cache parsed blocks by entity identity (kind:name:id).
		// Store both content and blocks so we can validate content hasn't changed.
		const cacheKey = `${ kind }:${ name }:${ id }`;
		const cached = parsedBlocksCache.get( cacheKey );
		let _blocks;

		if ( cached && cached.content === content ) {
			// Same content, return cached blocks (preserves clientIds)
			_blocks = cached.blocks;
		} else {
			// Content changed or no cache, parse and update cache
