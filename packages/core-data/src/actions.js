...
export const editEntityRecord =
	...
	( { select, dispatch } ) => {
		...
		// Tell the entity sync manager about the edit BEFORE it lands in the
		// store (see `EntitySyncManager.update` for the ordering caveat). It
		// receives the merged edits and the edit's intent, and decides what
		// to do with them.
	...
	async ( { select, resolveSelect, dispatch } ) => {
		...
		try {
			...
			try {
				...
				// (a direct `saveEntityRecord` call bypasses
				// `editEntityRecord`) and add anything it needs to the
				// request.
				...
				if ( isAutosave ) {
					...
					// can clear the persisted edits from state. The added
					// values never exist in the state edits, so comparing
					// against them would fail to match and leave the record
