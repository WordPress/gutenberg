...
describe('iteratePath', () => {
	...
	it('Yields the root first', () => {
		expect(Array.from(iteratePath(tree, ['target', 'bird']))[0]).toEqual(
			tree
		);
	});
...
describe('iterateDescendants', () => {
	...
	it('Yields children when there is just one level of nesting', () => {
		...
		expect(results).toEqual(expect.arrayContaining([red, blue, green]));
	});
...
describe('deepCopyLocksTreePath', () => {
	it('Returns a tree with a cloned path', () => {
		...
		expect(deepCopy.children.target.locks).toBe(tree.children.target.locks);
	...
	it('Creates missing branches', () => {
		...
		expect(Object.keys(target.children.bird.children)).toEqual(['blue']);
