...
describe('isValidHref', () => {
	...
	describe('URLs beginning with a protocol', () => {
		it('returns true for valid URLs', () => {
			...
			expect(isValidHref('http://test.com/with/path/separators')).toBe(
				true
			);
			expect(
...
describe('getFormatBoundary', () => {
	...
	describe('Invalid values', () => {
		...
		])('should return empty bounds if %s', (_ignored, [start, end]) => {
			const record = {
				formats: [
					null,
					[italicFormat],
	...
	describe('Single value out of bounds', () => {
		it.each([
			...
			['end is withins bounds but is less than start (inverted)', [4, 1]],
			...
			expect(getFormatBoundary(record, { type: 'core/bold' })).toEqual({
				start: 4,
				end: 6,
			});
		});
