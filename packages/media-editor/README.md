...
function MyMediaEditor( { mediaId } ) {
	const { media, isLoading } = useSelect( ( select ) => {
		const attachment = select( coreStore ).getEditedEntityRecord(
			'postType',
			'attachment',
			mediaId
		);
...
-   `media`: Media object from the WordPress REST API
-   `fields`: Array of field definitions (uses `Field` type from `@wordpress/dataviews`)
-   `onUpdate`: Callback when media is updated `(updates: Partial<Media>) => void`
