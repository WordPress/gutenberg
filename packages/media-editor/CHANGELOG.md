...
-   `MediaEditor`: open the settings panel on Crop where it docks beside the canvas, and on Details below that, where an open panel replaces the canvas and crop controls would have nothing to act on. Non-image media has no Crop tab and always opens on Details ([#81840](https://github.com/WordPress/gutenberg/pull/81840)).
...
-   `MediaEditor`: size the docked settings panel with the `--wpds-dimension-surface-width-sm` token (320px), up from the 280px it inherited from `$sidebar-width`, and dock it above the `large` breakpoint; below that it takes the whole body ([#81840](https://github.com/WordPress/gutenberg/pull/81840)).
...
-   `MediaEditor.ImageControls` is removed, and the `scope` prop is now inert. Both were added in the same unreleased cycle; the transform controls belong to the Crop panel, with a flat row under the canvas below `small` where no panel is docked, and the sidebar's open state comes from width rather than a persisted preference. `scope` is still accepted so existing callers compile ([#81840](https://github.com/WordPress/gutenberg/pull/81840)).
