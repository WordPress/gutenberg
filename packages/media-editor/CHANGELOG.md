...
-   `MediaEditor.HistoryActions` renders nothing while a panel covers the canvas. Reset, undo and redo act on the cropper, so leaving them beside the metadata fields put an enabled, destructive Reset next to a crop the user could not see. `renderFrame` gains `hasCanvas` so a frame can drop the container it would have put them in ([#81840](https://github.com/WordPress/gutenberg/pull/81840)).
