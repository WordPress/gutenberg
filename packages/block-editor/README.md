...
-   _mediaCrossOriginDomains_ `[Array]`: Array\<(''|'anonymous'|'use-credentials'|undefined)> For each target domain (i.e CDN), set a value for requesting media assets with CORS, allowing them to be used in a `<canvas>` as if they were loaded from the current origin.
