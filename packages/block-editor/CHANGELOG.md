...
-   Block Fields (experimental): show a connection indicator next to the field label for block attributes that support Block Bindings — muted when the attribute is unbound, highlighted in the synced color when it is bound, with the source name exposed to assistive technology and in a tooltip.
