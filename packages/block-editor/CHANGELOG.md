...
-   Expose the layout inspector panel as the private `LayoutPanel`, for blocks that manage the `layout` attribute themselves ([#pattern-block-inspector-parity (createPullRequest)](https://github.com/WordPress/gutenberg/pull/pattern-block-inspector-parity (createPullRequest))).
-   Inspector: Give the Pattern block (`core/block`) the full style panel set, in the Styles tab and in the single-panel layout, as template parts get, so its layout and custom CSS controls are reachable ([#pattern-block-inspector-parity (createPullRequest)](https://github.com/WordPress/gutenberg/pull/pattern-block-inspector-parity (createPullRequest))).
