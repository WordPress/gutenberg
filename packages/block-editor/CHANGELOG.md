...
-   Remove the dotted-underline label treatment and the inline reset affordance for inherited Global Styles, marking only a local override with a diamond indicator and an "Overrides inherited styles" tooltip ([#80649](https://github.com/WordPress/gutenberg/pull/80649)).
-   The override indicator's tooltip now shows the Global Styles breadcrumb path the overridden value is inherited from, e.g. "Overrides inherited styles from Blocks › Group › Variations › Subtitle" (block inspector only; the Global Styles screens show no breadcrumb) ([#80649](https://github.com/WordPress/gutenberg/pull/80649)).

### Internal

-   Remove the `IS_GUTENBERG_PLUGIN` gate on the inherited Global Styles treatment in the block inspector, so the inheritance affordances ship in WordPress Core builds ([#80555](https://github.com/WordPress/gutenberg/pull/80555)).
