...
-   Fix shift+arrow selection in table blocks extending in block document order (horizontally) instead of along the table grid. Shift+arrow now grows or shrinks a cell selection by one row or column in the arrow's direction, accounting for merged cells, whether the caret is in a cell's text or whole cells are selected.
-   Clear the native browser selection once a table cell multi-selection is established, matching regular block multi-selection. `MULTI_SELECT_SET` now updates the `initialPosition` state, and the selection observer passes an initial position for canvas drag selections, so a stale native range can no longer be re-interpreted as a cell selection after an operation (like merging cells) removes the selected cells from the DOM.
-   Fix rectangular table cell selection when the table contains merged cells. `cellIndex` is not span-aware, so merged cells shifted the computed columns of the cells around them and the selection rectangle covered the wrong cells. The selection now derives each cell's visual column from an occupancy grid of the table.
