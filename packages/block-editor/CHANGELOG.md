...
-   Block alignment: Explain in the alignment menu when the template rather than a block on the page limits width, with a way to edit that template. Hosts supply the explanation through a private setting, since the block editor cannot know what a template is ([#ZZZZZ](https://github.com/WordPress/gutenberg/pull/ZZZZZ)).
