...
-   The default block appender inside a block is now that block's empty state: it renders while the block has no inner blocks, whether or not the block is selected, and no longer renders next to content. Empty containers stay discoverable when unselected, and the absolutely positioned corner inserter that appeared inside selected containers with content is replaced by an inserter button in the block toolbar. Custom appenders and `renderAppender: false` are unaffected.
