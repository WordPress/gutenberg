...
-   Inspector: Let the Pattern block (`core/block`) show every inspector group while a section is being edited, as template parts already can, so its settings such as the wrapper element are reachable ([#ZZZZZ](https://github.com/WordPress/gutenberg/pull/ZZZZZ)).
