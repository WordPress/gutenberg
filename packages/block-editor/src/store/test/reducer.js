...
describe('state', () => {
	...
	describe('isUpdatingSameBlockAttribute()', () => {
		it('should return false if not updating block attributes', () => {
			...
			expect(isUpdatingSameBlockAttribute(action, previousAction)).toBe(
				false
			);
		});
	...
	describe('blocks()', () => {
		...
		describe('replace inner blocks', () => {
			...
			it('can replace a child block', () => {
				...
				expect(state.tree.chicken).not.toBe(existingState.tree.chicken);
			});
			...
			it('can insert a child block', () => {
				...
				expect(state.tree.chicken).not.toBe(existingState.tree.chicken);
				expect(state.tree[''].innerBlocks[0]).toBe(state.tree.chicken);
		...
		it('should key by reset blocks clientId', () => {
			[undefined, blocks(undefined, {})].forEach((original) => {
				...
				expect(state.tree[''].innerBlocks[0]).toBe(state.tree.bananas);
			});
		});
		...
		it('should replace the block', () => {
			...
			expect(state.tree[''].innerBlocks[0]).toBe(state.tree.wings);
		...
		it('should move the block up', () => {
			...
			expect(state.tree[''].innerBlocks[1]).toBe(state.tree.chicken);
