	...
	describe( 'blocks()', () => {
		...
		describe( 'blocks', () => {
			...
			describe( 'isPersistentChange', () => {
				...
				it( 'should flag only the next change as part of a combined operation', () => {
					let original = deepFreeze(
						blocks( undefined, {
							type: 'RESET_BLOCKS',
							blocks: [
					...
					expect( nextState.isLastBlockChangeCombinedOperation ).toBe(
						true
					);
					...
					expect(
						subsequentState.isLastBlockChangeCombinedOperation
					).toBeUndefined();
				} );

				it( 'should preserve combined operation markers when marking the next change as not persistent', () => {
					let original = deepFreeze(
						blocks( undefined, {
							type: 'RESET_BLOCKS',
							blocks: [
				...
				it( 'should flag INSERT_BLOCKS-style changes that add blocks', () => {
					...
					const insertState = blocks( original, {
						type: 'INSERT_BLOCKS',
						blocks: [
							{
								clientId: 'plum',
					...
					const replaceState = blocks( original, {
						type: 'REPLACE_BLOCKS',
						clientIds: [ 'plum' ],
						blocks: [
							{
