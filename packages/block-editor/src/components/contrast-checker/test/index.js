...
describe( 'ContrastChecker', () => {
	...
	test( 'should render component when the text and background colors do not meet AA WCAG guidelines.', () => {
		...
		expect(
			container.querySelector( '.components-notice' ).textContent
		).toBe(
	...
	test( 'should take into consideration wherever text is large or not', () => {
		const { container: wrapperSmallText } = render(
			<ContrastChecker
		...
		expect(
			wrapperSmallText.querySelector( '.components-notice' ).textContent
		).toBe(
			'This color combination may be hard for people to read. Try using a brighter background color and/or a darker text color.'
		);

		const { container: wrapperLargeText } = render(
			<ContrastChecker
				backgroundColor="#C44B4B"
				textColor="#000000"
				isLargeText={ true }
		...
		expect( wrapperLargeText.innerHTML ).toBe( '' );
	} );

	test( 'should take into consideration the font size passed', () => {
		const { container: wrapperSmallFontSize } = render(
			<ContrastChecker
				backgroundColor="#C44B4B"
				textColor="#000000"
				fontSize={ 23 }
		...
		expect(
			wrapperSmallFontSize.querySelector( '.components-notice' )
				.textContent
		).toBe(
			'This color combination may be hard for people to read. Try using a brighter background color and/or a darker text color.'
		);
	...
	test( 'should use isLargeText to make decisions if both isLargeText and fontSize props are passed', () => {
		...
		const { container: wrapperNoLargeText } = render(
			<ContrastChecker
				backgroundColor="#C44B4B"
				textColor="#000000"
				fontSize={ 24 }
		...
		expect(
			wrapperNoLargeText.querySelector( '.components-notice' ).textContent
		).toBe(
			'This color combination may be hard for people to read. Try using a brighter background color and/or a darker text color.'
		);
	} );
	...
	test( 'should render component when the colors meet AA WCAG guidelines but the text color only has alpha transparency with alpha checker enabled.', () => {
		...
		expect(
			container.querySelector( '.components-notice' ).textContent
		).toBe( 'Transparent text may be hard for people to read.' );
	} );
