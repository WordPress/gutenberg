...
const lastClickPoints = new WeakMap();

/**
 * Remembers the last pointer down position within the wrapper. When selecting
 * a block turns its editable element into an inert part of the editing host
 * mid-click, the browser drops the caret the mousedown placed;
 * `useEditableRoot` restores it from this point.
 *
 * @param {HTMLElement} node  Wrapper element.
 * @param {MouseEvent}  event Mouse event.
 */
export function setLastClickPoint( node, event ) {
	lastClickPoints.set( node, {
		x: event.clientX,
		y: event.clientY,
		time: Date.now(),
	} );
...
/**
 * Returns the last pointer down position within the wrapper, if it happened
 * recently enough to belong to the interaction being processed.
 *
 * @param {HTMLElement} node Wrapper element.
 *
 * @return {?{x: number, y: number}} The point, or null.
 */
export function getRecentClickPoint( node ) {
	const point = lastClickPoints.get( node );
	return point && Date.now() - point.time < 1000 ? point : null;
}

/**
 * Returns a collapsed range at the given client coordinates.
 *
 * @param {Document} ownerDocument The document.
 * @param {number}   x             Client X.
 * @param {number}   y             Client Y.
 *
 * @return {?Range} The caret range, or null.
 */
export function caretRangeFromPoint( ownerDocument, x, y ) {
