...
function mediaUpload(
	...
	const hasClientSide = clientSideFiles.length > 0;
	const hasServerSide = serverSideFiles.length > 0;

	// Track completion of both upload paths for onBatchSuccess.
	// onBatchSuccess should only fire once all files from both paths are done.
	let clientSideDone = ! hasClientSide;
	let serverSideDone = ! hasServerSide;

	function maybeCallBatchSuccess() {
		if ( clientSideDone && serverSideDone ) {
			onBatchSuccess?.();
		}
	}
