...
const BLOCK_CLASS = 'block-editor-block-list__block';
...
function createBlockElement( { textFieldCount = 0, innerTextFields = 0 } ) {
	const root = document.createElement( 'div' );
	root.className = BLOCK_CLASS;

	for ( let i = 0; i < textFieldCount; i++ ) {
		root.appendChild( createTextField() );
	}

	if ( innerTextFields > 0 ) {
		const inner = document.createElement( 'div' );
		inner.className = BLOCK_CLASS;
		for ( let i = 0; i < innerTextFields; i++ ) {
			inner.appendChild( createTextField() );
		}
...
describe( 'getListViewKeyboardFocusPosition', () => {
	afterEach( () => {
		document.body.replaceChildren();
	} );

	it( 'uses caret-at-end when the canvas wrapper is missing', () => {
		expect( getListViewKeyboardFocusPosition( null ) ).toBe( -1 );
	} );

	it( 'uses caret-at-end for a single text field', () => {
		const root = createBlockElement( { textFieldCount: 1 } );
		expect( getListViewKeyboardFocusPosition( root ) ).toBe( -1 );
	} );

	it( 'uses the first field when the block has multiple text fields', () => {
		const root = createBlockElement( { textFieldCount: 2 } );
		expect( getListViewKeyboardFocusPosition( root ) ).toBe( 0 );
	} );

	it( 'ignores text fields that belong to inner blocks', () => {
		const root = createBlockElement( { innerTextFields: 2 } );
		expect( getListViewKeyboardFocusPosition( root ) ).toBe( -1 );
	} );
} );
