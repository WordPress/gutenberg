...
/**
 * Returns a class based on duotone filter slug.
 *
 * @param {string} duotoneSlug Slug of the duotone filter.
 *
 * @return {string} String with the class corresponding to the duotone filter slug.
 *                  The class is generated by appending 'has-' followed by duotoneSlug in kebabCase and ending with '-duotone-filter'.
 */
export function getDuotoneClass( duotoneSlug ) {
	return `has-${ kebabCase( duotoneSlug ) }-duotone-filter`;
}
				...
				onChange={ ( newColors ) => {
					...
					if ( newColors ) {
						const slug = duotonePalette.find( ( { colors } ) =>
							colors.every(
								( color, index ) => newColors[ index ] === color
							)
						)?.slug;
						if ( slug ) {
							newDuotone = slug;
							...
							newStyle.color.duotone = newColors;
						}
					} else {
						delete newStyle.color.duotone;
					}
...
function addDuotoneClasses( props, blockType, attributes ) {
	...
	props.className = classnames( props.className, getDuotoneClass( duotone ) );
...
const withDuotoneStyles = createHigherOrderComponent(
	( BlockListBlock ) => ( props ) => {
		...
		if ( ! duotoneSupport || ( ! customDuotone && ! namedDuotone ) ) {
