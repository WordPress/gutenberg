...
-   **Provider**: A transport layer that syncs CRDT document updates between peers. The Gutenberg plugin can provide an experimental HTTP polling provider; plugins can supply their own provider via the `sync.providers` filter.
...
### Experimental HTTP polling provider

When the **Polling provider** experiment under **Real-Time Collaboration** is enabled, the default provider (`src/providers/http-polling/`) uses HTTP polling to exchange updates with a central sync server. When the experiment is disabled, the default provider list is empty. Plugins can still add a provider through the `sync.providers` filter. A shared polling manager batches updates for all rooms (entities) into a single request per poll cycle. See [the HTTP polling README](./src/providers/http-polling/README.md) for full details including the REST API format, sync protocol, and compaction.
