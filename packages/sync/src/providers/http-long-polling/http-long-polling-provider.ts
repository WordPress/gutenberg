...
import { longPollingManager } from './long-polling-manager';
...
/**
 * Event types for HttpLongPollingProvider.
 * ObservableV2 expects event handlers as functions.
 */
type HttpLongPollingEvents = {
	status: ( status: ConnectionStatus ) => void;
};

/**
 * Yjs provider that uses HTTP long polling for real-time synchronization.
 * The server holds requests open until updates are available, so remote
 * changes arrive with low latency, and held requests are aborted and
 * re-issued when local changes need to be sent.
 */
class HttpLongPollingProvider extends ObservableV2< HttpLongPollingEvents > {
	...
	public disconnect(): void {
		...
		longPollingManager.unregisterRoom( this.options.room );
	...
	/**
	 * Handle synchronization events from the long-polling manager.
	 */
...
/**
 * Create a provider creator function for the HttpLongPollingProvider
 */
export function createHttpLongPollingProvider(): ProviderCreator {
	return async ( {
		...
		const provider = new HttpLongPollingProvider( {
			awareness,
			// debug: true,
			room,
			ydoc,
