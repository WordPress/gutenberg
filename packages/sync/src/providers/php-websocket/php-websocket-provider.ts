...
import { webSocketSyncManager } from './websocket-manager';
...
/**
 * Event types for PhpWebSocketProvider.
 * ObservableV2 expects event handlers as functions.
 */
type PhpWebSocketEvents = {
	status: ( status: ConnectionStatus ) => void;
};

/**
 * Yjs provider that synchronizes through the experimental PHP WebSocket
 * sync server using JSON messages that share the HTTP polling semantics.
 */
class PhpWebSocketProvider extends ObservableV2< PhpWebSocketEvents > {
	...
	/**
	 * Connect to the WebSocket server and initialize sync.
	 */
	public connect(): void {
		this.log( 'Connecting' );
	...
	public disconnect(): void {
		...
		webSocketSyncManager.unregisterRoom( this.options.room );
	...
	/**
	 * Handle synchronization events from the WebSocket manager.
	 */
...
/**
 * Create a provider creator function for the PhpWebSocketProvider
 */
export function createPhpWebSocketProvider(): ProviderCreator {
	return async ( {
		awareness,
		objectType,
		objectId,
		...
		const provider = new PhpWebSocketProvider( {
			awareness,
			// debug: true,
			room,
			ydoc,
