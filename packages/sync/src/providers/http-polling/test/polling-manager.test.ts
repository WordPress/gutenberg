...
describe( 'polling-manager', () => {
	...
	describe( 'initial sync signal', () => {
		it( 'fires onInitialSync after the first successful poll, even with no updates', async () => {
			mockPostSyncUpdate.mockResolvedValue( syncResponse );
			...
			// Not synced until the first poll returns the server's state.
			expect( onInitialSync ).not.toHaveBeenCalled();

			await jest.advanceTimersByTimeAsync( 0 );

			// An empty response still means the server has nothing more to
			// send, so the initial sync is complete without any document change
			// or sync step 2 handshake (the solo, paused-queue case).
			expect( onInitialSync ).toHaveBeenCalled();
		} );

		it( 'does not fire onInitialSync when the initial poll fails', async () => {
			mockPostSyncUpdate.mockRejectedValue( new Error( 'network' ) );
