...
import type {
	Action,
	NormalizedField,
	ViewTimeline as ViewTimelineType,
	ViewTimelineProps,
...
function ListItem< Item >( {
	...
	const renderedMediaField =
		showMedia && mediaField?.render ? (
			<div className="dataviews-view-timeline__media-wrapper">
				<mediaField.render
					item={ item }
					field={ mediaField }
					config={ { sizes: '52px' } }
						...
						( [ groupName, groupItems ] ) => (
							<VStack key={ groupName } spacing={ 2 }>
								<h3 className="dataviews-view-timeline__group-header">
									{ view.layout?.eventField === groupField.id
										? groupName
										: sprintf(
								...
								<div className="dataviews-view-timeline__group-items">
									{ groupItems.map( ( item ) => {
										const id =
											generateCompositeItemIdPrefix(
												item
