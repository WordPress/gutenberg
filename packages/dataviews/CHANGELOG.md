...
-   The `./wp` bundle (`@wordpress/dataviews/wp`) now requires WordPress 7.0 or later. It no longer inlines the `@wordpress/*` packages that WordPress does not ship as scripts (`@wordpress/ui`, `@wordpress/icons`, and `@wordpress/kebab-case`), nor `@wordpress/theme`, which only `@wordpress/ui` pulled in: consumers bundle those packages themselves, and their copy of `@wordpress/ui` resolves `@wordpress/theme` to the `wp.theme` script, which WordPress ships since 7.0. The bundle shrinks by about a quarter, shares a single copy of those packages and of the theme context with the consumer, and the `@base-ui/react`, `@daypicker/react`, and `colorjs.io` dependencies are no longer declared ([#82595](https://github.com/WordPress/gutenberg/pull/82595)).
...
-   DataForm: Use `ValidatedTextareaControl` from `@wordpress/ui` in the `textarea` control, instead of unlocking it from the `@wordpress/components` private APIs. This was the last private API used by the package, so the `@wordpress/private-apis` dependency is removed. ([#81984](https://github.com/WordPress/gutenberg/pull/81984))
