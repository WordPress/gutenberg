...
-   `resolveStyle`'s per-leaf source map now carries breadcrumb metadata — a `breadcrumb` array of part identifiers plus the `blockName` and `variation` slugs — describing the Global Styles path each inherited value comes from, so consumers can translate and surface it (the engine stays block-agnostic and does not resolve titles) ([#80649](https://github.com/WordPress/gutenberg/pull/80649)).
