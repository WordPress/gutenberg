...
describe( 'Menu', () => {
	...
	describe( 'pointer and keyboard interactions', () => {
		...
		it( 'should allow resetting checked radio items from a separate menu', async () => {
			const SiblingMenus = () => {
			...
			const openMenuA = screen.getByRole( 'button', {
				name: 'Open menu A',
			} );
			const openMenuB = screen.getByRole( 'button', {
				name: 'Open menu B',
			} );

			// Open the first menu and verify its checked state.
			await click( openMenuA );
