...
describe('Guide', () => {
	...
	it('renders one page at a time', () => {
		const wrapper = shallow(
			<Guide
				pages={[{ content: <p>Page 1</p> }, { content: <p>Page 2</p> }]}
			/>
		);
	...
	it('hides back button and shows forward button on the first page', () => {
		...
		expect(wrapper.find('.components-guide__forward-button')).toHaveLength(
			1
		);
		expect(wrapper.find('.components-guide__finish-button')).toHaveLength(
			0
		);
	});
	...
	it("doesn't display the page control if there is only one page", () => {
		const wrapper = shallow(<Guide pages={[{ content: <p>Page 1</p> }]} />);
	...
	it('calls onFinish when the finish button is clicked', () => {
		...
		const wrapper = shallow(
			<Guide onFinish={onFinish} pages={[{ content: <p>Page 1</p> }]} />
		);
