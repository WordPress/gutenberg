...
describe( 'FontSizePicker', () => {
	...
	describe( 'with ≤ 5 numeric (unitless) font sizes', () => {
		// Mirrors the reported bug: theme.json presets defined with a
		// numeric `size` (e.g. `"size": 12`) instead of a string with
		// units (e.g. `"size": "12px"`).
		const fontSizes = [
			{
				slug: 'small',
		...
		it( 'has no selection for a newly-inserted block (value is undefined)', async () => {
			await render(
				<FontSizePicker fontSizes={ fontSizes } value={ undefined } />
			);
		...
		it( 'selects the correct option when value matches a numeric preset', async () => {
			await render(
				<FontSizePicker fontSizes={ fontSizes } value={ 16 } />
			);
			expect(
				screen.getByRole( 'radio', { checked: true } )
			).toHaveAccessibleName( 'Medium' );
		...
		it( 'calls onChange with a number (not a string) when a font size is selected', async () => {
			const user = userEvent.setup();
			const onChange = jest.fn();
			await render(
				<FontSizePicker fontSizes={ fontSizes } onChange={ onChange } />
			...
			await user.click( screen.getByRole( 'radio', { name: 'Medium' } ) );
			expect( onChange ).toHaveBeenCalledTimes( 1 );
			expect( onChange ).toHaveBeenCalledWith( 16, fontSizes[ 1 ] );
		} );
