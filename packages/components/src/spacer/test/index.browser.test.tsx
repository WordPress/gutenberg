...
function expectCustomProperties(
	testId: string,
	expected: Record< string, string >
) {
	const computed = getComputedStyle( screen.getByTestId( testId ) );
	for ( const [ property, value ] of Object.entries( expected ) ) {
		expect( computed.getPropertyValue( property ).trim() ).toBe( value );
	}
}
...
describe( 'props', () => {
	...
	test( 'should render nested instances without passing spacing variables to children', () => {
		...
		expect(
			getComputedStyle( outerSpacer ).getPropertyValue(
				'--wp-components-spacer-padding'
			)
		).toBe( 'calc(4px * 4)' );
		expect(
			getComputedStyle( innerSpacer ).getPropertyValue(
				'--wp-components-spacer-padding'
			)
		).toBe( '' );
	} );
