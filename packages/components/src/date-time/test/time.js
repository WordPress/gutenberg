...
describe('TimePicker', () => {
	...
	it('should allow to set the time correctly when the PM period is selected', () => {
		...
		expect(onChangeSpy).toHaveBeenNthCalledWith(1, '1986-10-18T23:00:00');
		// When changing the hours to '6', we expect the time to be 6pm
		expect(onChangeSpy).toHaveBeenNthCalledWith(2, '1986-10-18T18:00:00');
	});
	...
	it('should reset the date when currentTime changed', () => {
		 ...
		 */
		expect(screen.getByText('AM').classList.contains('is-primary')).toBe(
			false
		);
		expect(screen.getByText('PM').classList.contains('is-primary')).toBe(
			true
		);
	});
	...
	it('Should set a time when passed a null currentTime', () => {
		...
		render(
			<TimePicker currentTime={null} onChange={onChangeSpy} is12Hour />
		);
