...
describe( 'ColorPicker', () => {
	...
	describe( 'visual picker', () => {
		...
		const getPointerPosition = ( pointer: HTMLElement ) => {
			...
			return {
				left: ( pointer.offsetLeft / surface.clientWidth ) * 100,
				top: ( pointer.offsetTop / surface.clientHeight ) * 100,
			};
		};

		const expectPointerPosition = (
			pointer: HTMLElement,
			expected: { left?: number; top?: number }
		) => {
			const position = getPointerPosition( pointer );
			if ( expected.left !== undefined ) {
				expect( position.left ).toBeCloseTo( expected.left );
			}
			if ( expected.top !== undefined ) {
				expect( position.top ).toBeCloseTo( expected.top );
			}
		};
		...
		it( 'updates visual saturation when HSL saturation is edited at black', async () => {
			...
			expectPointerPosition( pointer, { top: 100, left: 25 } );
		} );
	} );
