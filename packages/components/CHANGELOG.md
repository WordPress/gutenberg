...
-   `navigateRegions`: Pressing Escape moves focus out onto the surrounding region. On a region, Tab moves to the next one, backwards with Shift, and Enter, or Escape again, goes back in, to the place focus last left the region from or to its first tabbable. Handlers that claim a key first, popovers and modals among them, keep winning. Tab cycles exactly the regions the existing shortcuts visit, in document order, so a region nested in another, like a floating toolbar, comes right after its container ([#82310](https://github.com/WordPress/gutenberg/pull/82310)).
