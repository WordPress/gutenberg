...
-   `navigateRegions`: Pressing Escape moves focus out onto the surrounding region. On a region, Tab moves to the next one, backwards with Shift, and Enter goes back in, to the place focus last left the region from or to its first tabbable. Handlers that claim a key first, popovers and modals among them, keep winning. Cycling skips regions with nothing reachable inside, keeps regions whose controls are revealed on focus, and visits a region nested in another, like a floating toolbar, right after its container, in document order ([#82310](https://github.com/WordPress/gutenberg/pull/82310)).
