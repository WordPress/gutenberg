...
-   `navigateRegions`: Pressing Escape moves focus out onto the surrounding region, and on a region cycles to the next one, backwards with Shift. Handlers that claim the key first, popovers and modals among them, keep winning. Cycling now skips regions that are hidden or have no size ([#82310](https://github.com/WordPress/gutenberg/pull/82310)).
