...
-   `PaletteEdit`: Expose palette swatches as command buttons instead of a listbox, since activating a swatch opens the editor rather than selecting a value. Adds a `presentation` prop to `CircularOptionPicker` (`listbox` | `toggle-buttons` | `command-buttons`), forwarded through `ColorPalette`, `GradientPicker`, and `DuotonePicker`. Soft-deprecates `asButtons` (hidden from docs, still supported as an alias for `toggle-buttons`, with wrapper-level warnings) ([#81836](https://github.com/WordPress/gutenberg/pull/81836)).
