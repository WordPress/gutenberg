...
-   `BorderBoxControl`: Restore the split borders layout. The top and bottom controls are centred across the row and the right control sits at the end of its column again, after `BorderControl`'s own `margin: 0` began overriding both ([#PLACEHOLDER](https://github.com/WordPress/gutenberg/pull/PLACEHOLDER)).
