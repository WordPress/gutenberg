...
const clickMiddleOfElement = async ( driver, element, isParagraph = false ) => {
	...
	// Condition to make it work on iOS Paragraph blocks on both environments, the default calculation somehow doesn't work for this combination but works for others.
	if ( isParagraph === true && ! isAndroid() ) {
		action.press( { x: size.width / 2, y: location.y } );
	} else {
