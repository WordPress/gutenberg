...
The Style Engine provides a unified API for rendering styling for blocks across both client-side and server-side applications. It handles both low-level CSS generation from style objects and high-level global styles management for theme.json configurations.

## Key Features

-   **Low-level CSS Generation**: Generate CSS rules and declarations from style objects
-   **Global Styles API**: Read, write, and transform WordPress global styles (theme.json)
...
This package continues to evolve, and is likely to change. It has recently included
...
-   _config_ `GlobalStylesConfig | undefined`: Global styles configuration
-   _blockTypes_ `any[]`: Array of block types from WordPress blocks store
-   _options_ `GlobalStylesRenderOptions`: Options for rendering global styles

_Returns_

-   `[ any[], any ]`: Array of stylesheets and settings

### getBlockSelector

Determine the CSS selector for the block type and target provided, returning it if available.

_Parameters_

-   _blockType_ `BlockType`: The block's type.
-   _target_ `string`: The desired selector's target e.g. `root`, delimited string, or array path.
-   _options_ `{ fallback?: boolean; }`: Options object.
-   _options.fallback_ `{ fallback?: boolean; }`: Whether or not to fallback to broader selector.

_Returns_

-   `string | null`: The CSS selector or `null` if no selector available.
...
Get generated CSS for layout styles by looking up layout definitions provided in theme.json, and outputting common layout styles, and specific blockGap values.

_Parameters_

-   _props_ `{ layoutDefinitions?: Record< string, LayoutDefinition >; style?: GlobalStylesStyles; selector?: string; hasBlockGapSupport?: boolean; hasFallbackGapSupport?: boolean; fallbackGapValue?: string; }`: Layout styles configuration
-   _props.layoutDefinitions_ `Record< string, LayoutDefinition >`: Layout definitions from theme.json
-   _props.style_ `GlobalStylesStyles`: Style object for the block
-   _props.selector_ `string`: Selector to apply the styles to
-   _props.hasBlockGapSupport_ `boolean`: Whether the block supports block gap styles
-   _props.hasFallbackGapSupport_ `boolean`: Whether the block supports fallback gap styles
-   _props.fallbackGapValue_ `string`: Fallback gap value to use if block gap support is

_Returns_

-   `string`: Generated CSS rules for the layout styles

### getPalettes

Retrieves color and gradient related settings.

The arrays for colors and gradients are made up of color palettes from each origin i.e. "Core", "Theme", and "User".

_Parameters_

-   _settings_ `GlobalStylesConfig`: Global styles settings.

_Returns_

-   `MultiOriginPalettes`: Color and gradient related settings.

### getPresetVariableFromValue

Encodes a value to a preset variable format if it matches a preset. This is the inverse operation of getValueFromVariable().
...
-   _features_ `GlobalStylesSettings`: GlobalStylesContext settings object.
-   _blockName_ `string | undefined`: The name of a block (e.g., 'core/paragraph').
-   _variableStylePath_ `string`: The style path (e.g., 'color.text', 'typography.fontSize').
-   _presetPropertyValue_ `any`: The value to encode (e.g., '#ff0000').

_Returns_

-   `any`: The preset variable if found, otherwise the original value.
...
-   _ruleValue_ `UnresolvedValue`: A block style value that may contain a reference to a theme.json value.
-   _tree_ `GlobalStylesConfig | undefined`: A theme.json object.

_Returns_

-   The resolved value or incoming ruleValue.
...
-   _preset_ `TypographyPreset`: A typography preset object containing size and fluid properties.
-   _settings_ `GlobalStylesSettings`: Global styles settings object containing typography and layout settings.

_Returns_

-   A font-size value or the value of preset.size.
...
-   _features_ `GlobalStylesConfig`: GlobalStylesContext config (user, base, or merged). Represents the theme.json tree.
-   _blockName_ `string`: The name of a block as represented in the styles property. E.g., 'root' for root-level, and 'core/block-name' for blocks.
-   _variable_ `string | UnresolvedValue`: An incoming style value. A CSS var value is expected, but it could be any value.

_Returns_

-   `any`: The value of the CSS var, if found. If not found, returns the original variable argument.
...
-   _base_ `GlobalStylesConfig`: Base global styles (theme + WordPress defaults)
-   _user_ `GlobalStylesConfig`: User customizations

_Returns_

-   `GlobalStylesConfig`: Merged global styles configuration
