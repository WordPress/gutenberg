...
A single generated CSS rule, as returned by `getCSSRules`.
...
A style object, for example the value of a block's `attributes.style` object or the top-level styles in `theme.json`. Passed to `compileCSS` and `getCSSRules` to generate CSS.

### StyleOptions

Options that adjust how styles are generated by `compileCSS` and `getCSSRules`.
