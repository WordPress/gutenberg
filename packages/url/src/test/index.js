...
describe('isValidAuthority', () => {
	it('returns true if the authority is valid', () => {
		expect(isValidAuthority('user:password@www.test-this.com:1020')).toBe(
			true
		);
...
describe('getFilename', () => {
	it('returns the filename part of the URL', () => {
		...
		expect(getFilename('https://wordpress.org/image.jpg?query=test')).toBe(
			'image.jpg'
		);
	...
	it('returns undefined when the provided value does not contain a filename', () => {
		...
		expect(getFilename('http://localhost:8080/a/path/')).toBe(undefined);
		expect(getFilename('http://localhost:8080/?query=test')).toBe(
			undefined
		);
		expect(getFilename('http://localhost:8080/#anchor')).toBe(undefined);
...
describe('getQueryString', () => {
	it('returns the query string of a URL', () => {
		...
		expect(getQueryString('https://wordpress.org/test-path?query')).toBe(
			'query'
		);
		expect(
			getQueryString(
		...
		expect(getQueryString('https://test.com?foo=bar&foo=baz?test')).toBe(
	...
	it('returns undefined when the provided does not contain a url query string', () => {
		...
		expect(getQueryString('https://example.com/empty?')).toBeUndefined();
	});
});
...
describe('filterURLForDisplay', () => {
	...
	it('should preserve the original url if the url is short enough', () => {
		const url = filterURLForDisplay('http://www.wordpress.org/ig.jpg', 20);
