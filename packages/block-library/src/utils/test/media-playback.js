...
describe( 'media playback coordination', () => {
	...
	it( 'stops other audio and video elements when native media starts playing', () => {
		...
		const [ controlledVideo, videoWithoutControls ] =
		...
		].forEach( ( media ) => mockMediaPlaybackState( media ) );
		...
		expect( controlledVideo.pause ).toHaveBeenCalledTimes( 1 );
		expect( videoWithoutControls.pause ).toHaveBeenCalledTimes( 1 );
	} );

	it( 'stops audio and video elements when a playlist starts playing', () => {
		document.body.innerHTML = `
			<audio></audio>
			<video></video>
		`;
	...
	it( 'stops playlist players when audio or video starts playing', () => {
