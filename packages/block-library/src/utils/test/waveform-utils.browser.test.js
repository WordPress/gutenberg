...
describe( 'Waveform utilities', () => {
	...
	describe( 'applyWaveformPlayerStyles', () => {
		it( 'applies the waveform background color', () => {
			...
			expect( readStyle( waveformContainer ).backgroundColor ).toBe(
				'rgb(255, 238, 170)'
			);
		} );
		...
		it( 'applies the waveform background gradient', () => {
			...
			expect( readStyle( waveformContainer ).backgroundImage ).toBe(
				'linear-gradient(90deg, rgb(255, 0, 0) 0%, rgb(0, 0, 255) 100%)'
			);
		} );
		...
		it( 'applies the waveform player text color variables', () => {
			...
			const style = readStyle( container );
			expect( style.getPropertyValue( '--wfp-text-color' ) ).toBe(
				'#0000ff'
			);
			expect(
				style.getPropertyValue( '--wfp-text-secondary-color' )
			).toBe( '#0000ff' );
		} );
		...
		it( 'applies the waveform player play button color variable', () => {
			...
			expect(
				readStyle( container ).getPropertyValue( '--wfp-button-color' )
			).toBe( '#ff0000' );
		} );
		...
		it( 'applies the waveform player play button gradient', () => {
			...
			expect( readStyle( playButton ).backgroundImage ).toBe(
				'linear-gradient(90deg, rgb(255, 0, 0) 0%, rgb(0, 0, 255) 100%)'
			);
			expect(
				readStyle( container ).getPropertyValue( '--wfp-button-color' )
		...
		it( 'removes the waveform background color when cleared', () => {
			...
			expect( readStyle( waveformContainer ).backgroundColor ).not.toBe(
				'rgb(255, 238, 170)'
			);
		} );
	...
	describe( 'styleSvgIcons', () => {
		it( 'should set white fill for dark button colors', () => {
			...
			expect( readStyle( path ).fill ).toBe( 'rgb(255, 255, 255)' );
		} );
		...
		it( 'should set black fill for light button colors', () => {
			...
			expect( readStyle( path ).fill ).toBe( 'rgb(0, 0, 0)' );
		} );
		...
		it( 'should style multiple SVG paths', () => {
			...
			expect( readStyle( path1 ).fill ).toBe( 'rgb(255, 255, 255)' );
			expect( readStyle( path2 ).fill ).toBe( 'rgb(255, 255, 255)' );
		} );
	...
	describe( 'setupPlayButtonArtwork', () => {
		...
		it( 'should not modify play button icon paths', () => {
			...
			expect( readStyle( path ).fill ).not.toBe( 'rgb(255, 255, 255)' );
		} );
