...
function render_block_core_post_navigation_link( $attributes, $content ) {
	...
	/*
	 * Border and spacing serialization is skipped for this block so those styles
	 * can be withheld from the empty wrapper rendered when there is no adjacent
	 * post. The wrapper itself is kept for backward compatibility.
	 */
	$support_styles = '' === $content
		...
		: block_core_post_navigation_link_get_border_and_spacing_styles( $attributes );
...
/**
 * Generates the border and spacing class names and inline styles for the
 * `core/post-navigation-link` block.
 *
 * Mirrors the default border and spacing block support serialization, which the
 * block opts out of so the styles are only applied when a link is rendered.
 *
 * @since 7.2.0
 *
 * @param array $attributes The block attributes.
 * @return array {
 *     Border and spacing class names and inline styles.
 *
 ...
 * }
 */
function block_core_post_navigation_link_get_border_and_spacing_styles( $attributes ) {
