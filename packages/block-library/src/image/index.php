...
function gutenberg_get_image_width_height( $block_attributes, $image_meta = null ) {
	...
	if ( empty( $block_attributes['fileWidth'] ) || empty( $block_attributes['fileHeight'] ) ) {
		// External image? May be scaled.
		if ( $block_attributes['align'] !== 'full' &&
			$block_attributes['align'] !== 'wide' &&
			! empty( $block_attributes['scale'] ) &&
			$block_attributes['scale'] < 1 ) {

			// Scale is relative to the block width.
			$image_width = round( $block_witdh['default'] * $block_attributes['scale'] );
	...
	if ( $image_file_width < $image_meta['width'] &&
		...
		// TODO: shall we upscale an image here..?
	...
	if ( $block_attributes['align'] === 'full' ) {
		...
		// TODO: allow upscaling? Looks ugly, but some may expect it?
...
function render_block_core_image( $block_attributes = array(), $html = '' ) {
	...
	if ( ! empty( $block_attributes['id'] ) ) {
		...
		// Something's wrong. Broken or very old image meta? Incorrect attachment id?
	...
	$image_attributes = array(
		...
		'height' => ! empty( $image_dimensions[1] ) ? (int) $image_dimensions[1] : '',
	);
