...
const DiscussionsBoard = ( {
	...
	useEffect(() => {
		const threadClassName = contentRef.current?.classList?.value
			.split(' ')
			.find(className => className.startsWith('block-editor-collab__'));

		setThreadId( threadClassName?.slice('block-editor-collab__'.length || null ) );
	}, [contentRef]);

	// Fetch the current post, current user, and the selected block clientId.
	const { postId, allThreads, currentThread, currentUser, clientId } = useSelect( ( select ) => {
		const post       = select( editorStore ).getCurrentPost();
		const collabData = post?.meta?.collab ? JSON.parse( post.meta.collab ) : [];
		...
		return { 
			...
			clientId: select( blockEditorStore ).getSelectedBlockClientId() || null,
		}
	}, [ threadId ] );
	...
	const getUpdatedComments = (newComment, threadKey) => ({
		...
		[threadKey]: {
			isResolved,
			comments: [...(allThreads[threadKey]?.comments || []), newComment],
		},
	});
	...
	const saveComment = async () => {
		...
		const updatedComments = getUpdatedComments(newComment, threadKey);
	...
	const markThreadAsResolved = async (resolved) => {
		...
		if (resolved) {
			...
			updateBlockAttributes(clientId, { className: clientId.replace(`block-editor-collab__${threadId}`, '') });
		} else {
			updatedComments[threadId] = { ...updatedComments[threadId], isResolved: resolved };
		}
	...
	const deleteComment = async (commentId) => {
		...
		if (currentComments.length === 0) {
			...
			updatedComments[threadId] = { ...allThreads[threadId], comments: currentComments };
		}
	...
	return (
		...
		{ showConfirmation && (
			...
			>
				...
				<Button variant='primary' onClick={ () => confirmAndMarkThreadAsResolved() }>
					{__('Yes')}
				</Button>
