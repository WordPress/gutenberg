...
const ALLOWED_BLOCKS = variations.map((v) => `core/social-link-${v.name}`);
...
function SocialLinksEdit({
	...
	function renderPlaceholder() {
		return [...new Array(innerBlocks.length || 1)].map((_, index) => (
	...
	if (!shouldRenderFooterAppender && activeInnerBlocks.length === 0) {
		return (
			<View style={styles.placeholderWrapper}>{renderPlaceholder()}</View>
		);
	}
