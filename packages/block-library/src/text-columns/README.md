# Text Columns (deprecated)

<!-- START TOKEN(Autogenerated block API docs) -->
**Name:** `core/text-columns`
**Category:** [design](/block-editor/reference-guides/core-blocks/core-blocks-design/)
**API Version:** [3](/block-editor/reference-guides/block-api/block-api-versions/)
**Block Type:** [Static](/block-editor/getting-started/fundamentals/static-dynamic-rendering/) (saved in post content)

> This block is deprecated. Please use the Columns block instead.

## Attributes

_Defined via the [`attributes`](/block-editor/reference-guides/block-api/block-attributes/) property in block.json._

| Attribute | [Type](/block-editor/reference-guides/block-api/block-attributes/#type-validation) | [Default](/block-editor/reference-guides/block-api/block-attributes/#default-value) | Description |
|-----------|------|---------|-------------|
| `content` | `array` | `[{},{}]` | [Source](/block-editor/reference-guides/block-api/block-attributes/#value-source): `query`. [Selector](/block-editor/reference-guides/block-api/block-attributes/#value-source): `p` |
| `columns` | `number` | `2` | — |
| `width` | `string` | — | — |

## Supports

_Defined via the [`supports`](/block-editor/reference-guides/block-api/block-supports/) property in block.json._

- [`inserter`](/block-editor/reference-guides/block-api/block-supports/#inserter): `false`
- [`interactivity`](/block-editor/reference-guides/block-api/block-supports/#interactivity):
  - `clientNavigation`: `true`

## Block Markup

This is a [**static block**](/block-editor/getting-started/fundamentals/static-dynamic-rendering/). The markup is saved directly in the post content.

```html
<!-- wp:text-columns {"content":[{},{}],"columns":2} -->
<!-- Content... -->
<!-- /wp:text-columns -->
```

## Source

- [block.json](https://github.com/WordPress/gutenberg/tree/trunk/packages/block-library/src/text-columns/block.json) ([reference](/block-editor/reference-guides/block-api/block-metadata/))
- [edit.js](https://github.com/WordPress/gutenberg/tree/trunk/packages/block-library/src/text-columns/edit.js)
- [save.js](https://github.com/WordPress/gutenberg/tree/trunk/packages/block-library/src/text-columns/save.js)

<!-- END TOKEN(Autogenerated block API docs) -->
