...
describe('when an image is attached', () => {
	it('edits the image', async () => {
		const { getByLabelText, getByText } = render(
			<CoverEdit attributes={attributes} setAttributes={setAttributes} />
		);
	...
	it('edits the focal point with a slider', async () => {
		...
		fireEvent(getByTestId('Slider Y-Axis Position'), 'valueChange', '52');
	...
	it('edits the focal point with a text input', async () => {
		...
		fireEvent.press(editFocalPointButton);
		fireEvent.press(getByText((attributes.focalPoint.x * 100).toString()));
	...
	it('clears the media within cell button', async () => {
		...
		const clearMediaButton = await waitFor(() => getByText('Clear Media'));
...
describe('color settings', () => {
	it('sets a color for the overlay background when the placeholder is visible', async () => {
		...
		const block = await waitFor(() => getByA11yLabel('Cover block. Empty'));
		expect(block).toBeDefined();

		// Select a color from the placeholder palette
		const colorPalette = await waitFor(() => getByTestId('color-palette'));
		...
		const colorPaletteButton = await waitFor(() => getByTestId(COLOR_PINK));
	...
	it('sets a gradient overlay background when a solid background was already selected', async () => {
		...
		// Open the gradients
		const gradientsButton = await waitFor(() => getByA11yLabel('Gradient'));
