...
function render_block_core_comments_pagination_next( $attributes, $content, $block ) {
	...
	$arrow              = isset( $block->context['paginationArrow'] ) ? $block->context['paginationArrow'] : '';
	$next_comments_link = get_next_comments_link( sprintf( '%1s %2s', $label, $arrow ), $max_page );

	$wrapper_attributes = get_block_wrapper_attributes( array( 'class' => 'wp-block-next-comments-link' ) );
