...
export default function ReusableBlockEdit({ attributes: { ref }, clientId }) {
	...
	const { isMissing, hasResolved, hasEdits } = useSelect(
		(select) => {
			...
			const hasResolvedBlock = select(coreStore).hasFinishedResolution('getEntityRecord', [
				'postType',
				'wp_block',
				ref,
			]);
			const blockHasEdits = select(coreStore).hasEditsForEntityRecord('postType', 'wp_block', ref);
			return {
		...
		<RecursionProvider>
			<div {...blockProps}>
				...
				<InspectorControls>
					<PanelBody>
						<TextControl label={__('Name')} value={title} onChange={setTitle} />
					</PanelBody>
				</InspectorControls>
				<BlockContentOverlay
					clientId={clientId}
