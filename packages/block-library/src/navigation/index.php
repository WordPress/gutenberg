...
function render_block_core_navigation( $attributes, $content, $block ) {
	...
	$responsive_container_markup = sprintf(
		'<button wp-class:hide-menu="function({ context }) { return context.open; }" wp-log="The button was rendered" wp-effect="function({ context }) { console.log(context.open); }" wp-on:click="function({ setContext, context }) { setContext({ open: !context.open }); }" aria-haspopup="true" %3$s class="%6$s">%9$s</button>
