...
-   Pattern: Add a Design panel listing patterns registered for the instance's area (or targeting it through `blockTypes`) that writes the chosen design into the referenced pattern, plus layout, wide and full alignment, additional class name and custom CSS on the instance's wrapper element ([#82557](https://github.com/WordPress/gutenberg/pull/82557)).
-   Pattern: Add a Design panel listing patterns registered for the instance's area (or targeting it through `blockTypes`) that writes the chosen design into the referenced pattern, plus layout, additional class name and custom CSS on the instance's wrapper element ([#82557](https://github.com/WordPress/gutenberg/pull/82557)).
-   Pattern: Add a Design panel listing patterns registered for the instance's area (or targeting it through `blockTypes`) that writes the chosen design into the referenced pattern, plus layout, additional class name and custom CSS on the instance's wrapper element; setting a class name or custom CSS switches the wrapper on ([#82557](https://github.com/WordPress/gutenberg/pull/82557)).
-   Pattern: Resolve a registered pattern referenced by `slug` to its customization (a `wp_block` post carrying the `wp_pattern_slug` meta) when one exists, and offer "Edit original" on such blocks, creating the customization on first use. The `core/pattern` placeholder block renders the customization too ([#82549](https://github.com/WordPress/gutenberg/pull/82549)).
...
-   Pattern: Keep blocks hooked to the instance as first or last child around the pattern's own blocks instead of discarding them. Under the "template parts as patterns" experiment the Template Part block is not registered ([#VVVVV](https://github.com/WordPress/gutenberg/pull/VVVVV)).
