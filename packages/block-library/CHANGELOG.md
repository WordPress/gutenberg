...
-   Embed: Declare the provider variations in `block.json` — each with the URL `patterns` that attribute an address to it and, where the provider serves one kind of media, its oEmbed type — so the editor and the server-side conversion match URLs against the same list, and a provider variation registered from PHP through the `get_block_type_variations` filter is matched like a declared one. The JavaScript entries keep only what data cannot hold: the icons and the `isActive` callbacks ([#82013](https://github.com/WordPress/gutenberg/pull/82013)).
