...
-   Navigation Link: Search every entity type from the link UI instead of only the link's own type, so a category, tag or post can be added without leaving the search box. The suggestions shown before anything is typed still lead with the link's own type ([#BBBBB](https://github.com/WordPress/gutenberg/pull/BBBBB)).
