...
-   Navigation: Suppress the overlay of a Navigation block an overlay template part reaches through a pattern, a synced pattern or a nested template part. Those expand their content in a render pass of their own, so the walk that suppressed nested overlays never saw the block, and it opened a second overlay inside the first and nested a `<nav>` landmark inside another.
