...
-   Term Template: Fall back to the Terms Query block's defaults when the markup leaves out `termQuery` keys such as `order`, `orderBy` or `hideEmpty`, instead of logging "Undefined array key" warnings and rendering empty terms in an unspecified order ([#83161](https://github.com/WordPress/gutenberg/pull/83161)).
