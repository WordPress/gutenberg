...
-   Pattern: Add optional `tagName` and `area` attributes. `area` (a template part area such as `header`, from the instance or from the pattern's registration) provides the default wrapper element, the icon and title, and a "Replace" action listing patterns of the same area; `tagName` wraps the rendered pattern in a `header`, `main`, `section`, `article`, `aside`, `footer` or `div` element regardless of area. Both have controls under Advanced ([#82551](https://github.com/WordPress/gutenberg/pull/82551)).
