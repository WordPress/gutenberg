...
-   Image, Gallery: On an already-published post, media selected into these blocks that is not attached to any post is now proposed for attaching, and appears in the save panel alongside any other pending changes so it can be reviewed or skipped per item. Before publishing, the pre-publish panel makes the same offer ([#66663](https://github.com/WordPress/gutenberg/issues/66663)).
