...
-   Navigation Link: Search every entity type from the link UI instead of only the link's own type, so a category, tag or post can be added without leaving the search box. Results lead with the link's own type, as do the suggestions shown before anything is typed. Attachments and post formats are left out, since a Navigation cannot point at either ([#83299](https://github.com/WordPress/gutenberg/pull/83299)).
