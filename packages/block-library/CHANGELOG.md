...
-   Preformatted, Verse, Code: Fix `WP_Embed::autoembed()` turning a plain-text URL on its own line into an oEmbed card on the front end. The Code block's existing escaping for this only ever protected the first isolated URL in a block; a second or third one (e.g. two Twitter links pasted into a Preformatted block) stayed unescaped. Preformatted and Verse now get the same, corrected protection ([#TBD]).
