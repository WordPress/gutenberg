...
-   Time to Read: Use the post from block context instead of the global post when calculating reading time, so the block displays the correct value inside a Query Loop on a Page when the post contains a `<!--more-->` tag.
