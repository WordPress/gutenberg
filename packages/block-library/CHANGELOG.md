...
-   Tab List: Resolve the caret position manually when a click on a tab title lands on the button itself (for example on an inline image, which doesn't receive pointer events), so the title's rich text still gets focus and inline images remain selectable and expose their width and alternative text controls ([#81497](https://github.com/WordPress/gutenberg/issues/81497)).
