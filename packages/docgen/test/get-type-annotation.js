...
describe('Type annotations', () => {
	...
	describe('simple types', () => {
		...
		it.each(keywordTypes)(
			...
			(paramType, expected) => {
				...
				expect(getTypeAnnotation(paramTag, node, 0)).toBe(expected);
			}
		);
		...
		it.each(keywordTypes)(
			...
			(returnType, expected) => {
				...
				expect(getTypeAnnotation(returnTag, node, 0)).toBe(expected);
			}
		);
	});
		...
		describe.each([
			...
			it.each([
				...
				(literalType, literalValue, literalResult) => {
					...
					expect(getTypeAnnotation(tag, node, 0)).toBe(literalResult);
				}
			);
		});
	});
	...
	describe('imports, parameterized types, rest types, operator types, type predicates, index accessed types', () => {
		const node =
			getImportsParameterizedRestOperatorPredicateIndexerTypeNode();
		...
		it('should get the type predicate return type', () => {
			expect(getTypeAnnotation(returnTag, node, 1)).toBe('foo is string');
		});
	});
	...
	describe('function argument array-destructuring', () => {
		...
		describe('tuple-type', () => {
			...
			it('should grab the whole type for the unqualified name', () => {
				expect(getTypeAnnotation(paramTag, node, 0)).toBe('[ T, S ]');
			});
