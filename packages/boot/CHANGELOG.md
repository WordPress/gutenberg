...
-   Size single-page layout to the framed `#wpcontent` under `admin-stage-rounded`, and expose `boot-is-full-canvas` so full-canvas editing can escape the rounded frame ([#NNNN](https://github.com/WordPress/gutenberg/pull/NNNN)).
...
-   Open an entity at the width its `viewport` search param asks for, so a navigation overlay meant for mobile is edited at mobile width ([#81617](https://github.com/WordPress/gutenberg/pull/81617)).
