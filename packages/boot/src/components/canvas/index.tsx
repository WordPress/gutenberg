	...
	const documentInfo = (
		<>
			<div className={ styles.document }>
				...
				{ canvas.previewLabel && (
					<span className={ styles[ 'document-label' ] }>
						{ canvas.previewLabel }
					</span>
				) }
				{ canvas.previewStatus === 'homepage' && (
					<DropdownMenu
						className={ styles[ 'document-options' ] }
						icon={ chevronDown }
...
function PreviewDeviceSwitcher( {
	...
	return (
		<ToggleGroupControl
			className={ styles[ 'device-switcher' ] }
...
function PreviewLiveSiteButton( { url }: { url?: string } ) {
	...
	return (
		<Button
			className={ styles[ 'live-site-button' ] }
			icon={ external }
...
function ExternalPreviewNotice( {
	...
	return (
		<Notice
			className={ styles[ 'external-notice' ] }
			status="warning"
			onDismiss={ onDismiss }
		>
			{ sprintf(
			...
			<a
				className={ styles[ 'external-notice-link' ] }
				href={ url }
				target="_blank"
...
function PreviewViewport( {
	...
	return (
		<div className={ styles.viewport } ref={ viewportRef }>
			<div
				ref={ frameRef }
				className={ clsx( styles.frame, `is-${ device }`, {
			...
			>
				...
				{ isResizable && (
					<>
						<button
							type="button"
							className={ `${ styles[ 'resize-handle' ] } is-start` }
						...
						<button
							type="button"
							className={ `${ styles[ 'resize-handle' ] } is-end` }
							aria-label={ __( 'Resize preview width' ) }
							title={ __( 'Resize preview width' ) }
							onPointerDown={ ( event ) =>
...
function PreviewToolbar( {
	...
	return (
		<div className={ styles.toolbar }>
			<div className={ styles[ 'toolbar-start' ] }>
				<PreviewEditButton
					editLink={ editLink }
...
export default function Canvas( { canvas }: CanvasProps ) {
	 ...
	 */
	const editorSettings = useMemo< Record< string, unknown > >( () => {
		...
		if ( canvas.isPreview ) {
			nextSettings.styles = [ { css: 'body{min-height:100vh;}' } ];
		}
	...
	// A route that points at a frontend URL previews the live site rather than
	// mounting an editor for it.
