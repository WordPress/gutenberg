...
describe('isKeyboardEvent', () => {
	...
	it('returns false for a superset of modifiers', () => {
		...
		const attachNode = attachEventListeners((event) => {
			expect(isKeyboardEvent.primary(event, 'm', isAppleOSFalse)).toBe(
				false
			);
		});
	...
	describe('primary', () => {
		...
		it('should identify modifier key when ⌘M is pressed', () => {
			expect.assertions(3);
			const attachNode = attachEventListeners((event) => {
				expect(isKeyboardEvent.primary(event, 'm', isAppleOSTrue)).toBe(
					true
				);
			});
	...
	describe('secondary', () => {
		it('should identify modifier key when Shift + Alt + Ctrl is pressed', () => {
			...
			const attachNode = attachEventListeners((event) => {
				expect(
					isKeyboardEvent.secondary(event, undefined, isAppleOSFalse)
				).toBe(true);
			});
	...
	describe('access', () => {
		...
		it('should identify modifier key when Shift + Alt + M is pressed', () => {
			expect.assertions(3);
			const attachNode = attachEventListeners((event) => {
				expect(isKeyboardEvent.access(event, 'm', isAppleOSFalse)).toBe(
					true
				);
			});
		...
		it('should identify modifier key when Ctrl + ⌥M is pressed', () => {
			expect.assertions(3);
			const attachNode = attachEventListeners((event) => {
				expect(isKeyboardEvent.access(event, 'm', isAppleOSTrue)).toBe(
					true
				);
			});
