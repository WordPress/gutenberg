...
export default class Editor extends Component {
	...
	render() {
		...
		const myMinHeight = Math.max( _minHeight, item.height );
		return (
			<View>
				<View style={ { flex: 1, flexDirection: 'row' } }>
					<Button title="Bold" color={ this.isFormatActive( 'bold' ) ? 'black' : 'gray' } onPress={ () => {
						this.onFormatPress( 'bold' );
					} } />
					<Button title="Italic" color={ this.isFormatActive( 'italic' ) ? 'black' : 'gray' } onPress={ () => {
						this.onFormatPress( 'italic' );
					} } />
					<Button title="Strikethrough" color={ this.isFormatActive( 'strikethrough' ) ? 'black' : 'gray' } onPress={ () => {
						this.onFormatPress( 'strikethrough' );
					} } />
				</View>
				<AztecView
					ref="_aztec"
