...
-   `--hot` – enables "Fast Refresh". The page will automatically reload if you make changes to the code. It requires that WordPress has the [`SCRIPT_DEBUG`](https://developer.wordpress.org/advanced-administration/debug/debug-wordpress/#script_debug) flag enabled. See [Fast Refresh](#fast-refresh) below for the additional setup it needs.
...
When you run `start --hot`, webpack enables [React Fast Refresh](https://webpack.js.org/guides/hot-module-replacement/) so that editing a component updates the page without a full reload and without losing component state. This mode needs a little extra setup that the default (non-hot) build does not.

##### Requirements

-   WordPress must have the [`SCRIPT_DEBUG`](https://developer.wordpress.org/advanced-administration/debug/debug-wordpress/#script_debug) constant set to `true`. WordPress Core registers the `wp-react-refresh-runtime` and `wp-react-refresh-entry` scripts (and adds them as dependencies of `react`) only when `SCRIPT_DEBUG` is enabled.
-   The [Gutenberg](https://wordpress.org/plugins/gutenberg/) plugin is **not** required. Earlier versions of these docs listed it as a requirement, but current versions of WordPress Core provide the Fast Refresh scripts on their own when `SCRIPT_DEBUG` is enabled.

##### Enqueue the runtime chunk

With `--hot`, webpack splits the webpack runtime into a separate `runtime.js` chunk (via `optimization.runtimeChunk: 'single'`) and emits it alongside your entry point in the output directory:
...
This `runtime.js` **must be enqueued before your entry script**, otherwise Fast Refresh will not work (and you may see runtime errors in the console). The `*.asset.php` files generated by [`DependencyExtractionWebpackPlugin`](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-dependency-extraction-webpack-plugin/) do not list this local chunk as a dependency, so it has to be enqueued manually. For example:
...
add_action(
	...
	function () {
		...
		// Only present when built with `--hot`; enqueue it first.
		if ( file_exists( $dir . 'runtime.js' ) ) {
			$runtime = require $dir . 'runtime.asset.php';
			wp_enqueue_script( 'my-plugin-runtime', $url . 'runtime.js', $runtime['dependencies'], $runtime['version'], true );
		}

		wp_enqueue_script( 'my-plugin', $url . 'index.js', $asset['dependencies'], $asset['version'], true );
	}
);
```

Because `runtime.js` only exists in the `--hot` build, guarding the enqueue with `file_exists()` lets the same code work for both `start --hot` and a regular production `build` (which inlines the runtime into the entry script).

##### Custom local domains (`--allowed-hosts`)

The dev server defaults to [`allowedHosts: 'auto'`](https://webpack.js.org/configuration/dev-server/#devserverallowedhosts), which only accepts requests for `localhost`. If you develop against a custom local domain (for example `mysite.local`, or a proxied wp-env/Local/Valet site), the dev server responds with `Invalid Host header` and Fast Refresh fails to connect. Pass `--allowed-hosts` — which is forwarded to `webpack serve` — to allow your domain. Use the `=` form:

```bash
# Allow a specific domain and its subdomains.
wp-scripts start --hot --allowed-hosts=.mysite.local

# Allow any host (least restrictive; convenient for local-only development).
wp-scripts start --hot --allowed-hosts=all
```
