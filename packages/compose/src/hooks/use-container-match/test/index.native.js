...
const TestComponent = ( { query } ) => {
	...
	return (
		<View onLayout={ onLayout } matches={ matches } />
	);
};
...
describe( 'useContainerMatch()', () => {
	it( 'should return "{ isSmall: false }"', () => {
		const component = renderWithOnLayout( <TestComponent query={ { isSmall: '< 200' } } /> );
		expect( component.props.matches ).toMatchObject( { isSmall: false } );
	} );

	it( 'should return "{ isMedium: true }"', () => {
		const component = renderWithOnLayout( <TestComponent query={ { isMedium: '> 250' } } /> );
		expect( component.props.matches ).toMatchObject( { isMedium: true } );
	} );

	it( 'should return "{ isSmall: false, isMedium: true, isLarge: false }"', () => {
		const component = renderWithOnLayout( <TestComponent query={ { isSmall: '< 200', isMedium: '> 250', isLarge: '> 400' } } /> );
		expect( component.props.matches ).toMatchObject( { isSmall: false, isMedium: true, isLarge: false } );
	} );

	it( 'should return "{ isMedium: true }" using default operator', () => {
		const component = renderWithOnLayout( <TestComponent query={ { isMedium: '200' } } /> );
		expect( component.props.matches ).toMatchObject( { isMedium: true } );
	} );
} );
