...
Used to filter the format type settings. It receives the settings and the name of the format to be registered as arguments.

_Example:_

Ensure a custom class is added to text using the bold format:

```js
const addCustomClassBoldFormat = ( settings, name ) => {
	if ( 'core/bold' !== name ) {
		return settings;
	}
	...
	return {
		...settings,
		className: 'my-custom-class-bold',
	};
};
addFilter( 'richText.registerFormatType', 'my-plugin/class-names/bold' , addCustomClassBoldFormat );
```
