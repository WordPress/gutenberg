...
When a token name appears on the left-hand side of a CSS declaration (for example, `'--wpds-color-fg-content-neutral: red;'`), the `bareToken` check is skipped for that occurrence. Those cases are instead reported by [`no-setting-ds-tokens`](./no-setting-ds-tokens.md).
