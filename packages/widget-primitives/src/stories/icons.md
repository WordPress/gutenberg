...
The metadata module may still declare a rendered element (`icon: calendar` from `@wordpress/icons`); a resolved reference from the record wins over it, and the element stands when the record carries none.
...
`useWidgetTypes` resolves references while it assembles each `WidgetType`, at the same boundary that resolves field-type names. Hosts receive a renderable `icon` and never see a reference; anything that is not an element is dropped at the boundary.
