...
`HostLink` is the anchor consumers mount for a link target. It reads the capability and mounts the application's own link or a plain anchor, so no consumer branches on it.
