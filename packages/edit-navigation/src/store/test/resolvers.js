...
describe( 'getNavigationPostForMenu', () => {
	...
	it( 'adds the colors to the main navigation block', () => {
		const menuId = 123;
		...
		// Get the core/navigation block
		const blockAttrs = persistPostAction.args[ 2 ].blocks[ 0 ].attributes;

		// Post link
		expect( blockAttrs ).toEqual(
			expect.objectContaining( {
				textColor: 'blue',
				backgroundColor: 'red',
				overlayTextColor: 'green',
		...
		expect( blockAttrs ).toEqual(
			expect.not.objectContaining( {
				className:
					'menu classes has-text-color has-blue-color has-background-color has-red-background-color',
			} )
		);
	} );
} );
