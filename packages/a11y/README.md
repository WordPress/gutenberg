...
This is the counterpart to the `useReducedMotion` hook in `@wordpress/compose`, for code that runs outside React such as block view scripts. It reads the preference once; callers that need to react to it changing should watch the media query themselves.
...
- `boolean`: Whether reduced motion is preferred.
