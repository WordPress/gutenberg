...
-   Save failures: Offer the failure to the clipboard. A save error notice now renders as a component rather than as raw HTML in a notice string, so every failure carries a copy button, ready to paste into a search or an assistant. Where the server said more than the notice does, that detail sits behind a `Collapsible` disclosure and stays findable by the browser's find-in-page while collapsed. Markup in a server message is stripped rather than the message being dropped ([#82496](https://github.com/WordPress/gutenberg/pull/82496)).
