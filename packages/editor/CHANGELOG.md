...
-   Suggest mode: resolve both halves of a suggested block replacement together. A block-switcher transform, a Group, or a paste over a selection is captured as a removal plus an insertion; accepting only the insertion left the original block behind as a duplicate, and rejecting only it left a hole. Both halves now carry a shared group id and are resolved by one decision, reporting through a single snackbar ([#81684](https://github.com/WordPress/gutenberg/pull/81684)).
