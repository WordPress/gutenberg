 ...
 * @return {?Object} The `core/suggestion` format at that character, or undefined.
 */
 ...
 * @param {Object} record Rich-text record.
 * @param {number} index  Character index.
 * @return {string|undefined} Marker id.
 */
function suggestionIdAt( record, index ) {
 ...
 *
 * @param {Object} record Rich-text record.
 * @param {number} index  A character index known to carry the marker.
 * @param {string} id     Marker id.
 * @return {?{start: number, end: number}} The marker's range.
 */
function markerRun( record, index, id ) {
	let start = index;
 ...
 *
 * @param {Object}        prev     Record before the edit.
 * @param {Object}        next     Record after the edit.
 * @param {Object}        range    Changed range from `analyzeFormatEdit`.
 * @param {number|string} authorId Id of the user making this edit.
 * @return {?{id: string, attributes: Object, range: {start:number, end:number}}}
 * The marker to extend, or null.
 */
function findExtendableFormatMarker( prev, next, range, authorId ) {
 ...
 * @param {number} start  Range start.
 * @param {number} end    Range end.
 * @return {string} HTML of the sliced run, marker-free.
 */
function sliceToUnmarkedHTML( record, start, end ) {
	return sliceToHTML(
 ...
 * @param {number|string} [options.authorId] Id of the user making the edit;
 *                                           required to extend their own marker.
 * @return {{ kind: 'format'|'none', range?: {start:number, end:number},
 *           beforeHTML?: string, afterHTML?: string, extendsId?: string,
 *           markerAttributes?: Object }} Plan.
 */
export function planFormatMarkers( prevValue, nextValue, { authorId } = {} ) {
	const prev = toRecord( prevValue );
	const next = toRecord( nextValue );
	if ( ! prev || ! next ) {
		return { kind: 'none' };
 ...
 * @param {Object}        [options]
 * @param {number|string} [options.id]       Marker (note) id; ignored by an extending plan.
