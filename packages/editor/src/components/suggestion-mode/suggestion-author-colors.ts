 ...
 *
 * @param threads Unresolved note threads.
 * @return Author id to display name, in thread order.
 */
function getInlineSuggestionAuthors( threads?: any[] ): Map< number, string > {
 ...
 *
 * @param value Decoded display name.
 * @return Value safe to place between double quotes.
 */
 ...
 *
 * @param threads Unresolved note threads (each with `author`, `author_name`
 *                and the `_wp_suggestion` meta payload).
 * @return Serialized CSS targeting `.wp-suggestion-a11y` pseudo-elements.
 */
export function buildSuggestionAuthorAnnouncementCss(
	threads?: any[]
): string {
	/*
