 ...
 * to the raw target.
 *
 * @param event Input/clipboard event.
 * @return The element the edit lands in, or null.
 */
function resolveEventElement( event: any ): any {
 ...
 *
 * @param event          Input/clipboard event.
 * @param selectionStart Block-editor `getSelectionStart()` value.
 * @return True when the event targets the selected rich text.
 */
export function isEventTargetSelectedRichText(
	event: any,
	selectionStart: any
): boolean {
 ...
 * @param options.preferTargetRanges Whether to consult
 *                                   `getTargetRanges()` before the live
 *                                   selection. Defaults to true.
 * @return Normalized offsets, or null.
 */
export function readEventRange(
	event: any,
	{ preferTargetRanges = true }: { preferTargetRanges?: boolean } = {}
 ...
 *
 * @param editable Rich-text editable root.
 * @param range    Range within the editable.
 * @return Normalized offsets, or null.
 */
function mapRangeToOffsets(
	editable: any,
	range: any
): { start: number; end: number } | null {
	try {
 ...
 * @param attributeKey Attribute key the write targets, when known.
 * @return Normalized offsets, or null.
 */
export function readLiveInlineSelection(
	clientId?: string,
