 ...
 * @return Attributes without suggestion metadata, returned by reference when
 ...
 * @return The block with suggestion state removed, returned by reference when
 * nothing changed.
 */
export function stripSuggestionDataFromBlock( block: any ): any {
 ...
 *
 * @param blocks Blocks about to be serialized to the clipboard.
 * @return Blocks without suggestion state, returned by reference when nothing
 * changed.
 */
export function stripSuggestionDataFromBlocks( blocks: any ): any {
	if ( ! Array.isArray( blocks ) ) {
		return blocks;
	}
	let changed = false;
	const next = blocks.map( ( block: any ) => {
