 ...
 */
function markContentEditable( element: Element ) {
 ...
 *
 * @param options
 * @param options.clientId     Block client id.
 * @param options.attributeKey `data-wp-block-attribute-key` value.
 * @return Elements.
 */
function createBlockEditable( {
	clientId,
	attributeKey,
}: {
	clientId: string;
...
describe( 'readEventRange', () => {
	 ...
	 *
	 * @return The editable and its two text nodes.
	 */
...
describe( 'readLiveInlineSelection', () => {
	 ...
	 * @return The editable and its two text nodes.
	 */
	function createBlockWithMark( { clientId }: { clientId: string } ) {
		const block = document.createElement( 'div' );
		block.setAttribute( 'data-block', clientId );
		const editable = document.createElement( 'p' );
		editable.className = 'block-editor-rich-text__editable';
