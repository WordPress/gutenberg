...
├── floating-add-note.tsx            FloatingAddNote - on-select "Add note" button floated next to a text selection
...
NotesSidebarContainer (index.jsx)         - gates on post type support
 └── NotesSidebar (index.jsx)             - owns sidebarRef + useNoteThreads + sidebar registration
      ...
      ├── FloatingAddNote                - popover next to a live text selection (inline notes)
