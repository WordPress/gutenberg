...
const { MediaModal } = unlock( mediaEditorPrivateApis );
...
/**
 * Mounts the unified MediaModal alongside existing editor modals.
 *
 * Bridges `@wordpress/editor`'s `usePostFields('attachment')` hook
 * into the modal, since `@wordpress/media-editor` cannot depend on
 * `@wordpress/editor`. Hosts both the browse picker and the editor —
 * the inner mode (browse vs. edit) is driven by the `core/media-editor`
 * store.
 *
 * @return {Element} The MediaModal component wired with attachment fields.
 */
