...
describe( 'Post actions', () => {
	describe( 'Distributed Editing document dirty state', () => {
		it( 'detects content drift from the server-confirmed base even when core edits are clean', () => {
			const baseContent =
				'<!-- wp:paragraph --><p>Server confirmed base.</p><!-- /wp:paragraph -->';
			const editedContent =
				'<!-- wp:paragraph --><p>Locally retained edit.</p><!-- /wp:paragraph -->';
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
			...
			registry
				...
				.setDistributedEditingSessionState( {
					clientBaseVersion: '4',
					serverVersion: '4',
					distributedEditingPostStateHash: 'hash-4',
		...
		it( 'treats sync-meta-only and core namespace differences as clean', () => {
			const baseContent = `<!-- wp:core/paragraph --><p>Same document.</p><!-- /wp:core/paragraph --><script type="wp/post-sync-meta" data-sync-meta-format="diff-match-patch">{"version":"4"}</script>`;
			const editedContent =
				'<!-- wp:paragraph --><p>Same document.</p><!-- /wp:paragraph -->';
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
			...
			expect(
				...
				isDirty: false,
				baseCanonicalContent: editedContent,
				currentCanonicalContent: editedContent,
			} );
		} );
	...
	describe( '__experimentalOpenDistributedEditingRiskyBlockReview()', () => {
		...
		it( 'refreshes server-backed review items before opening the pre-publish sidebar', async () => {
			const registry = createRegistryWithStores();
			const dispatch = registry.dispatch( editorStore );
			const select = registry.select( editorStore );
			const originalBody = document.body.innerHTML;
			...
			const post = {
				id: postId,
				type: 'post',
				title: 'Server review item opener post',
				content:
					...
					items: [
						{
							...
							createdAtGmt: '2026-06-25T16:32:00',
							updatedAtGmt: '2026-06-25T16:32:00',
							expiresAtGmt: '2026-07-02T16:32:00',
							proposedContentHash:
								'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
			...
			try {
				...
				expect( select.isPublishSidebarOpened() ).toBe( true );
				expect(
		...
		it( 'does not open an empty pre-publish sidebar when refresh clears stale review rows', async () => {
			const registry = createRegistryWithStores();
			const dispatch = registry.dispatch( editorStore );
			const select = registry.select( editorStore );
			const post = {
				id: postId,
				type: 'post',
				title: 'Stale review opener post',
				content:
					'<!-- wp:paragraph --><p>Clean server content.</p><!-- /wp:paragraph -->',
				status: 'publish',
			};
			...
			expect( select.isPublishSidebarOpened() ).toBe( false );
			expect(
				select.getDistributedEditingRiskyBlockReviewState()
			).toMatchObject( {
	...
	describe( '__experimentalMaybeRouteSavePostToDistributedEditingRiskyBlockReview()', () => {
		it( 'blocks normal save without relabeling Save when the enabled risky-block policy has no approved items', async () => {
			const registry = createRegistryWithStores();
	...
	describe( '__experimentalRefreshDistributedEditingRiskyBlockReviewItems()', () => {
		...
		it( 'clears review UI state when the server returns only stale count metadata', async () => {
			const registry = createRegistryWithStores();
			const dispatch = registry.dispatch( editorStore );
			const select = registry.select( editorStore );
			const post = {
				id: postId,
				type: 'post',
				title: 'Stale review item count post',
				content:
					'<!-- wp:paragraph --><p>Clean server content.</p><!-- /wp:paragraph -->',
				status: 'publish',
			};
	...
	describe( '__experimentalSyncDistributedEditingWithServer()', () => {
		it( 'publishes the newly confirmed copy and refreshes presence after a clean server sync', async () => {
			...
			apiFetch.setFetchHandler( async ( options ) => {
				...
				if (
					...
					serverStateRefetchCalls++;
					serverStateRefetchRequest = options;
			...
			expect( serverStateRefetchCalls ).toBe( 1 );
			expect( serverStateRefetchRequest.headers ).toEqual( {
				'If-None-Match': '"base-hash"',
			} );
			expect( serverStateRefetchRequest.parse ).toBe( false );
			expect( heartbeatCalls ).toBe( 1 );
			expect( presenceSnapshotCalls ).toBe( 1 );
			expect( normalSaveCalls ).toBe( 0 );
			expect( autosaveCalls ).toBe( 0 );
		...
		it( 'keeps unsafe block review pending when server sync sees the safe body is current', async () => {
			const safeServerContent =
				'<!-- wp:paragraph --><p>Safe author edit.</p><!-- /wp:paragraph --><!-- wp:html --><div>Original HTML.</div><!-- /wp:html -->';
			const post = {
				id: postId,
				type: 'post',
	...
	describe( '__experimentalSaveDistributedEditingRetryAfterProof()', () => {
		...
		it( 'applies partial-safe retry-save content to the editor while keeping unsafe review pending', async () => {
			...
			const authorUnsafeContent =
				'<!-- wp:paragraph --><p>Safe author edit.</p><!-- /wp:paragraph --><!-- wp:html --><script>alert("unsafe")</script><!-- /wp:html -->';
			const safeServerContent =
				'<!-- wp:paragraph --><p>Safe author edit.</p><!-- /wp:paragraph --><!-- wp:html --><div>Original HTML.</div><!-- /wp:html -->';
	...
	describe( 'savePost()', () => {
		...
		it( 'uses normal savePost for new auto-draft posts even when DE-RTC retry-save is enabled', async () => {
			const firstSavedContent = 'Created from post-new.php';
			const secondEditContent = 'Edited again after the first save.';
			const post = {
				id: postId,
				type: 'post',
				title: '',
			...
			apiFetch.setFetchHandler( async ( options ) => {
				...
				if (
					...
					retrySaveCalls++;
					throw {
						code: 'unexpected_retry_save',
						message: 'New posts must use normal WordPress save.',
					};
				}
			...
			registry
				...
				.receiveEntityRecords( 'postType', 'post', post );
			registry.dispatch( editorStore ).setupEditor( post, {
				title: 'New DE-RTC post',
				content: firstSavedContent,
				status: 'draft',
			} );
			registry.dispatch( editorStore ).updateEditorSettings( {
				distributedEditing: {
			...
			expect(
				registry.select( editorStore ).isEditedPostSaveable()
			).toBe( true );
			expect(
				registry
					.select( editorStore )
		...
		it( 'routes a dirty edit after confirmed retry-save through a fresh guarded save', async () => {
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
		...
		it( 'routes explicit retry-save protected local changes through freshness and guarded save', async () => {
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
