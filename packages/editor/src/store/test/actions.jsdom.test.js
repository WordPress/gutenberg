...
describe( 'Post actions', () => {
	...
	describe( 'savePost()', () => {
		...
		it( 'keeps the failure detail for save failures with an error message', async () => {
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
			...
			expect(
				unlock( registry.select( editorStore ) ).getSaveErrorNotice()
			).toEqual( {
				message:
					'Updating failed. We’ll try to save a backup in this browser. Please try updating again.',
				detail: 'Details from server.',
			} );
			// The notice is rendered by `SaveErrorNotice`, so that the detail
			// can be disclosed and copied, rather than by the notices store.
			expect( registry.select( noticesStore ).getNotices() ).toEqual(
				[]
			);
		} );

		it( 'strips markup from an error message rather than dropping it', async () => {
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
			...
			apiFetch.setFetchHandler( async ( options ) => {
				...
				if (
					...
					throw {
						code: 'test_save_failure',
						message:
							'<p>Details from <strong>server</strong>.</p><p>Try again.</p>',
					};
				}
			...
			expect(
				...
				// The block boundary survives as a line break, so the two
		...
		[
			...
			it( `omits the details disclosure for ${ code } failures`, async () => {
				...
				).toEqual( { message: expectedNotice, detail: null } );
			} );
		} );

		it( 'clears the failure notice once a save succeeds', async () => {
			const post = {
				id: postId,
				type: 'post',
				title: 'bar',
			...
			apiFetch.setFetchHandler( async ( options ) => {
				...
				if (
					...
					return { ...post, content: 'new bar' };
				}

				// Requested by the success notice, which needs the post type's
				// labels.
				if (
					method === 'GET' &&
					path.startsWith( '/wp/v2/types/post' )
			...
			shouldFail = false;
			registry.dispatch( editorStore ).editPost( { content: 'newer' } );
			await registry.dispatch( editorStore ).savePost();
