...
export function* savePost( options = {} ) {
	// TODO: If edits can be managed for multiple core data entities, the save
	// procedure should be reconsidered as persisting all outstanding edits, not
	// just for the single post being edited.
...
export function* resetEditorBlocks( blocks, options = {} ) {
	...
	// Defining blocks as a non-enumerable property prevents it from being
	// counted as dirtying or as a meaningful edit for persistence.
	Object.defineProperty( edit, '_blocks', {
		enumerable: false,
		value: yield* getBlocksWithSourcedAttributes( blocks ),
	} );

	if ( options.__unstableShouldCreateUndoLevel !== false ) {
		// Assigning content as a property descriptor allows for it to be
		// counted as dirtying, accurately reflecting the current blocks value,
		// while still only computing it when explicitly referenced (usually at
		// save time, but not necessarily so).
		Object.defineProperty( edit, 'content', {
			enumerable: true,
			// The `core-data` module manages edits by preserving property
			// descriptors. If not configurable, the property cannot be deleted
			// in the "cloned" edits state.
			// TODO: Consider whether this can be avoided as a concern for an
			// implementer, where `core-data` state could avoid using the
			// `delete` keyword and instead otherwise omit the property from
			// the clone.
			configurable: true,
			get() {
				// Since blocks may be more recently updated than the last time
				// content was assigned (as in multiple consecutive edits to
				// content treated as a single sequence of edits), a WeakMap is
				// used as a cache for the serialized result of blocks, per
				// blocks reference.
				...
				Object.defineProperty( this, 'content', {
					...
					configurable: true,
					get() {
						if ( ! cache.has( this._blocks ) ) {
							let blocksForSerialization = this._blocks;
							...
							cache.set( this._blocks, content );
						}

						return cache.get( this._blocks );
					},
				} );
