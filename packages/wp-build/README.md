...
| Stage | Plugins Applied | Input | Description |
|-------|-----------------|-------|-------------|
| `ltr` | autoprefixer → custom plugins | Source CSS | Generates LTR stylesheet |
| `rtl` | rtlcss | LTR output | Generates RTL stylesheet from LTR |
| `minify` | cssnano | CSS file | Minifies CSS for production |

**Note:** The RTL stylesheet is generated by applying rtlcss to the LTR output, ensuring consistent processing and avoiding duplicate autoprefixer/custom plugin execution.
