...
export default function StylesCanvas() {
	...
	return (
		<EditorContentSlotFill.Fill>
			<div className="edit-site-styles-canvas">
				<ResizableEditor enableResizing={ false }>
					{ /* eslint-disable-next-line jsx-a11y/no-noninteractive-element-interactions */ }
					<section
						className="edit-site-styles-canvas__section"
						ref={ focusOnMountRef }
					...
					>
						<Button
							size="compact"
							className="edit-site-styles-canvas__close-button"
							icon={ closeSmall }
							label={ __( 'Close' ) }
...
/**
 * Hook to check if the styles canvas is currently active.
 *
 * @return {boolean} Whether the styles canvas has content to display.
 */
export function useHasStylesCanvas() {
	const fills = useSlotFills( EditorContentSlotFill.name );
	return !! fills?.length;
}

// Backward compatibility exports
export { useHasStylesCanvas as useHasEditorCanvasContainer };
export { getStylesCanvasTitle as getEditorCanvasContainerTitle };
