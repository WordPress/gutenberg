...
-   **Not the widget's permissions.** A widget asks the server about its own entities, through core-data, and reflects the answer. The policy never asks the widget anything; the widget only sees one consequence, the same it would see in any read-only host: without `edit`, it receives no `setAttributes`.
