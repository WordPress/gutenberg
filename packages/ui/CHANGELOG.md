...
-   `SearchableChipSelect`, `SearchableChipSelectControl`, `Combobox.ChipWithRemove`: When items are selected, the chips toolbar is labelled, each chip is named from its label, each remove button uses a shared `Remove` name described by that label, each chip describes Backspace or Delete removal, and the search input includes the selected count and how to move to those chips. A Field or consumer description stays in place. When the field is read-only or disabled, the input keeps the selected count and omits the move instruction, and chips omit the Backspace or Delete hint. ([#82863](https://github.com/WordPress/gutenberg/pull/82863))
