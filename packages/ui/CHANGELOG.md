...
-   `Dialog` / `AlertDialog` / `Drawer`: Sticky header and footer chrome when the popup scrolls. `Dialog.Header` / `Dialog.Footer` and `Drawer.Header` / `Drawer.Footer` gain `sticky` (default `true`); `AlertDialog.Popup` gains `stickyHeader` and `stickyFooter` (default `true`) for the built-in title/description stack and the actions stack (error message included in the bottom chrome). Separator borders only show when there is off-screen content in that direction. Opt out on `Dialog` / `Drawer` with `sticky={ false }` on each subcomponent; on `AlertDialog` with `stickyHeader` / `stickyFooter` ([#77559](https://github.com/WordPress/gutenberg/pull/77559)).
