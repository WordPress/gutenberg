...
Use the `--_gcd-*` prefix only inside this package; treat these variables as implementation details, not public theming API. The `plugin-wpds/no-global-css-defense-custom-properties` Stylelint rule (shipped from this package) enforces this outside `@wordpress/ui` (this package disables it in `.stylelintrc.mjs`).
