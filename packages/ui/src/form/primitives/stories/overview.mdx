...
When a control is already accessibly labeled but still needs a visual label or description for layout consistency, use `Field.VisualLabel` and `Field.VisualDescription` outside `Field.Root`. These components do not carry semantics and are not associated with a control.
