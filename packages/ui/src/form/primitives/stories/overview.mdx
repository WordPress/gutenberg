...
When a control is already accessibly labeled but still needs a visual label for layout consistency, use `Field.VisualLabel` outside `Field.Root`. It does not carry semantics and is not associated with a control. Description text under a field or fieldset should use `Field.Description` or `Fieldset.Description` so it is associated.
