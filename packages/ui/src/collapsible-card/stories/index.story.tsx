...
/**
 * When `Card.FullBleed` is the **first child** of `CollapsibleCard.Header`, it
 * should extend flush to the card&apos;s top and side edges — same pattern as
 * `Card` — while content that follows stays padded.
 */
	...
	args: {
		...
		children: (
			<>
				...
				<CollapsibleCard.Content>
					<Text>
						The image above should bleed to the card&apos;s top and
						side edges when the panel is open.
					</Text>
				</CollapsibleCard.Content>
			</>
		),
...
/**
 * When `Card.FullBleed` is the **only child** of `CollapsibleCard.Header`, it
 * fills the header trigger area — same as `Card.FullBleedHeroOnly`.
 */
