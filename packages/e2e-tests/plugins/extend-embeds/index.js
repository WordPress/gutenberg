( function() {
	...
	const { data, apiFetch, hooks, element, components, editor, i18n, blockLibrary } = wp;
	...
	const { __ } = i18n;
	const { registerEmbedBlock } = blockLibrary;
	...
	const redditEmbedBlockSettings = {
	...
	const addEnhancedRedditBlockToEmbedDiscovery = ( blockSettings ) => {
		// Make sure that our enhanced reddit block is at the top of the
		// list so it is discovered first for URL -> Embed Block resolution.
		return [
			redditEmbedBlockSettings,
			...blockSettings,
		];
	};
	...
	addFilter(
		'blockLibrary.Embed.blockResolutionList',
		'test/extend-embeds',
		addEnhancedRedditBlockToEmbedDiscovery
	);

	registerEmbedBlock( redditEmbedBlockSettings );
