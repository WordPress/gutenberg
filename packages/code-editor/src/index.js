 ...
 */
import { isObject, filter, isEqual, without, contains, delay } from 'lodash';
...
function configureLinting( editor, settings ) {
	...
	function updateErrorNotice() {
		if ( settings.onUpdateErrorNotice && ! isEqual( currentErrorAnnotations, previouslyShownErrorAnnotations ) ) {
	...
	function getLintOptions() { // eslint-disable-line complexity
		...
		if ( 'javascript' === settings.codemirror.mode && settings.jshint ) {
			Object.assign( options.options, settings.jshint );
		}

		// Configure CSSLint.
		if ( 'css' === settings.codemirror.mode && settings.csslint ) {
			Object.assign( options.options, settings.csslint );
		}

		// Configure HTMLHint.
		if ( 'htmlmixed' === settings.codemirror.mode && settings.htmlhint ) {
			options.options.rules = Object.assign( {}, settings.htmlhint );
		...
		options.onUpdateLinting = ( function( onUpdateLintingOverridden ) {
			return function( annotations, annotationsSorted, cm ) {
				const errorAnnotations = filter( annotations, function( annotation ) {
				...
				if ( isEqual( errorAnnotations, currentErrorAnnotations ) ) {
					return;
				}
	...
	jQuery( document.body ).on( 'mousedown', function( event ) {
		if (
			...
			! jQuery.contains( editor.display.wrapper, event.target ) &&
			! jQuery( event.target ).hasClass( 'CodeMirror-hint' )
		) {
			updateErrorNotice();
		}
	} );
...
function configureTabbing( codemirror, settings ) {
	const $textarea = jQuery( codemirror.getTextArea() );
...
/**
	 ...
	 */
export function initialize( textarea, settings ) {
	let $textarea;
	...
	const instanceSettings = Object.assign( {}, defaultSettings, settings );
	instanceSettings.codemirror = Object.assign( {}, instanceSettings.codemirror );

	const codemirror = CodeMirror.fromTextArea( $textarea[ 0 ], instanceSettings.codemirror );
	...
	if ( codemirror.showHint ) {
		codemirror.on( 'keyup', function( editor, event ) { // eslint-disable-line complexity
			...
			const innerMode = CodeMirror.innerMode( codemirror.getMode(), token.state ).mode.name;
