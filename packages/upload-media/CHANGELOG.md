...
-   The error shown when a HEIC file cannot be converted now names the browser that failed and lists the browsers that do decode HEIC on the visitor's operating system, since HEIC decoding depends on OS-provided codecs. Exposed as the new `getHeicUnsupportedMessage` and `getHeicConversionAdvice` exports ([#81123](https://github.com/WordPress/gutenberg/issues/81123)).
