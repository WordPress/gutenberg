			...
			if ( isAnimated ) {
				...
				if ( ! hasTransparency ) {
					...
					if ( settings.gifConvert === 'prompt' ) {
						...
						// Ask the host to prompt the user. The gif conversion
						// lifecycle record above outlives this prompt; the
						// prompt only drives the modal.
						dispatch.requestUploadPrompt( {
							type: 'gif-conversion',
							itemId: id,
						} );
					}
