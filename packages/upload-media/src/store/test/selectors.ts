...
describe( 'selectors', () => {
	...
	describe( 'getItemNextRetryTimestamp', () => {
		it( 'should return the timestamp for an item with a scheduled retry', () => {
		...
		it( 'should return undefined for a non-existent item', () => {
			...
			expect(
				getItemNextRetryTimestamp( state, 'non-existent' )
			).toBeUndefined();
		} );
	} );
	...
	describe( 'getItemRetryCount', () => {
		...
		it( 'should return 0 for a non-existent item', () => {
			const state: State = {
				queue: [],
				queueStatus: 'active',
				blobUrls: {},
			...
			expect( getItemRetryCount( state, 'non-existent' ) ).toBe( 0 );
		} );
	} );
	...
	describe( 'hasExceededMaxRetries', () => {
		...
		it( 'should return true when retry count exceeds max retries', () => {
			const state: State = {
				queue: [
					{
						id: '1',
		...
		it( 'should use default max of 3 when retry settings are not configured', () => {
			const state: State = {
				queue: [
					{
						id: '1',
		...
		it( 'should return false for a non-existent item', () => {
			...
			// retryCount defaults to 0, which is less than 3
			expect( hasExceededMaxRetries( state, 'non-existent' ) ).toBe(
				false
			);
		} );
	} );
