...
describe( 'reducer', () => {
	...
	describe( `${ Type.CacheBlobUrl }`, () => {
		it( 'caches a blob URL for an item', () => {
			const initialState: State = {
				queueStatus: 'active',
				blobUrls: {},
				settings: {
			...
			const state = reducer( initialState, {
				...
				id: '1',
				blobUrl: 'blob:http://example.com/1',
			} );

			expect( state.blobUrls[ '1' ] ).toEqual( [
				'blob:http://example.com/1',
			] );
		} );

		it( 'appends to existing blob URLs for an item', () => {
			const initialState: State = {
				queueStatus: 'active',
				blobUrls: {
					'1': [ 'blob:http://example.com/1' ],
				},
				settings: {
					mediaUpload: jest.fn(),
				},
			...
			const state = reducer( initialState, {
				type: Type.CacheBlobUrl,
				id: '1',
				blobUrl: 'blob:http://example.com/2',
			} );

			expect( state.blobUrls[ '1' ] ).toEqual( [
				'blob:http://example.com/1',
				'blob:http://example.com/2',
			] );
		} );
	} );

	describe( `${ Type.RevokeBlobUrls }`, () => {
		it( 'removes blob URLs for an item', () => {
			const initialState: State = {
				queueStatus: 'active',
				blobUrls: {
					'1': [ 'blob:http://example.com/1' ],
					'2': [ 'blob:http://example.com/2' ],
				},
				settings: {
					mediaUpload: jest.fn(),
				},
			...
			expect( state.blobUrls[ '1' ] ).toBeUndefined();
			expect( state.blobUrls[ '2' ] ).toEqual( [
				'blob:http://example.com/2',
			] );
		} );
	} );
