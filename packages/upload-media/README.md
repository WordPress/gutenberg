...
Failed items are removed from the queue just like successful ones, so an empty queue on its own says nothing about whether anything was uploaded. This tally only ever grows: to learn how many items failed within a single batch, read it when the batch starts and subtract that from its later value.
...
-   `number`: Number of failed uploads since the editor loaded.
