...
In-flight uploads are mirrored to IndexedDB so they can be resumed after a page
reload or browser crash. On the next editor load, interrupted uploads are
detected and the user is offered the choice to **Resume** or **Discard** them.

Resume continues from the furthest completed pipeline step: an upload that was
interrupted after client-side processing but before the network upload resumes
at the upload step rather than reprocessing the image.

Reconnecting a resumed upload to its block requires the block to still exist
after the reload, which happens when the post was saved or locally autosaved
while the in-progress upload was present. When no matching block exists, the
upload still completes and the attachment appears in the Media Library.

Persistence is enabled by default and degrades gracefully when IndexedDB is
unavailable (e.g. some private-browsing modes), falling back to the in-memory
queue. Set the `durableQueue` setting to `false` to disable it.
