...
Call this from the main thread before processing a JXL image. The bytes come from the lazily loaded `wasm-vips/vips-jxl.wasm` module; they are wrapped in a Blob URL on demand (see `getWasmUrl`).

Takes a bare `ArrayBuffer` rather than a typed array so the RPC layer recognises it as transferable: comctx only fast-paths `ArrayBuffer` and `Array`, and walks anything else key by key, which for a multi-megabyte typed array means millions of allocations on the calling thread.

_Parameters_

-   _bytes_ `ArrayBuffer`: Inlined bytes of the vips-jxl.wasm module.
