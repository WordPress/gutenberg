...
Call this from the main thread before processing a JXL image. The bytes are the inlined `Uint8Array` from the lazily loaded `wasm-vips/vips-jxl.wasm` module; they are wrapped in a Blob URL on demand (see `getWasmUrl`).

_Parameters_

-   _bytes_ `Uint8Array< ArrayBuffer >`: Inlined bytes of the vips-jxl.wasm module.
