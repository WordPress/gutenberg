...
This is significantly faster than calling resizeImage() for each size, because decoding compressed formats like AVIF is expensive. By decoding once and using thumbnailImage() for each size, we avoid redundant decode work.
...
-   _sizes_ `BatchResizeSize[]`: Array of sizes to generate, each with a name and resize options.
-   _quality_ Desired quality (0-1).

_Returns_

-   `Promise< Record< string, BatchResizeResult > >`: Map of size names to processed file data plus dimensions.
