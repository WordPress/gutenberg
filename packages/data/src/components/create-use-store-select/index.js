...
export default function createUseStoreSelect( store ) {
	return function storeSpecificSelect( mapSelect, deps ) {
		// eslint-disable-next-line react-hooks/rules-of-hooks
		return useStoreSelect( store, mapSelect, deps );
	};
}
 ...
 */
function useStoreSelect( storeKey, _mapSelect, deps = [] ) {
	if ( Array.isArray( _mapSelect ) ) {
		deps = deps.concat( _mapSelect );
		const query = _mapSelect;
		_mapSelect = ( selectors ) =>
			query.map( ( selector ) => selectors[ selector ]() );
	}

	const mapSelect = useCallback( _mapSelect, [ storeKey, ...deps ] );
	...
	useIsomorphicLayoutEffect( () => {
		...
		const unsubscribe = registry.stores[ storeKey ].subscribe( onChange );
		return () => {
