...
-   `manifest.json`: New `_builtin` property, set on the icons WordPress renders in the admin bar and admin menu. It additionally registers the icon in the reserved `_builtin` collection, which is hidden from the icons REST API and, unlike `core`, cannot be unregistered. ({TODO})
