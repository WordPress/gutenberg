...
-   `manifest.json`: New `admin` property, set on the icons the WordPress admin interface renders. It additionally registers the icon in the `core-admin` collection, always as a non-public icon there, so that unregistering `core` cannot take it away from the admin interface. ([#83261](https://github.com/WordPress/gutenberg/pull/83261))
