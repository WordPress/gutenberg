...
-   `manifest.json`: New `admin` property, set on the icons the WordPress admin interface renders (`brush`, `chevron-left`, `comment`, `dashboard`, `home`, `link`, `media`, `menu`, `page`, `pencil`, `people`, `pin`, `plugins`, `plus`, `search`, `settings`, `sites`, `tool`, `update` and `wordpress`). It ships the icon to WordPress Core and registers it in the new `core-admin` collection, which is not public, so the icon is not exposed through the icons REST API or selectable in the Icon block, and unregistering `core` cannot take it away from the admin interface. ([#83261](https://github.com/WordPress/gutenberg/pull/83261), [#79451](https://github.com/WordPress/gutenberg/pull/79451))
