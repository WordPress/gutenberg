...
3. **Add the icon to `manifest.json`**: Add an entry for your new icon in `src/manifest.json`. The entry should include:
   ...
   - `keywords` (optional): An array of additional translatable search terms for the icon (e.g. `[ "hamburger", "navigation" ]` for a Menu icon). The icons registry matches them alongside the icon's name and label, and they are localized in the generated `manifest.php`. No bundled icon sets them yet.
