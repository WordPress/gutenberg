...
3. **Add the icon to `manifest.json`**: Add an entry for your new icon in `src/manifest.json`. The entry should include:
   ...
   - `keywords` (optional): An array of lowercase search terms for the icon, such as `[ "hamburger", "navigation" ]` for Menu. Any entry may have them: the icons registry and the icon library match them alongside the name and label, and so does the Storybook icon library, which reads this manifest rather than keeping its own list. Terms for icons shipped to Core are localized in the generated `manifest.php`.
