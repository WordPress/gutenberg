...
3. **Add the icon to `manifest.json`**: Add an entry for your new icon in `src/manifest.json`. The entry should include:
   ...
   - `admin` (optional): Set to `true` for an icon the WordPress admin interface renders. The icon then ships to WordPress Core and is registered in the `core-admin` collection, which is not public, so it is never exposed through the REST API or selectable in the Icon block. Independent of `public`: an icon can be in one collection, the other, or both.
