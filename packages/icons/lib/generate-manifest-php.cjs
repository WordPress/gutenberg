...
function generatePHPArray( manifest ) {
	...
	const phpEntries = manifest.map( ( item ) => {
		...
		'filePath' => '${ filePath }',${ publicEntry }${ adminEntry }
	),`;
	} );
 ...
 * Only includes icons which have `public` or `admin` set to true. Each entry
 * repeats those two flags, which tell the plugin's PHP registration which of
 * the `core` and `core-admin` collections the icon belongs to.
 */
async function generateManifestPHP() {
	const manifestJson = await readFile( MANIFEST_JSON_PATH, 'utf8' );
	const manifest = JSON.parse( manifestJson );
	const includedIcons = manifest.filter(
		( item ) => item.public === true || item.admin === true
	);
	const phpHeader = `<?php
...
`;
	const phpArray = generatePHPArray( includedIcons );
