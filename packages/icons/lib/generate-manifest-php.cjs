...
function generatePHPArray( manifest ) {
	...
	const phpEntries = manifest.map( ( item ) => {
		...
		'filePath' => '${ filePath }',${ publicEntry }${ builtinEntry }
	),`;
	} );
 ...
 * Only includes icons which have the `public` or `_builtin` key set.
 */
async function generateManifestPHP() {
	...
	const includedIcons = manifest.filter(
		( item ) => typeof item.public === 'boolean' || item._builtin === true
	);
	const phpHeader = `<?php
