...
const meta: Meta< typeof ThemeProvider > = {
	...
	argTypes: {
		...
		isRoot: {
			isRoot: false,
			description:
				'The `isRoot` prop is already set on the root instance that is part of Storybook global setup',
		},
		color: {
			control: false,
			description:
				'Use the global theme control in the top toolbar to set the color scheme',
		},
	},
	parameters: {
...
// No need to render a `ThemeProvider`, since controls are disabled and
// ThemeProvider is already being rendered in the global Storybook setup.
...
export const WithPicker: StoryObj< typeof ThemeProvider > = {
	render: ( args, { globals } ) => {
		const [ primary, setPrimary ] = useState( globals.primary );
		const [ fun, setFun ] = useState( 0 );

		useEffect( () => {
			setPrimary( globals.primary );
		}, [ globals.primary ] );
		...
		return (
			<ThemeProvider
				{ ...args }
				color={ {
					...JSON.parse( globals.themeColorPreset ),
					primary,
				} }
			>
					...
					>
						...
						<div>
							<input
								...
								step={ 0.05 }
								onChange={ ( e ) =>
									setFun( Number( e.target.value ) )
								}
							/>
							<label htmlFor="fun">Pick the amount of fun</label>
						</div>
					</div>
					{ args.children }
				</div>
	...
	args: {
		children: <DSTokensList />,
		// color: { bg: '#f0f0f0', primary: '#3858e9' },
	},
};
...
/**
 * Note: the following example is not impacted by the global theme control.
 */
...
export const AcrossIFrames: StoryObj< typeof ThemeProvider > = {
	render: ( args, { globals } ) => {
		return (
			<ThemeProvider
				{ ...args }
				color={ JSON.parse( globals.themeColorPreset ) }
			>
				{ args.children }
