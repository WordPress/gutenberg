...
-   A block type whose icon is a string of SVG markup now renders that icon. The markup is parsed with `DOMParser` and converted to an element during registration, so a `block.json` icon inlined from a `file:` path — by the build step or by server-side registration — displays in the inserter without registering the block in JavaScript ([#79658](https://github.com/WordPress/gutenberg/pull/79658)).
