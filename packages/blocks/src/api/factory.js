...
const isPossibleTransformForSource = ( transform, direction, blocks ) => {
	...
	const isValidForMultiBlocks =
		isWildcardBlockTransform( transform ) || ! isMultiBlock || transform.isMultiBlock;
...
const getBlockTypesForPossibleToTransforms = ( blocks ) => {
	...
	const blockNames = flatMap( possibleTransforms, ( transformation ) => transformation.blocks );
...
export const isWildcardBlockTransform = ( t ) =>
	t && t.type === 'block' && Array.isArray( t.blocks ) && t.blocks.includes( '*' );
...
export function getBlockTransforms( direction, blockTypeOrName ) {
	...
	if ( blockTypeOrName === undefined ) {
		return flatMap( getBlockTypes(), ( { name } ) => getBlockTransforms( direction, name ) );
	}
...
export function switchToBlockType( blocks, name ) {
	...
	const transformation =
		...
		findTransform(
			...
			( t ) =>
				t.type === 'block' &&
				( isWildcardBlockTransform( t ) || t.blocks.indexOf( sourceName ) !== -1 ) &&
				( ! isMultiBlock || t.isMultiBlock )
		);
...
export const getBlockFromExample = ( name, example ) => {
	return createBlock(
		name,
		example.attributes,
		map( example.innerBlocks, ( innerBlock ) => getBlockFromExample( innerBlock.name, innerBlock ) )
	);
};
