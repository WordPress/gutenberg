...
export const blockTraverser = ( joinResults, initialValue, getNextResult ) => {
	const recurse = ( block ) => {
		let output = initialValue;
		let blockIndex = 0;

		for ( const content of block.innerContent ) {
			output = joinResults(
				output,
				getNextResult(
					null === content ? block.innerBlocks[ blockIndex++ ] : content,
					recurse
				)
			);
		}
