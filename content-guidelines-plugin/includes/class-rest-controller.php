 ...
 */
class CG_REST_Controller extends WP_REST_Posts_Controller {
	...
	public function __construct() {
		parent::__construct( CG_Post_Type::POST_TYPE );
	}
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'category' => array(
							'description'       => __( 'Limit response to a specific guideline category.', 'content-guidelines' ),
							'type'              => 'string',
							'enum'              => CG_Post_Type::VALID_CATEGORIES,
							'sanitize_callback' => 'sanitize_text_field',
						),
						'block'    => array(
							'description'       => __( 'Limit response to guidelines for a specific block type.', 'content-guidelines' ),
							'type'              => 'string',
							'sanitize_callback' => 'sanitize_text_field',
						),
						'status'   => array(
							'description'       => __( 'Limit response to guidelines with a specific status.', 'content-guidelines' ),
							'type'              => 'string',
							'enum'              => CG_Post_Type::VALID_STATUSES,
							'sanitize_callback' => 'sanitize_text_field',
						),
					),
				),
	...
	public function get_guidelines_permissions_check( $request ) { // phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
		...
		if ( ! current_user_can( $post_type->cap->read ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'Sorry, you are not allowed to view the guidelines.', 'content-guidelines' ),
	...
	public function create_item( $request ) {
		...
		if ( $existing ) {
			return new WP_Error(
				...
				__( 'Guidelines already exist. Use PATCH to update.', 'content-guidelines' ),
				array( 'status' => 400 )
			);
		}
		...
		$prepared->post_title = __( 'Guidelines', 'content-guidelines' );
	...
	public function prepare_item_for_response( $post, $request ) {
		...
		if ( rest_is_field_included( 'guideline_categories', $fields ) ) {
			$guideline_categories = CG_Post_Type::get_guideline_categories_from_meta( $post->ID );
	...
	protected function save_guideline_categories_to_meta( $post_id, $categories ) {
		...
		foreach ( CG_Post_Type::CATEGORY_META_KEYS as $category ) {
		...
		if ( isset( $categories['blocks'] ) && is_array( $categories['blocks'] ) ) {
			foreach ( $categories['blocks'] as $block_name => $block_data ) {
				$meta_key = CG_Post_Type::block_name_to_meta_key( $block_name );
	...
	protected function sanitize_guideline_categories( $categories ) {
		...
		$valid_categories = CG_Post_Type::VALID_CATEGORIES;
	...
	public function get_item_schema() {
		...
		$this->schema = array(
			...
			'properties' => array(
				'id'                   => array(
					'description' => __( 'Unique identifier for the guidelines.', 'content-guidelines' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
				'status'               => array(
					'description' => __( 'The status of the guidelines (draft or publish).', 'content-guidelines' ),
					'type'        => 'string',
					'enum'        => CG_Post_Type::VALID_STATUSES,
					'context'     => array( 'view', 'edit' ),
				),
				'guideline_categories' => array(
					'description' => __( 'The guideline categories and their content.', 'content-guidelines' ),
					'type'        => 'object',
					'context'     => array( 'view', 'edit' ),
					'arg_options' => array(
						'validate_callback' => static function ( $value ) {
							if ( ! is_array( $value ) && ! is_object( $value ) ) {
								return new WP_Error(
									'rest_invalid_param',
									__( 'guideline_categories must be a JSON object.', 'content-guidelines' ),
									array( 'status' => 400 )
								);
							}
							return true;
				...
				'date'                 => array(
					'description' => __( 'The date the guidelines were created, in the site\'s timezone.', 'content-guidelines' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'date_gmt'             => array(
					'description' => __( 'The date the guidelines were created, as GMT.', 'content-guidelines' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'modified'             => array(
					'description' => __( 'The date the guidelines were last modified, in the site\'s timezone.', 'content-guidelines' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'modified_gmt'         => array(
					'description' => __( 'The date the guidelines were last modified, as GMT.', 'content-guidelines' ),
					'type'        => 'string',
					'format'      => 'date-time',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				...
				'author'               => array(
					'description' => __( 'The ID of the author of the guidelines.', 'content-guidelines' ),
					'type'        => 'integer',
					'context'     => array( 'view', 'edit' ),
					'readonly'    => true,
				),
