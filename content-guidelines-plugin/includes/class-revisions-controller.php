 ...
 */
class CG_Revisions_Controller extends WP_REST_Revisions_Controller {
		...
		register_rest_route(
			...
			array(
				array(
					...
					'args'                => array(
						'parent' => array(
							'description' => __( 'The ID for the parent of the revision.', 'content-guidelines' ),
							'type'        => 'integer',
						),
						'id'     => array(
							'description' => __( 'Unique identifier for the revision to restore.', 'content-guidelines' ),
							'type'        => 'integer',
						),
					),
				),
	...
	public function prepare_item_for_response( $item, $request ) {
		...
		if ( rest_is_field_included( 'guideline_categories', $fields ) ) {
			...
			$guideline_categories         = CG_Post_Type::get_guideline_categories_from_meta( $item->ID );
	...
	public function restore_revision_permissions_check( $request ) {
		...
		if ( ! $parent || $this->parent_post_type !== $parent->post_type ) {
			return new WP_Error(
				'rest_post_not_found',
				__( 'Guidelines not found.', 'content-guidelines' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! current_user_can( $post_type->cap->edit_posts ) ) {
			return new WP_Error(
				'rest_cannot_restore',
				__( 'Sorry, you are not allowed to restore revisions.', 'content-guidelines' ),
	...
	public function restore_revision( $request ) {
		...
		if ( ! $revision || 'revision' !== $revision->post_type || (int) $revision->post_parent !== (int) $parent->ID ) {
			return new WP_Error(
				...
				__( 'Revision not found.', 'content-guidelines' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( ! $result ) {
			return new WP_Error(
				'rest_cannot_restore',
				__( 'Could not restore revision.', 'content-guidelines' ),
				array( 'status' => 500 )
			);
		}
