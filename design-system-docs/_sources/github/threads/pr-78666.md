...
type: pull_request
state: MERGED
merged_at: 2026-05-27T19:55:41Z
url: https://github.com/WordPress/gutenberg/pull/78666
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Code Quality","[Package] Theme"]
---

# PR #78666: @wordpress/theme: deduplicate addFallbackToVar helper

- URL: https://github.com/WordPress/gutenberg/pull/78666
- Author: ciampo
- Created: 2026-05-26T09:06:41Z
- Updated: 2026-05-27T19:56:25Z
- Comments: 6 of 6
- Review threads: 1 of 1
- Changed files: 4 of 4
- Diff: ../pr-diffs/78666.diff

## Body

## What?

Addresses task **B4** from [#77462](https://github.com/WordPress/gutenberg/issues/77462) — deduplicates the `addFallbackToVar` helper that lived in both `postcss-plugins/add-fallback-to-var.ts` and `postcss-plugins/ds-token-fallbacks.mjs`.
...
- New `packages/theme/src/postcss-plugins/add-fallback-to-var.mjs` holds the single generic implementation (takes the token fallback map as a parameter, with optional `escapeQuotes`). JSDoc preserves the type information that the previous `.ts` file expressed inline.
- `packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs` is now a thin wrapper that pre-binds the package's generated fallback map and re-exports `addFallbackToVar`. The PostCSS, esbuild, and Vite plugins keep their existing imports unchanged.
- The old `add-fallback-to-var.ts` is removed; its companion unit test now imports the new `.mjs` source.

## Testing Instructions

- `npm run test:unit -- packages/theme` — all 38 tests still pass (including the 14-case `add-fallback-to-var` suite and the 5-case prebound `ds-token-fallbacks` suite).
- The published `./postcss-plugins/postcss-ds-token-fallbacks`, `./esbuild-plugins/esbuild-ds-token-fallbacks`, and `./vite-plugins/vite-ds-token-fallbacks` entry points are unchanged on disk, so external consumers see no behavior change.

## Checklist

- [x] My code is tested.
- [x] My code follows the WordPress code style.
- [x] I've tested my changes with keyboard and screen readers. _(N/A — internal build helper refactor with no user-facing surface.)_
- [x] My code has proper inline documentation.
- [x] I've included developer documentation if appropriate.
- [x] I've updated all React Native files affected by the changes. _(N/A.)_
- [x] I've added an entry to the changelog of packages I've changed.

## Issue comments

### github-actions on 2026-05-26T09:15:24Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4542675569
...
### github-actions on 2026-05-26T09:39:53Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4542945689

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 09abf9f01feb396dcf955199d9fbdc855917da05.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26443059759
📝  Reported issues:
- #77197 in `/test/e2e/specs/interactivity/router-navigate.spec.ts`

### aduth on 2026-05-26T20:51:25Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4548739177

Where was `add-fallback-to-var.ts` being used before these changes? As best I can tell, it wasn't, and we could remove it altogether in favor of what already existed in `ds-token-fallbacks`?

### github-actions on 2026-05-26T20:51:36Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4548740320
...
### ciampo on 2026-05-27T09:07:50Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4553104201

> Where was `add-fallback-to-var.ts` being used before these changes? As best I can tell, it wasn't, and we could remove it altogether in favor of what already existed in `ds-token-fallbacks`?

@aduth the duplication here was introduced by design, as explained in https://github.com/WordPress/gutenberg/pull/75586 's description:

> ### `addFallbackToVar` function
> A reusable function that takes a CSS value string and replaces bare `var(--wpds-*)` calls with `var(--wpds-*, <fallback>)`. It's idempotent (skips vars that already have a fallback). This is the core transform that downstream build plugins will use.
> 
> Two versions:
> 
> * `add-fallback-to-var.ts` — generic, takes the fallback map as a parameter (testable in isolation)
> * `ds-token-fallbacks.mjs` — pre-binds the generated fallback map for convenience

So, in this PR, we continue to honor the approach, but apply an optimization that was [initially deferred](https://github.com/WordPress/gutenberg/pull/75586#discussion_r2827674963)

### ciampo on 2026-05-27T19:54:24Z

URL: https://github.com/WordPress/gutenberg/pull/78666#issuecomment-4558153712
...
#### aduth on 2026-05-26T20:47:38Z

URL: https://github.com/WordPress/gutenberg/pull/78666#discussion_r3306754122
...
Maybe not directly related, but what's the point of this variable, vs. accessing the import value directly?

#### ciampo on 2026-05-27T09:22:26Z

URL: https://github.com/WordPress/gutenberg/pull/78666#discussion_r3309836935
Path: packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs
...
Leftover from previous PRs, but as you point out, it can be removed. Done!

## Changed files

- packages/theme/src/postcss-plugins/add-fallback-to-var.mjs (ADDED, +47/-0)
- packages/theme/src/postcss-plugins/add-fallback-to-var.ts (DELETED, +0/-44)
- packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs (MODIFIED, +9/-27)
- packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts (MODIFIED, +1/-1)
