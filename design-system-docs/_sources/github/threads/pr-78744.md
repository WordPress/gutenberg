...
type: pull_request
state: OPEN
url: https://github.com/WordPress/gutenberg/pull/78744
matched_labels: ["[Package] UI"]
all_labels: ["[Package] UI"]
---

# PR #78744: UI: Widen Pick'd upstream prop types to forward all `@base-ui/react` props

- URL: https://github.com/WordPress/gutenberg/pull/78744
- Author: ciampo
- Created: 2026-05-27T15:25:17Z
- Updated: 2026-06-10T15:53:07Z
- Comments: 6 of 6
- Review threads: 9 of 9
- Changed files: 6 of 6
- Diff: ../pr-diffs/78744.diff
...
### github-actions on 2026-05-27T15:35:05Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4556064072
...
| `build/modules/boot/index.min.js` | 19.4 kB |
...
| `build/scripts/editor/index.min.js` | 463 kB |
...
| `build/styles/editor/style-rtl.css` | 29.5 kB |
| `build/styles/editor/style-rtl.min.css` | 24.9 kB |
| `build/styles/editor/style.css` | 29.5 kB |
...
### github-actions on 2026-05-28T09:23:12Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4562542759

<!-- flaky-tests-report-comment -->
**Flaky tests detected in ff05cc2e9774a45ae1a9a8d1164a4cb6acb39994.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26564773593
📝  Reported issues:
- #78689 in `/test/e2e/specs/editor/plugins/wp-editor-meta-box.spec.js`
- #78706 in `/test/e2e/specs/editor/various/autosave.spec.js`

### ciampo on 2026-05-28T10:01:17Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4562885515

@WordPress/gutenberg-components as an experiment, I'm trying to widen the component types and, instead of `Pick`ing props from Base UI, just expose the whole props surface.

I asked an AI agent to comment which props are effectively added (and removed) for each component, and the results are written in the inline comments above.

From the start, it's obvious that, if we widen the types, we add support for features that we never tested and/or may not want to support — to demonstrate this, I added a comment for `Tooltip.Root` https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316869218

Before moving on with the effort, I think it's important that we calibrate on what approach we want to take

### mirka on 2026-05-29T19:47:25Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4579286150

I kind of share your hesitation for overexposing props on `Tooltip`, given how easily it can be misused. The misuse angle is probably less of a concern on higher-level overlays like `Drawer` or `AlertDialog`.

Personally I'm most concerned with the misuse angle. I don't mind if we expose upstream props without testing (people will report if they try an advanced prop and it doesn't work for some reason), and I don't mind changes in upstream behavior. Most use cases won't touch advanced props anyway, so I mostly see it as a balance between maintenance cost and misuse risk.

### aduth on 2026-06-03T21:01:31Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4616682660

I'll be curious to dive into this in more detail tomorrow, but my initial feeling is that maybe it's okay that we overexpose, even for higher-risk props, since these are low-level primitives that are designed with more flexibility in mind? I personally feel like our amount of intervention should slide in proportion to the level of abstraction of the component, and the lowest level of components should have the lowest amount of it (i.e. they're intended for more granular control and advanced use-cases).

At the very least, we could change our process from default-opt-in to default-opt-out and explicitly omit the props we're most concerned about.

### ciampo on 2026-06-10T15:53:07Z

URL: https://github.com/WordPress/gutenberg/pull/78744#issuecomment-4671977279

Exposing more props will require additional initial work in documenting and adding Storybook examples — something to keep in mind, not necessarily a blocker.

Re. misuse, we could add specific documentation (in Storybook pages, dedicated Storybook examples, and JSDocs) to guard against those misuses. Definitely not something that will ensure correct usage, but at least it's written somewhere.

## Review threads

### Review thread 1

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595597
Path: packages/ui/src/tooltip/types.ts
...
**`Tooltip.Root` — resolved props: 2 → 12 (+10)**

The runtime wrapper is `<_Tooltip.Root { ...props } />`, so the widening simply lifts the previously-`Pick`'d type to the full upstream surface.

- **Previously listed (Pick'd):** `children`, `disabled`
- **Newly exposed:** `actionsRef`, `defaultOpen`, `defaultTriggerId`, `disableHoverablePopup`, `handle`, `onOpenChange`, `onOpenChangeComplete`, `open`, `trackCursorAxis`, `triggerId`

#### ciampo on 2026-05-28T09:48:48Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316869218
Path: packages/ui/src/tooltip/types.ts
...
- Do we want to allow consumers to control the open state of the tooltip?
- `handle` needs to be exposed alongside the `handle` prop on `Tooltip.Trigger`, and the `Tooltip.createHandle()` function. Is that something we want to support? (see [detached triggers](https://base-ui.com/react/components/tooltip#detached-triggers))
- The `children` prop on base UI also supports a callback function that is used in conjunction with the `payload` prop on `Tooltip.Trigger` to re-use the same tooltip to display different contents depending on the trigger (see [multiple triggers](https://base-ui.com/react/components/tooltip#detached-triggers))

### Review thread 2

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595604
Path: packages/ui/src/popover/types.ts

```diff
@@ -7,29 +7,17 @@ export type PortalProps = ComponentProps< typeof _Popover.Portal >;
...
**`Popover.Root` — resolved props: 5 → 10 (+5)**

Runtime is `<_Popover.Root { ...props } />`, full pass-through.

- **Previously listed (Pick'd + local `children`):** `children`, `defaultOpen`, `modal`, `onOpenChange`, `open`
- **Newly exposed:** `actionsRef`, `defaultTriggerId`, `handle`, `onOpenChangeComplete`, `triggerId`

### Review thread 3

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595607
Path: packages/ui/src/popover/types.ts
...
**`Popover.Trigger` — net +3 base-ui props, −3 React legacy cleanup**

Runtime is `<_Popover.Trigger ref={ ref } { ...props } />`. Switching from `ComponentProps< 'button' > & Pick< _Popover.Trigger.Props, 'openOnHover' | 'delay' | 'closeDelay' >` to `ComponentProps< typeof _Popover.Trigger >` swaps in base-ui's `Trigger.Props` directly.

- **Newly exposed (base-ui-specific):** `handle`, `nativeButton`, `payload`
- **Still listed:** `closeDelay`, `delay`, `disabled`, `openOnHover`, local `children`, all `<button>` / `aria-*` / event-handler / form attributes (`form`, `formAction`, `formMethod`, `name`, `type`, `value`, …).
- **Removed (no real loss):** `color`, `defaultChecked`, `defaultValue` — React's `HTMLAttributes<HTMLButtonElement>` exposes these on every button for historical reasons; base-ui's `Trigger.Props` already drops them.

### Review thread 4

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595610
Path: packages/ui/src/popover/types.ts
...
**`Popover.Popup` — cleanup only (+0 base-ui props, −3 React legacy keys)**

Introspecting `_Popover.Popup.Props` confirms it only adds `initialFocus` / `finalFocus` on top of the `<div>` surface — the props I waved at in the PR description (`keepMounted`, `actionsRef`, etc.) actually live on `_Popover.Root`, not `_Popover.Popup`.

So the public-key delta is just:

- **Still listed:** `initialFocus`, `finalFocus`, local `backdrop` / `children` / `portal` / `positioner` / `variant`, plus the standard `<div>` / `aria-*` / event-handler surface.
- **Removed (junk):** `color`, `defaultChecked`, `defaultValue` (React's `<div>` legacy keys; base-ui's `Popup.Props` already drops them).

Value of the change here is type-definition cleanliness, not new API surface.

### Review thread 5

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595614
Path: packages/ui/src/dialog/types.ts
...
**`Dialog.Root` — resolved props: 7 → 11 (+4)**

Runtime is `<_Dialog.Root modal={ modal } { ...props }>` (with a `modal` default), so the widening lifts the previously-`Pick`'d set to the full upstream surface.

- **Previously listed (Pick'd + local `children`):** `children`, `defaultOpen`, `disablePointerDismissal`, `modal`, `onOpenChange`, `onOpenChangeComplete`, `open`
- **Newly exposed:** `actionsRef`, `defaultTriggerId`, `handle`, `triggerId`

`children` is intentionally narrowed back to `ReactNode` via `Omit< _Dialog.Root.Props, 'children' >`: the wrapper consumes children to wrap them in `DialogModalProvider`, so base-ui's render-fn-children form can't be honored.

### Review thread 6

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595621
Path: packages/ui/src/dialog/types.ts
...
**`Dialog.Popup` — cleanup only (+0 base-ui props, −3 React legacy keys)**

Same story as `Popover.Popup` above: `_Dialog.Popup.Props` only adds `initialFocus` / `finalFocus` on top of the `<div>` surface. Public-key delta is just the −3 cleanup of `color` / `defaultChecked` / `defaultValue` from React's `<div>` legacy. Locals (`children`, `portal`, `size`) and the standard HTML/ARIA surface are unchanged.

### Review thread 7

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595624
...
**`Drawer.Root` — resolved props: 8 → 17 (+9), the largest gain in the PR**

Runtime is `<_Drawer.Root modal={ modal } swipeDirection={ swipeDirection } { ...props }>` (with defaults for `modal` and `swipeDirection`), so the widening lifts the previously-`Pick`'d set to the full upstream surface.

- **Previously listed (Pick'd + locally-redeclared `swipeDirection` + local `children`):** `children`, `defaultOpen`, `disablePointerDismissal`, `modal`, `onOpenChange`, `onOpenChangeComplete`, `open`, `swipeDirection`
- **Newly exposed:** `actionsRef`, `defaultSnapPoint`, `defaultTriggerId`, `handle`, `onSnapPointChange`, `snapPoint`, `snapPoints`, `snapToSequentialPoints`, `triggerId`

Notes:
- `swipeDirection` is now inherited from upstream rather than re-declared locally; the only thing lost is the WP-flavored JSDoc on it. Happy to re-add an explicit interface override if we want to preserve that doc.
- `children` is narrowed back to `ReactNode` via `Omit< _Drawer.Root.Props, 'children' >`, same rationale as `Dialog.Root`.

### Review thread 8

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595630
Path: packages/ui/src/drawer/types.ts
...
**`Drawer.Popup` — cleanup only (+0 base-ui props, −3 React legacy keys)**

Same story as `Popover.Popup` / `Dialog.Popup`: `_Drawer.Popup.Props` only adds `initialFocus` / `finalFocus` on top of the `<div>` surface. Public-key delta is the −3 cleanup of `color` / `defaultChecked` / `defaultValue`. Locals (`children`, `portal`, `size`) and the standard HTML/ARIA surface are unchanged.

### Review thread 9

#### ciampo on 2026-05-28T09:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/78744#discussion_r3316595634
Path: packages/ui/src/alert-dialog/types.ts

```diff
@@ -62,8 +62,7 @@ export interface TriggerProps extends ComponentProps< 'button' > {
 }
 
 export interface PopupProps
```

**`AlertDialog.Popup` — cleanup only (+0 base-ui props, −3 React legacy keys)**

Same story as the other `*.Popup` widenings: `_AlertDialog.Popup.Props` only adds `initialFocus` / `finalFocus` on top of the `<div>` surface. Public-key delta is the −3 cleanup of `color` / `defaultChecked` / `defaultValue`. Locals (`cancelButtonText`, `children`, `confirmButtonText`, `description`, `intent`, `portal`, `stickyFooter`, `stickyHeader`, `title`) are unchanged.

For visibility: `AlertDialog.Root` is deliberately left Pick'd (no widening). The wrapper owns the confirm-state machine and overrides `onOpenChange` / `onOpenChangeComplete` / `actionsRef` on the underlying `_AlertDialog.Root`, so exposing those upstream props would be a footgun.

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +1/-0)
- packages/ui/src/alert-dialog/types.ts (MODIFIED, +1/-2)
- packages/ui/src/dialog/types.ts (MODIFIED, +7/-14)
- packages/ui/src/drawer/types.ts (MODIFIED, +6/-25)
- packages/ui/src/popover/types.ts (MODIFIED, +3/-15)
- packages/ui/src/tooltip/types.ts (MODIFIED, +1/-1)
