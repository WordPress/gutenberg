...
type: pull_request
state: MERGED
merged_at: 2026-01-16T16:25:04Z
url: https://github.com/WordPress/gutenberg/pull/74625
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] Private APIs","[Package] UI"]
---

# PR #74625: Add `Tooltip` component to `@wordpress/ui`

- URL: https://github.com/WordPress/gutenberg/pull/74625
- Author: retrofox
- Created: 2026-01-14T19:51:49Z
- Updated: 2026-01-29T12:54:13Z
- Comments: 4 of 4
- Review threads: 12 of 12
- Changed files: 11 of 11
- Diff: ../pr-diffs/74625.diff
...
### github-actions on 2026-01-14T19:52:00Z

URL: https://github.com/WordPress/gutenberg/pull/74625#issuecomment-3751377137
...
### github-actions on 2026-01-14T20:02:53Z

URL: https://github.com/WordPress/gutenberg/pull/74625#issuecomment-3751485360
...
| :--- | :---: | :---: |
| `build/scripts/private-apis/index.min.js` | 1.08 kB | +3 B (+0.28%) |
...
### ciampo on 2026-01-16T13:44:42Z

URL: https://github.com/WordPress/gutenberg/pull/74625#issuecomment-3760086608

@retrofox I'm going to take this PR to the finish line, since it's a dependency for both `Tabs` and `IconButton` (which I'm working on)

### retrofox on 2026-01-17T17:32:53Z

URL: https://github.com/WordPress/gutenberg/pull/74625#issuecomment-3764135060

> @retrofox I'm going to take this PR to the finish line, since it's a dependency for both `Tabs` and `IconButton` (which I'm working on)

Thanks, @ciampo. That's great becase we also need the IconButton component.

## Review threads

### Review thread 1 (resolved)

#### aduth on 2026-01-14T20:31:44Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2691949466
...
We probably want to set a `delay={ 0 }` on the provider to avoid real-time test delays.

### Review thread 2 (resolved)

#### aduth on 2026-01-14T20:32:25Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2691951436
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Building on my previous comment, if we negate the delay, I wonder if we can get rid of this? Human-time sleeps like this are not ideal for tests since they artificially increase the test runtime.

### Review thread 3

#### aduth on 2026-01-14T20:35:16Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2691959650
Path: packages/ui/src/tooltip/popup.tsx
...
I wonder if the tooltips should be consistently black, regardless of the theme. Existing tooltips show this way by default, where "default" is effectively "light" mode. We could wrap a `ThemeProvider` to apply a fixed background color. Ideally this would be using a CSS token property, but I'm not exactly sure if `ThemeProvider` supports that currently.

#### retrofox on 2026-01-14T21:44:48Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2692165558
Path: packages/ui/src/tooltip/popup.tsx
...
Wrapped the Tooltip popup with ThemeProvider using `color={{ bg: '#1e1e1e' }}` to ensure a consistent dark background regardless of theme mode. This required adding `@wordpress/ui` to the allowed private API modules and setting up lock-unlock to access the private ThemeProvider.

Open to using a CSS token instead of the hardcoded hex value if there's a semantic token for "always dark surface".

#### aduth on 2026-01-15T21:30:35Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2696027406
Path: packages/ui/src/tooltip/popup.tsx
...
Hard-coding seems okay for now since there's not a semantic token, as long as we include a comment to this effect (like suggested by @ciampo).

### Review thread 4 (resolved)

#### aduth on 2026-01-14T20:37:34Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2691967512
Path: packages/ui/src/tooltip/style.module.css
...
I think we'll want to use [padding tokens here](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/tokens/dimension.json). While there's some parallel discussion happening in #74556, I think this component fits the definition of ["surface" target](https://github.com/WordPress/gutenberg/tree/trunk/packages/theme#token-naming) that we could use the existing surface padding tokens.

#### retrofox on 2026-01-14T21:25:11Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2692100771
Path: packages/ui/src/tooltip/style.module.css
...
#### aduth on 2026-01-14T20:38:56Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2691971561
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Let's omit all these these "dependencies" docblock comments. They're no longer required after #73616 and we're excluding them in the rest of the files in this package.

### Review thread 6 (resolved)

#### ciampo on 2026-01-15T09:35:39Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693647787
Path: packages/ui/src/tooltip/popup.tsx
...
#### ciampo on 2026-01-15T09:37:08Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693652385
Path: packages/ui/src/tooltip/style.module.css
...
```suggestion
		line-height: 1.4; // TODO: use DS token for line height (or `Text` component when presets are ready
```

### Review thread 8

#### ciampo on 2026-01-15T09:38:18Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693656484
Path: packages/ui/src/tooltip/provider.tsx
...
These comments are not necessary anymore in `@wordpress/ui` (see other files in the package)

#### aduth on 2026-01-15T21:32:38Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2696033424
Path: packages/ui/src/tooltip/provider.tsx
...
There's still a handful more of these comments, but honestly at this point I think we should have a lint to _forbid_ their use and enforce it consistently.

### Review thread 9 (resolved)

#### ciampo on 2026-01-15T09:40:22Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693663450
...
@aduth @mirka looks like `@wordpress/ui` will have to list `@wordpress/theme` as a `dependency`. 

Wrapping parts of the components in a `ThemeProvider` will likely be necessary also in other portalled components anyway, right?

#### aduth on 2026-01-15T21:29:33Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2696024842
Path: packages/ui/package.json
...
Yeah this seems inevitable to me.

### Review thread 10 (resolved)

#### ciampo on 2026-01-15T09:41:29Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693666658
Path: packages/ui/src/tooltip/test/index.test.tsx
...
is the `await waitFor( () => {` still necessary, even with a `0` delay? (here and everywhere else)

#### retrofox on 2026-01-15T14:09:26Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2694542854
Path: packages/ui/src/tooltip/test/index.test.tsx
...
The test passes without `waitFor`, but I prefer keeping it for:

1. **Semantic clarity**: The popup appearing is conceptually async (state change → render → portal mount), even if the current implementation resolves fast enough
2. **Resilience**: If base-ui adds internal async logic, tests won't break
3. **Documentation**: Shows that the popup doesn't exist until after the hover triggers the open state

Happy to remove if you feel strongly about it, but I think it serves as good documentation of the expected behavior. Adding a comment for now.

#### ciampo on 2026-01-16T13:43:50Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2698568516
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Thank you for the eloquent and well-formatted reply

### Review thread 11 (resolved)

#### ciampo on 2026-01-15T09:43:48Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2693674501
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Should we refactor the `delay={0}` prop so that we need to specify it only once?

Either by creating a `commonTestingProps` variable at the top of the file (and then spreading `{...commonTestingProps}` on ever provider instance), or by wrapping `Tooltip.Provider`in a local component that sets `delay={0}` and then forwards all the props

#### retrofox on 2026-01-15T14:12:55Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2694554670
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Thanks for the tip. Added a `TestProvider` component here 455027523192279a32ac1e20b9618970f607e08e

### Review thread 12

#### simison on 2026-01-17T15:06:34Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2701210097
Path: packages/private-apis/src/implementation.ts
...
This one slipped here twice now, it was already on line 46.

https://github.com/WordPress/gutenberg/blob/bceabb971c43e4d066a09013f282f54d41f3e885/packages/private-apis/src/implementation.ts#L39-L46

#### retrofox on 2026-01-17T17:29:21Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2701297642
Path: packages/private-apis/src/implementation.ts
...
thanks. let's clean it in a follow up

#### ciampo on 2026-01-29T12:54:13Z

URL: https://github.com/WordPress/gutenberg/pull/74625#discussion_r2741504357
Path: packages/private-apis/src/implementation.ts
...
https://github.com/WordPress/gutenberg/pull/75051

## Changed files

- packages/private-apis/src/implementation.ts (MODIFIED, +1/-0)
- packages/ui/src/index.ts (MODIFIED, +1/-0)
- packages/ui/src/tooltip/index.ts (ADDED, +6/-0)
- packages/ui/src/tooltip/popup.tsx (ADDED, +64/-0)
- packages/ui/src/tooltip/provider.tsx (ADDED, +8/-0)
- packages/ui/src/tooltip/root.tsx (ADDED, +8/-0)
- packages/ui/src/tooltip/stories/index.story.tsx (ADDED, +97/-0)
- packages/ui/src/tooltip/style.module.css (ADDED, +20/-0)
- packages/ui/src/tooltip/test/index.test.tsx (ADDED, +87/-0)
- packages/ui/src/tooltip/trigger.tsx (ADDED, +15/-0)
- packages/ui/src/tooltip/types.ts (ADDED, +26/-0)
