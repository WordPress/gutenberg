...
type: pull_request
state: MERGED
merged_at: 2026-05-14T09:27:52Z
url: https://github.com/WordPress/gutenberg/pull/78168
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #78168: Overlays: Extend positioner slot pattern to Popover, Select, Autocomplete

- URL: https://github.com/WordPress/gutenberg/pull/78168
- Author: ciampo
- Created: 2026-05-11T14:09:25Z
- Updated: 2026-05-14T09:28:35Z
- Comments: 3 of 3
- Review threads: 6 of 6
- Changed files: 26 of 26
- Diff: ../pr-diffs/78168.diff
...
<details>
<summary>Slot pattern rule (added to CONTRIBUTING.md)</summary>
...
High-level wrappers that hide `Popup` (e.g. `IconButton` → `Tooltip`) re-expose the same slot props by the same name and shape. Add new slot props only when a concrete consumer needs them.
...
### github-actions on 2026-05-11T14:23:38Z

URL: https://github.com/WordPress/gutenberg/pull/78168#issuecomment-4421585837
...
### github-actions on 2026-05-12T09:26:43Z

URL: https://github.com/WordPress/gutenberg/pull/78168#issuecomment-4429116898

<!-- flaky-tests-report-comment -->
**Flaky tests detected in bab432413ed1105ce91564f90fae586291f2e477.**
...
### github-actions on 2026-05-12T16:34:33Z

URL: https://github.com/WordPress/gutenberg/pull/78168#issuecomment-4432681902
...
#### copilot-pull-request-reviewer on 2026-05-12T15:32:14Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3227734076
...
The new `positioner` slot API isn’t covered by tests. Since `Popover.Popup` now relies on `renderSlotWithChildren(positioner, <Positioner />, …)`, please add a regression test that passes a custom `positioner={ <Popover.Positioner … /> }` (e.g. with a `data-testid`/`className`) and asserts the custom positioner wrapper is used and still receives the injected popup subtree as children.

### Review thread 2 (resolved)

#### copilot-pull-request-reviewer on 2026-05-12T15:32:14Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3227734120
Path: packages/ui/src/form/primitives/select/popup.tsx
...
`Select.Popup` now supports a `positioner` slot, but there are no tests validating that a provided `positioner` element is actually used (and that children injection works). Please add a test similar to the existing portal tests that passes `positioner={ <Select.Positioner data-testid="…" /> }` and asserts the positioner element is present around the popup.

### Review thread 3 (resolved)

#### copilot-pull-request-reviewer on 2026-05-12T15:32:15Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3227734145
...
`Autocomplete.Popup` now supports a `positioner` slot, but the current test suite only exercises the `portal` slot and ref forwarding. Please add a regression test that supplies `positioner={ <Autocomplete.Positioner data-testid="…" /> }` and asserts the custom positioner is rendered and wraps the popup content (verifying `renderSlotWithChildren` integration).

### Review thread 4 (resolved)

#### mirka on 2026-05-13T12:48:07Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3234343953
...
I believe these should all be using our own `ComponentProps` utility?

```suggestion
export type PositionerProps = ComponentProps <
```

#### ciampo on 2026-05-14T08:48:29Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3240189096
Path: packages/ui/src/form/primitives/autocomplete/types.ts
...
Done, and applied to `Portal` components too, that until now were also using `ComponentPropsWithoutRef` instead of our internal `ComponentProps`

### Review thread 5 (resolved)

#### mirka on 2026-05-13T12:50:37Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3234360871
Path: packages/ui/src/form/primitives/autocomplete/positioner.tsx
...
Do you prefer this over spreading the entire `ITEM_POPUP_POSITIONER_PROPS` object? I kind of get the point, but passing them one by one also seems prone to mistakes over time, like when we add more default properties. Perhaps not too important at this time though?

#### ciampo on 2026-05-14T08:45:13Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3240174057
Path: packages/ui/src/form/primitives/autocomplete/positioner.tsx
...
#### mirka on 2026-05-13T12:54:49Z

URL: https://github.com/WordPress/gutenberg/pull/78168#discussion_r3234389151
...
- packages/ui/src/alert-dialog/types.ts (MODIFIED, +2/-4)
- packages/ui/src/dialog/types.ts (MODIFIED, +2/-2)
- packages/ui/src/drawer/types.ts (MODIFIED, +2/-2)
- packages/ui/src/form/primitives/autocomplete/index.ts (MODIFIED, +2/-1)
- packages/ui/src/form/primitives/autocomplete/popup.tsx (MODIFIED, +17/-21)
- packages/ui/src/form/primitives/autocomplete/positioner.tsx (ADDED, +32/-0)
- packages/ui/src/form/primitives/autocomplete/stories/index.story.tsx (MODIFIED, +1/-0)
- packages/ui/src/form/primitives/autocomplete/test/index.test.tsx (MODIFIED, +41/-0)
- packages/ui/src/form/primitives/autocomplete/types.ts (MODIFIED, +12/-4)
- packages/ui/src/form/primitives/constants.ts (MODIFIED, +7/-4)
- packages/ui/src/form/primitives/select/index.ts (MODIFIED, +2/-1)
- packages/ui/src/form/primitives/select/popup.tsx (MODIFIED, +30/-34)
- packages/ui/src/form/primitives/select/positioner.tsx (ADDED, +36/-0)
- packages/ui/src/form/primitives/select/stories/index.story.tsx (MODIFIED, +1/-0)
- packages/ui/src/form/primitives/select/test/index.test.tsx (MODIFIED, +28/-0)
- packages/ui/src/form/primitives/select/types.ts (MODIFIED, +12/-2)
- packages/ui/src/popover/index.ts (MODIFIED, +12/-1)
- packages/ui/src/popover/popup.tsx (MODIFIED, +28/-45)
- packages/ui/src/popover/positioner.tsx (ADDED, +45/-0)
- packages/ui/src/popover/root.tsx (MODIFIED, +5/-2)
- packages/ui/src/popover/stories/index.story.tsx (MODIFIED, +91/-47)
- packages/ui/src/popover/test/index.test.tsx (MODIFIED, +36/-1)
- packages/ui/src/popover/types.ts (MODIFIED, +13/-15)
- packages/ui/src/tooltip/types.ts (MODIFIED, +3/-5)
