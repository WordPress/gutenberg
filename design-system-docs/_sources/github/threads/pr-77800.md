...
type: pull_request
state: MERGED
merged_at: 2026-04-30T15:24:22Z
url: https://github.com/WordPress/gutenberg/pull/77800
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77800: ui/Drawer: Polish open/close animation, fix swipe on content padding

- URL: https://github.com/WordPress/gutenberg/pull/77800
- Author: ciampo
- Created: 2026-04-29T12:42:16Z
- Updated: 2026-05-05T09:45:55Z
- Comments: 4 of 4
- Review threads: 2 of 2
- Changed files: 6 of 6
- Diff: ../pr-diffs/77800.diff

## Body

## What?

`Drawer` popup polish, in three pieces:

1. **Restore the slide-out close animation** when the popup closes via close button, Escape, or backdrop click — previously it snapped to its closed offset instead of sliding.
2. **Fade the popup elevation shadow** alongside the slide so it no longer pops in / out at the screen edge.
3. **Engage mouse-drag swipe-dismiss in the popup-edge padding gutter.** Base UI carves out `[data-drawer-content]` from mouse-drag swipe so text selection inside the body keeps working; before this PR, our `Drawer.Content` *was* the marker, so the visual padding gutter inherited that carve-out and could not be grabbed with the mouse. Most of the drawer surface looked draggable but silently no-oped under pointer down on desktop.

<details>
<summary>Why the close animation broke</summary>

The popup `transform` transition was nested inside `&[data-open], &[data-swiping]`. Base UI removes `[data-open]` at the start of the close sequence, so on a non-swipe dismissal neither selector matched, the `transition` declaration disappeared, and the per-direction `&[data-ending-style] { transform: translate(±100%) }` rule applied with nothing to interpolate.

Moving the transition to the `.popup` top level inside the existing `@media not (prefers-reduced-motion)` guard mirrors what `Dialog` already does and what the Base UI [animation handbook](https://base-ui.com/react/handbook/animation) and [drawer docs](https://base-ui.com/react/components/drawer) prescribe for `[data-starting-style]` / `[data-ending-style]` based transitions. `will-change: transform` stays scoped to the in-motion states (now including `[data-ending-style]`), so we still avoid retaining a transform layer while idle.

The original gating was [intentional](https://github.com/WordPress/gutenberg/pull/76690) (`Drawer: Scope motion layers and horizontal safe-area`) — the optimization is fine, the over-scoping was the regression.

</details>

<details>
<summary>Why the elevation fade</summary>

At `[data-starting-style]` / `[data-ending-style]` the popup is translated fully past the viewport edge, but its `box-shadow` blur still bleeds back into the viewport, appearing as a faint halo at mount/unmount. Most visible in `modal={ false }` and `modal="trap-focus"` modes where there is no backdrop to mask it.

Setting `box-shadow: none` at those two states and adding `box-shadow` to the existing transition makes the shadow grow in alongside the slide. `none` interpolates against the resolved token value as a list of zero-shadow layers per the [CSS Backgrounds spec](https://www.w3.org/TR/css-backgrounds-3/#shadow-list), so we don't need to know how many layers `--wpds-elevation-lg` expands to.

The other overlay primitives (`Dialog`, `AlertDialog`, `Popover`, `Tooltip`, `Select`, `Autocomplete`) don't need this — they either have no entry/exit animation or already animate `opacity` through 0, which fades the shadow with the rest of the popup. The drawer is the only one that intentionally keeps the popup opaque while it slides.

</details>

<details>
<summary>Why scope the swipe carve-out tightly</summary>

In [`DrawerViewport.tsx`](https://github.com/mui/base-ui/blob/master/packages/react/src/drawer/viewport/DrawerViewport.tsx), Base UI's `onPointerDown` handler bails out when `getElementAtPoint(...).closest('[data-drawer-content]')` matches — so mouse-drag never engages over the body, on purpose, so that text selection there keeps working.

When `Drawer.Content` *is* `[data-drawer-content]`, that carve-out covers the visible padding gutter too, even though the gutter is just empty space the user perceives as part of the panel chrome. Scoping the marker tightly around the body children — `Drawer.Content` now renders an outer styled `<div>` (the actual scroll container, with `data-wp-ui-overlay-scroll-container` and the forwarded `ref` / `onScroll` / `className` / props) and wraps Base UI's `_Drawer.Content` inside it — leaves the padding gutter outside the carve-out. Pointer-downs there resolve to the popup ancestor and engage swipe; pointer-downs on real body content still match `[data-drawer-content]` via `closest()` and bail, preserving text selection.

Two consequences worth noting:

- **Shared CSS softens `>` to descendant matching** for the non-pinned `.content > .header` / `.content > .footer` rules in `overlay-chrome.module.css`, so they traverse the new wrapper. `Dialog` and `AlertDialog` don't render the wrapper — direct nested chrome matches identically — but consumers who already wrap nested chrome in custom intermediate elements inside `*.Content` will now see the documented padding-collapse apply.
- **The marker is rendered with the default `<div>` (block)**, not `display: contents`. A boxless marker would silently neuter `useOverlayScrollStateAttributes`'s `ResizeObserver` for dynamic body growth (the hook observes the scroll container's direct children, and `ResizeObserver` does not fire for boxless elements). Keeping the marker as a real auto-height block lets the resize signal propagate as the hook's JSDoc invariant promises.

</details>

## Testing Instructions

In Storybook, open `Design System / Components / Drawer` and try:

1. **Close animation** — open any drawer story, then close via the close button, Escape, and a backdrop click. The popup should slide back out (~450ms) toward its anchored edge, not snap. Repeat with `swipeDirection` set to each of `up`, `down`, `left`, `right` (use `All Sides` or the controls).
2. **Mouse-drag swipe-dismiss** — grab the popup with a mouse drag and pull it past the anchored edge. Now works from anywhere on the popup surface that *isn't* live body content (text, links, focusable elements): the chrome, the popup-edge padding gutter, the empty space below short body content, etc. The release should still use the velocity-scaled duration; no regression.
3. **Mouse-drag bail-out is preserved** — start a mouse drag from inside a paragraph, link, or focusable child of `Drawer.Content`. Swipe should *not* engage; selection / click should behave normally.
4. **Touch swipe** (DevTools touch emulation or a real device) — swipe from anywhere in the popup. Vertical drawers gate on the scroll edge; horizontal drawers engage immediately.
5. **Elevation fade** — `Non Modal` story is the clearest test (no backdrop). Open and close: the shadow should ramp in alongside the slide rather than appearing instantly. In modal mode (`Default`) the effect is subtler but should still look smooth.
6. **Scroll behavior** — open a story whose body overflows. Body still scrolls; sticky chrome separators still highlight; native scrollbar still sits at the popup edge (no inset gutter); `tabindex="0"` toggles correctly when content stops or starts overflowing (both initially and as content mutates at runtime).
7. **Reduced motion** — enable OS-level "Reduce motion" (or DevTools → _Rendering → Emulate `prefers-reduced-motion: reduce`_) and close the drawer. It should close instantly with no transition. The shadow should also not animate.

### Testing Instructions for Keyboard

1. Tab to the trigger and press <kbd>Enter</kbd> / <kbd>Space</kbd> to open.
2. Press <kbd>Escape</kbd> — the drawer should slide out and focus should return to the trigger.
3. Tab to the in-header close button and press <kbd>Enter</kbd> — same expectation.
4. Open a drawer with overflowing body. Tab into the body region and arrow-scroll — the scroll region should be reachable as a tab stop while overflowing, and the tab stop should disappear once content fits.

## Screenshots or screencast

| Before | After |
| --- | --- |
| <video src="https://github.com/user-attachments/assets/4b2abc7f-b77d-4b30-ae82-fb2d2487a823" /> | <video src="https://github.com/user-attachments/assets/c6826113-4c8a-48c3-8a08-cfd8d86c302d" /> |

## Use of AI Tools

Authored with the help of an AI assistant (Cursor); diagnosis, code changes, and PR text were reviewed by a human. Per the [WordPress AI Guidelines](https://make.wordpress.org/ai/handbook/ai-guidelines/).

## Issue comments

### github-actions on 2026-04-29T12:53:26Z

URL: https://github.com/WordPress/gutenberg/pull/77800#issuecomment-4343828910
...
| `build/styles/components/style.css` | 17.5 kB |
...
### github-actions on 2026-04-29T16:20:49Z

URL: https://github.com/WordPress/gutenberg/pull/77800#issuecomment-4345546828

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 5d78f3e9dc8750959033ce390377e4e97755c5ef.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25160940518
📝  Reported issues:
...
- #67172 in `/test/e2e/specs/site-editor/template-revert.spec.js`

### github-actions on 2026-04-29T22:42:37Z

URL: https://github.com/WordPress/gutenberg/pull/77800#issuecomment-4348054126
...
### ciampo on 2026-04-30T15:01:00Z

URL: https://github.com/WordPress/gutenberg/pull/77800#issuecomment-4353572223

Merging as E2E test failures can't be related to this PR (I don't think the code in this PR is used in Gutenberg yet)

## Review threads

### Review thread 1 (resolved)

#### copilot-pull-request-reviewer on 2026-04-30T14:14:11Z

URL: https://github.com/WordPress/gutenberg/pull/77800#discussion_r3168557725
Path: packages/ui/src/drawer/test/index.test.tsx

```diff
@@ -810,6 +810,66 @@ describe( 'Drawer', () => {
 			);
 		} );
...
+		// - The marker must render as a real block-level element, not
+		//   `display: contents`, so `useOverlayScrollStateAttributes`'s
+		//   `ResizeObserver` (which observes the scroll container's
+		//   direct children) actually fires on dynamic body growth.
...
+			// `display: contents` would silently neuter the hook's
+			// ResizeObserver (no box, no resize entries) — guard
...
+				marker as HTMLElement
+			).display;
+			expect( computedDisplay ).not.toBe( 'contents' );
```

The `getComputedStyle(...).display` assertion won’t provide a reliable signal here: the unit test environment maps `*.css`/`*.scss` to an empty object (no styles are applied), so `getComputedStyle` will almost always report the default `'block'` and won’t catch a regression introduced via CSS modules. Consider instead asserting against inline styling (e.g., absence of a `style` attribute / `toHaveStyle`), or limiting the test to structural guarantees (marker element type/placement) that Jest can actually observe.
```suggestion
			// In this test environment CSS module styles are not applied,
			// so computed styles are not a reliable signal. Limit the
			// assertion to observable inline styling.
			expect( ( marker as HTMLElement ).style.display ).not.toBe(
				'contents'
			);
```
...
#### mirka on 2026-04-30T15:55:58Z

URL: https://github.com/WordPress/gutenberg/pull/77800#discussion_r3169207740
Path: packages/ui/src/drawer/content.tsx
...
Rest props can still include `render`, which is now forwarded onto this plain div instead of the `Drawer.Content`.

#### ciampo on 2026-04-30T20:42:43Z

URL: https://github.com/WordPress/gutenberg/pull/77800#discussion_r3170746447
Path: packages/ui/src/drawer/content.tsx
...
good catch, i'll open a small follow-up fix. Curious that TS didn't flag it?

In fact, what do you think the expected behaviour should be? Forwarding `...props` to `Drawer.Content` ?

#### ciampo on 2026-05-05T09:45:55Z

URL: https://github.com/WordPress/gutenberg/pull/77800#discussion_r3187478835
Path: packages/ui/src/drawer/content.tsx
...
@mirka fix proposed in https://github.com/WordPress/gutenberg/pull/77941

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +9/-0)
- packages/ui/src/drawer/content.tsx (MODIFIED, +10/-7)
- packages/ui/src/drawer/style.module.css (MODIFIED, +41/-9)
- packages/ui/src/drawer/test/index.test.tsx (MODIFIED, +56/-0)
- packages/ui/src/drawer/types.ts (MODIFIED, +1/-1)
- packages/ui/src/utils/css/overlay-chrome.module.css (MODIFIED, +21/-4)
