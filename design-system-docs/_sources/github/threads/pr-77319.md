...
type: pull_request
state: MERGED
merged_at: 2026-04-17T09:23:35Z
url: https://github.com/WordPress/gutenberg/pull/77319
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Code Quality","[Package] UI"]
---

# PR #77319: ui/Tabs: Fix `act()` warnings in tests

- URL: https://github.com/WordPress/gutenberg/pull/77319
- Author: ciampo
- Created: 2026-04-14T10:55:39Z
- Updated: 2026-04-17T09:24:18Z
- Comments: 7 of 7
- Review threads: 0 of 0
- Changed files: 2 of 2
- Diff: ../pr-diffs/77319.diff
...
### github-actions on 2026-04-14T11:07:35Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4243391503
...
### github-actions on 2026-04-14T11:25:21Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4243496721
...
### jsnajdr on 2026-04-14T11:36:53Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4243562232

It's a bummer we need to switch to fake timers 🙁 Isn't there a better way?

Ariakit has similar issues with some updates running after random timeouts, outside `act()`. And `@ariakit/test` has utilities to handle this.

Another targeted fix could be an `await act( () => sleep( 17 ) )` call at the critical places, if there are not too many.

### github-actions on 2026-04-14T11:52:32Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4243647462

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 225ca41bbe22688d6af57bf9c6e94e62b739dc75.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24555982757
📝  Reported issues:
...
### ciampo on 2026-04-14T12:18:38Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4243808117

@jsnajdr yeah disabling fake timers is not great, but I'm not sure that `@ariakit/test`'s approach is great either? Replaging `render` and `user-event` from `@testing-library/react` feels intrusive and not very portable (and would be a large rewrite).

> Another targeted fix could be an await act( () => sleep( 17 ) ) call at the critical places, if there are not too many.

Claude Opus counts 132 `await` calls in the tests, and given that those warnings are non-deterministic, it's hard to estimate how many `await act( () => sleep( 17 ) )` would be actually necessary.

I considered two other alternatives:

1. suppress `act()` warnings by filtering out the specific error message in `console.error` at the test suite level (which would stop working if the warning string changes, and would potentially make the `collectUncaughtErrors` logic complex)
2. opt out of the React act-environment to avoid the warnings — assuming that most of the code is already tested in Base UI (which also, from what I gather, ignores act warnings)

I updated the PR to follow the latter approach, @jsnajdr let me know what you think

### jsnajdr on 2026-04-15T11:57:27Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4251772068

With some help from Cursor I found the root cause of the warnings. There are only two tests that trigger them. The ones that wait 50ms for validation:
```js
await new Promise( ( resolve ) => setTimeout( resolve, 50 ) );
```
These waits are not wrapped in `act`. After I wrap them, the rAF callback will fire _inside_ the `act` environment and everything will be good again:
```js
await act( () => new Promise( ( resolve ) => setTimeout( resolve, 50 ) ) );
```
Please try this fix and verify if it works for you.

### ciampo on 2026-04-17T08:35:35Z

URL: https://github.com/WordPress/gutenberg/pull/77319#issuecomment-4266538062

@jsnajdr you are right, your suggested fix seems to work in my tests too.
