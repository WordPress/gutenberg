...
type: pull_request
state: MERGED
merged_at: 2026-02-19T13:49:54Z
url: https://github.com/WordPress/gutenberg/pull/75697
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Package] Theme"]
---

# PR #75697: Theme: Add design token for interactive non-link elements

- URL: https://github.com/WordPress/gutenberg/pull/75697
- Author: mirka
- Created: 2026-02-18T18:07:59Z
- Updated: 2026-02-19T13:51:50Z
- Comments: 5 of 5
- Review threads: 1 of 1
- Changed files: 6 of 6
- Diff: ../pr-diffs/75697.diff

## Body

## What?

I'm proposing we add a new `--wpds-cursor-control` design token to `@wordpress/theme`, intended for the `cursor` style of interactive controls that are not links (e.g. buttons, checkboxes, and toggles).

## Why?

There's a growing consensus that `cursor: pointer` is overused on non-link interactive elements (#63756), and we'd like to start aligning with that direction. However, this is a potentially contentious change, and it may end up as a user preference in the future, or even revert it.

Extracting the cursor value into a design token lets us decouple the policy decision from the individual component styles. Components can adopt the token now, and the actual value can be changed in one place later — whether that's a global switch, a user preference, or a different default.

The token currently defaults to `default`.

## How?

- New token file `tokens/cursor.json` using Terrazzo's `string` extension type.
- Registered in `terrazzo.config.ts` and rebuilt via `npm run build:tokens`.

## Testing Instructions

1. Verify the token builds: `cd packages/theme && npm run build:tokens`
2. Check `packages/theme/src/prebuilt/css/design-tokens.css` contains `--wpds-cursor-control: default;`

## Issue comments

### github-actions on 2026-02-18T18:11:15Z

URL: https://github.com/WordPress/gutenberg/pull/75697#issuecomment-3922373605
...
### github-actions on 2026-02-18T18:20:03Z

URL: https://github.com/WordPress/gutenberg/pull/75697#issuecomment-3922414819
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 14.8 kB | +13 B (+0.09%) |
| `build/styles/theme/style-rtl.css` | 2.76 kB | +62 B (+2.3%) |
| `build/styles/theme/style-rtl.min.css` | 1.41 kB | +13 B (+0.93%) |
| `build/styles/theme/style.css` | 2.76 kB | +62 B (+2.3%) |
| `build/styles/theme/style.min.css` | 1.41 kB | +13 B (+0.93%) |
...
### github-actions on 2026-02-18T18:36:32Z

URL: https://github.com/WordPress/gutenberg/pull/75697#issuecomment-3922490430

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 453da03e34d30c651c9e41404a2b73305f85d202.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22151732147
📝  Reported issues:
- #75667 in `/test/e2e/specs/site-editor/page-list.spec.js`

### jasmussen on 2026-02-19T08:19:26Z

URL: https://github.com/WordPress/gutenberg/pull/75697#issuecomment-3925477472

Thanks for the PR, good motivation.

It is indeed a contentious change: I'm pro "default" as that is the operating system way forward. Yet that stance, as it turns out, is so unusual for other contributors that the absence of the "pointer" cursor on buttons is often flagged as a _bug_. But since that's a personal opinion, I'm happy to defer to a consensus, which leads me to another personal opinion (softly felt at that): this just needs a decision, and I don't think we need to surface this as an option. If that decision is to go pointer-by-default today, presumably we can change our minds again in a couple of years if the winds change?

But that's also me not blocking this, I'm not against options at all, and if this is option-worthy, happy to support it.

### ciampo on 2026-02-19T09:35:34Z

URL: https://github.com/WordPress/gutenberg/pull/75697#issuecomment-3925909455

IMO the right approach is:

- extract the parameter to a theme setting, like this PR does
- the theme defaults to the "correct" setting, ie. `default` pointer (no cursor)
- for better integration in Gutenberg, we will set it to `pointer`, so that new UI components and old UI components will behave the same way
- once all UI is refactored to either the new UI components, or to consuming the DS tokens for cursor styling, we'll be able to "flip the switch" consistently across Gutenberg UI if we so wish (likely behind a user preference or something).
...
#### mirka on 2026-02-18T18:10:41Z

URL: https://github.com/WordPress/gutenberg/pull/75697#discussion_r2823748134
Path: packages/theme/tokens/cursor.json
...
#### ciampo on 2026-02-19T09:39:46Z

URL: https://github.com/WordPress/gutenberg/pull/75697#discussion_r2826616132
Path: packages/theme/tokens/cursor.json
...
I like`control`, it's an established term that already implicitly excludes links.

## Changed files

- packages/theme/CHANGELOG.md (MODIFIED, +4/-0)
- packages/theme/docs/tokens.md (MODIFIED, +6/-0)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +1/-0)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +1/-0)
- packages/theme/terrazzo.config.ts (MODIFIED, +1/-0)
- packages/theme/tokens/cursor.json (ADDED, +9/-0)
