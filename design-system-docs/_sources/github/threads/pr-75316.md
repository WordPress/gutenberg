...
type: pull_request
state: MERGED
merged_at: 2026-02-09T14:01:54Z
url: https://github.com/WordPress/gutenberg/pull/75316
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Developer Documentation","[Package] Components","Storybook"]
---

# PR #75316: Storybook: Fix missing props on certain components

- URL: https://github.com/WordPress/gutenberg/pull/75316
- Author: mirka
- Created: 2026-02-06T18:58:45Z
- Updated: 2026-02-09T14:03:02Z
- Comments: 3 of 3
- Review threads: 1 of 1
- Changed files: 9 of 9
- Diff: ../pr-diffs/75316.diff

## Body

## What?

Fixes missing prop types and descriptions in the Storybook props table for `Modal`, `Tooltip`, `Popover`, and `Disabled`.

## Why?

After #71208 added `displayName` to context objects, `react-docgen-typescript` started misidentifying these context objects as the component in each file. For example, it would find `ModalContext` (via its `displayName` assignment) instead of `Modal`, resulting in an empty or incorrect props table.

## How?

Move context objects (and their `displayName` assignments) out of the component files into separate `context.ts` files, so `react-docgen-typescript` no longer picks them up when analyzing the component.

The simplest fix would have been to remove the `displayName` assignments, but given that separate context files are already a convention in this package anyway, I opted for that approach.

## Testing Instructions

1. Run `npm run storybook:dev`.
2. Check the props table in the Docs tab for Modal, Tooltip, Popover, and Disabled. Each should display prop types and descriptions.

## Issue comments

### github-actions on 2026-02-06T19:00:53Z

URL: https://github.com/WordPress/gutenberg/pull/75316#issuecomment-3862089832
...
### github-actions on 2026-02-06T19:16:57Z

URL: https://github.com/WordPress/gutenberg/pull/75316#issuecomment-3862149252
...
| `build/scripts/editor/index.min.js` | 323 kB | -156 B (-0.05%) |
...
### mirka on 2026-02-09T13:21:21Z

URL: https://github.com/WordPress/gutenberg/pull/75316#issuecomment-3871673142

> Should we remove the `displayName` assignments anyway, in case they will encourage this pattern from resurfacing in the future?

It does seem like we can't get named context providers without an explicit display name assignment, so I wouldn't go as far as that.

## Review threads

### Review thread 1

#### mirka on 2026-02-06T19:00:43Z

URL: https://github.com/WordPress/gutenberg/pull/75316#discussion_r2775584011
...
For this component specifically, it was also necessary to remove the display name assignment on the main component.

This has no effect anyway, because the component is already named (React dev tools should continue showing the correct component name).
...
- packages/components/src/modal/index.tsx (MODIFIED, +1/-19)
- packages/components/src/popover/context.ts (ADDED, +4/-0)
- packages/components/src/popover/index.tsx (MODIFIED, +1/-14)
- packages/components/src/tooltip/context.ts (ADDED, +8/-0)
- packages/components/src/tooltip/index.tsx (MODIFIED, +2/-22)
