...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/46099
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #46099: `Navigator`: Split `NaviagtorContainer` from `NavigatorProvider`

- URL: https://github.com/WordPress/gutenberg/pull/46099
- Author: noisysocks
- Created: 2022-11-28T05:20:21Z
- Updated: 2022-11-29T08:04:08Z
- Comments: 5 of 5
- Review threads: 1 of 1
- Changed files: 23 of 23
- Diff: ../pr-diffs/46099.diff
...
### codesandbox on 2022-11-28T05:20:23Z

URL: https://github.com/WordPress/gutenberg/pull/46099#issuecomment-1328560639

<img alt="CodeSandbox logo" src="https://raw.githubusercontent.com/codesandbox/open-in-codesandbox/main/assets/csb.light.svg#gh-dark-mode-only" /><img alt="CodeSandbox logo" src="https://raw.githubusercontent.com/codesandbox/open-in-codesandbox/main/assets/csb.svg#gh-light-mode-only" />&nbsp; Open in CodeSandbox <a href="https://codesandbox.io/p/github/WordPress/gutenberg/update/split-navigator-container-from-provider?workspace=%7B%22gitSidebarPanel%22:%22PR%22,%22sidebarPanel%22:%22GIT%22%7D">Web Editor</a> | <a href="https://codesandbox.io/p/vscode?owner=WordPress&repo=gutenberg&branch=update/split-navigator-container-from-provider">VS Code</a> | <a href="https://codesandbox.io/p/vscode?owner=WordPress&repo=gutenberg&branch=update/split-navigator-container-from-provider&insiders=true">VS Code Insiders</a>

<!-- open-in-codesandbox:complete -->

### noisysocks on 2022-11-28T05:26:18Z

URL: https://github.com/WordPress/gutenberg/pull/46099#issuecomment-1328563574

@ciampo: What do you think of this refactor? Needed so that I can have navigator links for the Styles panel that live outside of the Styles panel in https://github.com/WordPress/gutenberg/pull/45960. (You click on the block in the Style Book and it navigates to the block's screen in Styles.)

Two alternative approaches I can think of:

1. Make `StyleBook` a `Fill` that lives inside `GlobalStylesUI` and fills a `Slot` in `Editor`. `StyleBook` would then have access to `NaviatorContext`. 
2. Refactor `NavigatorProvider` to allow controlled state e.g. `<NavigatorProvider path={ path } setPath={ setPath }>`. We could then put `path` inside the `edit-site` redux store.

Alternative 1 above is kinda tempting. But would splitting `NavigatorProvider` and `NavigatorContainer` be useful outside of this use case? Keen to hear any insight you have 😀

### github-actions on 2022-11-28T05:30:15Z

URL: https://github.com/WordPress/gutenberg/pull/46099#issuecomment-1328565246
...
| :--- | :---: | :---: |
| `build/block-editor/index.min.js` | 179 kB | +12 B (0%) |
| `build/components/index.min.js` | 203 kB | +86 B (0%) |
| `build/edit-post/index.min.js` | 34.5 kB | +4 B (0%) |
| `build/edit-site/index.min.js` | 61.1 kB | +13 B (0%) |
...
### noisysocks on 2022-11-29T07:05:08Z

URL: https://github.com/WordPress/gutenberg/pull/46099#issuecomment-1330174671

> 1. Make `StyleBook` a `Fill` that lives inside `GlobalStylesUI` and fills a `Slot` in `Editor`. `StyleBook` would then have access to `NaviatorContext`.

I'm going to go with this approach. That way all of the work can stay in `@wordpress/edit-site`.

### ciampo on 2022-11-29T08:04:08Z

URL: https://github.com/WordPress/gutenberg/pull/46099#issuecomment-1330232778

I never actually wondered if `Slot`s  inherit the context of ther `Fill` — good to know!

## Review threads

### Review thread 1

#### noisysocks on 2022-11-28T05:30:43Z

URL: https://github.com/WordPress/gutenberg/pull/46099#discussion_r1033140458
...
- packages/block-editor/src/components/inserter/block-patterns-tab.js (MODIFIED, +44/-39)
- packages/block-editor/src/components/inserter/media-tab/media-tab.js (MODIFIED, +45/-40)
- packages/components/CHANGELOG.md (MODIFIED, +4/-0)
- packages/components/src/index.js (MODIFIED, +1/-0)
- packages/components/src/navigator/index.ts (MODIFIED, +1/-0)
- packages/components/src/navigator/navigator-back-button/README.md (MODIFIED, +1/-1)
- packages/components/src/navigator/navigator-back-button/component.tsx (MODIFIED, +17/-13)
- packages/components/src/navigator/navigator-button/README.md (MODIFIED, +1/-1)
- packages/components/src/navigator/navigator-button/component.tsx (MODIFIED, +19/-15)
- packages/components/src/navigator/navigator-container/README.md (ADDED, +21/-0)
- packages/components/src/navigator/navigator-container/component.tsx (ADDED, +89/-0)
- packages/components/src/navigator/navigator-container/index.ts (ADDED, +1/-0)
- packages/components/src/navigator/navigator-provider/README.md (MODIFIED, +16/-13)
- packages/components/src/navigator/navigator-provider/component.tsx (MODIFIED, +48/-44)
- packages/components/src/navigator/navigator-screen/README.md (MODIFIED, +1/-1)
- packages/components/src/navigator/navigator-screen/component.tsx (MODIFIED, +15/-12)
- packages/components/src/navigator/stories/index.tsx (MODIFIED, +121/-117)
- packages/components/src/navigator/test/index.tsx (MODIFIED, +70/-67)
- packages/components/src/navigator/types.ts (MODIFIED, +7/-0)
- packages/edit-post/src/components/preferences-modal/test/__snapshots__/index.js.snap (MODIFIED, +2/-2)
- packages/edit-site/src/components/global-styles/ui.js (MODIFIED, +29/-29)
- packages/interface/src/components/preferences-modal-tabs/index.js (MODIFIED, +71/-71)
