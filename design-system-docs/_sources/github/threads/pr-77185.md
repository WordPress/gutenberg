...
type: pull_request
state: MERGED
merged_at: 2026-04-20T09:09:19Z
url: https://github.com/WordPress/gutenberg/pull/77185
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Feature] UI Components","[Package] Components"]
---

# PR #77185: Autocomplete: Skip stale triggers from completed mentions

- URL: https://github.com/WordPress/gutenberg/pull/77185
- Author: Mamaduka
- Created: 2026-04-09T09:36:34Z
- Updated: 2026-04-20T09:09:56Z
- Comments: 5 of 5
- Review threads: 4 of 4
- Changed files: 5 of 5
- Diff: ../pr-diffs/77185.diff
...
### github-actions on 2026-04-09T10:33:57Z

URL: https://github.com/WordPress/gutenberg/pull/77185#issuecomment-4213465968
...
### github-actions on 2026-04-09T10:47:00Z

URL: https://github.com/WordPress/gutenberg/pull/77185#issuecomment-4213538096
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 265 kB | +189 B (+0.07%) |
...
### github-actions on 2026-04-09T12:49:59Z

URL: https://github.com/WordPress/gutenberg/pull/77185#issuecomment-4214343419

<!-- flaky-tests-report-comment -->
**Flaky tests detected in e0c743195646bfe12c3bfaeaf16a2a78d20e025e.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24401902237
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### Mamaduka on 2026-04-14T13:30:27Z

URL: https://github.com/WordPress/gutenberg/pull/77185#issuecomment-4244261882

Thanks for the review, @ciampo! I pushed some changes for inline feedback.

I think there's still room for improvement and maybe for unifying the logic. I'll try to experiment with it.

Ideally, we would want to resolve most of the points mentioned in https://github.com/WordPress/gutenberg/issues/77007. It would have been simpler with a "fresh trigger only" model, but that might introduce regressions.

### Mamaduka on 2026-04-16T17:50:02Z

URL: https://github.com/WordPress/gutenberg/pull/77185#issuecomment-4262256110

Thanks for the review, @mirka!

@ciampo, is this good to merge for you?

## Update

Okay, I'm going to merge this. We can continue the discussion in https://github.com/WordPress/gutenberg/issues/77007. Unfortunately, I think the remaining items for that issue are "won't fix", though I need to re-evaluate it once more.

## Review threads

### Review thread 1

#### ciampo on 2026-04-10T15:09:55Z

URL: https://github.com/WordPress/gutenberg/pull/77185#discussion_r3065115710
Path: packages/components/src/autocomplete/get-autocomplete-match.ts

```diff
@@ -108,6 +117,20 @@ export function getAutocompleteMatch(
 		return null;
 	}
...
- move the clearing logic before the `getAutocompleteMatch` call (checking if text has meaningfully changed beyond just the completion),
- replace `startsWith` with a stricter equality-only check (with trailing-space tolerance?). The `startsWith` check seems designed to handle `@username ` (with trailing space), but `trimEnd()` already handles that? `startsWith` may not be needed at all?

### Review thread 2 (resolved)

#### ciampo on 2026-04-10T15:11:29Z

URL: https://github.com/WordPress/gutenberg/pull/77185#discussion_r3065123974
Path: packages/components/src/autocomplete/index.tsx

```diff
@@ -108,38 +108,62 @@ export function useAutocomplete( {
 		state;
...
To keep the same behavior as the original code, we should add a `RESET` dispatch and a focus the `contentRef`:
...
#### ciampo on 2026-04-10T15:13:20Z

URL: https://github.com/WordPress/gutenberg/pull/77185#discussion_r3065133658
Path: packages/components/src/autocomplete/test/get-autocomplete-match.ts

We could add new tests for the `lastCompletion` parameter to check that:

- A match is suppressed when `lastCompletion.value` equals `textWithoutTrigger`;
- A match is suppressed when `textWithoutTrigger` starts with `lastCompletion.value`;
- A match is NOT suppressed when the `completer` name differs;
- A match is NOT suppressed when the text doesn't match the last completion

### Review thread 4 (resolved)

#### ciampo on 2026-04-10T15:20:57Z

URL: https://github.com/WordPress/gutenberg/pull/77185#discussion_r3065171503
Path: test/e2e/specs/editor/various/autocomplete-and-mentions.spec.js

```diff
@@ -569,4 +566,178 @@ test.describe( 'Autocomplete (@firefox, @webkit)', () => {
...
I tried to think about a more deterministic way to perform the check and avoid the timeout, but didn't come up with anything

#### Mamaduka on 2026-04-14T13:24:46Z

URL: https://github.com/WordPress/gutenberg/pull/77185#discussion_r3079759081
Path: test/e2e/specs/editor/various/autocomplete-and-mentions.spec.js
...
I tried a couple of methods, but unfortunately, this was the most "reliable" one.
...
- test/e2e/specs/editor/various/autocomplete-and-mentions.spec.js (MODIFIED, +175/-4)
