...
type: pull_request
state: MERGED
merged_at: 2026-01-07T17:00:25Z
url: https://github.com/WordPress/gutenberg/pull/74296
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #74296: UI: Add `Fieldset` primitives

- URL: https://github.com/WordPress/gutenberg/pull/74296
- Author: mirka
- Created: 2025-12-31T15:26:00Z
- Updated: 2026-01-07T18:00:55Z
- Comments: 4 of 4
- Review threads: 2 of 2
- Changed files: 13 of 13
- Diff: ../pr-diffs/74296.diff
...
Add `Fieldset` primitive components to `@wordpress/ui`.

## Why?

These adopt the same label/description styles used in `BaseControl` in `@wordpress/components` (and `Field` in `@wordpress/ui`), but provide `fieldset` semantics instead. They are modular from the outset, and based on [Base UI](https://base-ui.com/react/components/fieldset).

## How?

A context-based `Description` subcomponent is added by us to the Base UI components. This will be necessary in things like our `RadioControl` replacement, which has a description for the entire group.
...
<img width="589" height="290" alt="Storybook story for Fieldset" src="https://github.com/user-attachments/assets/8c1e0be1-0771-41ba-86e6-068d26b470b7" />

## Issue comments

### github-actions on 2025-12-31T15:26:09Z

URL: https://github.com/WordPress/gutenberg/pull/74296#issuecomment-3702388261
...
### github-actions on 2025-12-31T15:37:37Z

URL: https://github.com/WordPress/gutenberg/pull/74296#issuecomment-3702404777
...
### aduth on 2026-01-02T15:04:06Z

URL: https://github.com/WordPress/gutenberg/pull/74296#issuecomment-3705542410
...
### mirka on 2026-01-07T16:06:25Z

URL: https://github.com/WordPress/gutenberg/pull/74296#issuecomment-3719600930

> Regarding the included example, I wonder if the discussion at #63734 would be relevant to how we'd want to organize the subcomponents (i.e. description first)?

Definitely. I'm still having some discussions and doing some experiments on this matter, so I'll update everything (including `@wordpress/components`) at once as soon as we have a decision.

## Review threads

### Review thread 1

#### aduth on 2026-01-02T15:11:19Z

URL: https://github.com/WordPress/gutenberg/pull/74296#discussion_r2657880716
Path: packages/ui/src/form/primitives/fieldset/style.module.css
...
Should it use `Stack` ? I guess this is a similar discussion to https://github.com/WordPress/gutenberg/pull/73931#issuecomment-3687479221 as far as how much we lean on CSS and tolerance for HTML element nesting vs. flattening 🙃

#### mirka on 2026-01-07T16:49:31Z

URL: https://github.com/WordPress/gutenberg/pull/74296#discussion_r2669221286
Path: packages/ui/src/form/primitives/fieldset/style.module.css
...
I was thinking about this and one gotcha I noticed was that, if a component takes a `render` prop, then every time a consumer wants to render as a different HTML tag our default stack styling is going to be overwritten if we style our component with a `render={ <Stack /> }` .

Sometimes this might come in handy if the consumer actually wanted to wipe out the default styles, but then again we wouldn't be giving any kind of consistent developer experience because some styles will still often be set with normal CSS as well.

```jsx
function MyLibraryComponent( props ) {
	return (
		<div
			render={ <Stack /> }
			className={ styles[ 'library-component' ] }
			{ ...props }
		/>
	);
}

<MyLibraryComponent render={ <section /> } />;
```

And I'm not confident how good of an idea it is to do nested renders `render={ <Stack render={ restProps.render } /> }` or do `mergeProps` every time this needs to happen.

This makes me lean towards avoiding using `render` to style base components that take a `render` prop from the consumer.

#### ciampo on 2026-01-07T18:00:55Z

URL: https://github.com/WordPress/gutenberg/pull/74296#discussion_r2669539523
Path: packages/ui/src/form/primitives/fieldset/style.module.css
...
On one hand, the idea of re-using our components internally feels very cool and aligned with the concepts of building modular and inter-composable components.

On the other hand, I'm also slightly intimidated about having internal code that looks like `render={ <Stack render={ restProps.render } />` as it may be more difficult to maintain long-term and even have some unexpected consequences in terms of prop merging.

I'm not too worried about consistency, since IMO it should be completely fine that consumers use CSS+design tokens to apply styles.

I also like to frame the question in terms of utility: I don't think that, in this case, using `Stack` brings a lot of value to `Fieldset`, and therefore using CSS may be a better trade-off?

### Review thread 2

#### aduth on 2026-01-02T15:27:18Z

URL: https://github.com/WordPress/gutenberg/pull/74296#discussion_r2657905955
Path: packages/ui/src/form/primitives/fieldset/stories/index.story.tsx
...
#### mirka on 2026-01-07T16:07:22Z

URL: https://github.com/WordPress/gutenberg/pull/74296#discussion_r2669070196
Path: packages/ui/src/form/primitives/fieldset/stories/index.story.tsx
...
- packages/ui/src/form/primitives/field/root.tsx (MODIFIED, +2/-0)
- packages/ui/src/form/primitives/fieldset/context.tsx (ADDED, +13/-0)
- packages/ui/src/form/primitives/fieldset/description.tsx (ADDED, +36/-0)
- packages/ui/src/form/primitives/fieldset/details.tsx (ADDED, +46/-0)
- packages/ui/src/form/primitives/fieldset/index.ts (ADDED, +6/-0)
- packages/ui/src/form/primitives/fieldset/legend.tsx (ADDED, +17/-0)
- packages/ui/src/form/primitives/fieldset/root.tsx (ADDED, +42/-0)
- packages/ui/src/form/primitives/fieldset/stories/index.story.tsx (ADDED, +67/-0)
- packages/ui/src/form/primitives/fieldset/style.module.css (ADDED, +12/-0)
- packages/ui/src/form/primitives/fieldset/test/index.test.tsx (ADDED, +135/-0)
- packages/ui/src/form/primitives/fieldset/types.ts (ADDED, +35/-0)
