...
type: pull_request
state: OPEN
url: https://github.com/WordPress/gutenberg/pull/76604
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Package] Theme"]
---

# PR #76604: Theme: Enrich `design-tokens.js` export with more token data

- URL: https://github.com/WordPress/gutenberg/pull/76604
- Author: mirka
- Created: 2026-03-17T22:16:05Z
- Updated: 2026-04-20T17:01:11Z
- Comments: 9 of 9
- Review threads: 3 of 3
- Changed files: 9 of 9
- Diff: ../pr-diffs/76604.diff

## Body

## What?

Replaces the flat `string[]` default export of `@wordpress/theme/design-tokens.js` with two named exports that carry full token metadata:

- `tokens` — an object keyed by CSS variable name, with `$description`, `$type`, `group`, and default values for each token.
- `groups` — an object keyed by group name (derived from the source token file), with an array of member token names as values.

```js
import { tokens, groups } from '@wordpress/theme/design-tokens.js';

tokens['--wpds-border-radius-md'];
// {
...
tokens['--wpds-color-bg-interactive-brand-strong'];
// {
//   $description: 'Background color for interactive elements with brand tone and strong emphasis.',
//   $type: 'color',
//   group: 'color',
//   // No `modes` — value depends on --wp-admin-theme-color at runtime.
// }

groups.border;
// ['--wpds-border-radius-xs', '--wpds-border-radius-sm', …]
```

## Why?

The previous export was a flat list of CSS variable names used only for lint rule validation. Storybook stories and documentation that need to display token metadata (descriptions, types, values) currently hardcode this information, which drifts from the source of truth in `tokens/*.json`.

By exposing the data that the Terrazzo build already has access to, consumers like stories and docs can stay in sync without duplication. Having both the DTCG `$value` (structured, for programmatic use) and `css` (string, for documentation contexts) covers the common access patterns.

## How?

Updated the `terrazzo-plugin-known-wpds-css-variables` Terrazzo plugin to read `$description`, `$type`, and `$value` from the token data and output the two named exports. Default values are nested under `modes.default` to allow future mode expansion (e.g. compact, high-contrast). Brand-dependent color tokens (those whose runtime value depends on `--wp-admin-theme-color`) omit the `modes` property since they have no stable default.

Updated the two consumers of the previous default export:

- `packages/eslint-plugin/rules/no-unknown-ds-tokens.js` — switched from `new Set(tokenList)` to `new Set(Object.keys(tokenMap))`.
- `packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.mjs` — same.

Also moved the Chrome font-weight 500-to-499 workaround ([chromium#40552893](https://issues.chromium.org/issues/40552893)) from the CSS plugin transform layer into `typography.json` directly, so that `$value` and `css` are consistent in the output.

### Design decisions

- **DTCG naming**: `$description`, `$type`, and `$value` use the DTCG `$`-prefix convention. `css` and `modes` are not DTCG terms and use plain names.
- **`modes` namespace**: Values are nested under `modes.default` rather than flat on the token entry, anticipating future mode support.
- **Brand token exclusion**: Tokens referencing `--wp-admin-theme-color` have no stable default. Rather than exposing a misleading fallback, these entries omit `modes` entirely.

### Known limitation: type narrowing

The generated file is `.mjs`, so TypeScript infers widened types (`$type` is `string`, not the literal `"dimension"`). Consumers can't use discriminated union narrowing based on `$type` to infer the shape of `$value`. Full literal type inference would require `as const`, which is TypeScript-only syntax. Once the project's minimum Node version is raised to 24+ (which supports native TypeScript execution via type stripping), we can convert the generated file to `.ts` and add `as const`.
...
```
npx jest --config test/unit/jest.config.js packages/eslint-plugin/rules/__tests__/no-unknown-ds-tokens.js --no-coverage
npx jest packages/theme/src/stylelint-plugins/test/no-unknown-ds-tokens.test.ts --no-coverage
```
...
### github-actions on 2026-03-17T22:43:10Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4078436900
...
| `build/scripts/editor/index.min.js` | 358 kB |
...
### github-actions on 2026-03-17T23:43:09Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4078708022

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 2202947935bbcb195c68ebfad388788a25ef90ea.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23445569685
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### aduth on 2026-03-19T19:19:46Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4092721817

> By exposing the metadata that the Terrazzo build already has access to, consumers like stories and docs can stay in sync without duplication.

What would this look like in practice?

### mirka on 2026-03-23T13:29:39Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4110614627

> > By exposing the metadata that the Terrazzo build already has access to, consumers like stories and docs can stay in sync without duplication.
> 
> What would this look like in practice?

This was motivated by seeing Storybook files [like this](https://github.com/WordPress/gutenberg/pull/76097/changes#diff-82610f1e8b3e25464afae0881aad6d89bb2baed6069ff057e34e2d2efa90de85), which was manually duplicating the token descriptions. Another use case is token values in Framer Motion setups, where we cannot use CSS variables.

https://github.com/WordPress/gutenberg/blob/4c2851a0ebe332334d9a03f50c9747d551ce3f30/packages/components/src/utils/dropdown-motion.ts#L4-L15

It's also useful in Emotion files, which currently cannot be processed by the fallback injector (#75872).

### github-actions on 2026-03-23T14:51:13Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4111224803
...
### aduth on 2026-03-25T20:11:31Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4129468176

I'll give this a more detailed look tomorrow, but thinking out loud a bit on initial thoughts: I'm a bit confused about what we want this file to be, and what should be the sources of truth. It looks like it duplicates a slice of what we could expect to point people to the original token values for, but then also mixes in some CSS-specific concerns. Could we, for example, structure this more as a pointer or mapping of the tokens and equivalent CSS properties and expect people to use the original token data instead.

Curious what you think, since I haven't fully made up my mind either. Everything in "prebuilt" is ultimately a derivation of the original tokens, so maybe this is fine as just one more that's optimized for common usage patterns in JavaScript.

### mirka on 2026-03-26T15:43:43Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4136089741

> Could we, for example, structure this more as a pointer or mapping of the tokens and equivalent CSS properties and expect people to use the original token data instead.
...
### aduth on 2026-03-26T15:50:30Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4136138927

> One thought I had was whether we should just expose the original json files. Those are in a standardized format, and so consumers could then manipulate them however they wish. The current approach in this PR is kind of like a convenience layer for that.

Yeah, this is exactly what I was thinking, or... more accurately, what I thought we already did 😅 They're sort-of consumable as-is in JavaScript if we expose them (better with [bleeding-edge JSON modules supports](https://github.com/tc39/proposal-json-modules)).

After letting this sit in my brain overnight, I think this can work. As you say, it's just a convenience to that data. And a derivation of the source tokens in the same way the existing CSS or other prebuild artifacts are. It could be helpful to have a clearer articulation of all that we expect it contain, because right now it cherry-picks a few details from tokens and combines with extra things like CSS, group.

### ciampo on 2026-03-27T16:41:01Z

URL: https://github.com/WordPress/gutenberg/pull/76604#issuecomment-4143867659

> One thought I had was whether we should just expose the original json files

Having `terrazzo` should make it a lot easier to manipulate and expose the data in the right format — I guess it's just a matter of picking the best compromise. Less redundancy, but more computation to derive data <-> more duplication of the same info in different formats, easier use of such information

## Review threads

### Review thread 1 (resolved)

#### ciampo on 2026-03-18T13:08:20Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2953325350
Path: packages/theme/src/prebuilt/js/design-tokens.mjs

I'd expect the `postbuild` npm script in the `@wordress/theme` package to format this correctly, maybe the `mjs` extension is ignored by prettier?

#### mirka on 2026-03-23T12:55:23Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2974862687
Path: packages/theme/src/prebuilt/js/design-tokens.mjs

I think that was just a faulty AI commit 😅

### Review thread 2

#### aduth on 2026-03-19T19:19:16Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2962206405
...
#### mirka on 2026-03-23T14:10:09Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2975295080
Path: packages/theme/CHANGELOG.md
...
#### mirka on 2026-03-23T15:20:35Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2975744651
Path: packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts
...
#### aduth on 2026-03-25T20:10:22Z

URL: https://github.com/WordPress/gutenberg/pull/76604#discussion_r2990774739
Path: packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts
...
- packages/eslint-plugin/rules/no-unknown-ds-tokens.js (MODIFIED, +2/-3)
- packages/theme/CHANGELOG.md (MODIFIED, +4/-0)
- packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts (MODIFIED, +55/-13)
- packages/theme/docs/tokens.md (MODIFIED, +19/-19)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +1/-1)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +2579/-142)
- packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.mjs (MODIFIED, +2/-2)
- packages/theme/terrazzo.config.ts (MODIFIED, +0/-22)
- packages/theme/tokens/typography.json (MODIFIED, +2/-2)
