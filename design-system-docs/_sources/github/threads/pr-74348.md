...
type: pull_request
state: MERGED
merged_at: 2026-01-07T08:07:55Z
url: https://github.com/WordPress/gutenberg/pull/74348
...
# PR #74348: Build: use .mjs extensions for build-module files

- URL: https://github.com/WordPress/gutenberg/pull/74348
- Author: jsnajdr
- Created: 2026-01-05T12:47:48Z
- Updated: 2026-01-07T08:08:41Z
- Comments: 3 of 3
- Review threads: 0 of 0
- Changed files: 103 of 103
- Diff: ../pr-diffs/74348.diff

## Body

Attempt to fix the CJS build issues described in https://github.com/WordPress/gutenberg/pull/73822#issuecomment-3710178745. The plan is to:
1. Remove `type: module` from `package.json` files, because that enables strict Node.js compat in esbuild (https://github.com/evanw/esbuild/issues/2623) which makes it practically impossible to use `default` exports while producing CJS versions of packages.
2. Instead of `type: module` use the `.mjs` extension for all ESM files. The idea is that we don't use `.js` extensions, for which `type: module` provides the default format, but we always specify the module format explicitly, with the `.cjs` or `.mjs` extension.

Let's see what CI checks think about this.

## Issue comments

### github-actions on 2026-01-05T13:04:53Z

URL: https://github.com/WordPress/gutenberg/pull/74348#issuecomment-3710342554
...
### github-actions on 2026-01-05T17:37:24Z

URL: https://github.com/WordPress/gutenberg/pull/74348#issuecomment-3711423282
...
| `build/styles/block-editor/style-rtl.css` | 18.2 kB |
| `build/styles/block-editor/style-rtl.min.css` | 15.6 kB |
| `build/styles/block-editor/style.css` | 18.2 kB |
| `build/styles/block-editor/style.min.css` | 15.6 kB |
...
| `build/styles/block-library/categories/style-rtl.css` | 209 B |
| `build/styles/block-library/categories/style-rtl.min.css` | 152 B |
| `build/styles/block-library/categories/style.css` | 217 B |
| `build/styles/block-library/categories/style.min.css` | 152 B |
...
| `build/styles/block-library/style-rtl.css` | 20.1 kB |
...
| `build/styles/edit-site/style-rtl.css` | 21.4 kB |
| `build/styles/edit-site/style-rtl.min.css` | 16.4 kB |
| `build/styles/edit-site/style.css` | 21.5 kB |
...
### jsnajdr on 2026-01-06T20:06:09Z

URL: https://github.com/WordPress/gutenberg/pull/74348#issuecomment-3716131005

@manzoorwanijk @aduth @youknowriad This is now ready as a followup to #73822. It should fix the issues that Jetpack encounters when trying to use the `next` packages in https://github.com/Automattic/jetpack/pull/46397.

I had to give up on `"type": "module"` completely because esbuild makes certain assumptions about the CJS version of the modules that are not true (https://github.com/evanw/esbuild/issues/2623), and can't be changed through any configuration or plugin.

Instead, the published files in `build` and `build-module` consistently use the `.mjs` and `.cjs` extensions. So that there is never any ambiguity about the `.js` extension that would need to be resolved by looking at `"type"` in `package.json`.

After some additional fixes here and there, renaming a few files (mostly stylelint) from `.js` to `.mjs`, this is all green and ready to go.

One major package I found that uses this convention is [stylelint](https://app.unpkg.com/stylelint). No `type` field in `package.json`, dual ESM/CJS modules with `.mjs`/`.cjs` extensions, and these files referenced in `exports`.
...
- packages/a11y/package.json (MODIFIED, +3/-4)
- packages/abilities/package.json (MODIFIED, +3/-4)
- packages/admin-ui/package.json (MODIFIED, +2/-3)
- packages/annotations/package.json (MODIFIED, +2/-3)
- packages/api-fetch/package.json (MODIFIED, +2/-3)
- packages/asset-loader/package.json (MODIFIED, +2/-3)
- packages/autop/package.json (MODIFIED, +2/-3)
- packages/blob/package.json (MODIFIED, +2/-3)
- packages/block-directory/package.json (MODIFIED, +2/-3)
- packages/block-editor/package.json (MODIFIED, +5/-6)
- packages/block-library/package.json (MODIFIED, +11/-12)
- packages/block-serialization-default-parser/package.json (MODIFIED, +2/-3)
- packages/blocks/package.json (MODIFIED, +4/-5)
- packages/boot/package.json (MODIFIED, +3/-4)
- packages/commands/package.json (MODIFIED, +2/-3)
- packages/components/.stylelintrc.mjs (RENAMED, +0/-0)
- packages/components/CHANGELOG.md (MODIFIED, +1/-1)
- packages/components/package.json (MODIFIED, +2/-3)
- packages/compose/package.json (MODIFIED, +2/-3)
- packages/core-abilities/package.json (MODIFIED, +3/-4)
- packages/core-commands/package.json (MODIFIED, +2/-3)
- packages/core-data/package.json (MODIFIED, +3/-4)
- packages/customize-widgets/package.json (MODIFIED, +2/-3)
- packages/data-controls/package.json (MODIFIED, +2/-3)
- packages/data/package.json (MODIFIED, +2/-3)
- packages/dataviews/CHANGELOG.md (MODIFIED, +1/-1)
- packages/dataviews/package.json (MODIFIED, +2/-3)
- packages/date/package.json (MODIFIED, +2/-3)
- packages/deprecated/package.json (MODIFIED, +2/-3)
- packages/dom-ready/package.json (MODIFIED, +2/-3)
- packages/dom/package.json (MODIFIED, +2/-3)
- packages/e2e-test-utils-playwright/package.json (MODIFIED, +2/-2)
- packages/edit-post/package.json (MODIFIED, +2/-3)
- packages/edit-site-init/package.json (MODIFIED, +2/-3)
- packages/edit-site/package.json (MODIFIED, +2/-3)
- packages/edit-widgets/package.json (MODIFIED, +2/-3)
- packages/editor/package.json (MODIFIED, +4/-5)
- packages/element/package.json (MODIFIED, +2/-3)
- packages/escape-html/package.json (MODIFIED, +2/-3)
- packages/fields/package.json (MODIFIED, +2/-3)
- packages/format-library/package.json (MODIFIED, +2/-3)
- packages/global-styles-engine/package.json (MODIFIED, +2/-3)
- packages/global-styles-ui/package.json (MODIFIED, +2/-3)
- packages/hooks/package.json (MODIFIED, +2/-3)
- packages/html-entities/package.json (MODIFIED, +2/-3)
- packages/i18n/package.json (MODIFIED, +2/-3)
- packages/icons/package.json (MODIFIED, +2/-3)
- packages/image-cropper/package.json (MODIFIED, +2/-3)
- packages/interactivity-router/package.json (MODIFIED, +4/-5)
- packages/interactivity/package.json (MODIFIED, +3/-3)
- packages/interface/package.json (MODIFIED, +4/-5)
- packages/is-shallow-equal/package.json (MODIFIED, +2/-3)
- packages/jest-console/package.json (MODIFIED, +2/-3)
- packages/jest-puppeteer-axe/package.json (MODIFIED, +2/-3)
- packages/keyboard-shortcuts/package.json (MODIFIED, +2/-3)
- packages/keycodes/package.json (MODIFIED, +2/-3)
- packages/latex-to-mathml/package.json (MODIFIED, +4/-5)
- packages/lazy-editor/package.json (MODIFIED, +3/-4)
- packages/list-reusable-blocks/package.json (MODIFIED, +2/-3)
- packages/media-fields/package.json (MODIFIED, +2/-3)
- packages/media-utils/package.json (MODIFIED, +2/-3)
- packages/notices/package.json (MODIFIED, +2/-3)
- packages/nux/package.json (MODIFIED, +4/-5)
- packages/patterns/package.json (MODIFIED, +4/-5)
- packages/plugins/package.json (MODIFIED, +2/-3)
- packages/preferences-persistence/package.json (MODIFIED, +2/-3)
- packages/preferences/package.json (MODIFIED, +2/-3)
- packages/primitives/package.json (MODIFIED, +2/-3)
- packages/priority-queue/package.json (MODIFIED, +2/-3)
- packages/private-apis/package.json (MODIFIED, +2/-3)
- packages/react-i18n/package.json (MODIFIED, +2/-3)
- packages/redux-routine/package.json (MODIFIED, +2/-3)
- packages/report-flaky-tests/package.json (MODIFIED, +2/-2)
- packages/reusable-blocks/package.json (MODIFIED, +4/-5)
- packages/rich-text/package.json (MODIFIED, +4/-5)
- packages/route/package.json (MODIFIED, +3/-4)
- packages/router/package.json (MODIFIED, +2/-3)
- packages/server-side-render/package.json (MODIFIED, +2/-3)
- packages/shortcode/package.json (MODIFIED, +2/-3)
- packages/style-engine/package.json (MODIFIED, +2/-3)
- packages/sync/package.json (MODIFIED, +2/-3)
- packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts (MODIFIED, +1/-1)
- packages/theme/package.json (MODIFIED, +6/-7)
- packages/theme/src/prebuilt/js/design-tokens.mjs (RENAMED, +0/-0)
- packages/theme/src/stylelint-plugins/no-setting-wpds-custom-properties.mjs (RENAMED, +0/-0)
- packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.mjs (RENAMED, +1/-1)
- packages/theme/src/stylelint-plugins/test/.stylelintrc.json (MODIFIED, +2/-2)
- packages/theme/terrazzo.config.ts (MODIFIED, +1/-1)
- packages/token-list/package.json (MODIFIED, +2/-3)
- packages/ui/package.json (MODIFIED, +2/-3)
- packages/undo-manager/package.json (MODIFIED, +2/-3)
- packages/upload-media/package.json (MODIFIED, +2/-3)
- packages/url/package.json (MODIFIED, +2/-3)
- packages/viewport/package.json (MODIFIED, +2/-3)
- packages/views/package.json (MODIFIED, +2/-3)
- packages/vips/package.json (MODIFIED, +2/-3)
- packages/warning/package.json (MODIFIED, +2/-3)
- packages/widgets/package.json (MODIFIED, +2/-3)
- packages/wordcount/package.json (MODIFIED, +2/-3)
- packages/workflow/package.json (MODIFIED, +3/-4)
- packages/wp-build/package.json (MODIFIED, +1/-1)
- packages/wp-build/src/build.mjs (MODIFIED, +4/-3)
- tools/eslint/import-resolver.js (MODIFIED, +3/-1)
