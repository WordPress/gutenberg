...
type: pull_request
state: MERGED
merged_at: 2026-02-06T16:25:29Z
url: https://github.com/WordPress/gutenberg/pull/75183
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #75183: @wordpress/ui: add Dialog component

- URL: https://github.com/WordPress/gutenberg/pull/75183
- Author: ciampo
- Created: 2026-02-03T17:50:31Z
- Updated: 2026-03-03T18:00:13Z
- Comments: 16 of 16
- Review threads: 14 of 14
- Changed files: 16 of 16
- Diff: ../pr-diffs/75183.diff
...
Add `Dialog` component to `@wordpress/ui`, the successor for the `Modal` component in `@wordpress/components`
...
Use Base UI primitives and build on top of it
...
![Screenshot 2026-02-06 at 13 00 41](https://github.com/user-attachments/assets/78ea0d7b-8cef-4a3e-ac3d-26df9e82cc32)

## Follow ups

- [ ] Rename `Heading` to `Title`, consider removing `Header`, rename `CloseIcon` to `CloseIconButton`?
- [ ] Update the guidance added in #74778 to reference this component instead of `Modal`
- [ ] Create a new `ConfirmDialog` component. As [surfaced in my review comment for #74778](https://github.com/WordPress/gutenberg/pull/74778#pullrequestreview-3712957501), the guidelines surface a need for an abstraction supporting workflows including a destructive button. I think we could extend the "Confirm Dialog" pattern proposed here with a dedicated higher-level component abstraction that supports this use-case.
- [ ] Update `DataViews` `RenderModal` to use this component [instead of `Modal`](https://github.com/WordPress/gutenberg/blob/523bfc66a061e9e3eb211190ea028845635e0094/packages/dataviews/src/components/dataviews-item-actions/index.tsx#L105-L131), since `DataViews` is already starting to use the new theming and components
- [ ] Add explicit support to one or more [advanced use cases](https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3847470789)
- [ ] https://github.com/WordPress/gutenberg/issues/75291

## Issue comments

### github-actions on 2026-02-03T17:55:53Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3842794608
...
### ciampo on 2026-02-03T17:56:05Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3842795446

@copilot can you review this PR?

### copilot-swe-agent on 2026-02-03T17:56:14Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3842796053

@ciampo I've opened a new pull request, #75184, to work on those changes. Once the pull request is ready, I'll request review from you.

### github-actions on 2026-02-03T18:02:09Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3842823054
...
| `build/styles/edit-site/style-rtl.min.css` | 15.9 kB |
| `build/styles/edit-site/style.css` | 19.8 kB |
| `build/styles/edit-site/style.min.css` | 15.9 kB |
...
### github-actions on 2026-02-04T10:45:51Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3846689005

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 18e4f4ff574d263344079c83cfe0fe14869046ed.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/21672744028
📝  Reported issues:
- #69177 in `/test/e2e/specs/editor/blocks/navigation-colors.spec.js`

### aduth on 2026-02-04T13:06:00Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3847350471

A few things we can follow-up with separately once this lands that this work helps unlock:

- Update the guidance added by @jameskoster in #74778 to reference this component instead of `Modal`
- Create a new `ConfirmDialog` component. As [surfaced in my review comment for #74778](https://github.com/WordPress/gutenberg/pull/74778#pullrequestreview-3712957501), the guidelines surface a need for an abstraction supporting workflows including a destructive button. I think we could extend the "Confirm Dialog" pattern proposed here with a dedicated higher-level component abstraction that supports this use-case.
- Update `DataViews` `RenderModal` to use this component [instead of `Modal`](https://github.com/WordPress/gutenberg/blob/523bfc66a061e9e3eb211190ea028845635e0094/packages/dataviews/src/components/dataviews-item-actions/index.tsx#L105-L131), since `DataViews` is already starting to use the new theming and components (cc @oandregal )

### ciampo on 2026-02-04T13:31:04Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3847470789

@aduth Apart from addressing feedback, I pushed a couple more updates:

- added `defaultOpen` and `modal` props, as they feel necessary to an MVP of the component;
- added a new `full` size preset (dialog goes full screen, apart from some padding to show the backdrop);
- refactored how the dialog title is specified by the consumer: (cc @mirka too)
  - removed the `title` prop and reverted to using base ui's native strategy of specifying the title via `Dialog.Title`
  - added runtime checks (in the same style as in https://github.com/WordPress/gutenberg/pull/75170) to have the component throw an error if the Dialog.Title is not provided, or has empty text content.
  - this feels like a better solution to me, more coherent with the API approach we've been using, which lets base ui fully handle the dialog label, while also adding a strong guardrail against omitting the title

---

Apart from the above, we should understand which of the following patterns/examples (from the base ui docs) we want to officially support and document:

- [nested dialogs](https://base-ui.com/react/components/dialog#nested-dialogs), including the slightly more advanced [close confirmation](https://base-ui.com/react/components/dialog#close-confirmation) example (I assume we do)
- [outside scroll dialog](https://base-ui.com/react/components/dialog#outside-scroll-dialog) (this is more something for the @WordPress/gutenberg-design folks to address)
- [inside scroll dialog](https://base-ui.com/react/components/dialog#inside-scroll-dialog) (this is more something for the @WordPress/gutenberg-design folks to address)
- [Placing elements outside the popup](https://base-ui.com/react/components/dialog#placing-elements-outside-the-popup) (this is more something for the @WordPress/gutenberg-design folks to address)
- [Detached triggers](https://base-ui.com/react/components/dialog#detached-triggers)
- [Multiple triggers](https://base-ui.com/react/components/dialog#multiple-triggers), including [Controlled mode with multiple triggers](https://base-ui.com/react/components/dialog#controlled-mode-with-multiple-triggers)




Adding support and documenting these use cases can happen in follow-ups, too.

### jameskoster on 2026-02-04T16:34:53Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3848488437

Can we set a default `size`, (maybe `medium`) applied to both regular and confirm dialogs?

### oandregal on 2026-02-06T09:12:39Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3858969895

> Update DataViews RenderModal to use this component [instead of Modal](https://github.com/WordPress/gutenberg/blob/523bfc66a061e9e3eb211190ea028845635e0094/packages/dataviews/src/components/dataviews-item-actions/index.tsx#L105-L131), since DataViews is already starting to use the new theming and components (cc @oandregal )

Perhaps it's a good idea to track `wordpress/ui` usage in DataViews in a tracking issue that list the new ui components as they are created? So that there's a single place to look for status and work. In terms of timing/help, it's going to be tight for me to help with this before Feb 19th (WordPress 7.0 beta).

### jameskoster on 2026-02-06T10:02:13Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3859299929

Noting that https://github.com/WordPress/gutenberg/pull/75173 has landed. Should we set up a basic side sheet variant in this PR, or handle it separately?

### ciampo on 2026-02-06T10:42:13Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3859606660

> Noting that #75173 has landed. Should we set up a basic side sheet variant in this PR, or handle it separately?

Let's work on that separately, and understand its priority. There are already a lot of high-priority tasks on my list (ConfirmDialog with destructive actions, Card, and other additions to the ui library), and I'd like to get this one closed.

Could you create a separate issue with some context about it and maybe even some rough specs?

### ciampo on 2026-02-06T14:37:51Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3860826805

Pushed a few more changes:

- used DS token for font weight
- added box shadow using DS elevation token
- set default size to `medium`, added an additional `stretch` option (ie. no max width, for better customizability, which is the current default behaviour of the legacy `Modal` component)
- added a responsive min-width to better first small viewports
- reworked Storybook example, replacing different size-related stories with a single "All size" story
- Tweaked Storybook controls for the `modal` prop
- Removed unnecessary timeouts in tests

I believe this PR is now ready for a final review round

### ciampo on 2026-02-06T16:25:13Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3861317534

@WordPress/gutenberg-components @jameskoster  I'll go ahead and merge this PR, and start the work on `ConfirmDialog`.

Happy to iterate in follow-ups if anything comes up via later feedback. (I also added a section in the PR description to track the main follow-up tasks already discussed)

### jameskoster on 2026-02-06T16:30:50Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3861361981

The only thing I'm unsure about is the `stretch` size. Is that required for anything? I'm not aware of any in-product use cases for it.

### ciampo on 2026-02-06T17:18:36Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3861665148

> The only thing I'm unsure about is the `stretch` size. Is that required for anything? I'm not aware of any in-product use cases for it.

Given this is a low-level compoment, I thought we'd need to have a `size` option without a fixed max-width, useful to add ad-hoc customization on the popup size (for example, the `Modal` component currently behaves like that by default).

Instead of calling it `default` or leaving the prop as `undefined`, I opted to call it `stretch`.

### oandregal on 2026-03-03T18:00:13Z

URL: https://github.com/WordPress/gutenberg/pull/75183#issuecomment-3992692936

> Perhaps it's a good idea to track wordpress/ui usage in DataViews in a tracking issue that list the new ui components as they are created? So that there's a single place to look for status and work. In terms of timing/help, it's going to be tight for me to help with this before Feb 19th (WordPress 7.0 beta).

I've created https://github.com/WordPress/gutenberg/issues/76100 to track this work.

## Review threads

### Review thread 1 (resolved)

#### aduth on 2026-02-03T20:55:19Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2760959693
...
Do we need this? Is it reasonable to expect that someone would have to style dialog content this way?

From the styles included in this pull request, my impression is that the text should already have the color applied ([source](https://github.com/WordPress/gutenberg/blob/3be2f91acd98a63a00dd5a3c7e195ee272658b2d/packages/ui/src/dialog/style.module.css#L39))?

#### ciampo on 2026-02-04T10:40:22Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763342368
Path: packages/ui/src/dialog/stories/index.story.tsx
...
You're right, no need. Removed

### Review thread 2 (resolved)

#### aduth on 2026-02-03T21:04:44Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2761000793
...
Are these tests actually slow enough to warrant this? Maybe we can omit it?

```suggestion
```

#### ciampo on 2026-02-04T10:40:34Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763343117
Path: packages/ui/src/dialog/test/index.test.tsx
...
Yup, no need for this either

### Review thread 3 (resolved)

#### aduth on 2026-02-03T21:05:46Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2761005140
Path: packages/ui/src/dialog/header.tsx
...
Wondering if these components should be using `useRender` to allow `render` prop overrides, and to deal with some of the concerns like class merging, ref merging, etc. (per [documented design principles](https://github.com/WordPress/gutenberg/blob/trunk/packages/ui/README.md#core-design-principles))

#### ciampo on 2026-02-04T10:43:43Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763355492
Path: packages/ui/src/dialog/header.tsx
...
Switched to `useRender`

### Review thread 4 (resolved)

#### copilot-pull-request-reviewer on 2026-02-04T09:57:53Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763161449
Path: packages/ui/src/dialog/test/index.test.tsx
...
The test expects the heading element to be an HTMLHeadingElement, but the Heading component is typed as HTMLDivElement in its forwardRef and types.ts. Based on how Base UI Dialog.Title works and the pattern used in other Base UI components (like Fieldset.Legend which also uses HTMLDivElement), the test should expect HTMLDivElement instead of HTMLHeadingElement.
```suggestion
		expect( headingRef.current ).toBeInstanceOf( HTMLDivElement );
```

#### ciampo on 2026-02-04T10:51:07Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763385068
Path: packages/ui/src/dialog/test/index.test.tsx
...
I'll update the ref type of the `Heading` element instead to be `HTMLHeadingElement`

### Review thread 5 (resolved)

#### copilot-pull-request-reviewer on 2026-02-04T09:57:53Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763161485
Path: packages/ui/src/dialog/close-icon.tsx
...
Props are spread before the icon and label defaults, which means user-provided icon or label values will be overridden. Since CloseIconProps types define both icon and label as optional, allowing them to be customized, the defaults should be provided first and then props should be spread after them. This would allow users to override the default close icon or label if needed.
```suggestion
						icon={ close }
						label={ __( 'Close' ) }
						{ ...props }
...
#### ciampo on 2026-02-04T10:44:24Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763358288
Path: packages/ui/src/dialog/close-icon.tsx
...
#### copilot-pull-request-reviewer on 2026-02-04T12:37:13Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2763796701
...
Missing blank line between interface definitions. There should be a blank line between `PopupProps` and `ActionProps` for consistency with the other interface definitions in this file.
```suggestion
}
```
...
#### ciampo on 2026-02-04T13:33:56Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2764051150
...
Should we wrap the dialog's popup content in a themeprovider, similarly to what we do for other portalled UI?

#### aduth on 2026-02-04T20:51:38Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2765929160
Path: packages/ui/src/dialog/popup.tsx
...
Hm, I feel like we have to? For the theme context to be respected. I don't think it needs to be like what we do in Tooltip with hard-coding a fixed background color, but without it, none of the ancestor's color/density configuration would make its way into the dialog content?

If that tracks, then we should probably document this somewhere; i.e. every portaled element should handle ThemeProvider forwarding.

#### ciampo on 2026-02-06T16:19:12Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774926437
Path: packages/ui/src/dialog/popup.tsx
...
#### jameskoster on 2026-02-04T16:23:46Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2764847994
...
Should we use a token here?

#### ciampo on 2026-02-06T11:52:15Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2773761922
Path: packages/ui/src/dialog/style.module.css
...
Yup, switched to `--wpds-font-weight-medium`

### Review thread 9 (resolved)

#### ciampo on 2026-02-06T14:39:19Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774489595
Path: storybook/preview.jsx

```diff
@@ -241,6 +241,14 @@ export const parameters = {
...
@mirka Without this, our docs would show non-default stories first depending on the alphabetical order.

#### ciampo on 2026-02-06T15:18:14Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774669171
Path: storybook/preview.jsx
...
I see this was already addressed in https://github.com/WordPress/gutenberg/pull/75295, I will remove the changes from my PR

### Review thread 10 (resolved)

#### copilot-pull-request-reviewer on 2026-02-06T14:49:18Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774533702
Path: packages/ui/src/dialog/test/index.test.tsx
...
Using a fixed `setTimeout(..., 50)` to "ensure validation has run" makes the test slower and can be flaky on slower CI. Prefer waiting on a deterministic condition (e.g. `waitFor` around an expectation) rather than sleeping.
```suggestion
			// Wait for the dialog to appear and for validation to run without errors
			await waitFor( () => {
				expect( screen.getByRole( 'dialog' ) ).toBeInTheDocument();
				expect( onError ).not.toHaveBeenCalled();
			} );
...
#### ciampo on 2026-02-06T15:51:05Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774807278
Path: packages/ui/src/dialog/test/index.test.tsx
...
#### copilot-pull-request-reviewer on 2026-02-06T14:49:19Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774533734
Path: packages/ui/src/dialog/types.ts
...
The `size` prop docs say `'full'` "stretches to fill available width and height", but the CSS still enforces `max-height: calc(100vh - ...)`/`width: calc(100vw - ...)`, so it won't actually fill the viewport. Either update the docs to match the intended behavior (full-with-padding) or adjust the `is-full` styles to remove the max constraints so it truly fills the viewport.

#### ciampo on 2026-02-06T15:50:55Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774806457
Path: packages/ui/src/dialog/types.ts
...
URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774533755
Path: packages/ui/src/dialog/test/index.test.tsx
...
`screen.findByRole('dialog')` waits for the element to appear and only then rejects, which makes this test slower and can be flaky depending on default timeouts. Prefer asserting absence with `queryByRole('dialog')` (or `expect(screen.queryByRole(...)).not.toBeInTheDocument()`) when you don't want to wait.
```suggestion
			expect( screen.queryByRole( 'dialog' ) ).not.toBeInTheDocument();
```

#### ciampo on 2026-02-06T15:50:39Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774805330
Path: packages/ui/src/dialog/test/index.test.tsx
...
I disagree — the `findByRole` was used to make use of its 1s retry window to give time for any async render to occur to Dialog and really make sure that no errors are thrown

### Review thread 13 (resolved)

#### copilot-pull-request-reviewer on 2026-02-06T14:49:19Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774533783
Path: packages/ui/src/dialog/test/index.test.tsx
...
#### copilot-pull-request-reviewer on 2026-02-06T16:05:33Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774870554
Path: packages/ui/src/dialog/types.ts
...
The `PopupProps.size` docs say `'full'` “stretches to fill available width and height”, but the `.popup` base styles still enforce `width: calc(100vw - 2 * padding)` and `max-height: calc(100vh - 2 * padding)`, and `.is-full` only sets `height: 100vh` (which is still constrained by `max-height`). Either update the docs to match the actual “full within viewport padding” behavior, or adjust the `.is-full` styles to remove the max constraints (and potentially set `width/height: 100vw/100vh`) so `full` truly fills the viewport.

#### ciampo on 2026-02-06T16:11:08Z

URL: https://github.com/WordPress/gutenberg/pull/75183#discussion_r2774894127
Path: packages/ui/src/dialog/types.ts
...
the docs already mention the padding.

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +1/-0)
- packages/ui/src/dialog/action.tsx (ADDED, +22/-0)
- packages/ui/src/dialog/close-icon.tsx (ADDED, +32/-0)
- packages/ui/src/dialog/context.tsx (ADDED, +113/-0)
- packages/ui/src/dialog/footer.tsx (ADDED, +26/-0)
- packages/ui/src/dialog/header.tsx (ADDED, +26/-0)
- packages/ui/src/dialog/index.ts (ADDED, +10/-0)
- packages/ui/src/dialog/popup.tsx (ADDED, +46/-0)
- packages/ui/src/dialog/root.tsx (ADDED, +14/-0)
- packages/ui/src/dialog/stories/index.story.tsx (ADDED, +177/-0)
- packages/ui/src/dialog/style.module.css (ADDED, +114/-0)
- packages/ui/src/dialog/test/index.test.tsx (ADDED, +309/-0)
- packages/ui/src/dialog/title.tsx (ADDED, +39/-0)
- packages/ui/src/dialog/trigger.tsx (ADDED, +14/-0)
- packages/ui/src/dialog/types.ts (ADDED, +93/-0)
