...
type: pull_request
state: MERGED
merged_at: 2026-02-20T11:11:12Z
url: https://github.com/WordPress/gutenberg/pull/75586
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","Backported to WP Core","[Package] Theme"]
---

# PR #75586: Theme: Add design token fallback generation

- URL: https://github.com/WordPress/gutenberg/pull/75586
- Author: mirka
- Created: 2026-02-16T20:54:25Z
- Updated: 2026-02-26T09:40:01Z
- Comments: 3 of 3
- Review threads: 6 of 6
- Changed files: 8 of 8
- Diff: ../pr-diffs/75586.diff

## Body

## What?

Adds infrastructure for generating fallback values for design system tokens (`--wpds-*`). This allows components using these tokens to render correctly even without a global design tokens stylesheet.

A follow-up PR (#75589) will add the build plugins that inject these fallbacks into CSS and JS files.

## Why?

Components built with `@wordpress/ui` use design tokens as CSS custom properties (e.g. `var(--wpds-color-fg-default)`). The default values for these are provided by a global design tokens stylesheet from `@wordpress/theme`, which will only be available in WP 7.1+. Without fallback values, the tokens resolve to nothing in older environments, which blocks adoption of `@wordpress/ui` in plugins that need to support earlier WP versions.

By baking in static fallback values at build time, `@wordpress/theme` becomes a progressive enhancement — theming kicks in when available, but components still render correctly without it.

For brand-derived tokens (the ones influenced by the WP admin theme color), the fallback expressions use `var(--wp-admin-theme-color)` and `color-mix()` so the components still respond to the admin color scheme — even without `@wordpress/theme`.

## How?

### Terrazzo plugin (`terrazzo-plugin-ds-token-fallbacks`)

A new Terrazzo build plugin generates a `design-token-fallbacks.mjs` file containing a map of every `--wpds-*` token to its fallback expression. It runs during `npm run build:tokens`.

The fallback for each token is determined as follows:
- **Non-brand tokens** — use the literal default-mode value from the design token definitions (e.g. `#1e1e1e`, `4px`).
- **Brand tokens** (derived from the primary color ramp) — compute a `color-mix(in oklch, var(--wp-admin-theme-color, <seed>) N%, black/white)` expression that approximates the token's value using the admin theme color. If the approximation exceeds a deltaE threshold, falls back to the literal hex.
- **Hard-coded overrides** — a small set of tokens (`fg-interactive-brand-strong`, `fg-interactive-brand-strong-active`) are pinned to `#fff` for contrast safety on strong brand backgrounds.
...
A Storybook story (`brand-fallbacks.story.tsx`) that visually demonstrates the brand token fallback rendering for review.

## Testing Instructions

1. Run `npm run build:tokens` in `packages/theme` — verify `src/prebuilt/js/design-token-fallbacks.mjs` is generated.
2. Run the unit tests: `npx jest packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts`
3. See the "Brand color fallbacks" story in Storybook (`npm run storybook:dev`, navigate to Theme > Color Ramps > Brand Fallbacks).


https://github.com/user-attachments/assets/0f4efbc2-4561-4187-bea2-15ddd226e4f4
...
### github-actions on 2026-02-16T21:06:01Z

URL: https://github.com/WordPress/gutenberg/pull/75586#issuecomment-3910508859
...
### github-actions on 2026-02-16T21:58:57Z

URL: https://github.com/WordPress/gutenberg/pull/75586#issuecomment-3910671571
...
### mirka on 2026-02-25T11:11:19Z

URL: https://github.com/WordPress/gutenberg/pull/75586#issuecomment-3958524768

Manually cherry picked to `wp/7.0` branch at #75901

## Review threads

### Review thread 1

#### mirka on 2026-02-16T21:30:21Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2814097779
...
#### mirka on 2026-02-16T21:55:28Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2814153981
...
#### mirka on 2026-02-16T21:58:31Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2814161183
Path: packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts
...
https://github.com/WordPress/gutenberg/blob/1c26a4c6574e266a8a8e671f1e0420c568715f4d/packages/base-styles/_mixins.scss#L511-L517

### Review thread 4 (resolved)

#### ciampo on 2026-02-19T12:43:43Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2827657835
Path: packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts
...
Nit: since the primary seed is constant, this could be refactored outside of the function?

#### mirka on 2026-02-19T21:10:18Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2830183352
Path: packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts
...
Addressed in 065bf3f469f2f82c49c9f88320448811493a929c

### Review thread 5

#### ciampo on 2026-02-19T12:46:47Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2827674963
...
#### mirka on 2026-02-19T21:07:00Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2830170216
Path: packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs
...
#### ciampo on 2026-02-19T12:48:25Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2827683930
Path: packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts
...
#### mirka on 2026-02-19T21:08:15Z

URL: https://github.com/WordPress/gutenberg/pull/75586#discussion_r2830174870
Path: packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts
...
- packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/index.ts (ADDED, +296/-0)
- packages/theme/bin/terrazzo-plugin-ds-token-fallbacks/test/index.test.ts (ADDED, +30/-0)
- packages/theme/src/color-ramps/stories/brand-fallbacks.story.tsx (ADDED, +165/-0)
- packages/theme/src/postcss-plugins/add-fallback-to-var.ts (ADDED, +29/-0)
- packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs (ADDED, +29/-0)
- packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts (ADDED, +91/-0)
- packages/theme/src/prebuilt/js/design-token-fallbacks.mjs (ADDED, +157/-0)
- packages/theme/terrazzo.config.ts (MODIFIED, +4/-0)
