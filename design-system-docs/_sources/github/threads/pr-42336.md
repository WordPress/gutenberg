...
type: pull_request
state: MERGED
merged_at: 2022-07-12T11:29:58Z
url: https://github.com/WordPress/gutenberg/pull/42336
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Performance","[Type] Code Quality","[Package] Data","[Package] Components","[Package] Blocks","[Package] Block library","[Package] Block editor","[Tool] Create Block","[Package] Edit Site"]
---

# PR #42336: Lodash: Refactor away from `_.isObject()`

- URL: https://github.com/WordPress/gutenberg/pull/42336
- Author: tyxla
- Created: 2022-07-11T15:26:18Z
- Updated: 2022-07-12T11:30:31Z
- Comments: 2 of 2
- Review threads: 1 of 1
- Changed files: 12 of 12
- Diff: ../pr-diffs/42336.diff
...
### github-actions on 2022-07-11T15:35:11Z

URL: https://github.com/WordPress/gutenberg/pull/42336#issuecomment-1180561944
...
| :--- | :---: | :---: |
| `build/block-editor/index.min.js` | 153 kB | -5 B (0%) |
| `build/blocks/index.min.js` | 47 kB | +4 B (0%) |
| `build/components/index.min.js` | 230 kB | -7 B (0%) |
| `build/data/index.min.js` | 7.99 kB | +18 B (0%) |
...
### tyxla on 2022-07-12T10:16:44Z

URL: https://github.com/WordPress/gutenberg/pull/42336#issuecomment-1181581999

Thanks for the feedback, @ciampo 😉 makes a lot of sense to go through those.

> in lodash, the null check isn't performed with strict equality

That's fair, however:

* We have ESLint rules that forbid non-strict checks
* `!= null` means "is not null and is not undefined", but in this case, the undefined check is not necessary because `typeof undefined` is not `object`, so that's covered by the other part of the condition.

> in lodash, isObject returns true also for functions

Yes, thanks for bringing that up. I've actually manually audited these instances and saw that we never seek to invoke the results anywhere. I think this demonstrates one of the big problems with Lodash - concealing behavior, and sometimes even illogical behavior. Who could guess that `isObject()` will return `true` for functions as well? Who could have guessed that they should have used `isObjectLike` instead? I think removing such usages also contributes to cleaner and less error-prone code because of unveiling such subtle behavior.

## Review threads

### Review thread 1

#### ciampo on 2022-07-12T10:09:03Z

URL: https://github.com/WordPress/gutenberg/pull/42336#discussion_r918792015
Path: packages/blocks/src/api/registration.js

```diff
@@ -131,6 +131,10 @@ import { store as blocksStore } from '../store';
...
On which basis did we decide when to refactor the code "inline" vs creating a `isObject` utility function?
...
#### tyxla on 2022-07-12T10:13:22Z

URL: https://github.com/WordPress/gutenberg/pull/42336#discussion_r918795812
Path: packages/blocks/src/api/registration.js
...
I'd disagree, while it adds verbosity, I don't see why we'd always want to keep it a separate function. I think it's enough that we are specifically explicitly excluding the `null` case, and it's a JavaScript quirk, so not much we can do about it.

## Changed files

- .eslintrc.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/color.js (MODIFIED, +10/-3)
- packages/block-editor/src/hooks/utils.js (MODIFIED, +5/-2)
- packages/block-library/src/utils/clean-empty-object.js (MODIFIED, +6/-2)
- packages/blocks/src/api/registration.js (MODIFIED, +5/-1)
- packages/blocks/src/api/serializer.js (MODIFIED, +3/-2)
- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/shortcut/index.js (MODIFIED, +1/-6)
- packages/create-block/lib/templates.js (MODIFIED, +2/-2)
- packages/data/src/controls.js (MODIFIED, +4/-5)
- packages/data/src/registry.js (MODIFIED, +5/-1)
- packages/edit-site/src/components/global-styles/global-styles-provider.js (MODIFIED, +6/-2)
