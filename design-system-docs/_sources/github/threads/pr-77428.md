...
type: pull_request
state: MERGED
merged_at: 2026-04-22T14:31:48Z
url: https://github.com/WordPress/gutenberg/pull/77428
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Code Quality","[Package] UI"]
---

# PR #77428: Add no-unsafe-render-order ESLint rule

- URL: https://github.com/WordPress/gutenberg/pull/77428
- Author: mirka
- Created: 2026-04-16T13:50:53Z
- Updated: 2026-04-22T14:32:40Z
- Comments: 3 of 3
- Review threads: 1 of 1
- Changed files: 9 of 9
- Diff: ../pr-diffs/77428.diff
...
- add a `no-unsafe-render-order` ESLint rule for the `VisuallyHidden` and `Link`/`Text` render-prop footguns
- update `VisuallyHidden` and `Link` guidance to only recommend the safe composition direction

This rule will be enabled in the recommended set of lint rules in the wordpress eslint package.
...
## How?
- detect `render={ <VisuallyHidden /> }` when `VisuallyHidden` is imported as a named import
- detect `Link render={ <Text /> }` when `Link` and `Text` are imported as named imports
- support named import aliases and local named imports in `packages/ui` via `checkLocalImports`

## Testing Instructions
1. Run `./node_modules/.bin/eslint packages/eslint-plugin/rules/no-unsafe-render-order.js packages/eslint-plugin/rules/__tests__/no-unsafe-render-order.js eslint.config.mjs packages/ui/src/visually-hidden/visually-hidden.tsx packages/ui/src/link/stories/index.story.tsx`.
2. Run `./node_modules/.bin/jest packages/eslint-plugin/rules/__tests__/no-unsafe-render-order.js --runInBand`.
3. Verify the rule reports `<Dialog.Title render={ <VisuallyHidden /> }>` and `<Link render={ <Text /> }>` but does not report `<VisuallyHidden render={ <Dialog.Title /> }>`.

## Issue comments

### github-actions on 2026-04-16T14:01:02Z

URL: https://github.com/WordPress/gutenberg/pull/77428#issuecomment-4260657101
...
### github-actions on 2026-04-16T14:13:49Z

URL: https://github.com/WordPress/gutenberg/pull/77428#issuecomment-4260745493
...
### github-actions on 2026-04-16T15:01:27Z

URL: https://github.com/WordPress/gutenberg/pull/77428#issuecomment-4261106289

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 2614c86a6fdc3e07bd7a0bbf49f2eab5cfa7bbd3.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24515114123
📝  Reported issues:
...
#### ciampo on 2026-04-21T15:39:29Z

URL: https://github.com/WordPress/gutenberg/pull/77428#discussion_r3118637772
Path: packages/eslint-plugin/README.md

```diff
@@ -121,6 +121,7 @@ The granular rulesets will not define any environment globals. As such, if they
...
- eslint.config.mjs (MODIFIED, +11/-0)
...
- packages/ui/src/link/stories/index.story.tsx (MODIFIED, +5/-11)
- packages/ui/src/visually-hidden/visually-hidden.tsx (MODIFIED, +4/-20)
