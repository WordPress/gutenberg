...
type: pull_request
state: MERGED
merged_at: 2026-05-11T16:39:22Z
url: https://github.com/WordPress/gutenberg/pull/77861
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Breaking Change","[Package] UI"]
---

# PR #77861: Support object values in Select primitive

- URL: https://github.com/WordPress/gutenberg/pull/77861
- Author: mirka
- Created: 2026-04-30T20:32:27Z
- Updated: 2026-05-11T16:40:16Z
- Comments: 3 of 3
- Review threads: 14 of 14
- Changed files: 6 of 6
- Diff: ../pr-diffs/77861.diff

## Body

## What?

Updates the `Select` primitive so it can be used with object item values, matching Base UI's value model more closely.

## Why?
Base UI Select supports object item values, but our wrapper previously typed item values as strings and default-rendered item text from the raw value. This was not a good API in hindsight, because most consumers will want to model their data with separate value/labels anyway. It also makes it impossible to receive the full item object when custom-rendering the `Trigger`.

## How?
- Aligns the primitive value types with Base UI's `any` value typing.
- Stops defaulting `Select.Item` children to the raw `value`; item content should be provided explicitly.
- Adds a unit test covering object values passed through to custom trigger content.
- Updates Select primitive stories to use object item values with `items` arrays.

## Testing Instructions
1. Run `npm run lint:js packages/ui/src/form/primitives/select`.
2. Run `npm run test:unit packages/ui/src/form/primitives/select/test/index.test.tsx`.
3. Run `npm run build`.
4. Open Storybook and review `Design System/Components/Form/Primitives/Select`.

## Issue comments

### github-actions on 2026-04-30T21:47:45Z

URL: https://github.com/WordPress/gutenberg/pull/77861#issuecomment-4356380206
...
### github-actions on 2026-05-04T18:40:32Z

URL: https://github.com/WordPress/gutenberg/pull/77861#issuecomment-4373544635
...
### github-actions on 2026-05-04T19:12:21Z

URL: https://github.com/WordPress/gutenberg/pull/77861#issuecomment-4373777459

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 6a18eeee4d207379504b8d6c14e1066598e33191.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25672855028
📝  Reported issues:
...
#### mirka on 2026-04-30T20:52:16Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3170791373
...
#### ciampo on 2026-05-05T09:53:29Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187533203
Path: packages/ui/src/form/primitives/select/types.ts

```diff
@@ -8,7 +8,7 @@ export type PortalProps = ComponentPropsWithoutRef< typeof _Select.Portal >;
...
Optionally, the wrapper can keep per‑instance type safety with a small generic:
...
#### ciampo on 2026-05-05T09:54:00Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187536192
Path: packages/ui/src/form/primitives/select/root.tsx

```diff
@@ -2,5 +2,5 @@ import { Select as _Select } from '@base-ui/react/select';
...
Related to the previous comment, this could become
...
which means that consumers could get inference from `defaultValue` / `value` and a strongly‑typed `onValueChange`, wihtout (I think) any changes to the current Storybook code?

### Review thread 4 (resolved)

#### ciampo on 2026-05-05T09:59:34Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187567207
Path: packages/ui/src/form/primitives/select/types.ts

```diff
@@ -53,7 +53,7 @@ export type SelectItemProps = Omit<
 	/**
...
I know this is matching upstream, but could we use `unknown` instead?

### Review thread 5 (resolved)

#### ciampo on 2026-05-05T10:03:46Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187590333
Path: packages/ui/src/form/primitives/select/trigger.tsx

```diff
@@ -8,6 +8,16 @@ import { InputLayout } from '../input-layout';
...
Assuming that consumers either use `''` directly or wrap it in an object with a `value` key could have some unintended consequences:

- **Shape coupling**: Now that the primitive accepts arbitrary object values, the `{ value: '' }` shape is a hidden requirement to opt into placeholder styling. A consumer modeling items as `{ id, name }` (a perfectly reasonable shape) gets no placeholder state, with no error or hint.
- **Discoverability**: This convention isn't documented anywhere in the JSDoc or the `WithEmptyValueOption` story copy. The story only describes the `items` array side of things.

Looking at Base UI' APIs, it looks like `Select.Value`'s `state` already carries a `placeholder: boolean` (and `Select.Root` documents `null` as the canonical "no selection" sentinel, optionally paired with a null item in `items`). Maybe we could use `state.placeholder` to cover the unset case cleanly?
The remaining "selected‑but‑empty" case (where the empty option *is* a real selection) is genuinely a wrapper concern, but it's worth scoping the heuristic to that case only.

Separately, we should probably document the `{ value: '' }` convention in `SelectItemProps`'s JSDoc and in the `WithEmptyValueOption` story description to make it more explicit?

#### mirka on 2026-05-08T00:59:19Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3205583804
Path: packages/ui/src/form/primitives/select/trigger.tsx
...
I didn't realize Base UI Select supports placeholders now. I'm making this change first to align with upstream: #78076.

### Review thread 6 (resolved)

#### ciampo on 2026-05-05T10:10:26Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187624968
Path: packages/ui/src/form/primitives/select/test/index.test.tsx

```diff
@@ -4,6 +4,38 @@ import { createRef } from '@wordpress/element';
 import * as Select from '../index';
...
Related to the previous comment, the `isEmptyValue` change in `Trigger` isn't really tested.

Since the placeholder logic is now load‑bearing for every object‑valued select, we should probably test that the function is matching as expected when  `defaultValue` is `''` (string variant) and when it is `{ value: '', label: 'Select' }` (object variant). And potentially, a negative test asserting it does not apply when `defaultValue` is a non‑empty object.

At the moment, the only way to test it is to check for the `is-placeholder` class name, which is not great. Maybe a dedicated data attribute? Or a simple unit test on that function alone, without testing the whole `Trigger` component?

Separately, is it worth assessing that `onValueChange` receives the full object on selection? The current test only verifies the trigger text, which is also covered indirectly.

#### mirka on 2026-05-08T21:05:35Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211345082
Path: packages/ui/src/form/primitives/select/test/index.test.tsx
...
`isEmptyValue` logic removed due to #78076. `onValueChange` test added.

### Review thread 7 (resolved)

#### ciampo on 2026-05-05T10:11:19Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187630182
...
Super nit: maybe clearer to show an explicit refactor example ?
...
#### ciampo on 2026-05-05T10:22:00Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3187686326
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx

```diff
@@ -87,15 +97,15 @@ const withEmptyOptionItems = [
 		disabled: true,
 	},
 	{
-		value: 'Item 2',
...
Maybe a bit more explicit about what's going on with `value` and `label` in `Trigger`?


```
/**
 * Pass an `items` array of `{ value, label }` objects to `Select.Root` so
 * `Select.Trigger` can auto‑resolve the selected label without a custom render
 * function, and so the form value serializes correctly. An option with an
 * empty‑string `value` (typically `disabled`) doubles as a placeholder.
 */
```

#### mirka on 2026-05-08T21:04:13Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211338989
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx
...
#### ciampo on 2026-05-08T21:46:54Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211537225
Path: packages/ui/src/form/primitives/select/test/index.test.tsx

```diff
@@ -4,6 +4,48 @@ import { createRef } from '@wordpress/element';
 import * as Select from '../index';
...
Could we add a test for when there's no explicit trigger render function? Something like

```tsx
it( 'auto-resolves trigger label from items when value is an object', () => {
    const users = [ { value: '1', label: 'User 1' }, { value: '2', label: 'User 2' } ];
    render(
        <Select.Root defaultValue={ users[ 0 ] } items={ users }>
            <Select.Trigger />
            <Select.Popup>
                { users.map( ( o ) => (
                    <Select.Item key={ o.value } value={ o }>{ o.label }</Select.Item>
                ) ) }
            </Select.Popup>
        </Select.Root>
    );
...
#### ciampo on 2026-05-08T21:49:27Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211547176
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx

```diff
@@ -223,10 +274,11 @@ const customOptions = [
  */
 export const WithCustomTriggerAndItem: Story = {
```

Could we make use of the generic inference here and type it as

```suggestion
StoryObj< typeof Select.Root< ( typeof customOptions )[ number ] > >;
```

#### mirka on 2026-05-11T13:24:28Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3219234781
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx
...
This doesn't really add anything, because `Trigger` can't see what the `Root` types are.

### Review thread 11 (resolved)

#### ciampo on 2026-05-08T21:51:11Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211554575
Path: packages/ui/src/form/primitives/select/types.ts
...
Should we consider marking `children` required for `Select.Item` ?

Maybe not (especially to avoid TS errors when composing via `render` prop), but worth at lease considering

#### mirka on 2026-05-11T13:26:16Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3219248146
Path: packages/ui/src/form/primitives/select/types.ts
...
Yes, considering the `render` case I'd say this would potentially be an annoyance, compared to the negligible benefit (since it's already very obvious when you forget to pass `children `to `Item`).

### Review thread 12 (resolved)

#### ciampo on 2026-05-08T22:00:04Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3211589315
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx

Should we make it more explicit that the `items` prop is required for label auto-resolution to work, and that the items must have `{ value, label }` shape (or you need `itemToStringLabel`), and equality is `Object.is` by default (so consumers passing equivalent-but-different objects will silently mismatch)?

None of this is in our wrapper's JSDoc:
- A short JSDoc note on `SelectRootProps`/`SelectItemProps`.value mentioning that object values rely on items for label resolution and `Object.is` for equality (with `isItemEqualToValue` as the escape hatch).
- A dedicated `"object item values"` story (or a paragraph on WithCustomTriggerAndItem) that's the place you'd link people to.

### Review thread 13 (resolved)

#### ciampo on 2026-05-11T14:53:25Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3219866827
Path: packages/ui/src/form/primitives/select/stories/index.story.tsx

```diff
@@ -62,6 +69,7 @@ export const Compact: Story = {
  */
...
Does it make sense to spread `...Default.Args` here? The `items` array in `Default` doesn't really match what's rendered in the minimal story, and could be confusing?

Maybe we could also add a note in the JSDoc about it? Something like `Note: this story uses primitive string values (rather than object values like the other stories) because labels and values are identical here, so the `items` prop is unnecessary.`

### Review thread 14 (resolved)

#### ciampo on 2026-05-11T14:54:10Z

URL: https://github.com/WordPress/gutenberg/pull/77861#discussion_r3219872914
Path: packages/ui/src/form/primitives/select/test/index.test.tsx
...
- packages/ui/src/form/primitives/select/item.tsx (MODIFIED, +1/-1)
- packages/ui/src/form/primitives/select/root.tsx (MODIFIED, +13/-2)
- packages/ui/src/form/primitives/select/stories/index.story.tsx (MODIFIED, +92/-42)
- packages/ui/src/form/primitives/select/test/index.test.tsx (MODIFIED, +70/-4)
- packages/ui/src/form/primitives/select/types.ts (MODIFIED, +3/-5)
