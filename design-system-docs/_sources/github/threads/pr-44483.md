...
type: pull_request
state: MERGED
merged_at: 2022-09-29T01:09:16Z
url: https://github.com/WordPress/gutenberg/pull/44483
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #44483: FontSizePicker: Replace SCSS with Emotion + components

- URL: https://github.com/WordPress/gutenberg/pull/44483
- Author: noisysocks
- Created: 2022-09-27T05:18:58Z
- Updated: 2022-09-29T05:16:41Z
- Comments: 4 of 4
- Review threads: 3 of 3
- Changed files: 5 of 5
- Diff: ../pr-diffs/44483.diff
...
## How?
I put [The Black Parade](https://www.youtube.com/watch?v=AUx1Yr2Mwbc) on and got to work.
...
### github-actions on 2022-09-27T05:27:49Z

URL: https://github.com/WordPress/gutenberg/pull/44483#issuecomment-1258996650
...
I noticed that the Reset button has a custom height so thought that I may as well make this custom height respect `__unstable-large` so that it looks less weird. Happy to not do this if you prefer. We won't use the Reset button after https://github.com/WordPress/gutenberg/pull/44180 so it's of no real consequence either way.

> The custom font size slider should be full width, while it's not in this PR

Good catch, will look into that 👍

### noisysocks on 2022-09-28T06:33:16Z

URL: https://github.com/WordPress/gutenberg/pull/44483#issuecomment-1260451052
...
### noisysocks on 2022-09-28T23:48:00Z

URL: https://github.com/WordPress/gutenberg/pull/44483#issuecomment-1261578559
...
#### noisysocks on 2022-09-27T05:19:44Z

URL: https://github.com/WordPress/gutenberg/pull/44483#discussion_r980749288
...
#### ciampo on 2022-09-27T12:12:39Z

URL: https://github.com/WordPress/gutenberg/pull/44483#discussion_r981159431
Path: packages/components/src/font-size-picker/index.tsx

```diff
@@ -298,42 +304,45 @@ const UnforwardedFontSizePicker = (
 								</FlexItem>
 								{ withReset && (
 									<FlexItem isBlock>
-										<Button
...
It looks like this classname isn't used anywhere in the repo (plus, it should have a `components-font-size-picker` namespace anyway) — I believe we're safe in deleting it
...
#### ciampo on 2022-09-27T12:14:17Z

URL: https://github.com/WordPress/gutenberg/pull/44483#discussion_r981160961
Path: packages/components/src/font-size-picker/index.tsx
...
Is there a reason why `__nextHasNoMarginBottom` is not being passed to `RangeControl` ?

```suggestion
							__nextHasNoMarginBottom={ __nextHasNoMarginBottom }
```

Same applies for the `ToggleGroupControl` component a few lines above.

## Changed files

- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/font-size-picker/index.tsx (MODIFIED, +47/-42)
- packages/components/src/font-size-picker/style.scss (DELETED, +0/-78)
- packages/components/src/font-size-picker/styles.ts (ADDED, +44/-0)
