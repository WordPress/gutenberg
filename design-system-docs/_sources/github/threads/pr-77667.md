...
type: pull_request
state: MERGED
merged_at: 2026-04-29T12:12:38Z
url: https://github.com/WordPress/gutenberg/pull/77667
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77667: CollapsibleCard: Prevent focus ring clipping by content overflow

- URL: https://github.com/WordPress/gutenberg/pull/77667
- Author: t-hamano
- Created: 2026-04-25T02:55:13Z
- Updated: 2026-04-29T12:13:28Z
- Comments: 12 of 12
- Review threads: 1 of 1
- Changed files: 4 of 4
- Diff: ../pr-diffs/77667.diff
...
### github-actions on 2026-04-25T02:56:55Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4317681579
...
### github-actions on 2026-04-25T03:07:56Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4317726788
...
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 341 kB | +45 B (+0.01%) |
| `build/scripts/edit-site/index.min.js` | 264 kB | +58 B (+0.02%) |
| `build/scripts/editor/index.min.js` | 424 kB | +62 B (+0.01%) |
...
### github-actions on 2026-04-25T03:25:26Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4317803533

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 0d5ff69c977dc648b935d040b5cad52752bc1579.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25104284582
📝  Reported issues:
- #77720 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### jameskoster on 2026-04-27T11:23:20Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4326548863

This obviously increases the gap between the header content and the card content. I suspect this is not ideal from a design POV. 

I'm assuming the problematic `overflow: hidden` style on the content is a requirement for the animation. Could it make sense to only apply that style _during_ the animation?

### mirka on 2026-04-27T12:44:48Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4327059403

@ciampo Looking at #76678 again, could we maybe only hide overflow in the full bleed container?

### ciampo on 2026-04-27T14:03:51Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4327607652

Given the outer `overflow: clip`, we may be able to remove `oveflow: hidden` entirely on the `.content` className without affecting the animation.

@t-hamano do you mind trying this approach instead?

### t-hamano on 2026-04-28T13:11:29Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4335545210

@ciampo Thanks for the review!

> we may be able to remove `oveflow: hidden` entirely on the `.content` className without affecting the animation.

Unfortunately, I realized that the animation doesn't work in Firefox when expanding content. This occurs on both Windows and MacOS.

I suspect this is related to how `scrollHeight` is calculated in Firefox. When I remove `overflow: hidden`, `--collapsible-panel-height` seems to instantly switch from `0` to `auto`.

https://github.com/user-attachments/assets/d11868b8-002b-422e-aeff-d0d098736c95

Another idea is to apply overflow: visible once the expansion finishes, and remove it when collapsing starts, though it might feel a bit hacky 🤔

### ciampo on 2026-04-28T14:02:32Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4335924489
...
> The CSSOM View spec says scrollHeight should reflect the element's "scrolling area". An element with overflow: visible doesn't establish a scrolling box, so per spec scrollHeight === clientHeight. [MDN states this explicitly](https://developer.mozilla.org/en-US/docs/Web/API/Element/scrollHeight): "If the element's content can fit without a need for vertical scrollbar, its scrollHeight is equal to clientHeight."
> 
> Firefox: follows the spec → with height: 0 + overflow: visible, scrollHeight returns 0, so the panel can't compute a target height and the animation snaps.
> Chrome/Safari: deviate from the spec and return content size regardless of overflow, so the animation works.


Toggling `overflow: hidden` on/off based on the `transitionend` event of the `useOpenChangeComplete` base ui callback prop is also the only possible solution that comes to mind

### t-hamano on 2026-04-29T04:39:18Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4340834361

Sorry, I made a mistake when rebasing 🙈

### t-hamano on 2026-04-29T04:45:40Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4340860520

I'm a little unsure about the code, but it now works correctly in Firefox.

https://github.com/user-attachments/assets/558db44a-c6c8-45e0-8f8e-252b4aa38a08

### ciampo on 2026-04-29T10:37:59Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4342924369

> I'm a little unsure about the code

Yeah, I don't like that we're using a base ui internal.


I took the initiative and landed on a solution that looks cleaner, and seems to work well on Firefox/Safari/Chrome on MacOS. It uses base ui's state exposed via a callback version of `className`, allowing us to read `open` and `transitionStatus`. I pushed the changes and updated the PR description accordingly.

Prior to that, I tried a [more verbose solution](https://github.com/WordPress/gutenberg/pull/77667/commits/3237f4a5234bb2441eaa1c6b159ba7a57c2502aa) (based on `transitionend` event listener) which also worked, but required more code.

@t-hamano let me know if the current solution (or the previous one) work for you!

### t-hamano on 2026-04-29T11:17:11Z

URL: https://github.com/WordPress/gutenberg/pull/77667#issuecomment-4343186358
...
#### ciampo on 2026-04-28T12:42:55Z

URL: https://github.com/WordPress/gutenberg/pull/77667#discussion_r3154177975
Path: packages/ui/src/collapsible-card/style.module.css

Nit: to clarify the intent of this rule


```suggestion
		/* Card.Content's `.header + .content` selector doesn't match here because
		    Collapsible.Panel sits between Card.Header and Card.Content. Re-apply
		    the zero top padding manually so the header/content gap matches Card. */
...
- routes/experiments-home/style.scss (MODIFIED, +0/-5)
