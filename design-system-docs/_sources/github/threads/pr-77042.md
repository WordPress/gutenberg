...
type: pull_request
state: MERGED
merged_at: 2026-04-06T12:34:22Z
url: https://github.com/WordPress/gutenberg/pull/77042
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Focus] Accessibility (a11y)","[Package] Components"]
---

# PR #77042: `ValidatedRangeControl`: Fix aria-label rendered as [object Object]

- URL: https://github.com/WordPress/gutenberg/pull/77042
- Author: mirka
- Created: 2026-04-03T18:05:53Z
- Updated: 2026-04-06T18:42:59Z
- Comments: 3 of 3
- Review threads: 1 of 1
- Changed files: 3 of 3
- Diff: ../pr-diffs/77042.diff

## Body

## What?

Fixes `ValidatedRangeControl`'s `aria-label` being rendered as `[object Object]` in the browser.

## Why?

`ControlWithError` appends a "(Required)" or "(Optional)" suffix to the child component's `label` via `appendRequiredIndicator`. This function always wraps the result in a JSX fragment (`<>label (Required)</>`), which is a React element (i.e. an object).

This works fine for components like `InputControl` where the label is rendered as visible content inside a `<label>` element. But `RangeControl` passes `label` directly to `aria-label` on both its internal `<input type="range">` and the number `<input>`, and `aria-label` expects a string. The object gets coerced to `"[object Object]"`.

## How?

`appendRequiredIndicator` now checks whether the incoming label is a `string`. If so, it returns a concatenated string (e.g. `"Opacity (Required)"`) so that `aria-label` receives valid content. For non-string React node labels, it still falls back to the JSX fragment.

Adds a test that renders `ValidatedRangeControl` with `required` and asserts both the slider and spinbutton have the expected accessible name.

## Testing Instructions

1. `npm run storybook:dev`
2. Navigate to **Validated Form Controls / ValidatedRangeControl / Default**.
3. Inspect the slider and number input elements. Confirm `aria-label` reads `"Range (Required)"` (not `"[object Object]"`).
...
### github-actions on 2026-04-03T18:21:34Z

URL: https://github.com/WordPress/gutenberg/pull/77042#issuecomment-4184635762
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 265 kB | +37 B (+0.01%) |
...
### github-actions on 2026-04-03T18:31:22Z

URL: https://github.com/WordPress/gutenberg/pull/77042#issuecomment-4184669206
...
### github-actions on 2026-04-03T18:39:09Z

URL: https://github.com/WordPress/gutenberg/pull/77042#issuecomment-4184699857

<!-- flaky-tests-report-comment -->
**Flaky tests detected in c870174beea2ba6dbcc0f8de36353d20056ea364.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24030712165
📝  Reported issues:
...
#### aduth on 2026-04-06T14:03:27Z

URL: https://github.com/WordPress/gutenberg/pull/77042#discussion_r3039816461
Path: packages/components/src/validated-form-controls/control-with-error.tsx

I wonder if there'd be a way to set up TypeScript types in such a way (e.g. with generics) so that we could have caught the fact that we're passing a React element to RangeControl's `label` prop (invalid based on its types). 

Or if we want to be stricter about what we accept as field labels (as string-only) if we expect to be able to use them in contexts like attribute values.

#### mirka on 2026-04-06T18:42:59Z

URL: https://github.com/WordPress/gutenberg/pull/77042#discussion_r3041067699
Path: packages/components/src/validated-form-controls/control-with-error.tsx

> Or if we want to be stricter about what we accept as field labels (as string-only) if we expect to be able to use them in contexts like attribute values.

Good point. I think the longer-term fix here is to tighten the contract rather than make `ControlWithError` more generic. `@wordpress/ui` components are consistent about string-only labels anyway, so it matches the direction were moving toward.

## Changed files

- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/validated-form-controls/control-with-error.tsx (MODIFIED, +17/-12)
- packages/components/src/validated-form-controls/test/control-with-error.tsx (MODIFIED, +28/-1)
