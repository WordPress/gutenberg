...
type: pull_request
state: MERGED
merged_at: 2026-05-01T17:08:24Z
url: https://github.com/WordPress/gutenberg/pull/77811
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI","[Package] Grid"]
---

# PR #77811: Grid: add `renderResizeHandle` render prop

- URL: https://github.com/WordPress/gutenberg/pull/77811
- Author: retrofox
- Created: 2026-04-29T18:09:40Z
- Updated: 2026-05-12T18:20:34Z
- Comments: 6 of 6
- Review threads: 0 of 0
- Changed files: 8 of 8
- Diff: ../pr-diffs/77811.diff

## Body

## What?

Adds a `renderResizeHandle` render prop on `DashboardGrid` that lets consumers swap the default corner-triangle resize handle for a custom element. Side change: the resize handle no longer mounts outside edit mode, removing the dnd-kit `<DndContext>` and `useDraggable` subscription that every tile carried at rest.

Part of #77616

## Why?

The resize handle was hard-coded inside `GridItem` (`packages/grid/src/grid-item.tsx`) and consumers had no way to change the icon, position, or visual without forking the component. Surfaces that want a different visual language for the handle (or that need to react to gesture state) couldn't.

While exposing the handle, we noticed that every tile mounted a dnd-kit context plus a draggable hook even when the grid was not in edit mode — work that never ran. Skipping the mount is a small, contained perf win that also drops the now-unreachable `disabled` flag from the public render-prop contract.

## How?

The render prop is invoked inside the inner `<ResizeHandle>` component, after `useDraggable` has produced the gesture wiring. The grid keeps ownership of the `<DndContext>`, the throttled delta loop, and the step-to-grid logic; consumers receive `ref`, `listeners`, `attributes`, `verticalResizable`, `isResizing` (sourced from `useDraggable().isDragging`) and `itemId`.

The render prop is forwarded `DashboardGrid` → `GridItem` → `ResizeHandleWrapper` → `ResizeHandle`. When omitted, the default corner-triangle div renders unchanged.

Types are centralized in `packages/grid/src/types.ts`: `ResizeDelta`, `ResizeHandleRenderProps`, `ResizeHandleProps`, and `GridItemProps` now live there and are imported by the components. `ResizeHandleRenderProps` and `ResizeDelta` are re-exported from the package barrel.

In `GridItem`, `<ResizeHandle>` is rendered only when `! disabled`. The `disabled` field is removed from `ResizeHandleRenderProps` and `ResizeHandleProps`, the `.is-disabled` CSS rule is dropped, and the README/story are updated accordingly.

The Storybook story `Custom Resize Handle` shows an example handle built from a tiny inline SVG (diagonal stroke + corner triangle) wrapped in a div that carries the gesture wiring and an `isResizing`-driven opacity transition.

### Commits

- `Grid: add renderResizeHandle render prop` — feature.
- `Grid: skip ResizeHandle mount outside edit mode` — perf optimization, removes `disabled` flag.
- `Grid: refine custom resize handle story` — Storybook example polish.

## Testing

- [ ] Open the `Grid/DashboardGrid > Custom Resize Handle` story and verify the custom handle replaces the default corner triangle.
- [ ] In edit mode, drag the handle to resize a tile — width/height step, preview overlay, and `onChangeLayout` all behave the same as the default handle.
- [ ] Toggle `editMode` off via the controls panel — the handle disappears (no DOM, no dnd-kit subscription mounted).
- [ ] In the `Custom Resize Handle` story, confirm the handle's opacity drops to 0.5 while a resize gesture is active and snaps back on release.
- [ ] Run `npx tsc --build packages/grid/tsconfig.json` and `npx jest packages/grid` — both should pass.


https://github.com/user-attachments/assets/c74b9adc-7e42-4c94-a2ca-e2e64ddfa1dc



## Follow-ups

- [ ] Lock the document cursor during the resize gesture so the `nwse-resize` cue isn't lost when the pointer leaves the handle's hit area.

## Issue comments

### github-actions on 2026-04-29T18:09:52Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4346301830
...
### github-actions on 2026-04-29T18:27:44Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4346431829
...
| `build/scripts/editor/index.min.js` | 426 kB |
...
### github-actions on 2026-04-29T19:57:49Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4347081514

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 65ec2c1150b9be72f24b07caf16dd13ef23f2afe.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25222686293
📝  Reported issues:
...
### simison on 2026-04-30T09:06:57Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4351143954

Is this needed for something specifically for the Dashboard experiment, or adding "in case"? Consistent corner handle one _cannot_ change seems like a good thing! :-)

### retrofox on 2026-04-30T12:05:02Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4352289288

> Is this needed for something specifically for the Dashboard experiment, or adding "in case"? Consistent corner handle one _cannot_ change seems like a good thing! :-)

Fair question, and worth answering honestly: this isn't user-voice driven, no one filed it as a request. It comes from working on a real consumer (the dashboard surface) and hitting an asymmetry I think is worth addressing at the package level. Three points:

**1. `@wordpress/grid` It's a context-agnostic primitive.

The package isn't tied to the dashboard, the editor, or any specific surface.
It's a layout primitive that any plugin page, custom screen, or third-party consumer can pick up.

That framing is what justified the package in #77562 in the first place. Locking the resize handle's visual to the primitive constrains every future consumer to a single aesthetic, regardless of context.

**2. There's already an asymmetry with `actionableArea`.**
Consumers have full control over what goes inside `actionableArea`: add a settings button, a menu, a status indicator, whatever fits the surface.
If a consumer styles those actions to match their own design language, the resize handle is the one element they can't reach. So a tile ends up with consumer-styled actions next to a fixed corner triangle owned by the primitive. That's a real gap, not a hypothetical one.

**3. Visual consistency belongs to a higher layer.**
I agree consistency is a good thing. The layout primitive isn't the right place to enforce it.
The dashboard work we're doing introduces a chrome/tile component that owns the visual contract for the surface: actionable elements, drag affordances, focus styles, and yes, the resize handle.
IMO, that's the right place to standardize, because it knows the surface. The grid primitive shouldn't pre-decide what every consumer's resize handle looks like.

That said, if we're not convinced the asymmetry argument is sufficient on its own, I'm fine putting this on hold and revisiting it once the dashboard chrome work is more visible.

### simison on 2026-04-30T12:57:34Z

URL: https://github.com/WordPress/gutenberg/pull/77811#issuecomment-4352633386
...
- packages/grid/README.md (MODIFIED, +57/-0)
- packages/grid/src/grid-item.tsx (MODIFIED, +16/-80)
- packages/grid/src/grid.tsx (MODIFIED, +84/-82)
- packages/grid/src/index.ts (MODIFIED, +6/-1)
- packages/grid/src/resize-handle.module.css (MODIFIED, +0/-4)
- packages/grid/src/resize-handle.tsx (MODIFIED, +19/-48)
- packages/grid/src/stories/index.story.tsx (MODIFIED, +116/-2)
- packages/grid/src/types.ts (MODIFIED, +183/-0)
