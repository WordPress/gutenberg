...
type: pull_request
state: MERGED
merged_at: 2026-05-27T21:26:29Z
url: https://github.com/WordPress/gutenberg/pull/78741
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Package] Theme"]
---

# PR #78741: Theme: drop `density` support from `@wordpress/theme`

- URL: https://github.com/WordPress/gutenberg/pull/78741
- Author: ciampo
- Created: 2026-05-27T14:50:38Z
- Updated: 2026-05-27T21:27:12Z
- Comments: 2 of 2
- Review threads: 0 of 0
- Changed files: 15 of 15
- Diff: ../pr-diffs/78741.diff

## Body

## What

Addresses **T-X3** from #77462 — drop `density` support from `@wordpress/theme` for now, until a stable design is agreed on (see also #74556).

This PR removes:

- The `density` prop on `ThemeProvider`, along with its type in `ThemeProviderSettings`.
- The `data-wpds-density` DOM attribute that `ThemeProvider` was writing to its wrapper.
- The `compact` / `comfortable` mode overrides on `--wpds-dimension-padding-*` and `--wpds-dimension-gap-*`, both in the source token JSON and in the generated `tokens/modes/dimension.{compact,comfortable}.json` files.
- The `compact` / `comfortable` / `default` density mode selectors from the Terrazzo config, and the corresponding per-density CSS blocks in the prebuilt `design-tokens.css`.
- All `density` mentions from the package `README.md` and from `index.story.tsx`.

Default dimension token values continue to be emitted at `:root` exactly as before, so:

- Consumers that did not pass a `density` prop see no visual change.
- Consumers that did pass a `density` prop will see a TypeScript error (and will fall back to the default spacing). A grep across the repo found no such usage outside the package's own stories.

## Why

Per the stabilization review in #77462, density was flagged as an experimental feature with multiple open issues (propagation, asymmetric prop semantics) and an unresolved design direction. Stabilizing `ThemeProvider` as a public API is easier if we ship a smaller, more stable surface and revisit density separately.

## Testing instructions

1. `cd packages/theme && npm run build` should succeed and produce the same `design-tokens.css` that is committed here.
2. The package's `NestingAndInheriting` Storybook story still renders, only without the `density: ...` row in the debug output.
3. `var(--wpds-dimension-padding-*)` and `var(--wpds-dimension-gap-*)` resolve to the same default values as before.

## Issue comments

### github-actions on 2026-05-27T14:59:57Z

URL: https://github.com/WordPress/gutenberg/pull/78741#issuecomment-4555753291
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 19.4 kB | -155 B (-0.79%) |
| `build/scripts/theme/index.min.js` | 22 kB | -13 B (-0.06%) |
...
### github-actions on 2026-05-27T15:12:36Z

URL: https://github.com/WordPress/gutenberg/pull/78741#issuecomment-4555869135
...
- packages/theme/README.md (MODIFIED, +6/-14)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +0/-107)
- packages/theme/src/stories/index.story.tsx (MODIFIED, +4/-17)
- packages/theme/src/theme-provider.tsx (MODIFIED, +0/-2)
- packages/theme/src/types.ts (MODIFIED, +0/-9)
- packages/theme/terrazzo.config.ts (MODIFIED, +0/-21)
- packages/theme/tokens/dimension.json (MODIFIED, +0/-56)
- packages/theme/tokens/modes/dimension.comfortable.json (DELETED, +0/-64)
- packages/theme/tokens/modes/dimension.compact.json (DELETED, +0/-64)
- storybook/addons/design-system-theme/manager.ts (MODIFIED, +0/-11)
- storybook/decorators/with-design-system-theme.tsx (MODIFIED, +0/-2)
- storybook/preview.jsx (MODIFIED, +0/-1)
- storybook/stories/design-system/introduction.mdx (MODIFIED, +1/-1)
- storybook/stories/design-system/theme-example-application.story.tsx (MODIFIED, +3/-35)
