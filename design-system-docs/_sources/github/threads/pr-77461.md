...
type: pull_request
state: MERGED
merged_at: 2026-04-21T13:58:11Z
url: https://github.com/WordPress/gutenberg/pull/77461
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77461: Text: Apply both heading and paragraph CSS defenses unconditionally

- URL: https://github.com/WordPress/gutenberg/pull/77461
- Author: mirka
- Created: 2026-04-17T16:37:15Z
- Updated: 2026-04-21T13:58:55Z
- Comments: 3 of 3
- Review threads: 0 of 0
- Changed files: 2 of 2
- Diff: ../pr-diffs/77461.diff

## Body

## What?

Apply both the heading and paragraph entries from the unlayered CSS defense module to every `Text` render, regardless of the `variant` prop.

Follow up to #76783. Surfaced by a comment on #77372.

## Why?

`Text` decouples typographic variant from rendered element via the `render` prop, but the previous defense logic was gated on the variant name (`heading-*` or `body-*`). That meant common.css styles leaked through whenever the variant and the rendered element didn't match, e.g. `<Text variant="body-sm" render={ <h2 /> } />` or `Badge` (which is a `body-sm` `Text` rendered as a heading by consumers).

## How?

The heading and paragraph defenses in [`global-css-defense.module.css`](https://github.com/WordPress/gutenberg/blob/trunk/packages/ui/src/utils/css/global-css-defense.module.css) are already gated by compound selectors (`p.p` and `:is(h1, h2, h3, h4, h5, h6).heading`), so applying both classes unconditionally is a no-op when the element doesn't match. Drop the variant-based gating and let the selectors do the work.

## Testing Instructions

In Storybook, with the global CSS injector in the toolbar set to "WordPress", see the "With render prop" story for the `Text` component. Tweak the story file to render different `variant` and `render` combinations and see that there are no regressions.

## Issue comments

### github-actions on 2026-04-17T16:49:21Z

URL: https://github.com/WordPress/gutenberg/pull/77461#issuecomment-4269788242
...
| `build/scripts/edit-site/index.min.js` | 263 kB | -10 B (0%) |
| `build/scripts/editor/index.min.js` | 413 kB | -13 B (0%) |
...
### github-actions on 2026-04-17T17:08:21Z

URL: https://github.com/WordPress/gutenberg/pull/77461#issuecomment-4269895460

<!-- flaky-tests-report-comment -->
**Flaky tests detected in bd2daf7a27f77234159e57381c178e255181bc9a.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24724454884
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### github-actions on 2026-04-17T17:14:48Z

URL: https://github.com/WordPress/gutenberg/pull/77461#issuecomment-4269933662
