...
type: pull_request
state: MERGED
merged_at: 2026-03-03T10:16:35Z
url: https://github.com/WordPress/gutenberg/pull/76071
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components","[Package] Editor"]
---

# PR #76071: `Button`: Add `word-break: break-word`

- URL: https://github.com/WordPress/gutenberg/pull/76071
- Author: ntsekouras
- Created: 2026-03-03T08:32:03Z
- Updated: 2026-03-06T10:07:44Z
- Comments: 13 of 13
- Review threads: 0 of 0
- Changed files: 7 of 7
- Diff: ../pr-diffs/76071.diff
...
### github-actions on 2026-03-03T08:44:55Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3989544431
...
| :--- | :---: | :---: |
| `build/scripts/editor/index.min.js` | 349 kB | -15 B (0%) |
| `build/styles/components/style-rtl.css` | 17.6 kB | +12 B (+0.07%) |
| `build/styles/components/style-rtl.min.css` | 14.4 kB | +18 B (+0.13%) |
| `build/styles/components/style.css` | 17.7 kB | +12 B (+0.07%) |
| `build/styles/components/style.min.css` | 14.4 kB | +17 B (+0.12%) |
| `build/styles/editor/style-rtl.css` | 23.3 kB | -14 B (-0.06%) |
| `build/styles/editor/style-rtl.min.css` | 19.8 kB | -11 B (-0.06%) |
| `build/styles/editor/style.css` | 23.3 kB | -14 B (-0.06%) |
| `build/styles/editor/style.min.css` | 19.8 kB | -11 B (-0.06%) |
...
| `build/scripts/core-data/index.min.js` | 28.7 kB |
...
| `build/scripts/edit-site/index.min.js` | 245 kB |
...
### github-actions on 2026-03-03T10:03:42Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3989950013
...
### t-hamano on 2026-03-03T12:47:02Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3990785933
...
<img width="679" height="538" alt="image" src="https://github.com/user-attachments/assets/25e36794-87ba-42a3-bee9-f9b9772093d3" />

https://wordpress.github.io/gutenberg/?path=/docs/components-datetimepicker--docs

At the very least, we might need to check Storybook to see if any visual issues have occurred in any components that depend on the button component.

### ntsekouras on 2026-03-03T17:50:52Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3992641851

Thank you @t-hamano for catching this! I'll investigate more for other possible issues and see how we move forward.

### mirka on 2026-03-04T15:00:36Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3998101576

I wonder if `overflow-wrap: break-word` would have been a better choice here? It would prevent some of the issues, like the `DatePicker` one.

### ntsekouras on 2026-03-04T15:35:02Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3998319717

>I wonder if overflow-wrap: break-word would have been a better choice here? It would prevent some of the issues, like the DatePicker one.

If you want I can make this change. Alternative we could wait to see if there are any reports for this change.

In general the DatePicker one was a 'masked' issue, so I think it's fine we made the changes there.

### mirka on 2026-03-04T16:04:28Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3998505926

> If you want I can make this change. Alternative we could wait to see if there are any reports for this change.

From what I can gather, it does seem like `overflow-wrap: break-word` is the better option in modern-day CSS. `word-break: break-word` affects intrinsic sizing in flex/grid layouts (not what we want), and is marked as deprecated.

So I think it would be good to change regardless of any issue reports?

### ntsekouras on 2026-03-04T17:40:58Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3999108107

I tried replacing with `overflow-wrap: break-word;` but it seems it doesn't solve the initial issue we wanted:
<img width="546" height="197" alt="Screenshot 2026-03-04 at 7 40 28 PM" src="https://github.com/user-attachments/assets/d4a5e606-aad1-4230-8579-2ffed01a01d3" />



It seems it has to do with the `display:inline-flex`. This part from [mdn](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/overflow-wrap#break-word):
...
### mirka on 2026-03-04T18:13:12Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-3999275947

| Strategy | Min-content intrinsic size | Can shrink below min-content |
|---|---|---|
| `word-break: break-word` | Character-level (tiny) | Yes (inherently, since min-content is already tiny) |
| `overflow-wrap: break-word` | Word-level (natural) | No (`min-width: auto` prevents it) |
| `overflow-wrap: break-word` + `min-width: 0` | Word-level (natural) | Yes (parent can force it smaller) |

Maybe pair it with `min-width: 0`?

### ntsekouras on 2026-03-05T10:59:56Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-4004227755

>Maybe pair it with min-width: 0?

I had tried that and even tested by applying the same rule (min-width) to all parents to see if any node in-between with `min-width:auto` was the cause for not taking effect. 


With all my searching and explorations, I feel kind of stuck here about how to proceed.

1. Revert this PR and re-introduce (and add) the ad-hoc styles where we want them
2. Update to `overflow-wrap: anywhere;` which seems to be in theory the same with `word-break: break-word` - to not have the deprecated value
3. Something else?

### talldan on 2026-03-06T02:20:39Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-4009112686

I notice this also caused keyboard shortcuts to wrap in the editor settings menu:
<img width="200" alt="Screenshot 2026-03-06 at 10 01 13 am" src="https://github.com/user-attachments/assets/55b5586e-11f9-4da1-89a6-3fd19b9647c6" />

That might be easily fixed by adding a style to `components-menu-item__shortcut` that prevents wrapping.

edit: I've made an issue for this - https://github.com/WordPress/gutenberg/issues/76225

### ntsekouras on 2026-03-06T07:35:34Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-4010095893

Thanks @talldan. Button is a very common component and I think it's best to revert this PR and explore separately again if needed, with much more thorough testing and a clearer path forward.

### ciampo on 2026-03-06T10:07:44Z

URL: https://github.com/WordPress/gutenberg/pull/76071#issuecomment-4010800790

Yeah it's incredibly hard to apply changes to `Button` without causing regressions.
...
- packages/editor/src/components/page-attributes/parent.js (MODIFIED, +0/-1)
- packages/editor/src/components/post-author/panel.js (MODIFIED, +0/-1)
- packages/editor/src/components/post-url/panel.js (MODIFIED, +0/-1)
- packages/editor/src/components/post-url/style.scss (MODIFIED, +0/-5)
- test/e2e/specs/editor/plugins/custom-post-types.spec.js (MODIFIED, +2/-2)
