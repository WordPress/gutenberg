...
type: pull_request
state: MERGED
merged_at: 2026-04-24T14:51:57Z
url: https://github.com/WordPress/gutenberg/pull/77620
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components","[Package] Base styles"]
---

# PR #77620: ResizableBox: Inline handle z-index

- URL: https://github.com/WordPress/gutenberg/pull/77620
- Author: mirka
- Created: 2026-04-23T16:32:20Z
- Updated: 2026-04-24T14:52:35Z
- Comments: 3 of 3
- Review threads: 4 of 4
- Changed files: 3 of 3
- Diff: ../pr-diffs/77620.diff

## Body

## What?

Removes `.components-resizable-box__handle`, `.components-resizable-box__side-handle`, and `.components-resizable-box__corner-handle` from the shared `$z-layers` map in `packages/base-styles/_z-index.scss` and sets `z-index: 2` directly in `packages/components/src/resizable-box/style.scss`.

## Why?

These handles only need a local stacking value (above typical block focus outlines at `z-index: 1`, and ordering within the resize UI).

## Testing Instructions

1. Run Storybook for `@wordpress/components` or open the block editor.
2. Open a block that shows resize handles (Image, Cover, Spacer, Site Logo, grid resizer, etc.).
3. Confirm handles sit above the block focus outline, corners still work next to sides, and resizing behaves as before.
4. Optionally inspect handle elements in dev tools and confirm `z-index: 2`.

## Issue comments

### github-actions on 2026-04-23T16:49:02Z

URL: https://github.com/WordPress/gutenberg/pull/77620#issuecomment-4306208422
...
### github-actions on 2026-04-23T18:07:45Z

URL: https://github.com/WordPress/gutenberg/pull/77620#issuecomment-4306690915
...
### github-actions on 2026-04-24T13:53:10Z

URL: https://github.com/WordPress/gutenberg/pull/77620#issuecomment-4313711166

<!-- flaky-tests-report-comment -->
**Flaky tests detected in e5836d2648d2399fba96fb645b17f51b06f7fce9.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24893798628
📝  Reported issues:
...
#### mirka on 2026-04-23T18:07:09Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3132833881
...
#### ciampo on 2026-04-24T11:15:24Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137307196
Path: packages/components/src/resizable-box/style.scss

Probably safer not to do it, although I think that `re-resisable` may already be creating a stacking context on these handles with `absolute` positioning on a parent (plus the `z-index`)

### Review thread 2

#### ciampo on 2026-04-24T11:04:23Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137255506
Path: packages/components/src/resizable-box/style.scss

Should we add a CHANGELOG entry to mention the inlining of `z-index`, like mentioned in https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137231279 ?

#### mirka on 2026-04-24T13:21:50Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137958336
Path: packages/components/src/resizable-box/style.scss

https://github.com/WordPress/gutenberg/pull/77621#discussion_r3137713690

### Review thread 3

#### ciampo on 2026-04-24T11:09:03Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137278095
Path: packages/components/src/resizable-box/style.scss
...
Slightly out of scope, but it looks like all resize handles have the `.components-resizable-box__handle` class. In that case:
- these two extra rules could be redundant;
- we could add a comment above (line 18) like "Corner handles stack above side handles via DOM source order (see HANDLE_CLASSES in index.tsx) — no separate z-index needed"

#### mirka on 2026-04-24T13:18:39Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137940929
Path: packages/components/src/resizable-box/style.scss
...
I noticed too! Lots of smells overall, but I don't want to mess with this right now and create problems we hadn't considered.

### Review thread 4

#### ciampo on 2026-04-24T11:09:25Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137279786
Path: packages/components/src/resizable-box/style.scss
...
Nit: this could be refactored into a SASS variable?

#### mirka on 2026-04-24T13:21:20Z

URL: https://github.com/WordPress/gutenberg/pull/77620#discussion_r3137955669
Path: packages/components/src/resizable-box/style.scss
...
I thought about it too, but honestly I can't quickly tell if the z-indexes are all the same _as a conceptual truth_, or if they're just the same coincidentally 😅 Don't want to encode an untruth without untangling the actual structure…
