...
type: pull_request
state: MERGED
merged_at: 2026-04-10T16:51:53Z
url: https://github.com/WordPress/gutenberg/pull/76912
matched_labels: ["[Package] UI","[Package] Theme"]
all_labels: ["[Type] Code Quality","[Package] DataViews","[Package] Theme","[Package] UI"]
---

# PR #76912: Theme: Rename typography tokens to use "typography" prefix

- URL: https://github.com/WordPress/gutenberg/pull/76912
- Author: aduth
- Created: 2026-03-30T18:56:36Z
- Updated: 2026-04-15T11:06:43Z
- Comments: 7 of 7
- Review threads: 9 of 9
- Changed files: 37 of 37
- Diff: ../pr-diffs/76912.diff
...
- Updates typography tokens to add `typography-` prefix, removing `font-` from line-height tokens.
- Updates PostCSS plugin to raise an error when encountering a token that's not defined in the design system tokens, rather than silently returning the original value.
   - This is meant to help the changes clearer in migrating and validate the changes.

## Why?

- As described in #76838, not all typography tokens are related to font (e.g. line-height, letter spacing, etc.). The updated naming provides a more accurate grouping for typography tokens.
- Alignment between token grouping and file name (`typography.json` ➡️ `wpds-typography`), consistent with other tokens

## Testing Instructions

Verify there are no more occurrences of `--wpds-font-*` tokens in the codebase.

Verify that `npm run build` and `npm run lint:js` both pass. These would be expected to fail if there were invalid tokens, via PostCSS transform build-time errors and valid token linters.

## Use of AI Tools

Anthropic Claude Opus 4.6 in Plan Mode was used to plan and execute the build.

<details><summary>Rename plan</summary>

# Rename `wpds-font` to `wpds-typography`
...
This is a **breaking change** for `@wordpress/theme`. The package is used externally, so the CHANGELOG must document the rename with a migration table.

The TypeScript types (`FontFamily`, `FontSize`, `FontWeight`, `LineHeight`) are explicitly named in `terrazzo.config.ts` and do **not** need renaming -- they remain semantically correct.

## Naming Strategy

Sub-groups that correspond to CSS `font-*` properties retain the `font-` prefix. `line-height` is not a font property and drops the prefix:
...
Modify `[packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs](packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs)` to **throw an error** when a `var(--wpds-*)` reference is encountered that is not in the generated fallback map. Currently the function silently skips unknown tokens (line 31: `if (fallback === undefined) { return match; }`).

The change: replace the early return with a thrown `Error` including the unrecognized token name. This ensures any stale `--wpds-font-*` references cause a build failure with a clear message, both within Gutenberg and for external consumers using the build plugins.

The generic TypeScript version in `[add-fallback-to-var.ts](packages/theme/src/postcss-plugins/add-fallback-to-var.ts)` should remain lenient (it takes arbitrary fallback maps and is used in tests with partial maps).

Add a test case to `[add-fallback-to-var.test.ts](packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts)` verifying the error throw behavior by importing from `ds-token-fallbacks.mjs` or by adding a dedicated test for the strict behavior.

### 2. Token source file

Restructure `[packages/theme/tokens/typography.json](packages/theme/tokens/typography.json)`:

- Rename top-level key `"wpds-font"` to `"wpds-typography"`
- Rename sub-group `"family"` to `"font-family"`
- Rename sub-group `"size"` to `"font-size"`
- Rename sub-group `"weight"` to `"font-weight"`
- Keep `"line-height"` as-is

### 3. Terrazzo build config

Update `[packages/theme/terrazzo.config.ts](packages/theme/terrazzo.config.ts)`:

- Line 59: `token.id.startsWith('wpds-font.weight.')` -> `'wpds-typography.font-weight.'`
- Type patterns:
  - `wpds-font.family` -> `wpds-typography.font-family`
  - `wpds-font.size` -> `wpds-typography.font-size`
  - `wpds-font.weight` -> `wpds-typography.font-weight`
...
### 4. Regenerate prebuilt files

Run `npm run --workspace @wordpress/theme build:tokens` to regenerate:
...
- `packages/theme/src/prebuilt/ts/token-types.ts` (type names stay the same)
- `packages/theme/docs/tokens.md`

### 5. Consumer files (4 replacement patterns)

Apply these replacements across all consumer CSS/SCSS/TSX files:

1. `--wpds-font-family` -> `--wpds-typography-font-family`
2. `--wpds-font-size` -> `--wpds-typography-font-size`
3. `--wpds-font-weight` -> `--wpds-typography-font-weight`
4. `--wpds-font-line-height` -> `--wpds-typography-line-height`

`**@wordpress/ui`** (12 style files + 2 story files)
`**@wordpress/admin-ui`**(2 files)`**@wordpress/dataviews`** (1 file)
Storybook (1 file)

### 6. Test files

- `[packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts](packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts)`: Update 6 occurrences in mock fallback data and assertions
- `[packages/eslint-plugin/rules/__tests__/no-setting-ds-tokens.js](packages/eslint-plugin/rules/__tests__/no-setting-ds-tokens.js)`: Update 2 occurrences in test code strings

### 7. CHANGELOG

Add a breaking change entry to `[packages/theme/CHANGELOG.md](packages/theme/CHANGELOG.md)` under `## Unreleased` with the full migration table.

### 8. Verification

- Run `npm run --workspace @wordpress/theme build` to confirm token build succeeds
- Run JS tests for theme package: `npm run test:unit packages/theme`
- Run JS tests for eslint-plugin: `npm run test:unit packages/eslint-plugin`
- Grep for any remaining `wpds-font` references to confirm completeness
- The strict fallback error will also serve as a build-time safety net: if any `--wpds-font-` references survive in code processed by the PostCSS/esbuild/Vite plugins, the build will fail with a clear error
...
### github-actions on 2026-03-30T18:56:45Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4157403430
...
URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4157403453
...
- **Labels found**: [Package] DataViews, [Package] Theme, [Package] UI.
...
### github-actions on 2026-04-01T15:42:01Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4170990572
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 16.9 kB | +17 B (+0.1%) |
| `build/scripts/block-editor/index.min.js` | 340 kB | +19 B (+0.01%) |
| `build/scripts/edit-site/index.min.js` | 262 kB | +13 B (0%) |
| `build/scripts/editor/index.min.js` | 412 kB | +14 B (0%) |
| `build/styles/edit-post/style-rtl.css` | 4.02 kB | +4 B (+0.1%) |
| `build/styles/edit-post/style-rtl.min.css` | 2.71 kB | +12 B (+0.44%) |
| `build/styles/edit-post/style.css` | 4.02 kB | +4 B (+0.1%) |
| `build/styles/edit-post/style.min.css` | 2.71 kB | +11 B (+0.41%) |
| `build/styles/edit-site/style-rtl.css` | 20.9 kB | +13 B (+0.06%) |
| `build/styles/edit-site/style-rtl.min.css` | 17 kB | +17 B (+0.1%) |
| `build/styles/edit-site/style.css` | 20.9 kB | +11 B (+0.05%) |
| `build/styles/edit-site/style.min.css` | 17 kB | +17 B (+0.1%) |
| `build/styles/edit-widgets/style-rtl.css` | 5.35 kB | +6 B (+0.11%) |
| `build/styles/edit-widgets/style-rtl.min.css` | 4.01 kB | +10 B (+0.25%) |
| `build/styles/edit-widgets/style.css` | 5.34 kB | +6 B (+0.11%) |
| `build/styles/edit-widgets/style.min.css` | 4.02 kB | +13 B (+0.32%) |
| `build/styles/editor/style-rtl.css` | 26.6 kB | +11 B (+0.04%) |
| `build/styles/editor/style-rtl.min.css` | 22.5 kB | +18 B (+0.08%) |
| `build/styles/editor/style.css` | 26.7 kB | +12 B (+0.05%) |
| `build/styles/editor/style.min.css` | 22.5 kB | +18 B (+0.08%) |
...
### github-actions on 2026-04-01T16:39:04Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4171384299

<!-- flaky-tests-report-comment -->
**Flaky tests detected in af39c3f3667331d5872f35c4546adc8bc4c4df58.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24250559649
📝  Reported issues:
- #76012 in `/test/e2e/specs/site-editor/dataviews-list-layout-keyboard.spec.js`
...
### aduth on 2026-04-06T19:58:41Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4194667882

At the risk of reopening pandora's box, one alternative naming suggestion that came up as I was doing some final scrutinization of this pull request was `text-`. I don't think this name ever came up in our prior discussion? It's what GitHub Primer [uses](https://github.com/primer/primitives/blob/main/src/tokens/base/typography/typography.json5). It also avoids some confusion with ["typography" as a type in DTCG](https://www.designtokens.org/tr/drafts/format/#typography-component-references) meaning something different.

### mirka on 2026-04-07T11:47:51Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4198721929

> At the risk of reopening pandora's box, one alternative naming suggestion that came up as I was doing some final scrutinization of this pull request was `text-`. I don't think this name ever came up in our prior discussion?

It didn't! I just took some time to consider this though, and unfortunately I think it will be problematic because `text-` is already a namespace for CSS properties (starting with [`text-align`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/text-align) and many more going down the list). We may not have one now, but we can easily run into a `wpds-text-text-transform` situation.

### aduth on 2026-04-07T12:13:07Z

URL: https://github.com/WordPress/gutenberg/pull/76912#issuecomment-4198856115

> It didn't! I just took some time to consider this though, and unfortunately I think it will be problematic because `text-` is already a namespace for CSS properties (starting with [`text-align`](https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Properties/text-align) and many more going down the list). We may not have one now, but we can easily run into a `wpds-text-text-transform` situation.

That's a good point. With our [naming scheme](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/README.md#token-naming) maybe we could contort this to think of `text` as the "type" and `transform` as the "property", similar to what we do with `border-radius`, but there's a lot less collisions to deal with with "typography". The [DTCG overlap](https://www.designtokens.org/tr/drafts/format/#typography-component-references) may not be so bad in retrospect since while they have their own meaning, they both generally refer to the same type of grouping of typographical properties.

## Review threads

### Review thread 1

#### ciampo on 2026-03-31T10:32:37Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3015001518
Path: packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts

Should we add tests also for `ds-token-fallbacks.mjs` , since that's the actual file used by plugins?

#### aduth on 2026-04-06T17:36:23Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3040746821
Path: packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts

> Should we add tests also for `ds-token-fallbacks.mjs` , since that's the actual file used by plugins?

I missed this comment before, but added test coverage in 69ad9bad848ab489769fa8a9e7deb4f0ea93a436. In the process I noticed that we didn't really have effective TypeScript support for test files, so I improved this in 606b7a58b8bedc759c65b849adeadcde241f9af9.

### Review thread 2

#### ciampo on 2026-03-31T10:37:08Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3015022571
...
#### aduth on 2026-03-31T14:01:05Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3016130778
Path: packages/theme/CHANGELOG.md
...
#### ciampo on 2026-03-31T14:31:35Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3016319785
Path: packages/theme/CHANGELOG.md
...
#### mirka on 2026-04-01T19:16:29Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3024128913
Path: packages/theme/CHANGELOG.md
...
#### aduth on 2026-04-01T19:30:41Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3024198267
Path: packages/theme/CHANGELOG.md
...
I updated the `@wordpress/theme` package README in ac42c1801d463827e5396b0a445de1347f4e7619 to reflect how we might expect consumers to use the design tokens, including build toolings and linters. Does it read alright to you both?

### Review thread 3

#### ciampo on 2026-03-31T10:38:13Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3015027886
Path: packages/theme/src/postcss-plugins/add-fallback-to-var.ts
...
> The generic TypeScript version in [add-fallback-to-var.ts](packages/theme/src/postcss-plugins/add-fallback-to-var.ts) should remain lenient
...
Another consequence of making _both_ functions strict is that some false-positives in non-CSS contexts are caught, too (JS test strings, JSDoc examples) such as the `--wpds-border-fallback` CI error.
...
#### aduth on 2026-03-31T14:05:23Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3016155774
Path: packages/theme/src/postcss-plugins/add-fallback-to-var.ts
...
#### aduth on 2026-04-01T19:36:07Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3024223467
Path: packages/theme/src/postcss-plugins/add-fallback-to-var.ts
...
> Within WordPress, this stylesheet is loaded automatically.

In #76975, I opted to phrase this a little more vaguely as "Stylesheets are managed on your behalf in a WordPress context, so you don't need to worry about loading them yourself" to not misrepresent what is actually happening at a given time, because we don't load the stylesheet anywhere yet 😄

#### aduth on 2026-04-02T17:49:53Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3029468542
Path: packages/theme/README.md
...
I updated this in 72e68f82538056e70935cb95e8874e722d140442 to try to align with your proposal in #76975. Let me know if it reads better now 👀

### Review thread 5 (resolved)

#### mirka on 2026-04-02T16:27:09Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3029073318
Path: packages/theme/README.md
...
I believe the correct import path is `@wordpress/theme/design-tokens.css` now.

#### aduth on 2026-04-02T17:50:13Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3029470022
Path: packages/theme/README.md
...
Fixed as part of changes in 72e68f82538056e70935cb95e8874e722d140442.

### Review thread 6

#### mirka on 2026-04-02T16:29:52Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3029085651
Path: packages/theme/README.md
...
#### aduth on 2026-04-02T17:50:31Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3029471278
Path: packages/theme/README.md
...
Yeah, good point. 👍  I rephrased this in 72e68f82538056e70935cb95e8874e722d140442.

### Review thread 7

#### aduth on 2026-04-06T17:39:15Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3040761665
...
Our `tsconfig.base.json` [excludes test files](https://github.com/WordPress/gutenberg/blob/b31bdfa26f6b09a3e6e19d7ca5f8c8ff67d40677/tsconfig.base.json#L45). I don't know that I like that as a default, but in any case, that's why this is here: To make TypeScript check _all_ files in `src`.

### Review thread 8

#### mirka on 2026-04-06T17:48:48Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3040807094
Path: packages/theme/CHANGELOG.md
...
#### aduth on 2026-04-06T19:16:55Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3041218566
Path: packages/theme/CHANGELOG.md
...
Thanks 👍 Was a bad rebase resolution. Fixed in 0c3dfa8421fb75a7ec5a2731a529e93262392a76.

### Review thread 9

#### mirka on 2026-04-06T17:52:25Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3040824688
Path: packages/theme/README.md

```diff
@@ -13,6 +13,34 @@ A theming package that's part of the WordPress Design System. It has two parts:
...
We can append "This stylesheet is universal and does not have a separate RTL version." here like we did in #76975.

#### aduth on 2026-04-06T19:17:10Z

URL: https://github.com/WordPress/gutenberg/pull/76912#discussion_r3041219619
Path: packages/theme/README.md
...
Makes sense 👍 Added in e996c3f6ff4b38ab504819ac79c3f358299ebdf5.

## Changed files

- packages/admin-ui/src/breadcrumbs/style.scss (MODIFIED, +4/-4)
- packages/admin-ui/src/page/style.scss (MODIFIED, +7/-7)
- packages/dataviews/src/components/dataform-layouts/panel/style.scss (MODIFIED, +3/-3)
- packages/eslint-plugin/docs/rules/no-ds-tokens.md (MODIFIED, +1/-1)
- packages/eslint-plugin/rules/__tests__/no-ds-tokens.js (MODIFIED, +3/-3)
- packages/eslint-plugin/rules/__tests__/no-setting-ds-tokens.js (MODIFIED, +2/-2)
- packages/theme/CHANGELOG.md (MODIFIED, +13/-1)
- packages/theme/README.md (MODIFIED, +51/-22)
- packages/theme/docs/tokens.md (MODIFIED, +19/-19)
- packages/theme/src/postcss-plugins/add-fallback-to-var.ts (MODIFIED, +5/-1)
- packages/theme/src/postcss-plugins/ds-token-fallbacks.mjs (MODIFIED, +5/-1)
- packages/theme/src/postcss-plugins/test/add-fallback-to-var.test.ts (MODIFIED, +10/-9)
- packages/theme/src/postcss-plugins/test/ds-token-fallbacks.test.ts (ADDED, +39/-0)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +21/-19)
- packages/theme/src/prebuilt/js/design-token-fallbacks.mjs (MODIFIED, +18/-17)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +17/-17)
- packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.mjs (MODIFIED, +3/-3)
- packages/theme/terrazzo.config.ts (MODIFIED, +5/-5)
- packages/theme/tokens/typography.json (MODIFIED, +4/-4)
- packages/theme/tsconfig.src.json (MODIFIED, +2/-1)
- packages/ui/src/badge/style.module.css (MODIFIED, +4/-4)
- packages/ui/src/button/style.module.css (MODIFIED, +4/-4)
- packages/ui/src/card/stories/index.story.tsx (MODIFIED, +4/-4)
- packages/ui/src/collapsible-card/stories/index.story.tsx (MODIFIED, +5/-5)
- packages/ui/src/dialog/style.module.css (MODIFIED, +3/-3)
- packages/ui/src/empty-state/style.module.css (MODIFIED, +1/-1)
- packages/ui/src/form/primitives/input-layout/style.module.css (MODIFIED, +3/-3)
- packages/ui/src/link/style.module.css (MODIFIED, +1/-1)
- packages/ui/src/notice/style.module.css (MODIFIED, +5/-5)
- packages/ui/src/popover/style.module.css (MODIFIED, +3/-3)
- packages/ui/src/tabs/style.module.css (MODIFIED, +2/-2)
- packages/ui/src/text/style.module.css (MODIFIED, +49/-49)
- packages/ui/src/tooltip/style.module.css (MODIFIED, +2/-2)
- packages/ui/src/utils/css/field.module.css (MODIFIED, +10/-10)
- packages/ui/src/utils/css/global-css-defense.module.css (MODIFIED, +1/-1)
