...
type: pull_request
state: MERGED
merged_at: 2025-12-10T09:58:52Z
url: https://github.com/WordPress/gutenberg/pull/73793
matched_labels: ["Design System"]
all_labels: ["[Type] Enhancement","Design System"]
---

# PR #73793: Expand error family of tokens

- URL: https://github.com/WordPress/gutenberg/pull/73793
- Author: jameskoster
- Created: 2025-12-05T15:05:30Z
- Updated: 2026-01-07T13:08:57Z
- Comments: 6 of 6
- Review threads: 1 of 1
- Changed files: 6 of 6
- Diff: ../pr-diffs/73793.diff
...
- **Background tokens**: `bg-interactive-error` (normal, strong, weak) with resting, active, and disabled states
- **Foreground tokens**: `fg-interactive-error` (normal, strong) with resting, active, and disabled states
- **Stroke tokens**: `stroke-interactive-error` (normal) with resting and active states

Also: Remove `--wpds-color-stroke-interactive-brand-disabled` as it's not a token with any obvious use case currently. It can easily be added back later if necessary. 

Updates generated outputs (CSS, JS, JSON, TypeScript) and documentation to include these tokens.
...
1. Added token definitions in `packages/theme/tokens/color.json`:
   - Background interactive error tokens (normal, strong, weak) with resting/active/disabled states
   - Foreground interactive error tokens (normal, strong) with resting/active/disabled states
   - Stroke interactive error tokens (normal) with resting/active states
2. Regenerated prebuilt outputs:
   - CSS variables in `design-tokens.css`
   - JavaScript exports in `design-tokens.js`
   - Figma-compatible JSON in `figma.json`
   - TypeScript mappings in `color-tokens.ts`
3. Updated documentation in `ds-tokens.md` to include the new tokens in the reference table.
4. Updated TypeScript mappings to include the new tokens in the color token aliases.

## Issue comments

### github-actions on 2025-12-05T15:17:56Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3617334685

**Size Change:** +590 B (+0.02%) 

**Total Size:** 2.57 MB

| Filename | Size | Change |
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 103 kB | +527 B (+0.52%) |
| `build/scripts/theme/index.min.js` | 20.8 kB | +63 B (+0.3%) |
...
### github-actions on 2025-12-05T18:56:10Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3618147470
...
### jameskoster on 2025-12-08T11:31:07Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3626446971

> Hard to visualize without real-world usage, but we can tinker as we go 😄

Fair point haha. Here's a visualisation of the different tones and states:

<img width="456" height="856" alt="buttons" src="https://github.com/user-attachments/assets/61a978b5-4a24-4131-b217-b9702e101267" />

### ciampo on 2025-12-23T14:44:36Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3686891227

@jameskoster @aduth

I remember that a few months ago we discussed this very topic and decided _against_ having destructive buttons. We agreed that, instead, we'd use other tones + a confirmation dialog when needed for destructive actions — which influenced some decisions in the token architecture and naming.

What's the reason for reverting this decision?

### aduth on 2025-12-29T19:48:11Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3697360114

@jameskoster may have more insight, though I think the idea with additional danger tones was specifically for _irreversibly_ destructive actions, with the [current confirmation dialog pattern](https://wordpress.github.io/gutenberg/?path=/docs/components-confirmdialog--docs) using primary tones being sufficient for destructive, reversible decisions.

### jameskoster on 2026-01-07T13:08:57Z

URL: https://github.com/WordPress/gutenberg/pull/73793#issuecomment-3718790068

@ciampo @poligilad-auto worked on a revised workflow for destructive actions that included red accents for buttons in certain situations. See more in https://github.com/WordPress/gutenberg/issues/66810#issuecomment-3531947620

## Review threads

### Review thread 1

#### aduth on 2025-12-05T18:55:20Z

URL: https://github.com/WordPress/gutenberg/pull/73793#discussion_r2593659191
Path: packages/theme/docs/ds-tokens.md

```diff
@@ -110,6 +125,8 @@ Do not edit directly.
...
#### jameskoster on 2025-12-08T11:22:52Z

URL: https://github.com/WordPress/gutenberg/pull/73793#discussion_r2598226856
Path: packages/theme/docs/ds-tokens.md
...
#### aduth on 2025-12-08T14:54:13Z

URL: https://github.com/WordPress/gutenberg/pull/73793#discussion_r2598941675
Path: packages/theme/docs/ds-tokens.md
...
#### jameskoster on 2025-12-09T10:09:47Z

URL: https://github.com/WordPress/gutenberg/pull/73793#discussion_r2601952313
Path: packages/theme/docs/ds-tokens.md
...
- packages/theme/docs/ds-tokens.md (MODIFIED, +17/-1)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +17/-1)
- packages/theme/src/prebuilt/js/design-tokens.js (MODIFIED, +17/-1)
- packages/theme/src/prebuilt/json/figma.json (MODIFIED, +99/-3)
- packages/theme/src/prebuilt/ts/color-tokens.ts (MODIFIED, +26/-4)
- packages/theme/tokens/color.json (MODIFIED, +84/-4)
