...
type: pull_request
state: MERGED
merged_at: 2026-05-19T19:13:00Z
url: https://github.com/WordPress/gutenberg/pull/78438
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Developer Documentation","[Package] Theme"]
---

# PR #78438: Theme: Move token structure descriptions into tokens document

- URL: https://github.com/WordPress/gutenberg/pull/78438
- Author: aduth
- Created: 2026-05-19T13:22:19Z
- Updated: 2026-05-19T21:02:29Z
- Comments: 6 of 6
- Review threads: 0 of 0
- Changed files: 4 of 4
- Diff: ../pr-diffs/78438.diff

## Body

## What?

Moves token documentation about naming structure and purpose from `@wordpress/theme`'s `README.md` into its [`docs/tokens.md`](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/docs/tokens.md).

Related discussions:

- https://github.com/WordPress/gutenberg/pull/78186#issuecomment-4461146606
- https://github.com/WordPress/gutenberg/pull/78397#issuecomment-4479318738

## Why?

This is primarily aimed at enabling two things:

- The [WordPress Design System MCP](https://github.com/WordPress/gutenberg/tree/trunk/packages/design-system-mcp)'s `get_design_tokens` tool fetches this `tokens.md` document directly, so additional context here should prove valuable for AI agents to understand how to use design tokens effectively.
- Separately, I plan to add a new page to [the Storybook site](https://wordpress.github.io/gutenberg/) which surfaces this tokens document for human readers.

## How?

Because `tokens.md` is generated dynamically from the source tokens using a custom [Terrazzo](https://terrazzo.app/) plugin, the text is injected in the plugin code itself rather than modifying the `.md` file directly.

## Testing Instructions

Review the updated documents for accuracy:

- [README.md](https://github.com/WordPress/gutenberg/blob/update/theme-tokens-purpose/packages/theme/README.md)
- [tokens.md](https://github.com/WordPress/gutenberg/blob/update/theme-tokens-purpose/packages/theme/docs/tokens.md)
...
### github-actions on 2026-05-19T13:31:32Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4488255448
...
### github-actions on 2026-05-19T13:54:21Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4488464689

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 740fa9455d925a182dab3a3e1630474433e2643e.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26115155075
📝  Reported issues:
- #64334 in `/test/e2e/specs/site-editor/dataviews-list-layout-keyboard.spec.js`

### github-actions on 2026-05-19T14:02:19Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4488537359
...
### aduth on 2026-05-19T14:27:16Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4488761572

> As a future follow-up opportunity, do you think we should consider making the preamble more dynamic, ie. generated from the info in the `json` file rather than hardcoded?

Yeah, that's a good idea. I was also looking at opportunities to improve the descriptions of the tokens. For example, I think we do a really good job explaining semantic purpose for [elevation tokens](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/docs/tokens.md#elevation) use-cases and some of that might be worth considering for other tokens like [dimension](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/docs/tokens.md#dimension). The [color descriptions](https://github.com/WordPress/gutenberg/blob/trunk/packages/theme/docs/tokens.md#color) read pretty well to me personally, but the anecdotal observation that AI would choose to use interactive background colors for non-interactive elements might suggest there's some improvement opportunity there.

### aduth on 2026-05-19T17:36:24Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4490382952

I took these changes for a spin using a prompt from an anecdotal demo interface build that had previously used incorrect tokens (using interactive tokens for a chat bubble background in a hypothetical chat UI). It seemed to work a lot better this time using the `get_design_tokens` tool:

Prompt:

> If i were to build a custom UI for a chat interface using the wordpress design system, what design system css variable would i use for the background of a chat bubble message, and why?

AI response (Claude Opus 4.7):

> I'll check the WordPress Design System for the right token to use here.
>
> Good — I have the full token reference now. For chat bubble backgrounds, the right family is the `bg-surface-*` tokens, since bubbles are passive content containers (surfaces), not interactive controls or strokes.
> 
> The specific token depends on which bubble you're styling, because chat UIs almost always need two visually distinct bubbles so the user can tell their own messages from the other party's at a glance:
> 
> For the **other person's / incoming bubble**, use `--wpds-color-bg-surface-neutral` (or `-weak` if your chat sits on top of an already-neutral page background and you need the bubble to recede slightly). It's the default neutral container surface — quiet, doesn't compete with the message text, and lets the foreground content do the work.
> 
> For the **current user's / outgoing bubble**, use `--wpds-color-bg-surface-brand`. The `brand` surface token exists specifically to give a container a tonal identity without making it look like an interactive control. That's exactly the chat-bubble convention: "this one is mine," visually distinct, but it's not a button — you don't tap it to do anything.
> 
> A few important "why nots" so the choice is deliberate rather than just plausible:
> 
> Don't reach for `--wpds-color-bg-interactive-brand-strong` for the user bubble even though it's the same brand-colored visual family. The `interactive-*` tokens carry hover/active/disabled states and signal affordance — applying them to a static bubble tells users (and assistive tech consumers reading the styling intent) that the surface responds to interaction, which a message bubble generally doesn't.
> 
> Don't use the semantic-tone surfaces — `success`, `info`, `warning`, `caution`, `error` — for the default bubble background. Those tokens carry meaning. An `info`-tinted bubble reads as a system notice, not a message; an `error`-tinted bubble reads as a failed send. Reserve them for exactly those cases (e.g., `--wpds-color-bg-surface-error-weak` on a bubble that failed to deliver, `--wpds-color-bg-surface-info-weak` on a system message like "Encryption enabled").
> 
> For the matching text color inside each bubble, pair with `--wpds-color-fg-content-neutral` on the neutral bubble. The brand bubble is a little trickier — the token system doesn't expose a dedicated `fg-content-on-brand`, so you'd typically reach for the strongest neutral foreground or define the contrast pairing at your component layer.
> 
> If you want, I can sketch a small example component showing the two bubble variants wired up with these tokens.

The note about "the token system doesn't expose a dedicated `fg-content-on-brand`" is interesting. My understanding is that the same foreground token would apply on the brand background, but either way it's not really coming across very clearly.

### ciampo on 2026-05-19T21:02:22Z

URL: https://github.com/WordPress/gutenberg/pull/78438#issuecomment-4492016395
...
- packages/theme/README.md (MODIFIED, +1/-87)
- packages/theme/bin/terrazzo-plugin-ds-tokens-docs/index.ts (MODIFIED, +119/-0)
- packages/theme/docs/tokens.md (MODIFIED, +116/-0)
