...
type: pull_request
state: MERGED
merged_at: 2025-12-01T18:49:55Z
url: https://github.com/WordPress/gutenberg/pull/73625
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Performance","[Package] Theme"]
---

# PR #73625: Theme: better max chroma detection for taperChroma

- URL: https://github.com/WordPress/gutenberg/pull/73625
- Author: jsnajdr
- Created: 2025-11-27T16:15:18Z
- Updated: 2025-12-01T18:50:39Z
- Comments: 2 of 2
- Review threads: 3 of 3
- Changed files: 7 of 7
- Diff: ../pr-diffs/73625.diff

## Body

This PR is an optimization and cleanup of the `taperChroma` function. It speeds up the color ramp calculation again: my runs of the "buildRamps" unit test show an improvement from 1400ms to 800ms, almost 2x improvement.

The `taperChroma` function makes some colors in the color ramp more gray than they would normally be. It's very visible especially with very vivid colors. The following image shows a ramp for pure blue (`#00f`). The top ramp is with `taperChroma`, the lower ramp is without taper. There is big difference especially for `fgSurfaceX` colors, and also for `strokeX`.

<img width="1121" height="164" alt="Screenshot 2025-11-27 at 10 56 10" src="https://github.com/user-attachments/assets/9c3a752a-07f9-4d9a-a805-5513175ab666" />

And this PR does several changes to `taperChroma`:
- we calculate and cache the "max chroma for a given lightness and hue" value. This PR optimizes this calculation by rounding to more coarse steps. The entire LxH area (L=0.0-.01, H=0deg-360deg) is covered by 20x36 steps. That increases the number of cache hits a lot.
- `taperChroma` uses the sRGB gamut instead of P3. sRGB is where the final colors always are, so there's no point using P3.
- `taperChroma` no longer does any gamut mapping (`toGamut`). That's a separate step happening after `taperChroma`. Previously we were doing the gamut mapping twice.
- the algorithm to find the maximum chroma is greatly improved. The previous version was often giving too low chroma values. The new algorithm uses `toGamutCSS` to clamp a color with too high chroma to an in-gamut value. You can read more details in this colorjs.io doc: https://colorjs.io/docs/gamut-mapping. The doc describes a "bad" `lch.c` algorithm, and that's exactly what `taperChroma` was using previously. And it describes the "good" CSS algorithm, and that's what we're using now 🙂
- consistently use `PlainColorObject` type internally, instead of the more generic `ColorTypes`. That's mainly because `clone` accepts only `PlainColorObject`s.

The net result is a much faster and cleaner `taperChroma`, and imperceptible differences in generated colors.

## Issue comments

### github-actions on 2025-11-27T16:27:29Z

URL: https://github.com/WordPress/gutenberg/pull/73625#issuecomment-3586688938
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 72.6 kB | -6 B (-0.01%) |
| `build/scripts/theme/index.min.js` | 20.8 kB | -79 B (-0.38%) |
...
### github-actions on 2025-11-27T16:50:50Z

URL: https://github.com/WordPress/gutenberg/pull/73625#issuecomment-3586772643
...
#### jsnajdr on 2025-11-27T16:17:04Z

URL: https://github.com/WordPress/gutenberg/pull/73625#discussion_r2569439810
...
#### jsnajdr on 2025-11-27T16:49:35Z

URL: https://github.com/WordPress/gutenberg/pull/73625#discussion_r2569525072
Path: packages/theme/src/color-ramps/lib/taper-chroma.ts
...
<img width="410" height="322" alt="Image" src="https://github.com/user-attachments/assets/b5133432-7525-4993-bdae-57246ae24f54" />

That's exactly the surface we're calculating here. For each (L,H) point, the maximum chroma value C.

### Review thread 3

#### jsnajdr on 2025-11-27T16:50:33Z

URL: https://github.com/WordPress/gutenberg/pull/73625#discussion_r2569527177
Path: packages/theme/src/color-ramps/lib/taper-chroma.ts
...
- packages/theme/src/color-ramps/lib/default-ramps.ts (MODIFIED, +65/-65)
- packages/theme/src/color-ramps/lib/find-color-with-constraints.ts (MODIFIED, +11/-7)
- packages/theme/src/color-ramps/lib/index.ts (MODIFIED, +4/-4)
- packages/theme/src/color-ramps/lib/taper-chroma.ts (MODIFIED, +32/-63)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +20/-20)
- packages/theme/src/prebuilt/json/figma.json (MODIFIED, +20/-20)
- packages/theme/tokens/color.json (MODIFIED, +65/-65)
