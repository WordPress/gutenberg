...
type: pull_request
state: MERGED
merged_at: 2022-10-25T12:13:50Z
url: https://github.com/WordPress/gutenberg/pull/45241
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components"]
---

# PR #45241: BorderControl test: await floating-ui state changes when rendering/opening popover

- URL: https://github.com/WordPress/gutenberg/pull/45241
- Author: jsnajdr
- Created: 2022-10-24T11:43:53Z
- Updated: 2022-10-25T12:14:14Z
- Comments: 3 of 3
- Review threads: 1 of 1
- Changed files: 1 of 1
- Diff: ../pr-diffs/45241.diff

## Body

This PR fixes an issue where the `BorderControl` unit test fails with React 18 (see #45235) with an

> An update to %s inside a test was not wrapped in act(...)

error. It happens right after the `BorderControl` is rendered or rerendered, or after an `openPopover` call. Why would that happen given that `render` and `fireEvent` calls are automatically wrapped with `act()`? Because the component uses the `@floating-ui/react-dom` library, and that library [does `setState` in a promise handler](https://github.com/floating-ui/floating-ui/blob/59c546450c685bcdb5e4fb52cd3a297275c59edf/packages/react-dom/src/useFloating.ts#L53-L64), like:
```js
useLayoutEffect( () => {
  computePosition().then( ( data ) => {
    setData( data );
  } );
} );
```
Because the `act()` wrappers used by `render` and `fireEvent` are "synchronous", i.e, they are `act( () => {} )` calls that don't wait for promises to resolve, the `setData` call escapes outside the `act()`.

We need to add an "asynchronous" `act()`:
```js
await act( async () => {} );
```
to get the `setData` flushed before the promise returned by `act()` resolves.

This PR fixes the issue by making the `renderBorderControl` and `openPopover` functions asynchronous and by adding an `await act` call inside them.

The same fix is recommended by the Floating UI docs: https://floating-ui.com/docs/react-dom#testing
...
This issue was already somewhat discussed back in April in https://github.com/WordPress/gutenberg/pull/32765/files#r845193273.

## Issue comments

### github-actions on 2022-10-24T11:52:50Z

URL: https://github.com/WordPress/gutenberg/pull/45241#issuecomment-1288920807
...
### Mamaduka on 2022-10-25T11:23:24Z

URL: https://github.com/WordPress/gutenberg/pull/45241#issuecomment-1290393953

The failing PHPUnit tests are fixed now. See #45265.

### jsnajdr on 2022-10-25T11:27:08Z

URL: https://github.com/WordPress/gutenberg/pull/45241#issuecomment-1290398314

>  The failing PHPUnit tests are fixed now.
...
#### tyxla on 2022-10-25T09:14:45Z

URL: https://github.com/WordPress/gutenberg/pull/45241#discussion_r1004217866
Path: packages/components/src/border-control/test/index.js

```diff
@@ -22,28 +22,37 @@ const defaultBorder = {
 	width: '1px',
 };
...
I'm on the same page as @ciampo here - just like we did for #45208 it would be nice to use the opportunity to inline those `render()` calls. it doesn't help us much to reduce code by having helper functions, but it does obfuscate the test logic and makes it harder to follow.

#### jsnajdr on 2022-10-25T09:54:50Z

URL: https://github.com/WordPress/gutenberg/pull/45241#discussion_r1004261849
Path: packages/components/src/border-control/test/index.js
...
```js
renderAndWait( <BorderControl { ...props } /> );
```
making the wrapper a bit shorter, but let's do that in another PR. We could probably avoid all the `rerender` calls, too, by putting the `value` prop into local `useState` of a wrapper component that is actually tested. But let's do that independently, as it's not necessary for the React 18 upgrade.

> inline comments would definitely help, because `act()` around `element.focus()` seems pretty redundant.
...
- packages/components/src/border-control/test/index.js (MODIFIED, +139/-96)
