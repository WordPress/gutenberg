...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/75101
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Task","[Package] Theme"]
---

# PR #75101: Theme: Add dimension tokens for control height

- URL: https://github.com/WordPress/gutenberg/pull/75101
- Author: aduth
- Created: 2026-01-30T17:36:16Z
- Updated: 2026-03-24T19:28:11Z
- Comments: 8 of 8
- Review threads: 0 of 0
- Changed files: 59 of 59
- Diff: ../pr-diffs/75101.diff
...
Builds on: #75054
Related discussion: https://github.com/WordPress/gutenberg/pull/74652#discussion_r2733507053

Adds new `control-height` dimension tokens and updates `Button` to use them.

## Why?

Right now, this is exploratory, seeking to:

- Stress-test padding tokens in #75054.
   - At first I wondered whether these new tokens might allow us to move more in the direction of using padding to control button height, as buttons don't currently respond to theme density. However, in reflection of past discussions and some discussion with my friend Claude, controlling these heights explicitly may be preferable, particularly in how these controls after often arranged together (e.g. input next to a button).
- Apply "common control arrangement" height through design tokens for `control-height`.
   - The idea being that if we want these controls to have consistent height and respond collectively to theme density, we can achieve this through common design tokens that have density-specific adjustments.
   - These could then be used in other components like `Input`, and perhaps later expanded to incorporate other common control aspects like inline padding, gaps, and icon size.

This doesn't completely answer the question about the relation between component-specific sizing properties like `Button`'s `size=compact` or `Tab`'s `density`/`variant`, but it does provide a path for them to work together:

- Component-level props like `size` allow absolute control over the role that component plays (e.g. [toggle buttons are inherently compact](https://github.com/WordPress/gutenberg/blob/34b28b61de8447a57b2bb8735a215c20c85c65dd/packages/ui/src/button/types.ts#L31))
- Theme density is environmental/ambient, like a toolbar of related controls being compacted
...
### github-actions on 2026-01-30T17:36:24Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3824871427
...
### github-actions on 2026-01-30T17:47:39Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3824915358

**Size Change:** +1.09 kB (+0.04%) 

**Total Size:** 3 MB

| Filename | Size | Change |
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 14.7 kB | +65 B (+0.44%) |
| `build/styles/edit-site/style-rtl.css` | 22.1 kB | +113 B (+0.51%) |
| `build/styles/edit-site/style-rtl.min.css` | 17.1 kB | +54 B (+0.32%) |
| `build/styles/edit-site/style.css` | 22.1 kB | +113 B (+0.51%) |
| `build/styles/edit-site/style.min.css` | 17.1 kB | +53 B (+0.31%) |
| `build/styles/editor/style-rtl.css` | 26.1 kB | +122 B (+0.47%) |
| `build/styles/editor/style-rtl.min.css` | 20.6 kB | +57 B (+0.28%) |
| `build/styles/editor/style.css` | 26.1 kB | +123 B (+0.47%) |
| `build/styles/editor/style.min.css` | 20.6 kB | +58 B (+0.28%) |
| `build/styles/theme/style-rtl.css` | 2.81 kB | +117 B (+4.35%) |
| `build/styles/theme/style-rtl.min.css` | 1.44 kB | +48 B (+3.45%) |
| `build/styles/theme/style.css` | 2.81 kB | +117 B (+4.35%) |
| `build/styles/theme/style.min.css` | 1.44 kB | +48 B (+3.45%) |
...
### aduth on 2026-01-30T19:09:36Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3825247846
...
### jameskoster on 2026-02-02T12:39:08Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3834885415

Height arguably has application beyond controls; menu items, toolbars, chips, images (avatars), tabs. So I wonder if this should be a broader more generic scale?

I'm also curious how checkboxes and radios fit in. They're controls, but not covered by proposed the scale currently. Should they be? This may also hint towards a larger scale.

The density picture still isn't super clear to me which makes it difficult to comment on the interaction between `size` and `density`.

### aduth on 2026-02-02T17:01:44Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3836504369
...
Yeah one thing which occurs to me now after letting it sit for the weekend is that while there's precedent with the "small" and "compact" naming variants here, it doesn't do us any favors in how it overlaps with the "compact" density. Especially confusing is [the `compact` mode for the `compact` sub-token](https://github.com/WordPress/gutenberg/pull/75101/files#diff-b8ec86988a7516f85cbd284ec583faddfaa890a08d44a7c5f92aa5e24f22815bR147).

### jameskoster on 2026-02-03T11:39:42Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3840806278
...
### ciampo on 2026-02-04T15:54:54Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-3848270348
...
Re tokens in this PR, my gut feeling is that we may want more generic "size" tokens, to be applied to UI sizing in general

### aduth on 2026-03-24T19:28:07Z

URL: https://github.com/WordPress/gutenberg/pull/75101#issuecomment-4120827994
...
- packages/dataviews/src/components/dataviews-layouts/grid/composite-grid.tsx (MODIFIED, +4/-4)
...
- packages/theme/CHANGELOG.md (MODIFIED, +18/-0)
- packages/theme/README.md (MODIFIED, +2/-2)
- packages/theme/docs/tokens.md (MODIFIED, +20/-14)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +67/-43)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +12/-6)
...
- packages/theme/tokens/dimension.json (MODIFIED, +127/-64)
- packages/theme/tokens/modes/dimension.comfortable.json (MODIFIED, +59/-26)
- packages/theme/tokens/modes/dimension.compact.json (MODIFIED, +59/-26)
...
- packages/ui/src/button/style.module.css (MODIFIED, +5/-7)
