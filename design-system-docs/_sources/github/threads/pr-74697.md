...
type: pull_request
state: MERGED
merged_at: 2026-01-30T08:40:56Z
url: https://github.com/WordPress/gutenberg/pull/74697
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #74697: `@wordpress/ui`: add `IconButton`

- URL: https://github.com/WordPress/gutenberg/pull/74697
- Author: ciampo
- Created: 2026-01-16T17:09:56Z
- Updated: 2026-01-30T08:42:31Z
- Comments: 2 of 2
- Review threads: 5 of 5
- Changed files: 12 of 12
- Diff: ../pr-diffs/74697.diff
...
Add `IconButton` component to `@wordpress/ui`
...
![Screenshot 2026-01-29 at 14 40 57](https://github.com/user-attachments/assets/fa1ad58d-6204-4ad6-94a0-22747a620610)

## Issue comments

### github-actions on 2026-01-29T13:34:53Z

URL: https://github.com/WordPress/gutenberg/pull/74697#issuecomment-3817731283
...
### github-actions on 2026-01-29T13:41:33Z

URL: https://github.com/WordPress/gutenberg/pull/74697#issuecomment-3817766244
...
#### ciampo on 2026-01-29T13:43:03Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2741721145
Path: packages/ui/src/button/stories/index.story.tsx

For the `Pressed` storybook examples (here and in `IconButton`), I thought that it would better to load the component in its pressed state directly, and to rely on Storybook controls to toggle its pressed/active state (instead of a custom `render` story property)

### Review thread 2 (resolved)

#### copilot-pull-request-reviewer on 2026-01-29T13:45:45Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2741733574
...
The property key for `aria-pressed` is using double quotes here (`"aria-pressed"`), whereas the rest of the codebase (including the Button stories and types) consistently uses single-quoted keys like `'aria-pressed'`; aligning this to single quotes will keep it consistent and avoid running afoul of the standard quotes lint rule.

### Review thread 3 (resolved)

#### copilot-pull-request-reviewer on 2026-01-29T13:45:45Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2741733609
Path: packages/ui/src/icon-button/stories/index.story.tsx
...
The imported `useState` from `@wordpress/element` is not used in this story file anymore and should be removed to avoid lint warnings and keep the stories minimal.
```suggestion
...
#### copilot-pull-request-reviewer on 2026-01-29T13:45:46Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2741733653
Path: packages/ui/src/button/stories/index.story.tsx

```diff
@@ -166,27 +166,14 @@ export const Loading: Story = {
...
After simplifying the `Pressed` story to use a static `'aria-pressed'` arg, the `useState` import at the top of this file is now unused and should be removed to avoid lint warnings.

### Review thread 5

#### ciampo on 2026-01-29T14:00:18Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2741804643
...
I thought that adding `& { children?: unknown }` on the inner `function` arguments was more elegant than adding a `@ts-expect-error` on the `children` prop destructuring, but I'm happy to change approach as folks prefer

#### aduth on 2026-01-29T21:02:34Z

URL: https://github.com/WordPress/gutenberg/pull/74697#discussion_r2743538061
Path: packages/ui/src/icon-button/icon-button.tsx
...
- package-lock.json (MODIFIED, +1/-10)
- packages/ui/package.json (MODIFIED, +1/-0)
- packages/ui/src/button/stories/index.story.tsx (MODIFIED, +3/-16)
- packages/ui/src/icon-button/icon-button.tsx (ADDED, +64/-0)
- packages/ui/src/icon-button/index.ts (ADDED, +1/-0)
- packages/ui/src/icon-button/stories/index.story.tsx (ADDED, +128/-0)
- packages/ui/src/icon-button/style.module.css (ADDED, +8/-0)
- packages/ui/src/icon-button/test/index.test.tsx (ADDED, +86/-0)
- packages/ui/src/icon-button/types.ts (ADDED, +38/-0)
- packages/ui/src/index.ts (MODIFIED, +1/-0)
- packages/ui/src/utils/types.ts (MODIFIED, +8/-9)
