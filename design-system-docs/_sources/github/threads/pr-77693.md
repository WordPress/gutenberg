...
type: pull_request
state: MERGED
merged_at: 2026-04-27T15:50:20Z
url: https://github.com/WordPress/gutenberg/pull/77693
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #77693: ui: Forward style and className on *.Popup to inner Base UI Popup

- URL: https://github.com/WordPress/gutenberg/pull/77693
- Author: ciampo
- Created: 2026-04-27T12:51:14Z
- Updated: 2026-04-27T15:51:03Z
- Comments: 3 of 3
- Review threads: 0 of 0
- Changed files: 5 of 5
- Diff: ../pr-diffs/77693.diff

## Body

Follow-up to [#77648 (comment)](https://github.com/WordPress/gutenberg/pull/77648#issuecomment-4313669032).

## What?

Make `style` and `className` on `Popover.Popup`, `Tooltip.Popup`, and `Select.Popup` forward to the inner Base UI `Popup` (matching `Dialog.Popup`, `AlertDialog.Popup`, and `Drawer.Popup`) instead of the outer `Positioner`.

## Why?

The previous behavior was inconsistent across the overlay family and silently routed user props to a different element than the API name suggested. With every overlay now forwarding the same way, consumers can rely on a single mental model. Per-instance z-index overrides should use `<Overlay.Portal style={ { '--wp-ui-<overlay>-z-index': '9999' } } />` (uniformly demonstrated by #77648).

<details>
<summary>Forwarding contract before/after</summary>

| Component | Before | After |
| --- | --- | --- |
| `Dialog.Popup` | → Base UI `Popup` | → Base UI `Popup` (unchanged) |
| `AlertDialog.Popup` | → Base UI `Popup` | → Base UI `Popup` (unchanged) |
| `Drawer.Popup` | → Base UI `Popup` | → Base UI `Popup` (unchanged) |
| `Popover.Popup` | → Base UI **`Positioner`** | → Base UI **`Popup`** |
| `Tooltip.Popup` | → Base UI **`Positioner`** | → Base UI **`Popup`** |
| `Select.Popup` | → Base UI **`Positioner`** | → Base UI **`Popup`** |
...
- `className` is destructured and merged with the internal popup class via `clsx` before being passed to `<_Overlay.Popup>`. `style` flows implicitly via `...props`.
- `.positioner` CSS module class continues to own `z-index: var(--wp-ui-<overlay>-z-index, initial)` (the positioner is the absolutely-positioned element, so z-index has to live there) and the `box-sizing` reset, just without the user `className` mixed in.
- No JSDoc was added on `PopupProps` — the post-refactor behavior matches the silent siblings, so nothing surprising is left to document. The z-index escape hatch is documented uniformly via the Storybook stories aligned in #77648.

Audit of other props going to `Positioner`:

- All explicit positioning props (`align`, `alignOffset`, `anchor`, `arrowPadding`, `collisionAvoidance`, `collisionBoundary`, `collisionPadding`, `side`, `sideOffset`, `sticky`) — `Positioner`-only Base UI props, correctly stay on `Positioner`.
- `initialFocus`, `finalFocus` — already correctly on `Popup`.
- `...props` rest — already correctly forwarded to `Popup`.

So `style` and `className` were the only props going to the wrong element.

Base UI CSS variables exposed by `Positioner` (`--available-height`, `--available-width`, `--anchor-width`, …) cascade down and remain readable from the popup.
...
2. Open the existing `Popover`, `Tooltip`, and `Select` stories — especially `WithCustomZIndex` — and confirm rendering and z-index override still work.
3. `npm run test:unit -- packages/ui/src/popover packages/ui/src/tooltip packages/ui/src/form/primitives/select` — all suites pass.

### Testing Instructions for Keyboard

Open `Popover`, `Tooltip`, and `Select` stories and confirm keyboard interactions (Tab, Escape, arrow keys) are unchanged.
...
### github-actions on 2026-04-27T13:04:31Z

URL: https://github.com/WordPress/gutenberg/pull/77693#issuecomment-4327186465
...
### github-actions on 2026-04-27T13:22:25Z

URL: https://github.com/WordPress/gutenberg/pull/77693#issuecomment-4327309744

<!-- flaky-tests-report-comment -->
**Flaky tests detected in d552782fe4bf4b0db7e80e9421f47cfc8b826f1f.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24998242571
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### github-actions on 2026-04-27T13:38:00Z

URL: https://github.com/WordPress/gutenberg/pull/77693#issuecomment-4327420011
...
- packages/ui/src/form/primitives/select/popup.tsx (MODIFIED, +3/-8)
- packages/ui/src/popover/popup.tsx (MODIFIED, +5/-8)
- packages/ui/src/popover/types.ts (MODIFIED, +0/-10)
- packages/ui/src/tooltip/popup.tsx (MODIFIED, +2/-8)
