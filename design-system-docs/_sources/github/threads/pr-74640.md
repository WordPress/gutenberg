...
type: pull_request
state: MERGED
merged_at: 2026-01-15T18:12:24Z
url: https://github.com/WordPress/gutenberg/pull/74640
matched_labels: ["[Package] Components","[Package] Theme"]
all_labels: ["[Type] Build Tooling","[Package] Components","[Package] DataViews","[Package] Theme"]
---

# PR #74640: Infrastructure: Convert storybook to a workspace package

- URL: https://github.com/WordPress/gutenberg/pull/74640
- Author: manzoorwanijk
- Created: 2026-01-15T04:59:02Z
- Updated: 2026-01-16T15:29:36Z
- Comments: 5 of 5
- Review threads: 3 of 3
- Changed files: 7 of 7
- Diff: ../pr-diffs/74640.diff
...
### github-actions on 2026-01-15T04:59:12Z

URL: https://github.com/WordPress/gutenberg/pull/74640#issuecomment-3752911364
...
### github-actions on 2026-01-15T05:09:25Z

URL: https://github.com/WordPress/gutenberg/pull/74640#issuecomment-3752928667
...
### github-actions on 2026-01-15T06:22:26Z

URL: https://github.com/WordPress/gutenberg/pull/74640#issuecomment-3753073208

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 473d5adeb9e9340aaf447c89eb2a5c8d01c8a119.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/21021386883
📝  Reported issues:
- #71399 in `/test/e2e/specs/site-editor/template-registration.spec.js`

### manzoorwanijk on 2026-01-15T18:10:36Z

URL: https://github.com/WordPress/gutenberg/pull/74640#issuecomment-3756209242

> If I understand correctly, the e2e Storybook (`storybook:e2e:dev`) will no longer work with this setup when we move to pnpm? If so, were you planning a way around it? If making it into yet another workspace seems like too much, maybe we could even consider pruning/merging it somehow into the main Storybook instance (cc @ciampo).

That’s probably correct. We can make that a workspace as well.

### ciampo on 2026-01-16T09:07:14Z

URL: https://github.com/WordPress/gutenberg/pull/74640#issuecomment-3758877189

Maybe this is the right time to implement a more robust (and potentially even used in CI) VizReg setup. I think it's going to be especially useful now that we have DS tokens. Making a change to a token could have unexpected consequences, and VizReg could really give a hand there.

We could start with dedicated Storybook stories, and Playwright + Storybook Test Runner (with Docker to minimize OS-level deifferences when taking screenshots). Or even evaluate Chromatic, in case we can stay within the free tier

## Review threads

### Review thread 1 (resolved)

#### copilot-pull-request-reviewer on 2026-01-15T05:03:34Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693009173
...
The publishConfig section is unnecessary for a private package. Since this package has "private": true, it will never be published to npm, making the publishConfig redundant. Consider removing this section to avoid confusion.
```suggestion

```

#### manzoorwanijk on 2026-01-15T05:39:55Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693063481
Path: storybook/package.json
...
Linter makes it required.

### Review thread 2 (resolved)

#### copilot-pull-request-reviewer on 2026-01-15T05:03:34Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693009183
Path: storybook/package.json
...
Moving @storybook/react-vite and other Storybook packages to the workspace package dependencies may cause TypeScript resolution issues. Many story files in packages/components, packages/dataviews, packages/theme, and packages/ui import types from '@storybook/react-vite' (e.g., 'Meta', 'StoryFn', 'StoryObj'). While npm workspace dependency hoisting should make these packages available at the root level, this creates an implicit dependency where packages rely on types from dependencies of a sibling workspace package. Consider keeping @storybook/react-vite as a devDependency in the root package.json to make this dependency explicit and ensure reliable TypeScript type resolution across all packages.

#### manzoorwanijk on 2026-01-15T05:38:13Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693060336
Path: storybook/package.json
...
That is exactly what pnpm will solve. Fixed in https://github.com/WordPress/gutenberg/pull/74640/commits/473d5adeb9e9340aaf447c89eb2a5c8d01c8a119

### Review thread 3 (resolved)

#### copilot-pull-request-reviewer on 2026-01-15T05:03:35Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693009186
Path: storybook/package.json
...
The version specifications for Storybook dependencies have changed from exact versions to caret ranges. In the root package.json, most of these dependencies used exact versions (e.g., "10.1.11"), but here they use caret ranges (e.g., "^10.1.11"). While the 'storybook' package already used a caret range in the root, the addon packages (@storybook/addon-a11y, @storybook/addon-docs, @storybook/icons, @storybook/react-vite) and storybook-addon-tag-badges used exact versions. Consider using exact versions to maintain consistency with the original specification and avoid potential issues from minor version updates.
```suggestion
		"@storybook/addon-a11y": "10.1.11",
		"@storybook/addon-docs": "10.1.11",
		"@storybook/icons": "2.0.1",
...
#### manzoorwanijk on 2026-01-15T05:37:09Z

URL: https://github.com/WordPress/gutenberg/pull/74640#discussion_r2693058614
Path: storybook/package.json
...
- package-lock.json (MODIFIED, +23/-7)
...
- storybook/package.json (ADDED, +37/-0)
