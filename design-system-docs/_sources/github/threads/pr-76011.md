...
type: pull_request
state: MERGED
merged_at: 2026-03-02T20:20:54Z
url: https://github.com/WordPress/gutenberg/pull/76011
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #76011: InputLayout: Replace slot context with data attributes

- URL: https://github.com/WordPress/gutenberg/pull/76011
- Author: mirka
- Created: 2026-02-27T14:14:55Z
- Updated: 2026-03-06T19:06:18Z
- Comments: 4 of 4
- Review threads: 1 of 1
- Changed files: 7 of 7
- Diff: ../pr-diffs/76011.diff

## Body

## What?

Replaces the React context used for communicating slot type (prefix/suffix) between `InputLayout` and `InputLayout.Slot` with a DOM-based approach using `data-slot-type` attributes.

## Why?

`InputLayout` and `InputLayout.Slot` can come from different bundled versions of the `@wordpress/ui` package (for example in DataViews, where the `InputLayout` comes from the package and `InputLayout.Slot` comes from the consumer). When that happens, the React context identity doesn't match across bundles, so `useContext` silently returns `null` and the slot styling breaks.

Data attributes work across bundle boundaries since they're just DOM — no shared JS identity needed.

## How?

- `InputLayout` now wraps `prefix`/`suffix` content in a `<div data-slot-type="prefix|suffix">` with `display: contents` so it doesn't affect flex layout.
- `InputLayout.Slot` no longer reads context. It gets its directional padding from CSS ancestor selectors (`[data-slot-type="prefix"] &` / `[data-slot-type="suffix"] &`).
- Removed the `type` prop from `InputLayout.Slot` since standalone usage outside of `InputLayout` wasn't needed.
- Deleted `context.tsx`.

## Testing Instructions

No visual changes. Existing Storybook stories for `InputLayout` should render identically (padded prefix/suffix slots).

## Issue comments

### github-actions on 2026-02-27T14:16:25Z

URL: https://github.com/WordPress/gutenberg/pull/76011#issuecomment-3973189257
...
### github-actions on 2026-02-27T14:28:10Z

URL: https://github.com/WordPress/gutenberg/pull/76011#issuecomment-3973246939
...
### mirka on 2026-03-02T19:31:11Z

URL: https://github.com/WordPress/gutenberg/pull/76011#issuecomment-3986430919

> are we implying that we want to avoid using context at all in `@wordpress/ui` because of the possibility of being used across versions?

It's not always possible to avoid context (especially when coming from Base UI), so it's not a solution for every case. Sometimes the consumer package will need to handle it through its own architecture, for example by providing child components itself so the context instance matches.

I'm mainly hoping there aren't a lot of cases where we encounter this problem. And when we do, I'm hoping they can all ultimately be solved through that _package's_ architecture, not `@wordpress/ui`.

It just so happens that in this particular prefix/suffix case, we are bound to encounter a problem in a pre-existing DataViews API, and it's relatively simple to avoid context.

### github-actions on 2026-03-02T20:03:23Z

URL: https://github.com/WordPress/gutenberg/pull/76011#issuecomment-3986620581

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 509bf3664f342ee7c6a20519e6d64ea579777101.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22592431161
📝  Reported issues:
...
#### aduth on 2026-03-02T13:57:37Z

URL: https://github.com/WordPress/gutenberg/pull/76011#discussion_r2872606978
...
- packages/ui/src/form/primitives/input-layout/context.tsx (DELETED, +0/-36)
- packages/ui/src/form/primitives/input-layout/input-layout.tsx (MODIFIED, +17/-8)
- packages/ui/src/form/primitives/input-layout/slot.tsx (MODIFIED, +1/-15)
- packages/ui/src/form/primitives/input-layout/style.module.css (MODIFIED, +6/-2)
- packages/ui/src/form/primitives/input-layout/test/index.test.tsx (MODIFIED, +34/-1)
- packages/ui/src/form/primitives/input-layout/types.ts (MODIFIED, +1/-10)
