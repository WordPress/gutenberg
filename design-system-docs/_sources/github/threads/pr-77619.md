...
type: pull_request
state: MERGED
merged_at: 2026-04-24T13:55:16Z
url: https://github.com/WordPress/gutenberg/pull/77619
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components","[Package] Base styles"]
---

# PR #77619: FormToggle: Inline z-index

- URL: https://github.com/WordPress/gutenberg/pull/77619
- Author: mirka
- Created: 2026-04-23T15:52:59Z
- Updated: 2026-04-24T13:56:03Z
- Comments: 4 of 4
- Review threads: 4 of 4
- Changed files: 4 of 4
- Diff: ../pr-diffs/77619.diff

## Body

## What?

Removes `.components-form-toggle__input` from the shared `$z-layers` map in `packages/base-styles/_z-index.scss` and sets `z-index: 1` directly on the Form Toggle input in `packages/components/src/form-toggle/style.scss`.

Also adds an `isolation: isolate` to the component root.

## Why?

The Form Toggle input only ever needed a local stacking value of `1` above the visual thumb. Keeping it in the global z-index map added indirection without coordinating with other layers, so this inlines the value and drops the unused registry entry.

## Testing Instructions

1. Run Storybook for `@wordpress/components` (or open the app build that includes Form Toggle).
2. Open the Form Toggle story or any screen that uses toggles (e.g. block or editor settings).
3. Confirm the toggle still looks correct, the thumb sits under the invisible input for hit testing, and focus/hover states behave as before.
4. Optionally inspect the input in dev tools and confirm `z-index: 1` is applied.

## Issue comments

### github-actions on 2026-04-23T15:55:35Z

URL: https://github.com/WordPress/gutenberg/pull/77619#issuecomment-4305885287
...
### github-actions on 2026-04-23T16:08:14Z

URL: https://github.com/WordPress/gutenberg/pull/77619#issuecomment-4305971437
...
| :--- | :---: | :---: |
| `build/styles/components/style-rtl.css` | 17.3 kB | +10 B (+0.06%) |
| `build/styles/components/style-rtl.min.css` | 14.1 kB | +10 B (+0.07%) |
| `build/styles/components/style.css` | 17.4 kB | +10 B (+0.06%) |
| `build/styles/components/style.min.css` | 14.1 kB | +9 B (+0.06%) |
...
### github-actions on 2026-04-23T18:32:24Z

URL: https://github.com/WordPress/gutenberg/pull/77619#issuecomment-4306844330

<!-- flaky-tests-report-comment -->
**Flaky tests detected in a6db2c6c63d7664e9b4af2b2bd9e4f86f75c5e37.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24891337377
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### mirka on 2026-04-24T13:12:07Z

URL: https://github.com/WordPress/gutenberg/pull/77619#issuecomment-4313453213

> Also, for posterity, let's update the PR description mentioning `isolation: isolate`, pending [this convo](https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137291523)

Done 👍

> Finally, for this PR and all realted z-index PRs, should we have a parent tracking issue to track progress and to leave a dev note for future WP release?

No tracking issue because I can't easily pre-populate a todo list without actually looking into the instances. And similar to https://github.com/WordPress/gutenberg/pull/77621#discussion_r3137713690, a dev note is not really appropriate because these will generally only be relevant to `@wordpress/base-styles` consumers, and are thus not tied to WP releases. The changelog will collect all the changes, so I guess we could use that as a data source if we need to keep track of relevant work that was done.

## Review threads

### Review thread 1 (resolved)

#### ciampo on 2026-04-24T10:57:48Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137224056
...
#### ciampo on 2026-04-24T10:59:15Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137231279
...
#### mirka on 2026-04-24T13:01:23Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137848277
Path: packages/components/CHANGELOG.md
...
This can only possibly break on the `@wordpress/base-styles` side, see https://github.com/WordPress/gutenberg/pull/77621#discussion_r3137713690.

### Review thread 3 (resolved)

#### ciampo on 2026-04-24T10:59:51Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137234288
Path: packages/components/src/form-toggle/style.scss

```diff
@@ -151,7 +152,7 @@ $transition-duration: 0.2s;
 	opacity: 0;
 	margin: 0;
 	padding: 0;
...
#### ciampo on 2026-04-24T11:11:59Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137291523
Path: packages/components/src/form-toggle/style.scss

```diff
@@ -14,6 +14,7 @@ $transition-duration: 0.2s;
...
#### mirka on 2026-04-24T13:06:57Z

URL: https://github.com/WordPress/gutenberg/pull/77619#discussion_r3137877618
Path: packages/components/src/form-toggle/style.scss
