...
type: pull_request
state: MERGED
merged_at: 2025-11-11T18:22:21Z
url: https://github.com/WordPress/gutenberg/pull/72890
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Performance","[Package] Theme"]
---

# PR #72890: Theme: Inline values for primitive tokens in CSS output

- URL: https://github.com/WordPress/gutenberg/pull/72890
- Author: aduth
- Created: 2025-10-31T19:17:06Z
- Updated: 2025-11-11T18:23:07Z
- Comments: 6 of 6
- Review threads: 2 of 2
- Changed files: 13 of 13
- Diff: ../pr-diffs/72890.diff
...
* Reduce size of compiled stylesheet (see `packages/theme/src/prebuilt/css/design-tokens.css`)
* Makes primitive token values effectively private rather than relying on linting or naming conventions as a hopeful deterrent
* Changes should result in a slightly more performant `useThemeProviderStyles`
* This could enable further performance enhancements by allowing us to generate only the specific color ramp values that are actively in use
...
### github-actions on 2025-10-31T19:17:23Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3474583647
...
### github-actions on 2025-10-31T19:27:49Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3474610053

**Size Change:** -8.5 kB (-0.35%) 

**Total Size:** 2.41 MB

| Filename | Size | Change |  |
| :--- | :---: | :---: | :---: |
| `build/modules/boot/index.min.js` | 76.4 kB | -7.8 kB (-9.27%) | ✅ |
| `build/scripts/theme/index.min.js` | 20.8 kB | -694 B (-3.23%) |  |
...
### github-actions on 2025-10-31T19:44:57Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3474667378

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 9b888e1d6fe3cd1da551c3af4bf2a23aac53d200.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/19270841406
📝  Reported issues:
- #51533 in `/test/e2e/specs/editor/various/list-view.spec.js`

### aduth on 2025-11-06T20:17:58Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3499235266

@jsnajdr With what you're exploring in https://github.com/WordPress/gutenberg/pull/73004#issuecomment-3496422364 about limiting which colors we generate, I wonder if anything here could potentially lend to similar optimizations? What I like about this approach is that the [`color-tokens.ts`](https://github.com/WordPress/gutenberg/blob/9128c16ee0b94412bd55ba9b8216bac75009f97e/packages/theme/src/prebuilt/ts/color-tokens.ts) tells us exactly which ramp colors we need.

### jsnajdr on 2025-11-07T12:20:47Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3502253269

> limiting which colors we generate, I wonder if anything here could potentially lend to similar optimizations?

Every color generated in a ramp is used for two purposes:
- to be actually used by some color token. If it's not used, it didn't need to be generated. That's the obvious part.
- if it failed to meet a contrast requirement, it will trigger a seed adjustment. Even if it's an unused color at the moment, it still signals that there is something wrong with the seed color.

Should even an unused color trigger seed adjustments? There are two possible answers:
- yes, because we want the seed color to be long-term stable. We don't want it to suddenly change when some new color token starts using a yet-unused ramp color.
- no, because even with unadjusted seed, all contrast requirements of the colors that are actually used are met. There is no UI on the screen that could possibly have insufficient contrast.

### aduth on 2025-11-11T15:44:00Z

URL: https://github.com/WordPress/gutenberg/pull/72890#issuecomment-3517524004

I rebased after #72984 and with 9b888e1d6f added additional coverage for the private / semantic _dimension_ tokens added in #72984.

## Review threads

### Review thread 1 (resolved)

#### aduth on 2025-11-06T20:15:46Z

URL: https://github.com/WordPress/gutenberg/pull/72890#discussion_r2500688499
...
We'll want to think of a way to generalize this a bit more. In #72984, I'm adding some new primitive & semantic tokens for spacing / dimension values. Since this is targeted at color tokens specifically, that would mean the private CSS properties for dimensions would still be output. 

However, unlike color, we might not need to emit the same kind of mapping like what we're doing with `ts/color-tokens.ts`, since we're not computing anything dynamically at runtime. We just need to inline the values.

#### aduth on 2025-11-11T18:21:41Z

URL: https://github.com/WordPress/gutenberg/pull/72890#discussion_r2515207713
Path: packages/theme/terrazzo.config.ts
...
Resolved with 9b888e1d6fe3cd1da551c3af4bf2a23aac53d200

### Review thread 2 (resolved)

#### aduth on 2025-11-10T21:10:09Z

URL: https://github.com/WordPress/gutenberg/pull/72890#discussion_r2511979437
Path: packages/theme/bin/terrazzo-plugin-ds-tokens-docs/index.ts

```diff
@@ -47,23 +42,16 @@ export default function pluginDsTokenDocs( {
 						?.split( '.json' )[ 0 ] ?? 'Miscellaneous';
 
 				// Organize tokens in semantic/private, and group by category.
```

The added code comment below is meant to replace this.
...
- packages/theme/bin/terrazzo-plugin-ds-tokens-docs/index.ts (MODIFIED, +5/-24)
- packages/theme/bin/terrazzo-plugin-inline-alias-values/index.ts (ADDED, +84/-0)
- packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts (MODIFIED, +19/-39)
- packages/theme/docs/ds-tokens.md (MODIFIED, +0/-178)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +88/-428)
- packages/theme/src/prebuilt/json/figma.json (MODIFIED, +88/-918)
- packages/theme/src/prebuilt/ts/color-tokens.ts (ADDED, +117/-0)
- packages/theme/src/prebuilt/ts/design-tokens.ts (DELETED, +0/-356)
- packages/theme/src/stories/index.story.tsx (MODIFIED, +4/-18)
- packages/theme/src/test/token-id.test.ts (ADDED, +12/-0)
- packages/theme/src/token-id.ts (ADDED, +9/-0)
- packages/theme/src/use-theme-provider-styles.ts (MODIFIED, +20/-35)
- packages/theme/terrazzo.config.ts (MODIFIED, +14/-11)
