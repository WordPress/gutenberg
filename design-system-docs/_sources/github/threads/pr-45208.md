...
type: pull_request
state: MERGED
merged_at: 2022-10-25T10:55:44Z
url: https://github.com/WordPress/gutenberg/pull/45208
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #45208: Components: Improve `BorderBoxControl` tests

- URL: https://github.com/WordPress/gutenberg/pull/45208
- Author: tyxla
- Created: 2022-10-21T14:16:11Z
- Updated: 2022-10-25T10:56:08Z
- Comments: 1 of 1
- Review threads: 2 of 2
- Changed files: 1 of 1
- Diff: ../pr-diffs/45208.diff

## Body

## What?
This PR improves the `BorderBoxControl` component tests.
...
* Use `@testing-library/user-event` instead of `fireEvent` - primary motivation for this PR.
* No longer using `render()` in separate helper functions, rather run them inline.
* No longer using `screen` queries in separate helper functions either.

## Testing Instructions
Verify tests still pass: `npm run test:unit packages/components/src/border-box-control/test/index.js`

## Issue comments

### github-actions on 2022-10-21T14:27:42Z

URL: https://github.com/WordPress/gutenberg/pull/45208#issuecomment-1287044408
...
#### ciampo on 2022-10-24T10:41:57Z

URL: https://github.com/WordPress/gutenberg/pull/45208#discussion_r1003160879
Path: packages/components/src/border-box-control/test/index.js

```diff
@@ -241,35 +291,63 @@ describe( 'BorderBoxControl', () => {
 		} );
...
It would even better if the `spinbutton` had a `name` that we could use in the query (like we do for other buttons).

This change would apply for all of the `screen.getByRole( 'spinbutton' )` queries throughout the file

#### tyxla on 2022-10-25T09:09:02Z

URL: https://github.com/WordPress/gutenberg/pull/45208#discussion_r1004212006
Path: packages/components/src/border-box-control/test/index.js
...
Thanks, that's a good call! I've addressed that in https://github.com/WordPress/gutenberg/pull/45208/commits/21ad8ad92cfa8f9dd9e3af5bed7e851631b757b0 and actually did it for a few more of the elements we're querying. Looks much better now! 🙌

### Review thread 2

#### ciampo on 2022-10-24T10:47:44Z

URL: https://github.com/WordPress/gutenberg/pull/45208#discussion_r1003171693
Path: packages/components/src/border-box-control/test/index.js

```diff
@@ -210,27 +235,52 @@ describe( 'BorderBoxControl', () => {
...
It looks like this test is timing out — could it be somehow related to the refactor from `forEach` to a linear set of instructions?

#### tyxla on 2022-10-25T09:08:30Z

URL: https://github.com/WordPress/gutenberg/pull/45208#discussion_r1004211413
Path: packages/components/src/border-box-control/test/index.js
...
I think that's the reason - the user-event clicks are much more complicated than just simulating a single event. And because there's quite a few here, plus quite a few assertions between every click pair, this could take longer on some setups. For example, I never got to fail it locally. I've increased the timeout in https://github.com/WordPress/gutenberg/pull/45208/commits/4e6c822da787a3327c1867b83335704e6e5f266d to fix this, but since we're working with fake timers, it doesn't make the test slower, so we're good.

## Changed files

- packages/components/src/border-box-control/test/index.js (MODIFIED, +287/-122)
