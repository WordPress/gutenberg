...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/78304
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components","[Package] Block editor"]
---

# PR #78304: Autocomplete: Skip work for unselected blocks via isEnabled flag

- URL: https://github.com/WordPress/gutenberg/pull/78304
- Author: ellatrix
- Created: 2026-05-14T15:51:46Z
- Updated: 2026-05-18T14:43:11Z
- Comments: 4 of 4
- Review threads: 3 of 3
- Changed files: 4 of 4
- Diff: ../pr-diffs/78304.diff

## Body

## What?
Add an `isEnabled` option to `useAutocomplete` / `useAutocompleteProps` (and the block-editor wrappers) that short-circuits the autocomplete state machine, match scanning, and keydown listener attachment. `RichText` passes `isEnabled: isSelected` so only the focused block runs the full machinery.

## Why?
`useBlockEditorAutocompleteProps` runs on every render for every block, but autocomplete can only trigger from typing in the *selected* block. The trace shows ~30 ms self time during the big React commit split across 1000+ RichText instances; ~10 ms of that is genuinely conditional (after #78303's getter-export fix lands, which covers the rest).

## How?
- Add `isEnabled?: boolean` (defaults `true`) to `UseAutocompleteProps`.
- When `false`: the `textContent` memo returns `''`, the match `useEffect` bails, the keydown `useRefEffect` skips `addEventListener`, and `useCompleters` returns `EMPTY_ARRAY`.
- The hook still returns a stable `ref` so callers attach it unconditionally — the keydown listener wires up naturally on the next render once `isEnabled` flips to `true`.
- Block-editor's `RichText` passes `isEnabled: isSelected`.

## Testing
1. `npm run test:unit -- packages/components/src/autocomplete packages/block-editor/src/components/autocomplete` (37 tests pass).
2. In a post, type `/` in an empty paragraph — block inserter completer should appear as before.
3. Type `@` in a post that has `@mentions`-style completers wired — should work as before.
4. Click between blocks; confirm autocomplete activates only in the focused block.
...
### github-actions on 2026-05-14T15:52:15Z

URL: https://github.com/WordPress/gutenberg/pull/78304#issuecomment-4452296072
...
### github-actions on 2026-05-14T16:00:43Z

URL: https://github.com/WordPress/gutenberg/pull/78304#issuecomment-4452359245
...
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 345 kB | +20 B (+0.01%) |
| `build/scripts/components/index.min.js` | 266 kB | +40 B (+0.02%) |
...
### ellatrix on 2026-05-18T14:42:41Z

URL: https://github.com/WordPress/gutenberg/pull/78304#issuecomment-4478796349

Sorry, I don't know why Claude opened this behind my back, I never asked for this 😂

### ellatrix on 2026-05-18T14:43:02Z

URL: https://github.com/WordPress/gutenberg/pull/78304#issuecomment-4478799047
...
#### ciampo on 2026-05-15T08:08:03Z

URL: https://github.com/WordPress/gutenberg/pull/78304#discussion_r3246795496
...
#### Mamaduka on 2026-05-15T09:27:11Z

URL: https://github.com/WordPress/gutenberg/pull/78304#discussion_r3247202252
Path: packages/components/src/autocomplete/index.tsx
...
#### tyxla on 2026-05-17T21:07:18Z

URL: https://github.com/WordPress/gutenberg/pull/78304#discussion_r3255483199
Path: packages/components/src/autocomplete/index.tsx
...
#### ciampo on 2026-05-15T08:14:08Z

URL: https://github.com/WordPress/gutenberg/pull/78304#discussion_r3246818061
Path: packages/components/src/autocomplete/index.tsx
...
#### tyxla on 2026-05-17T21:01:23Z

URL: https://github.com/WordPress/gutenberg/pull/78304#discussion_r3255476334
...
- packages/block-editor/src/components/autocomplete/index.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/rich-text/index.js (MODIFIED, +1/-0)
- packages/components/src/autocomplete/index.tsx (MODIFIED, +26/-12)
- packages/components/src/autocomplete/types.ts (MODIFIED, +7/-0)
