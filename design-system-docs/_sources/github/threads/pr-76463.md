...
type: pull_request
state: MERGED
merged_at: 2026-03-17T17:09:27Z
url: https://github.com/WordPress/gutenberg/pull/76463
matched_labels: ["Design System"]
all_labels: ["[Type] Enhancement","Storybook","Design System"]
---

# PR #76463: Add ExampleApplication story for ThemeProvider to better demonstrate component theme-ability

- URL: https://github.com/WordPress/gutenberg/pull/76463
- Author: jameskoster
- Created: 2026-03-12T17:21:14Z
- Updated: 2026-03-18T13:36:39Z
- Comments: 4 of 4
- Review threads: 4 of 4
- Changed files: 2 of 2
- Diff: ../pr-diffs/76463.diff

## Body

https://github.com/user-attachments/assets/04d82b39-e1a3-4afa-8673-3e8b2dbe9b17

## What

Adds a new `ExampleApplication` Storybook story for `ThemeProvider` that renders a realistic mock application page, demonstrating how multiple `@wordpress/ui` components respond to theme changes together.

## Why

The existing `ThemeProvider` stories demonstrate token values and individual behaviors, but don't show how theming works in a real-world application context. Having a cohesive example makes it much easier to evaluate the effect of changing `primary` color, `bg` color, and `density` across a full UI -- buttons, cards, tabs, links, badges, and notices all responding in concert.

## How

- Added an `ExampleApplication` story that renders a two-column layout with a sidebar navigation and a main content area containing a header and settings cards.
- Uses `@wordpress/ui` components exclusively: `Button`, `Card`, `Badge`, `Tabs`, `Link`, `Notice`, `Stack`, and `Text`.
- All colors reference design tokens (`--wpds-color-*`), so every surface and text element adapts when `primary` or `bg` seed colors are changed via Storybook controls.
- Storybook controls are limited to `primary`, `bg`, and `density` to keep the demo focused.
- Added `@wordpress/ui` as a `devDependency` in `packages/theme/package.json` to support the import.

### Testing

1. Run `npm run storybook:dev -w storybook`
2. Navigate to **Theme / ThemeProvider / Example Application**
3. Use the `primary` color picker -- the brand button and links should update
4. Use the `bg` color picker -- all surface backgrounds should update
5. Toggle `density` between default, compact, and comfortable -- spacing should adjust throughout

## Issue comments

### github-actions on 2026-03-12T17:52:39Z

URL: https://github.com/WordPress/gutenberg/pull/76463#issuecomment-4048777242

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 1a220b999cb60c7bcba5d125139abf35ecb67d82.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23195216329
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### github-actions on 2026-03-13T16:46:12Z

URL: https://github.com/WordPress/gutenberg/pull/76463#issuecomment-4056499385
...
### github-actions on 2026-03-13T20:57:16Z

URL: https://github.com/WordPress/gutenberg/pull/76463#issuecomment-4057944807
...
| `build/modules/connectors/index.min.js` | 1.97 kB |
...
| `build/scripts/core-data/index.min.js` | 29.7 kB |
...
| `build/scripts/media-utils/index.min.js` | 75.1 kB |
...
| `build/scripts/sync/index.min.js` | 37.5 kB |
...
| `build/styles/editor/style-rtl.css` | 26.3 kB |
...
### mirka on 2026-03-17T16:41:47Z

URL: https://github.com/WordPress/gutenberg/pull/76463#issuecomment-4076389296
...
#### mirka on 2026-03-13T20:46:01Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2933689568
Path: .eslintrc.js

```diff
@@ -561,5 +561,11 @@ module.exports = {
...
This fixes the lint violations on CI.

### Review thread 2

#### mirka on 2026-03-13T21:04:04Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2933754758
...
- Demonstrate it using the standard `ThemeProvider` controls like the other `ThemeProvider` stories, but with `children` and `isRoot` hidden. The downside is that the `color` control is an object, and hard to play with.
- Move this story somewhere outside of the "Theme Provider" section, and demonstrate it using the Theming tool, instead of the Controls. Maybe add more color options to the tool, if that's better.
    ...
    <img width="300" height="330" alt="Theming tool" src="https://github.com/user-attachments/assets/02ae506e-6ebb-47b1-a7e7-100de9c3036e" />
...
#### jameskoster on 2026-03-16T14:53:44Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2940899450
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
#### ciampo on 2026-03-16T16:11:05Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2941416733
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
I agree with both 😅 in the [dedicated tokens story](https://wordpress.github.io/gutenberg/?path=/story/design-system-theme-theme-provider--with-picker), I opted for a picker directly in the code — although I'm not sure it's better than custom controls?

#### jameskoster on 2026-03-17T12:56:13Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2946583670
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
I updated to follow the With Picker story example. Do y'all think this is better? 

<img width="1414" height="871" alt="Screenshot 2026-03-17 at 12 55 34" src="https://github.com/user-attachments/assets/9cebf11e-2106-404c-aa17-eabf4d559bfb" />

#### ciampo on 2026-03-17T14:26:51Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2947177005
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
#### mirka on 2026-03-13T21:05:35Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2933760382
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
We might want some kind of description for this story. A JSDoc like this should work:

```suggestion
/**
 * An entire app can be themed. In this example…
 */
export const ExampleApplication: StoryObj< typeof ThemeProvider > = {
```
...
#### mirka on 2026-03-13T21:09:54Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2933775624
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
#### mirka on 2026-03-13T21:26:14Z

URL: https://github.com/WordPress/gutenberg/pull/76463#discussion_r2933843743
Path: storybook/stories/design-system/theme-example-application.story.tsx
...
- storybook/stories/design-system/theme-example-application.story.tsx (ADDED, +401/-0)
