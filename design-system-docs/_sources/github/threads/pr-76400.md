...
type: pull_request
state: MERGED
merged_at: 2026-03-12T20:10:17Z
url: https://github.com/WordPress/gutenberg/pull/76400
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components","Backported to WP Core"]
---

# PR #76400: TimePicker: Clamp month day to valid day for month

- URL: https://github.com/WordPress/gutenberg/pull/76400
- Author: aduth
- Created: 2026-03-11T14:59:35Z
- Updated: 2026-03-12T20:11:41Z
- Comments: 5 of 5
- Review threads: 3 of 3
- Changed files: 5 of 5
- Diff: ../pr-diffs/76400.diff
...
### github-actions on 2026-03-11T14:59:45Z

URL: https://github.com/WordPress/gutenberg/pull/76400#issuecomment-4039833564
...
Co-authored-by: anastis <anastis@git.wordpress.org>
```
...
### github-actions on 2026-03-11T15:19:12Z

URL: https://github.com/WordPress/gutenberg/pull/76400#issuecomment-4039969787
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 265 kB | +89 B (+0.03%) |
...
### github-actions on 2026-03-11T15:30:31Z

URL: https://github.com/WordPress/gutenberg/pull/76400#issuecomment-4040052971

<!-- flaky-tests-report-comment -->
**Flaky tests detected in cc98ff265d94faa7817252b20dff45043cc33c73.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22968072735
📝  Reported issues:
...
### aduth on 2026-03-11T18:30:13Z

URL: https://github.com/WordPress/gutenberg/pull/76400#issuecomment-4041262774

I found a different way to address the input value desync without needing #76399 (that pull request is still worth exploring, but has gnarly conflicts). Instead of statically assigning the `max` value of day as `31`, we can dynamically assign it based on the selected month. This also improves the user experience because it prevents the user from using the arrow keys to increment the value beyond a valid day for the month. In WordPress 6.9, a user can press <kbd>ArrowUp</kbd> from 28 February 2026 and it bizarrely switches to 29 March 2026. In this branch, pressing <kbd>ArrowUp</kbd> from 28 February 2026 does nothing.

Before (WordPress 6.9):

https://github.com/user-attachments/assets/75e99339-c765-48b7-a24d-d4d96734ffee 

After (this branch): 

https://github.com/user-attachments/assets/ee677421-811b-4d03-be87-685bef146014

### github-actions on 2026-03-12T20:11:41Z

URL: https://github.com/WordPress/gutenberg/pull/76400#issuecomment-4049706111

I just cherry-picked this PR to the wp/7.0 branch to get it included in the next release: 4fda7e74c5848136d0b369c8e545fc872ce25edd

## Review threads

### Review thread 1 (resolved)

#### mirka on 2026-03-12T12:58:46Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2924461837
...
Any reason why we're doing a `- 1` to the month in `values` but then adding back the one at both places they're used?

#### aduth on 2026-03-12T15:52:37Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2925651873
Path: packages/components/src/date-time/utils.ts
...
Hah, that's a good call-out. I imagine in some earlier iteration it was necessary because `formatDate( 'n' )` is month as 1-based ([source](https://www.php.net/manual/en/datetime.format.php)) but Date constructors expect month as 0-based ([source](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/Date#monthindex)), but that's just not how we use it anymore, so it does seem unnecessary.

#### aduth on 2026-03-12T16:37:14Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2925924900
Path: packages/components/src/date-time/utils.ts
...
Actually, I think it needs to be this way specifically because of the `updates` argument, where the argument is passed as a `0`-indexed month. We could probably update the call sites to make it 1-indexed, but I think operating in the JavaScript space it's more natural to align this around the `Date` conventions.

### Review thread 2 (resolved)

#### mirka on 2026-03-12T13:00:39Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2924471536
...
Would it be worth extracting a utility for this? It also appears in `setInConfiguredTimezone`.

#### aduth on 2026-03-12T15:53:40Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2925658142
Path: packages/components/src/date-time/time/index.tsx
...
Yeah, I think that's a good idea, also in how it would help provide some self-documentation by function name and allow for some more granular unit testing.

#### aduth on 2026-03-12T16:51:05Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2926009246
Path: packages/components/src/date-time/time/index.tsx
...
Extracted `getDaysInMonth` utility in 91d52e969b17ccd2da782881eb891965f85eeb66.

### Review thread 3 (resolved)

#### mirka on 2026-03-12T13:06:48Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2924504156
Path: packages/components/src/date-time/time/test/index.tsx

Could be worth adding a test for the leap year → non-leap year transition (e.g. starting on `2024-02-29`, then changing the year to 2025). The clamping in `setInConfiguredTimezone` handles it correctly, but it's a non-obvious edge case that would be nice to have guarded against future regressions.

#### aduth on 2026-03-12T16:51:19Z

URL: https://github.com/WordPress/gutenberg/pull/76400#discussion_r2926010771
Path: packages/components/src/date-time/time/test/index.tsx

Good idea! Added test case in 9137de28140b843001b213ff02e2feac08973548.

## Changed files

- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/date-time/test/utils.test.ts (MODIFIED, +8/-11)
- packages/components/src/date-time/time/index.tsx (MODIFIED, +2/-12)
- packages/components/src/date-time/time/test/index.tsx (MODIFIED, +69/-0)
- packages/components/src/date-time/utils.ts (MODIFIED, +18/-0)
