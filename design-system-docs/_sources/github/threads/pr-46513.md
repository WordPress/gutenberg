...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/46513
matched_labels: ["[Package] Components"]
all_labels: ["Needs Design Feedback","[Package] Components","[Feature] Colors"]
---

# PR #46513: Improve consistency across color and gradient panels

- URL: https://github.com/WordPress/gutenberg/pull/46513
- Author: richtabor
- Created: 2022-12-13T16:37:25Z
- Updated: 2025-02-10T16:45:26Z
- Comments: 2 of 2
- Review threads: 0 of 0
- Changed files: 3 of 3
- Diff: ../pr-diffs/46513.diff
...
### github-actions on 2022-12-13T16:50:55Z

URL: https://github.com/WordPress/gutenberg/pull/46513#issuecomment-1349049986
...
| :--- | :---: | :---: |
| `build/components/index.min.js` | 204 kB | -1 B (0%) |
| `build/components/style-rtl.css` | 11.7 kB | -2 B (0%) |
| `build/components/style.css` | 11.7 kB | -1 B (0%) |
...
| `build/edit-site/index.min.js` | 63.5 kB |
...
### jasmussen on 2022-12-14T09:01:59Z

URL: https://github.com/WordPress/gutenberg/pull/46513#issuecomment-1350672948

Good one, thank you for the PR, definitely want to make progress on these.

On the one hand, the custom color swatch was designed to be a specific height to add some character and not just be a rectangle. On the other hand, if the gradient box becomes taller, the padding around the left/right handles needs to increase to match this particular design where the gradient actually starts a bit indented from the edges. Tricky to reconcile those two easily.

Similarly, we're hoping to implement [a standard 40px controls grid](https://github.com/WordPress/gutenberg/issues/43423#issuecomment-1232540038) across the board. Just looking at that, the 48px size of the gradient control oddly stands out as not quite taller enough than 40px that it might as well _be_ 40px, or taller, such as 60px. Both tricky in their own right.

Another aspect is the borders: in the case of the swatches and colors, they are arguably needed, especially when the color _white_ is involved. But the border around the gradient box was specifically omitted because there are so many borders everywhere already. 

All of that is not to block the specific improvements here from landing, just that if @pablohoneyhoney has a moment to chime in, I'd love input. Otherwise, let's find a way forward!

Speaking of progress, it would be nice to revisit the gradient markers too — their style and even focus could use some love. I like the new focal point picker, perhaps some DNA can be shared.
...
- packages/components/src/custom-gradient-picker/style.scss (MODIFIED, +1/-0)
