...
type: pull_request
state: MERGED
merged_at: 2026-04-17T14:21:06Z
url: https://github.com/WordPress/gutenberg/pull/77420
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77420: Link: Remove underline from unstyled icon links

- URL: https://github.com/WordPress/gutenberg/pull/77420
- Author: mirka
- Created: 2026-04-16T10:28:00Z
- Updated: 2026-04-17T14:21:48Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 4 of 4
- Diff: ../pr-diffs/77420.diff

## Body

## What?

Prevent the `unstyled` `Link` variant from showing an underline when `openInNewTab` is enabled.

## Why?

The previous `openInNewTab` markup wrapped the label in a `.link-contents` span that re-applied underline styles. That was correct for default and neutral links, but it forced underline onto the label even when the root anchor was `unstyled`, which should not apply link chrome.

## How?

- Stop wrapping `openInNewTab` content in `.link-contents` and drop the `.has-link-icon` class on the anchor. Children are always a fragment: link text, then the optional new-tab icon.
- Style `.link-icon` as `inline-block` and `text-decoration: none` so the underline stays on the text for styled variants, and the decorative arrow does not pick up decoration. The `unstyled` variant keeps `text-decoration: none` on the anchor as before.
- Tests: assert the new-tab affordance stays accessible (`toBeVisible`, link accessible name includes the “opens in a new tab” phrase).

## Testing Instructions
1. Render a `Link` with `variant="unstyled"` and `openInNewTab` set to `true`.
2. Confirm the link text is not underlined.
3. Render a non-unstyled `Link` with `openInNewTab` set to `true`.
4. Confirm the link text is still underlined there.

## Screenshots

| Before | After |
|--------|-------|
|<img width="228" height="84" alt="Unstyled link, before" src="https://github.com/user-attachments/assets/f398cb03-c8ec-4258-b2a7-3c7877af3809" />|<img width="228" height="84" alt="Unstyled link, after" src="https://github.com/user-attachments/assets/b84783e6-0a9b-45c7-84de-c02d169d0efd" />|

## Issue comments

### github-actions on 2026-04-16T10:31:42Z

URL: https://github.com/WordPress/gutenberg/pull/77420#issuecomment-4259321070
...
### github-actions on 2026-04-16T10:41:20Z

URL: https://github.com/WordPress/gutenberg/pull/77420#issuecomment-4259375345
...
### github-actions on 2026-04-16T10:59:31Z

URL: https://github.com/WordPress/gutenberg/pull/77420#issuecomment-4259482774

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 8e5804a258a7b43f6a02b99403d293f13f1824cf.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24561162565
📝  Reported issues:
...
### aduth on 2026-04-16T14:36:21Z

URL: https://github.com/WordPress/gutenberg/pull/77420#issuecomment-4260912377

I'm sure there were reasons, but the way we've implemented the component so that it has a different element structure applying the underline depending on `opensInNewTab` seems prone to cause the exact types of maintenance headache we're dealing with here.

Revisiting that implementation, I expect it was done the way it was to avoid the underline extending to the pseudo-element, and the screen reader reading pseudo-element content? Based on some [light investigation](https://stackoverflow.com/a/20405045) and [testing](https://codepen.io/editor/aduth/pen/019d96ad-8185-7372-9e2c-ee665400b9dc), maybe we can address that with a combination of `display: inline-block;` and `text-underline: none;` on the added element, keeping the link contents and its styles as directly on the link element itself?

### mirka on 2026-04-17T10:46:02Z

URL: https://github.com/WordPress/gutenberg/pull/77420#issuecomment-4267354229
...
- packages/ui/src/link/link.tsx (MODIFIED, +12/-15)
- packages/ui/src/link/style.module.css (MODIFIED, +4/-16)
- packages/ui/src/link/test/index.test.tsx (MODIFIED, +27/-1)
