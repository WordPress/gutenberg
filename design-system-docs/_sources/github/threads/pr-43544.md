...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/43544
matched_labels: ["[Package] Components"]
all_labels: ["[Feature] Blocks","[Feature] UI Components","[Type] Regression","[Package] Components"]
---

# PR #43544: Fix block settings dropdown positioning when scrolling in the site editor

- URL: https://github.com/WordPress/gutenberg/pull/43544
- Author: talldan
- Created: 2022-08-24T04:29:14Z
- Updated: 2022-09-08T05:23:24Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 3 of 3
- Diff: ../pr-diffs/43544.diff
...
### github-actions on 2022-08-24T04:37:42Z

URL: https://github.com/WordPress/gutenberg/pull/43544#issuecomment-1225180937
...
| :--- | :---: | :---: |
| `build/block-library/index.min.js` | 186 kB | +33 B (0%) |
| `build/components/index.min.js` | 198 kB | +93 B (0%) |
| `build/core-data/index.min.js` | 15.5 kB | +65 B (0%) |
...
### ciampo on 2022-08-24T08:39:33Z

URL: https://github.com/WordPress/gutenberg/pull/43544#issuecomment-1225381515

This is unexpected — I thought that, for the "second gen" popover, all that mattered was the position of its anchor (and therefore it would be unaffected by what happens in the `iframe`).

Also, if the anchor and the popover are within the same document (which is the case for the settings dropdown), I also expected that `floating-ui` would be able to track its position correctly?

### talldan on 2022-08-24T08:59:36Z

URL: https://github.com/WordPress/gutenberg/pull/43544#issuecomment-1225436528
...
### ciampo on 2022-08-24T09:32:41Z

URL: https://github.com/WordPress/gutenberg/pull/43544#issuecomment-1225471925

I gave this PR a spin and noticed that:

- the "second gen" dropdown behaves differently depending on whether the toolbar is flipped or not
- the "second gen" dropdown doesn't resize when meeting the edge of the viewport in the site editor (while it does resize in the post editor) (could be fixed by https://github.com/WordPress/gutenberg/pull/43546 maybe?)
- when the "second gen" dropdown scroll past the bottom edge of the viewport, it causes the sidebar to show a scrollbar

https://user-images.githubusercontent.com/1083581/186383109-a496f1dc-7244-4f4f-ae86-bbc95cc0f192.mp4

> This is unexpected — I thought that, for the "second gen" popover, all that mattered was the position of its anchor (and therefore it would be unaffected by what happens in the `iframe`).
> 
> Also, if the anchor and the popover are within the same document (which is the case for the settings dropdown), I also expected that `floating-ui` would be able to track its position correctly?

I was hoping that some of these issues could be fixed by the different approach to `update/autoUpdate` that I've been taking in https://github.com/WordPress/gutenberg/pull/43335, but from a quick tests it didn't make a difference 😞 

> Ah, I imagine it's a slot/fill thing. 🤔

Maybe we need to tinker with the `bubblesVirtually` prop? (Maybe @aaronrobertshaw could be of help here, since he dealt with `Slot/Fill` when working on `ToolsPanel`)

### ciampo on 2022-08-29T08:18:58Z

URL: https://github.com/WordPress/gutenberg/pull/43544#issuecomment-1229937683

With https://github.com/WordPress/gutenberg/pull/43617 merged, I believe we can close this PR.

Feel free to re-open in case I was mistaken!
...
- packages/components/src/popover/index.js (MODIFIED, +98/-60)
- packages/components/src/popover/root-reference-document-context.js (ADDED, +10/-0)
