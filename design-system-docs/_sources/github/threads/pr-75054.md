...
type: pull_request
state: MERGED
merged_at: 2026-02-03T15:36:12Z
url: https://github.com/WordPress/gutenberg/pull/75054
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Package] DataViews","[Package] Theme"]
---

# PR #75054: Theme: Update dimension tokens

- URL: https://github.com/WordPress/gutenberg/pull/75054
- Author: jameskoster
- Created: 2026-01-29T13:59:20Z
- Updated: 2026-02-03T15:40:30Z
- Comments: 6 of 6
- Review threads: 2 of 2
- Changed files: 58 of 58
- Diff: ../pr-diffs/75054.diff
...
This PR refactors the dimension tokens in `@wordpress/theme`:

1. **Removes the `surface` segment from padding tokens** — Token names no longer include `surface` (e.g., `--wpds-dimension-padding-surface-sm` → `--wpds-dimension-padding-lg`).

2. **Updates padding scale** — Changed from `2xs`–`lg` to `xs`–`3xl`, with new intermediate values for a smoother progression. This will enable the use of padding tokens in components like `Button` which utilise `12px` inline padding in `@wordpress/components`.

2. **Updates gap scale** — Changed from `2xs`–`lg` to `xs`–`3xl`, with new intermediate values for a smoother progression. 
...
4. **Updates gap scale** — Changed from `2xs`–`xl` to `xs`–`2xl` for consistency
...
- **Simplified token structure** — Removing the `surface` segment from padding tokens reduces nesting, makes the token names more concise, and aligns padding with border tokens (see https://github.com/WordPress/gutenberg/pull/74617).
- **Conventional naming** — Starting scales at `xs` instead of `2xs` follows common conventions and provides a clear and consistent "smallest" anchor.

### How

The token source definitions were updated in `dimension.json`, with corresponding changes to the TypeScript types generator in `terrazzo.config.ts`. Documentation was updated in the theme package's README, changelog, and the auto-generated tokens reference.

CSS token usages were updated across the `@wordpress/ui` package, including the tooltip, badge, and form fieldset components, as well as shared CSS utilities.

The `@wordpress/dataviews` package required extensive updates, as many components use the `Stack` component with `gap` props. All gap values were migrated to preserve their original pixel values

Storybook fixtures and decorators were also updated to use the new token names.
...
- `npm run storybook`
- Compare stories against trunk for tooltip, badge, and form fieldset in `@wordpress/ui`.
- Compare stories against trunk for DataViews/Dataform.

## Issue comments

### github-actions on 2026-01-29T13:59:31Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3817868249
...
### github-actions on 2026-01-29T14:10:58Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3817941938

**Size Change:** +485 B (+0.02%) 

**Total Size:** 3 MB

| Filename | Size | Change |
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 14.6 kB | +31 B (+0.21%) |
| `build/scripts/block-editor/index.min.js` | 328 kB | -1 B (0%) |
| `build/scripts/editor/index.min.js` | 320 kB | -3 B (0%) |
| `build/styles/edit-site/style-rtl.css` | 22.1 kB | +41 B (+0.19%) |
| `build/styles/edit-site/style-rtl.min.css` | 17.1 kB | +32 B (+0.19%) |
| `build/styles/edit-site/style.css` | 22.1 kB | +41 B (+0.19%) |
| `build/styles/edit-site/style.min.css` | 17.1 kB | +32 B (+0.19%) |
| `build/styles/editor/style-rtl.css` | 26 kB | +52 B (+0.2%) |
| `build/styles/editor/style-rtl.min.css` | 20.6 kB | +36 B (+0.18%) |
| `build/styles/editor/style.css` | 26.1 kB | +49 B (+0.19%) |
| `build/styles/editor/style.min.css` | 20.5 kB | +35 B (+0.17%) |
| `build/styles/theme/style-rtl.css` | 2.69 kB | +42 B (+1.58%) |
| `build/styles/theme/style-rtl.min.css` | 1.39 kB | +28 B (+2.06%) |
| `build/styles/theme/style.css` | 2.69 kB | +42 B (+1.58%) |
| `build/styles/theme/style.min.css` | 1.39 kB | +28 B (+2.06%) |
...
### aduth on 2026-01-29T21:37:11Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3820503781

> Conventional naming — Starting scales at xs instead of 2xs follows common conventions and provides a clear and consistent "smallest" anchor.

Can you elaborate on "conventions" being followed here? It stood out to me how we now go up to 3xl (pretty big, lopsided) to achieve this, so we have `xs-3xl`. Wondering if a `2xs-2xl` would be more "balanced". I guess part of this depends how we want specific labels to map to values, since it'd have a difference (`md` is`12` in the former and `16` in the latter).

The more I think about it though, the more I think it's fine actually 😄 I think it's a better fit for labels to values.

### ciampo on 2026-01-30T08:49:38Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3822578201
...
IMO the important thing is that we create a scale that we can modify in the future without creating too much inconvenience for consumers:

- adding more steps at the two ends of the scale (eg `2xs`, `4xl`..) is going to perfectly ok;
- on the other hand, adding an extra step in the middle of the scale may be tricky
...
### jameskoster on 2026-01-30T09:42:22Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3822795873
...
### github-actions on 2026-01-30T10:10:20Z

URL: https://github.com/WordPress/gutenberg/pull/75054#issuecomment-3822921452

<!-- flaky-tests-report-comment -->
**Flaky tests detected in faa61fcb12268d65986837ed7b0cbb06d2c68cba.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/21511538575
📝  Reported issues:
- #74223 in `/test/e2e/specs/editor/various/block-bindings/post-meta.spec.js`

## Review threads

### Review thread 1 (resolved)

#### ciampo on 2026-01-30T08:50:29Z

URL: https://github.com/WordPress/gutenberg/pull/75054#discussion_r2745255002
...
Let's add an empty line before the next section

### Review thread 2

#### ciampo on 2026-02-03T14:56:48Z

URL: https://github.com/WordPress/gutenberg/pull/75054#discussion_r2759509157
Path: packages/theme/CHANGELOG.md
...
- packages/dataviews/README.md (MODIFIED, +1/-1)
- packages/dataviews/src/components/dataform-controls/date.tsx (MODIFIED, +5/-5)
...
- packages/dataviews/src/components/dataform-layouts/data-form-layout.tsx (MODIFIED, +1/-1)
...
- packages/dataviews/src/components/dataform-layouts/panel/dropdown.tsx (MODIFIED, +2/-2)
...
- packages/dataviews/src/components/dataform-layouts/regular/index.tsx (MODIFIED, +2/-2)
- packages/dataviews/src/components/dataform-layouts/row/index.tsx (MODIFIED, +2/-2)
...
- packages/dataviews/src/dataform/stories/validation.tsx (MODIFIED, +1/-1)
