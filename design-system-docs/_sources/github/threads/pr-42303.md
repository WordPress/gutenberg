...
type: pull_request
state: MERGED
merged_at: 2022-07-14T08:30:41Z
url: https://github.com/WordPress/gutenberg/pull/42303
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Type] Code Quality","[Package] Components"]
---

# PR #42303: ScrollLock: Covert component to TypeScript

- URL: https://github.com/WordPress/gutenberg/pull/42303
- Author: walbo
- Created: 2022-07-10T22:50:15Z
- Updated: 2022-07-14T08:31:11Z
- Comments: 1 of 1
- Review threads: 3 of 3
- Changed files: 6 of 6
- Diff: ../pr-diffs/42303.diff
...
## What?
Converts the `ScrollLock` component to TypeScript.

## Why?
Part of the @wordpress/components's TypeScript migration (https://github.com/WordPress/gutenberg/issues/35744).

## How?
- Refactors the current `ScrollLock` to TypeScript.
- Refactors the test to `@testing-library/react` and TypeScript
- Refactors storybook to TypeScript.

## Testing Instructions
1. Run npm run dev and ensure no typing errors
2. Fire up the Storybook and confirm the `ScrollLock` continues to function as expected

## Issue comments

### github-actions on 2022-07-10T22:59:49Z

URL: https://github.com/WordPress/gutenberg/pull/42303#issuecomment-1179816730
...
#### mirka on 2022-07-11T19:51:26Z

URL: https://github.com/WordPress/gutenberg/pull/42303#discussion_r918295718
Path: packages/components/src/scroll-lock/stories/index.tsx

```diff
@@ -7,32 +13,19 @@ import { useState } from '@wordpress/element';
...
#### mirka on 2022-07-11T21:49:44Z

URL: https://github.com/WordPress/gutenberg/pull/42303#discussion_r918377574
Path: packages/components/src/scroll-lock/stories/index.tsx

```diff
@@ -66,6 +59,23 @@ function ToggleContainer( props ) {
 	);
 }
...
#### ciampo on 2022-07-12T08:10:58Z

URL: https://github.com/WordPress/gutenberg/pull/42303#discussion_r918677353
Path: packages/components/src/scroll-lock/test/index.tsx
...
I wonder if using `toMatchStyleDIffSnapshot` here would make for a better test — basically checking that the difference between locked and unlocked state on `document.body` only produces an `overflow: hidden` style diff.

Otherwise, we could completely change the styles associated with the `lockscroll` CSS class and tests wouldn't break.

#### walbo on 2022-07-13T19:15:31Z

URL: https://github.com/WordPress/gutenberg/pull/42303#discussion_r920420157
Path: packages/components/src/scroll-lock/test/index.tsx
...
Not sure how to use `toMatchStyleDIffSnapshot` on the body here 🤔 I tried, but failed to get the style. Are you able to take a look at it @ciampo? Either in this PR or as a follow-up.

#### ciampo on 2022-07-13T21:18:07Z

URL: https://github.com/WordPress/gutenberg/pull/42303#discussion_r920522109
Path: packages/components/src/scroll-lock/test/index.tsx
...
Looking at the implementation of `toMatchStyleDiffSnapshot`, [it may not be able to get inline styles](https://github.com/WordPress/gutenberg/blob/1ef47334d7ab9b282a52c5f7ea80950abc4e6532/test/unit/config/matchers/to-match-style-diff-snapshot.js#L19).
...
- packages/components/src/scroll-lock/index.tsx (RENAMED, +34/-7)
- packages/components/src/scroll-lock/stories/index.tsx (RENAMED, +36/-26)
- packages/components/src/scroll-lock/test/index.js (DELETED, +0/-53)
- packages/components/src/scroll-lock/test/index.tsx (ADDED, +26/-0)
