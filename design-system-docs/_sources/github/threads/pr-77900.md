...
type: pull_request
state: MERGED
merged_at: 2026-05-04T10:28:30Z
url: https://github.com/WordPress/gutenberg/pull/77900
matched_labels: ["[Package] UI","[Package] Theme"]
all_labels: ["[Type] Code Quality","[Package] Core data","[Package] E2E Tests","[Package] Project management automation","[Package] Lazy import","dependencies","[Package] Theme","[Package] UI","[Package] Sync"]
---

# PR #77900: Upgrade and unify @types/node version

- URL: https://github.com/WordPress/gutenberg/pull/77900
- Author: manzoorwanijk
- Created: 2026-05-04T06:28:03Z
- Updated: 2026-05-05T11:51:50Z
- Comments: 12 of 12
- Review threads: 0 of 0
- Changed files: 15 of 15
- Diff: ../pr-diffs/77900.diff
...
### github-actions on 2026-05-04T06:34:47Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4368776750
...
### github-actions on 2026-05-04T06:43:47Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4368813615
...
### github-actions on 2026-05-04T08:34:22Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4369410537

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 8bf73589d81ca7b8509103e85eb27e365d54de65.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25312269514
📝  Reported issues:
...
### sirreal on 2026-05-04T09:38:37Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4369924278

> Bumped every @types/node entry to ^20.19.39 (latest in the 20.x line), including the root package.json

Is there a need to move this to the latest 20.x version? It does seem good to be consistent with the version used across the Gutenberg repository, but it doesn't seem necessary to require consumers to use the latest 20.x node version. In fact, this seems like it may encourage this duplication in consumers who are not very careful to avoid it by forcing a package upgrade that isn't essential.

It seems preferable to relax and widen the declared dependency. Consistency is good, but perhaps `20.17.0` (or whatever the most relaxed version was) is a better dependency to declare.

It does seem valuable to update this package in _our_ lockfile to the latest 20.x version and ensure that it is not duplicated there.

### manzoorwanijk on 2026-05-04T09:43:22Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4369969579

> but it doesn't seem necessary to require consumers to use the latest 20.x node version.

Good catch. Thanks. I will revert the change for e2e utils peer-dependency.

### manzoorwanijk on 2026-05-04T09:47:08Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4370005716

Updated. Thanks

### sirreal on 2026-05-04T09:51:37Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4370047849

All the package.json files are still upgrading the dependency. My comment was that they should align, but not necessarily upgrade to the latest version. I still see this in the diff:
...
I think these could either all target the previous minimum version range `^20.17.10`, or that `^20.19.0` range:
...
### manzoorwanijk on 2026-05-04T09:53:01Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4370059511

> All the package.json files are still upgrading the dependency. My comment was that they should align, but not necessarily upgrade to the latest version. I still see this in the diff:

But those are all `devDependencies` which stay only in this repo. Consumers will be affected only by `peerDependencies`, which I fixed.

### sirreal on 2026-05-04T09:59:28Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4370114623

> Consumers will be affected only by peerDependencies, which I fixed.

My mistake, you're right, thanks.

### ciampo on 2026-05-05T09:23:19Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4378001358

I'm not an expert of `npm` and related tooling, but I wonder if there is anything we can do to ensure alignment across the monorepo for such dependencies? Even a custom lint script which checks `package.jsons` and/or `package-lock.json` (or invokes `npm` commands, maybe `npm list` ?) and ensures that, for a given list of packages, we're using the same version across the repo?

Would there be value in such a check?

### manzoorwanijk on 2026-05-05T09:26:24Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4378020466

> I'm not an expert of `npm` and related tooling, but I wonder if there is anything we can do to ensure alignment across the monorepo for such dependencies? Even a custom lint script which checks `package.jsons` and/or `package-lock.json` (or invokes `npm` commands, maybe `npm list` ?) and ensures that, for a given list of packages, we're using the same version across the repo?
> 
> Would there be value in such a check?

There is a very popular tool for that - [Syncpack](https://syncpack.dev/). We could set it up with some rules to ensure we have consistency across the monorepo.

### manzoorwanijk on 2026-05-05T11:51:50Z

URL: https://github.com/WordPress/gutenberg/pull/77900#issuecomment-4378925674
...
- package-lock.json (MODIFIED, +22/-25)
...
- packages/lazy-import/package.json (MODIFIED, +1/-0)
