...
type: pull_request
state: MERGED
merged_at: 2026-02-12T08:50:03Z
url: https://github.com/WordPress/gutenberg/pull/75340
...
# PR #75340: Update Testing Library for React 19 compat

- URL: https://github.com/WordPress/gutenberg/pull/75340
- Author: jsnajdr
- Created: 2026-02-09T08:46:38Z
- Updated: 2026-02-12T08:51:34Z
- Comments: 9 of 9
- Review threads: 0 of 0
- Changed files: 13 of 13
- Diff: ../pr-diffs/75340.diff

## Body

For the React 19 upgrade (https://github.com/WordPress/gutenberg/issues/71336) we need to upgrade `@testing-library/react`. The 14.3.0 version we're currently using doesn't support React 19: it declares its React peer dependencies as `^18`, and also uses React 18 types.

One thing that will probably cause us headaches is `@testing-library/user-event`. We can't use the latest version because it's not compatible with `@ariakit/test`, and the needed fix (https://github.com/testing-library/user-event/pull/1265) hasn't been released for a year. That's why @aduth had to pin an older version in #74396. This older version depends on ancient `@types/react@17.0.42`, two major versions behind.

One way to address this woud be to avoid using `user-event` and `@ariakit/test` utils together. As soon as you use the custom `render` or `click` utils from Ariakit, and also initialize the `user-event` library, they start fighting over patching the native `Element.prototype.focus` method. We have 5 test suites where this happens. One of the libraries would need to be removed from these.

## Issue comments

### github-actions on 2026-02-09T08:46:48Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3870214498
...
### github-actions on 2026-02-09T08:58:59Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3870283682
...
### ciampo on 2026-02-09T11:34:16Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3871192711

I guess that using either `@ariakit/test` or `@testing-library/user-event` for any given suite feels like the simplest approach, hoping that it won't hit other limitations or issues

### jsnajdr on 2026-02-09T12:42:55Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3871489666

Another thing we could do is to fixup the `userEvent.setup()` call. With ESLint, ban direct calls to `userEvent.setup()`, and require users to call our wrapper instead:
```js
...
### ciampo on 2026-02-09T12:50:38Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3871526309

> This would make the two libs compatible again.

Then this would be my favourite fix, with the least amount of changes to the existing code. We should add a comment to explain why we're doing it, and maybe even link to the `user-event` fix that hasn't been merged yet.

### aduth on 2026-02-09T18:43:32Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3873332230

Asking from a place of ignorance: Why do we use both `@ariakit/test` and `@testing-library/user-event`? Looking at how we use the former, it feels like we could easily do the same with the latter? i.e. Could we consolidate our usage?

(Although the lack of recent maintainer activity in `@testing-library/user-event` is a bit concerning)

### ciampo on 2026-02-09T21:44:36Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3874029928

> Asking from a place of ignorance: Why do we use both `@ariakit/test` and `@testing-library/user-event`? Looking at how we use the former, it feels like we could easily do the same with the latter? i.e. Could we consolidate our usage?
> 
> (Although the lack of recent maintainer activity in `@testing-library/user-event` is a bit concerning)

Ariakit-based components tend to need `@ariakit/test` given how they internally usually update asynchronously, which wouldn't work with `@testing-library/user-event`

### jsnajdr on 2026-02-11T09:07:57Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3883162147

All issues in this PR are now solved and it's ready.

After trying out a few alternatives, I solved the `@testing-library/user-event` issue by 1) using the latest version; 2) patching the latest version with `jest.mock`. Now it effectively includes the unreleased bugfix and `@ariakit/test` and `@testing-library/user-event` now can happily co-live again.

The `user-event` upgrade also solved one issue in `RadioControl` unit tests where number of clicks and `onChange` events was twice as expected. I think it's this `user-event` fix published in 14.6.1: https://github.com/testing-library/user-event/pull/1130

### aduth on 2026-02-11T13:47:54Z

URL: https://github.com/WordPress/gutenberg/pull/75340#issuecomment-3884549771
...
- test/unit/config/global-mocks.js (MODIFIED, +36/-0)
