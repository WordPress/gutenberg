...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/74980
matched_labels: ["Design System","[Package] Theme"]
all_labels: ["[Type] Enhancement","Design System","[Package] Theme"]
---

# PR #74980: Add composite type tokens

- URL: https://github.com/WordPress/gutenberg/pull/74980
- Author: jameskoster
- Created: 2026-01-27T17:16:51Z
- Updated: 2026-02-11T11:09:22Z
- Comments: 19 of 19
- Review threads: 0 of 0
- Changed files: 6 of 6
- Diff: ../pr-diffs/74980.diff

## Body

@aduth is this what you had in mind for #74391?

## Issue comments

### github-actions on 2026-01-27T17:27:45Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3806505290

**Size Change:** +4.24 kB (+0.14%) 

**Total Size:** 2.99 MB

| Filename | Size | Change |  |
| :--- | :---: | :---: | :---: |
| `build/modules/boot/index.min.js` | 14.8 kB | +255 B (+1.75%) |  |
| `build/styles/edit-site/style-rtl.css` | 22.4 kB | +441 B (+2.01%) |  |
| `build/styles/edit-site/style-rtl.min.css` | 17.2 kB | +226 B (+1.33%) |  |
| `build/styles/edit-site/style.css` | 22.4 kB | +440 B (+2%) |  |
| `build/styles/edit-site/style.min.css` | 17.2 kB | +226 B (+1.33%) |  |
| `build/styles/editor/style-rtl.css` | 26.4 kB | +483 B (+1.86%) |  |
| `build/styles/editor/style-rtl.min.css` | 20.7 kB | +253 B (+1.24%) |  |
| `build/styles/editor/style.css` | 26.4 kB | +478 B (+1.84%) |  |
| `build/styles/editor/style.min.css` | 20.7 kB | +249 B (+1.22%) |  |
| `build/styles/theme/style-rtl.css` | 3.02 kB | +369 B (+13.92%) | ⚠️ |
| `build/styles/theme/style-rtl.min.css` | 1.58 kB | +223 B (+16.37%) | ⚠️ |
| `build/styles/theme/style.css` | 3.02 kB | +369 B (+13.92%) | ⚠️ |
| `build/styles/theme/style.min.css` | 1.58 kB | +223 B (+16.37%) | ⚠️ |
...
| `build/scripts/block-library/index.min.js` | 301 kB |
...
| `build/scripts/core-data/index.min.js` | 65 kB |
...
| `build/scripts/editor/index.min.js` | 318 kB |
...
| `build/scripts/media-utils/index.min.js` | 73.3 kB |
...
| `build/styles/block-library/style-rtl.css` | 19.9 kB |
| `build/styles/block-library/style-rtl.min.css` | 16.5 kB |
| `build/styles/block-library/style.css` | 20 kB |
| `build/styles/block-library/style.min.css` | 16.4 kB |
...
### aduth on 2026-01-27T17:29:15Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3806512842

The source tokens make sense and match what I'd expect with #74391. The CSS, JavaScript, and Markdown outputs are a little less clear.

The individual CSS properties are probably fine, if a bit verbose, as long as we'd have the `Text` component mentioned in https://github.com/WordPress/gutenberg/issues/72784#issuecomment-3751776941 to manage the heavy lifting of piecing them together.

The JavaScript and Makrdown outputs look wrong, though. These should match the actual CSS properties (and are used for the [ESLint](https://github.com/WordPress/gutenberg/pull/74325)/[Stylelint](https://github.com/WordPress/gutenberg/pull/74226) "known" tokens linting), but they're only including the prefix and not the property (e.g. `--wpds-font-body-xl` instead of `--wpds-font-body-xl-font-size`, etc.).

### aduth on 2026-01-27T17:58:37Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3806653043

I guess we also have #74127 which partly overlaps with #74391 😅

### jameskoster on 2026-01-28T09:59:50Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3810245356

I'm not 100% certain I interpreted the feedback correctly, but I pushed some changes 🙂

### aduth on 2026-01-28T15:50:26Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3812099166

I pushed some minor changes in 871bea8dd79e796a0e4d4a694fc8965d125ae0fb to generalize the behavior, since looking at how it's implemented, this seems to be a common token value type internal to Terrazzo. It should also make it easier for us to maintain.

Couple other thoughts on my mind:

- I noticed that Terrazzo will be adding shorthand CSS property options in a future release (https://github.com/terrazzoapp/terrazzo/pull/589), which could simplify some of how these properties end up being consumed.
- We might want to revisit the discussion of the top-level grouping name "font" in this context. This was last left off in https://github.com/WordPress/gutenberg/pull/73931#issuecomment-3715388812, where I wondered if we'd want a dedicated grouping specifically for "typography" to contain these tokens, or if "typography" would be the better top-level grouping to contain both the presets and the individual font properties. As it stands, I wonder how we feel about "font" in e.g. "--wpds-font-body-xl-*"

### jameskoster on 2026-01-28T16:26:08Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3812305807

Yeah 'font' might not be the best category name, and now you mention it I'm curious if we want _font_-family, _font_-size, _font_-weight in this instance... 🤔 

`--wpds-font-body-xl-font-family` could be simplified to (something like) `--wpds-text-body-xl-family` without losing much.

### aduth on 2026-01-28T19:48:00Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3813525372

I think we'd end up fighting against Terrazzo and the DTCG spec to try to make that happen, because the name is generated from [standardized properties](https://www.designtokens.org/tr/2025.10/format/#typography)). 

The alternative with changing (or adding) a top-level grouping would have it look like, e.g.:

- `--wpds-type-body-xl-font-family`
- Or: `--wpds-typography-body-xl-font-family`

### jameskoster on 2026-01-29T15:52:14Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3818553652

Got it. I suppose either suggestion works.

### aduth on 2026-01-29T20:44:29Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3820222272

Also looping @ciampo here since he raised the question initially in https://github.com/WordPress/gutenberg/pull/73931#discussion_r2643875814 .

Thinking about it some more, I agree with (a) @ciampo's original observation that some primitives don't really fit the definition of "font" (e.g. line-height, letter-spacing), (b) it definitely doesn't fit the composite tokens like these, and (c) it might be nicer if we could still maintain them together.

With that in mind, perhaps `type` is a better grouping for all of them.

So we'd have `type.json` with values like...

- `--wpds-type-font-size-xl` (or `--wpds-type-size-xl`. My initial preference was toward dropping `font-`, but I think we'd want consistency with the composite naming, which leads back to the problem discussed in https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3813525372)
- `--wpds-type-font-family-heading` (or `--wpds-type-family-heading`)
- `--wpds-type-line-height-xl` (definitely no `font-` needed here)
- `--wpds-type-heading-xl-font-family` (or `--wpds-type-preset-heading-xl-font-family` if we want to group them)
   - Also [reiterating](https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3812099166) that Terrazzo will soon ship shorthand support, so we'd also have shorter options like `--wpds-type-heading-xl`

I'm also fine with `typography` which exchanges some verbosity for clarity.

### ciampo on 2026-02-04T17:28:14Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3848766980

- I like `type` for the group heading (shorter)
- I'd probably lean on the side of keeping `font-` for `size`/`weight`/`family` etc.. , mostly for the added clarity and similarity with the CSS property names

---

Apart from that, I have a more general reflection re. composite tokens.

Adding composite tokens adds complexity to our setup, and creates quite a few more DS tokens (of which a few are actually aliasing the same values).

Do we think they are necessary? In other words, do we think they bring much value, given that we're going to expose these presets anyway through the `Text` and `Heading` components? We could also consider providing these presets via documentation / CSS utilitys / react hook utilities?

cc @mirka since we had discussed tokens and styles reusable utilities in the past.

### mirka on 2026-02-04T22:17:43Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3850027548

> In other words, do we think they bring much value, given that we're going to expose these presets anyway through the `Text` and `Heading` components? We could also consider providing these presets via documentation / CSS utilitys / react hook utilities?

I generally lean towards this. Presets of multiple values would probably be more sustainable to abstract as components or CSS utility hooks or whatever. Similar things I was planning to do this way are focus ring styles and animations. This is based upon the notion that an infinitely growing list of tokens is bad (which may be an incorrect notion, I don't know).

It may be useful to agree on some criteria for what justifies a new design token, versus some other way to encapsulate variables.

---

> * I like `type` for the group heading (shorter)

I have a relatively strong preference for `typography`. It's longer but I think `type` is way too ambiguous in this context.

### jameskoster on 2026-02-05T15:13:12Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3854308956

Yes good points, consumers should be primarily interacting with `Text` and from that perspective the value of these tokens diminishes. Happy to close this out for now.

### aduth on 2026-02-05T18:24:31Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3855411875

With what's being discussed, is the concern about the tokens themselves, or the proliferation of CSS properties? Conceptually we could still tokenize these as part of the design system. Typography is a built-in concept within the DTCG spec. The CSS we generate is an artifact of those tokens, but it's an incidental side effect independent of the tokens themselves. We could exclude these specific tokens from the CSS, still maintain them in the token JSON, and optionally generate some artifact that's more tightly consumed by the `Text` component. 

I was partly considering whether we'd want these for Figma integration, though I don't know that with how Figma imports DTCG tokens as variables that it would help in how Figma manages text styles.

### mirka on 2026-02-05T19:52:30Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3855874200

> The CSS we generate is an artifact of those tokens, but it's an incidental side effect independent of the tokens themselves. We could exclude these specific tokens from the CSS, still maintain them in the token JSON, and optionally generate some artifact that's more tightly consumed by the `Text` component.

Interesting, I hadn't thought about it that way — managing values in a central data store, that doesn't necessarily have to be emitted in the design-tokens.css. I honestly don't know what specific benefits there are to having that central data store, but if there are, the downsides are definitely mitigated by not emitting everything to the design-tokens.css.

### ciampo on 2026-02-06T11:08:31Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3859779877

@aduth what would this alternative solution look like, in practice?

Defining tokens in JSON, and then having some custom terrazzo code to generate "presets" in CSS so that they can be consumed by `Text` and `Heading`?

### aduth on 2026-02-06T13:57:49Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3860637553

To be honest, I'm not sure there's much value in following the approach, other than "correctness" in codifying this within our token set. At least without some use-case like being able to import those into Figma, for example.

But yeah, the idea would be that we filter them out of the CSS output, and create a custom Terrazzo plugin that emits some JavaScript (or CSS, or JSON) that's not broadly loaded or shared, but instead only used by the Text components.

### jameskoster on 2026-02-06T15:32:18Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3861076387

In case it influences anything; to my knowledge Figma doesn't support importing text styles (composite tokens) currently.

### ciampo on 2026-02-06T16:05:25Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3861226389

Should we start with the simplest approach (ie. use `Text` + documentation to create presets) and add more refined/complex solutions if needed?

### aduth on 2026-02-10T17:08:18Z

URL: https://github.com/WordPress/gutenberg/pull/74980#issuecomment-3879464421
...
- packages/theme/bin/terrazzo-plugin-ds-tokens-docs/index.ts (MODIFIED, +12/-2)
- packages/theme/bin/terrazzo-plugin-known-wpds-css-variables/index.ts (MODIFIED, +12/-2)
- packages/theme/docs/tokens.md (MODIFIED, +55/-19)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +108/-0)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +36/-0)
- packages/theme/tokens/typography.json (MODIFIED, +87/-0)
