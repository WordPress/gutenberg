...
type: pull_request
state: MERGED
merged_at: 2026-04-20T14:32:52Z
url: https://github.com/WordPress/gutenberg/pull/77112
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","Storybook","[Package] UI"]
---

# PR #77112: Storybook: Fix component descriptions in manifest files

- URL: https://github.com/WordPress/gutenberg/pull/77112
- Author: aduth
- Created: 2026-04-07T20:47:10Z
- Updated: 2026-04-20T14:33:39Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 13 of 13
- Diff: ../pr-diffs/77112.diff
...
<img width="629" height="142" alt="image" src="https://github.com/user-attachments/assets/1601eea6-3a21-417a-954e-8283f2eaff17" />
...
- Fix this more correctly, e.g. through package updates if available and a newer version fixes this issue.
- Or, keep this approach, and enforce the import convention through linting so it doesn't reemerge in the future.
...
The proposed fix was based on an observation of how this wasn't problematic with the output of some components that already used this approach for importing components from its source (`Card`, `Collapsible`, `Dialog`, `Notice`). It's a hacky fix around how Storybook (more specifically, likely `react-docgen-typescript`) is unable to follow the imports. The hypothesis is that it chooses Badge description because the previous code points to the root barrel index and [Badge is the first export in that file](https://github.com/WordPress/gutenberg/blob/7144f285edfff772e327f2cf40f8252c0dab31aa/packages/ui/src/index.ts#L1).
...
2. View manifest output (e.g. `open storybook/build/manifests/components.html`)
3. Observe correct description for `AlertDialog.Root` and other `@wordpress/ui` components
...
### github-actions on 2026-04-07T20:58:52Z

URL: https://github.com/WordPress/gutenberg/pull/77112#issuecomment-4202169047
...
### github-actions on 2026-04-07T21:40:37Z

URL: https://github.com/WordPress/gutenberg/pull/77112#issuecomment-4202364156

<!-- flaky-tests-report-comment -->
**Flaky tests detected in d8531bcaa27fe141e89bddc8fec591a50d3d6198.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24578852248
📝  Reported issues:
...
### aduth on 2026-04-15T18:07:38Z

URL: https://github.com/WordPress/gutenberg/pull/77112#issuecomment-4254365065

> * Fix this more correctly, e.g. through package updates if available and a newer version fixes this issue.

See: #77382

### github-actions on 2026-04-17T17:46:17Z

URL: https://github.com/WordPress/gutenberg/pull/77112#issuecomment-4270116414
...
### aduth on 2026-04-17T17:47:55Z

URL: https://github.com/WordPress/gutenberg/pull/77112#issuecomment-4270125931

Marking this ready for review so that we can improve component documentation for MCP being introduced in #77159. The Storybook upgrade is stalled in #77382 due to a couple upstream Storybook bugs. In d8531bcaa27fe141e89bddc8fec591a50d3d6198 I added lint enforcement to enforce the pattern for the components to be picked up correctly.

It might also be something we want to apply to `@wordpress/components`, which #77159 now includes, and which also has the same issue.
...
- eslint.config.mjs (MODIFIED, +31/-3)
- packages/ui/src/alert-dialog/stories/index.story.tsx (MODIFIED, +2/-2)
...
- packages/ui/src/form/primitives/select/stories/index.story.tsx (MODIFIED, +1/-1)
- packages/ui/src/popover/stories/index.story.tsx (MODIFIED, +2/-1)
- packages/ui/src/tabs/stories/index.story.tsx (MODIFIED, +2/-1)
- packages/ui/src/tooltip/stories/index.story.tsx (MODIFIED, +2/-1)
