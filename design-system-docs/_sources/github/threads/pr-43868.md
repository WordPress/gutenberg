...
type: pull_request
state: MERGED
merged_at: 2022-09-05T18:32:05Z
url: https://github.com/WordPress/gutenberg/pull/43868
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #43868: RadioControl: Clean up CSS

- URL: https://github.com/WordPress/gutenberg/pull/43868
- Author: mirka
- Created: 2022-09-05T16:55:58Z
- Updated: 2022-11-28T08:35:42Z
- Comments: 7 of 7
- Review threads: 0 of 0
- Changed files: 3 of 3
- Diff: ../pr-diffs/43868.diff
...
### github-actions on 2022-09-05T17:05:38Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1237314897
...
### louwie17 on 2022-11-16T11:44:44Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1316868834

@mirka just wanted to note that this change did break styling for component users that relied on the radio control options being nested directly under the `components-base-control__field` as the `VStack` component add's an extra element.

It would make more sense to add styling updates to the `css` file for something like that (the very one's you removed), or instead provide an extra css class that is always the parent of the radio control options.
In addition to the fact that `VStack` is also listed as an experimental component still.

### ciampo on 2022-11-16T14:08:04Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1317080050

Hey @louwie17 , unfortunately internal DOM structure and internal classnames are not part of the public API surface of the component. We consider those as implementation details, and therefore we can't guarantee backwards compatibility on those aspects.

That's why we actively discourage style overrides on the components (especially the ones relying on internal DOM structure or classnames), since they are particularly prone to breakages when the implementation details of a component change.

### mirka on 2022-11-17T06:08:00Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1318132937
...
### louwie17 on 2022-11-17T09:32:29Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1318343892

@ciampo & @mirka thanks for the responses, we do indeed try to keep the overrides to a minimum where we can, and understand things could change ( as the impact of Gutenberg updates has usually caused very minimal conflicts)

I do find this change a slight exception, mostly because the radio buttons have been directly nested since it's conception, and we make use of an experimental component to change that. But anyway, I guess that's just my opinion.

> Just to add: Instead, if you let us know what exactly you wanted to achieve with those style overrides, we may be able to improve the component implementation to support that kind of styling in a more robust way.
...
### ciampo on 2022-11-17T17:12:44Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1318950758

> Instead, if you let us know what exactly you wanted to achieve with those style overrides, we may be able to improve the component implementation to support that kind of styling in a more robust way.
...
### louwie17 on 2022-11-28T08:35:42Z

URL: https://github.com/WordPress/gutenberg/pull/43868#issuecomment-1328716515

> That sounds reasonable! Could you open a new issue with that request, so that we can discuss the exact specs?

Thanks, I created a feature request for it [here](https://github.com/WordPress/gutenberg/issues/46103).
...
- packages/components/src/radio-control/index.tsx (MODIFIED, +27/-23)
- packages/components/src/radio-control/style.scss (MODIFIED, +0/-17)
