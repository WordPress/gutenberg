...
type: pull_request
state: MERGED
merged_at: 2021-09-28T16:50:55Z
url: https://github.com/WordPress/gutenberg/pull/35185
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #35185: Replace tinycolor2 with colord on some components.

- URL: https://github.com/WordPress/gutenberg/pull/35185
- Author: jorgefilipecosta
- Created: 2021-09-28T12:18:23Z
- Updated: 2021-09-28T16:51:19Z
- Comments: 1 of 1
- Review threads: 2 of 2
- Changed files: 7 of 7
- Diff: ../pr-diffs/35185.diff
...
### github-actions on 2021-09-28T12:26:21Z

URL: https://github.com/WordPress/gutenberg/pull/35185#issuecomment-929155231
...
| :--- | :---: | :---: |
| `build/components/index.min.js` | 214 kB | +3.79 kB (+2%) |
...
#### Mamaduka on 2021-09-28T13:49:18Z

URL: https://github.com/WordPress/gutenberg/pull/35185#discussion_r717597550
Path: packages/components/src/color-palette/index.js

```diff
@@ -33,9 +36,9 @@ export default function ColorPalette( {
 				selectedIconProps={
 					value === color
 						? {
-								fill: tinycolor
...
-									.toHexString(),
+								fill: colord( color ).isLight()
+									? '#000'
+									: '#fff',
```

I think this change caused the failing unit tests, but I couldn't spot any regression when testing manually.

#### jorgefilipecosta on 2021-09-28T14:23:00Z

URL: https://github.com/WordPress/gutenberg/pull/35185#discussion_r717632071
Path: packages/components/src/color-palette/index.js
...
I tried a simplification but it end up failing on some edge cases, our tests were good because they caught that. In practical terms, it would be unnoticeable because it means using white instead of black in backgrounds where they are almost as readable no matter the option. But I corrected it and now the result should be exactly the same as before.

### Review thread 2

#### Mamaduka on 2021-09-28T13:51:15Z

URL: https://github.com/WordPress/gutenberg/pull/35185#discussion_r717599516
Path: packages/components/src/duotone-picker/utils.js
...
If anyone else is wondering about `/ 255` removal:

`colord().brightness()` returns number from 0-1, while `tinycolor().getBrightness` from 0-255.

#### jorgefilipecosta on 2021-09-28T13:54:28Z

URL: https://github.com/WordPress/gutenberg/pull/35185#discussion_r717602754
Path: packages/components/src/duotone-picker/utils.js
...
Exactly colord brightness already returns a value from 0 to 1 as we want so there is no need to divide by 255.
...
- packages/components/src/color-palette/index.js (MODIFIED, +41/-32)
- packages/components/src/color-palette/test/__snapshots__/index.js.snap (MODIFIED, +6/-6)
- packages/components/src/custom-gradient-picker/utils.js (MODIFIED, +5/-2)
- packages/components/src/duotone-picker/utils.js (MODIFIED, +5/-2)
