...
type: pull_request
state: MERGED
merged_at: 2026-05-04T14:01:58Z
url: https://github.com/WordPress/gutenberg/pull/77854
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI","[Package] Grid"]
---

# PR #77854: Grid: prevent resize runaway near viewport edge

- URL: https://github.com/WordPress/gutenberg/pull/77854
- Author: retrofox
- Created: 2026-04-30T16:30:59Z
- Updated: 2026-05-12T18:21:58Z
- Comments: 3 of 3
- Review threads: 0 of 0
- Changed files: 2 of 2
- Diff: ../pr-diffs/77854.diff

## Body

## What?

Tame dnd-kit's auto-scroll on the resize handle of `@wordpress/grid` and compensate the resize preview overlay for the resulting scroll.

## Why?

When resizing a tile in `editMode` and the cursor reached the bottom edge of the viewport, the gesture entered a runaway loop: the page kept scrolling down, dnd-kit's `delta` (in document coordinates) inflated with the scroll, the tile grew another row per frame, the page expanded further, and so on. Releasing the gesture often left a tile of dozens of rows that the user did not intend.

Auto-scroll is designed for drag-and-drop, where the dragged element should follow the cursor visually as the document scrolls. A resize is a magnitude gesture; default auto-scroll feeds the scroll growth back into the resize delta and produces the loop.

A simple `autoScroll: false` would prevent the runaway but loses the visibility benefit of auto-scroll near the edge. This PR keeps auto-scroll, but tunes it so it only engages on a deliberate edge push and runs slowly enough for the user to interrupt by releasing.

## How?

`<DndContext>` around the resize handle uses tuned options:

- `threshold: { x: 0.005, y: 0.005 }` — the trigger zone is the last 0.5% of each axis (≈5px in a 1000px viewport), so passive proximity does not engage auto-scroll.
- `acceleration: 1` — vs. dnd-kit's default of 10, scrolling is ~10× slower so the user can stop the gesture before the tile grows past intent.

The preview overlay in `GridItem` is computed from the resize delta minus the tile's bounding-rect drift. With auto-scroll active, both quantities now move with the document scroll. The overlay calculation captures the document scroll position at gesture start and subtracts the scroll change since then, so the overlay tracks the cursor's viewport-space position rather than drifting with the document under it.

The outer `DndContext` (used for tile drag-to-reorder) is untouched and keeps its default auto-scroll behavior, which is genuinely useful there.

## Testing

1. Storybook: open `Grid/DashboardGrid` in `EditMode` with a numeric `rowHeight`.
2. Place a tile near the viewport bottom (resize the browser window if needed).
3. Grab its bottom-right resize handle and drag downward, past the viewport's lower edge, then hold the cursor still.
4. Expected: the page does not auto-scroll on passive proximity; only when the cursor is in the very last sliver of the viewport. Once it does, the page scrolls slowly, and releasing commits the size the cursor was at. The dashed preview overlay tracks the cursor in viewport space.
5. Sanity check: drag the handle within the visible area and confirm normal resize still works in both axes.


**Before**

https://github.com/user-attachments/assets/e7077393-bd54-4329-b94a-59079888414c


**After**

https://github.com/user-attachments/assets/134c60c6-b60b-4600-927b-fe4ca10f8531

## Issue comments

### github-actions on 2026-04-30T16:31:16Z

URL: https://github.com/WordPress/gutenberg/pull/77854#issuecomment-4354279084
...
### github-actions on 2026-04-30T16:44:00Z

URL: https://github.com/WordPress/gutenberg/pull/77854#issuecomment-4354371069
...
### github-actions on 2026-05-01T17:00:35Z

URL: https://github.com/WordPress/gutenberg/pull/77854#issuecomment-4360525861

<!-- flaky-tests-report-comment -->
**Flaky tests detected in e52bed9c4bfc9070188891343e8d1a00c49bd019.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25319406751
📝  Reported issues:
- #77710 in `/test/e2e/specs/widgets/editing-widgets.spec.js`
...
- packages/grid/src/grid-item.tsx (MODIFIED, +45/-7)
- packages/grid/src/resize-handle.tsx (MODIFIED, +17/-1)
