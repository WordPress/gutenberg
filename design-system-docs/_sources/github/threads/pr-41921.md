...
type: pull_request
state: MERGED
merged_at: 2022-06-28T07:29:21Z
url: https://github.com/WordPress/gutenberg/pull/41921
matched_labels: ["[Package] Components"]
all_labels: ["[Feature] UI Components","[Type] Code Quality","[Package] Components"]
---

# PR #41921: Heading: Complete TypeScript migration of component

- URL: https://github.com/WordPress/gutenberg/pull/41921
- Author: walbo
- Created: 2022-06-23T20:10:39Z
- Updated: 2022-06-28T07:29:46Z
- Comments: 3 of 3
- Review threads: 3 of 3
- Changed files: 9 of 9
- Diff: ../pr-diffs/41921.diff
...
## What?
Completes the migration of `Heading` component to TypeScript.

## Why?
Part of the @wordpress/components's TypeScript migration (https://github.com/WordPress/gutenberg/issues/35744).

## How?
- Moved types to `types.js`
- Refactored test to use TypeScript
- Updated docs

## Testing Instructions
1. Run npm run dev and ensure no typing errors
2. Fire up the Storybook and confirm the `Heading` continues to function as expected
3. Test the various Headings within the block/site editors in Gutenberg still work

## Issue comments

### github-actions on 2022-06-23T20:20:11Z

URL: https://github.com/WordPress/gutenberg/pull/41921#issuecomment-1164830104
...
### mirka on 2022-06-24T12:16:24Z

URL: https://github.com/WordPress/gutenberg/pull/41921#issuecomment-1165516176

> While testing the component in the Storybook I noticed that the `variant` control is a [radio](https://github.com/WordPress/gutenberg/blob/trunk/packages/components/src/heading/stories/index.tsx#L22) button. This means you can't toggle off the `muted` variant once it's clicked. We should probably tweak that while we're here.

I don't feel super strongly about it, but for context, here are the pros/cons that were discussed: https://github.com/WordPress/gutenberg/pull/38842#discussion_r813249434

If y'all think it's worth addressing, another way to go might be:
...
### walbo on 2022-06-27T21:41:40Z

URL: https://github.com/WordPress/gutenberg/pull/41921#issuecomment-1167937746

> > While testing the component in the Storybook I noticed that the `variant` control is a [radio](https://github.com/WordPress/gutenberg/blob/trunk/packages/components/src/heading/stories/index.tsx#L22) button. This means you can't toggle off the `muted` variant once it's clicked. We should probably tweak that while we're here.
> 
> I don't feel super strongly about it, but for context, here are the pros/cons that were discussed: [#38842 (comment)](https://github.com/WordPress/gutenberg/pull/38842#discussion_r813249434)
> 
> If y'all think it's worth addressing, another way to go might be:
> 
> ```js
...
Implemented the `undefined` option. Seem like a bettere solution since we don't have to change the templat 👍

## Review threads

### Review thread 1 (resolved)

#### aaronrobertshaw on 2022-06-24T04:39:20Z

URL: https://github.com/WordPress/gutenberg/pull/41921#discussion_r905710172
Path: packages/components/src/heading/README.md
...
Nit: Given the type is for both numbers and strings, should we reflect that here for accuracy?

`export type HeadingSize = 1 | 2 | 3 | 4 | 5 | 6 | '1' | '2' | '3' | '4' | '5' | '6';`

### Review thread 2 (resolved)

#### aaronrobertshaw on 2022-06-24T04:44:43Z

URL: https://github.com/WordPress/gutenberg/pull/41921#discussion_r905711960
Path: packages/components/src/heading/test/index.tsx
...
Nit: If we are changing the previous uses of `container.firstChild` to `.children[ 0 ]` should we also do the same here?
```suggestion
		expect( container.children[ 0 ] ).toHaveAttribute( 'role', 'heading' );
		expect( container.children[ 0 ] ).toHaveAttribute( 'aria-level', '3' );
```

#### mirka on 2022-06-24T11:51:47Z

URL: https://github.com/WordPress/gutenberg/pull/41921#discussion_r905988711
Path: packages/components/src/heading/test/index.tsx
...
If anything, I'd prefer we simplify in [this direction](https://github.com/WordPress/gutenberg/compare/refactor/heading-to-typescript...refactor/heading-to-typescript-tests?expand=1#diff-a4c73bed0ef8cc8f0e9883a24338969e5f48c16aab6b97fe297b9d887f19df0d), so we don't have to reach into containers directly. I think this style better matches the Testing Library paradigm. What do folks think?

### Review thread 3 (resolved)

#### aaronrobertshaw on 2022-06-24T04:45:40Z

URL: https://github.com/WordPress/gutenberg/pull/41921#discussion_r905712272
Path: packages/components/src/heading/test/index.tsx
...
Nit: Same as above, should we be consistent with use of `.firstChild` or `.children[ 0 ]`

## Changed files

- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/heading/README.md (MODIFIED, +4/-3)
- packages/components/src/heading/component.tsx (MODIFIED, +2/-2)
- packages/components/src/heading/hook.ts (MODIFIED, +1/-42)
- packages/components/src/heading/stories/index.tsx (MODIFIED, +5/-1)
- packages/components/src/heading/test/__snapshots__/index.tsx.snap (RENAMED, +0/-0)
- packages/components/src/heading/test/index.js (DELETED, +0/-67)
- packages/components/src/heading/test/index.tsx (ADDED, +68/-0)
- packages/components/src/heading/types.ts (ADDED, +29/-0)
