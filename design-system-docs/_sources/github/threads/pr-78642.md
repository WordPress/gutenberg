...
type: pull_request
state: MERGED
merged_at: 2026-05-27T12:59:27Z
url: https://github.com/WordPress/gutenberg/pull/78642
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] Editor","[Package] UI"]
---

# PR #78642: UI: `Tooltip.Provider` — forward upstream `closeDelay` and `timeout` props

- URL: https://github.com/WordPress/gutenberg/pull/78642
- Author: ciampo
- Created: 2026-05-25T17:48:38Z
- Updated: 2026-05-27T13:37:13Z
- Comments: 3 of 3
- Review threads: 2 of 2
- Changed files: 3 of 3
- Diff: ../pr-diffs/78642.diff
...
### github-actions on 2026-05-25T17:57:52Z

URL: https://github.com/WordPress/gutenberg/pull/78642#issuecomment-4536182790
...
### github-actions on 2026-05-25T18:19:34Z

URL: https://github.com/WordPress/gutenberg/pull/78642#issuecomment-4536307034

<!-- flaky-tests-report-comment -->
**Flaky tests detected in d4d67e37b5ad25e6f5572a7461f0603aaffe5dda.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26510660426
📝  Reported issues:
- #78689 in `/test/e2e/specs/editor/plugins/wp-editor-meta-box.spec.js`

### github-actions on 2026-05-25T19:11:01Z

URL: https://github.com/WordPress/gutenberg/pull/78642#issuecomment-4536601875
...
#### ciampo on 2026-05-25T19:04:44Z

URL: https://github.com/WordPress/gutenberg/pull/78642#discussion_r3299614654
Path: packages/ui/src/tooltip/types.ts

```diff
@@ -11,7 +11,7 @@ export type RootProps = Pick< _Tooltip.Root.Props, 'disabled' | 'children' >;
 
 export type ProviderProps = Pick<
 	_Tooltip.Provider.Props,
-	'delay' | 'children'
+	'delay' | 'closeDelay' | 'timeout' | 'children'
```

@mirka not sure if you prefer `ComponentProps< typeof _Tooltip.Provider >` here?

#### mirka on 2026-05-26T20:29:02Z

URL: https://github.com/WordPress/gutenberg/pull/78642#discussion_r3306657596
Path: packages/ui/src/tooltip/types.ts
...
As in, should we use our `ComponentProps` utility? I don't think we can here because `Provider` doesn't render an HTML element. The utility adds in `className` and `render`.

Or do you mean "should we use `_Tooltip.Provider.Props` fully instead of picking explicitly"? I'm generally leaning toward yes. I haven't been omitting upstream props unless I have a clear reason.

#### ciampo on 2026-05-27T10:19:48Z

URL: https://github.com/WordPress/gutenberg/pull/78642#discussion_r3310163537
Path: packages/ui/src/tooltip/types.ts
...
> should we use our ComponentProps utility? I don't think we can here 

True, oversight on my end. But the real question was

> "should we use _Tooltip.Provider.Props fully instead of picking explicitly"? I'm generally leaning toward yes

Cool, I'll make those changes.

Should we do the same for other `*.Provider` components throughout the package, too? Or is that a per-component decision

#### mirka on 2026-05-27T13:37:13Z

URL: https://github.com/WordPress/gutenberg/pull/78642#discussion_r3311126074
Path: packages/ui/src/tooltip/types.ts
...
For components I've been working on, I'm generally not `Pick`ing props (regardless of whether it's a Provider subcomponent or not). I think in the beginning we had a little hesitation there in fear of upstream behavior changes, but at this point I don't have much hesitation left. It's not realistic to maintain a back compat layer for upstream changes anyway.

### Review thread 2 (resolved)

#### mirka on 2026-05-26T20:29:49Z

URL: https://github.com/WordPress/gutenberg/pull/78642#discussion_r3306661727
...
+-   `Tooltip.Provider`: Forward the upstream `closeDelay` and `timeout` props ([#78642](https://github.com/WordPress/gutenberg/pull/78642)).
```

Nit: Props were always forwarded — this is a types change.

## Changed files

- packages/editor/src/components/collaborators-presence/avatar/test/index.tsx (MODIFIED, +8/-3)
- packages/ui/CHANGELOG.md (MODIFIED, +4/-0)
- packages/ui/src/tooltip/types.ts (MODIFIED, +1/-4)
