...
type: pull_request
state: MERGED
merged_at: 2026-03-20T17:55:20Z
url: https://github.com/WordPress/gutenberg/pull/74719
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Status] Blocked","[Package] UI"]
---

# PR #74719: Add EmptyState component to @wordpress/ui

- URL: https://github.com/WordPress/gutenberg/pull/74719
- Author: retrofox
- Created: 2026-01-17T19:59:37Z
- Updated: 2026-03-20T17:55:58Z
- Comments: 19 of 19
- Review threads: 6 of 6
- Changed files: 17 of 17
- Diff: ../pr-diffs/74719.diff

## Body

## What?

Adds a new `EmptyState` compound component to the `@wordpress/ui` package. This component displays placeholder content when a page or section has no data to show.
...
The component follows the compound component pattern (like `Tooltip`), providing flexible composition:
...
- `EmptyState.Root` - Container with centered flex layout
- `EmptyState.Visual` - Generic visual container (for custom illustrations)
- `EmptyState.Icon` - Styled icon with circular border treatment
- `EmptyState.Title` - Heading text (h2)
- `EmptyState.Description` - Supporting descriptive text
- `EmptyState.Actions` - Container for action buttons (responsive layout)

### Implementation details

- Uses `@base-ui/react` primitives (`useRender`, `mergeProps`)
- CSS uses semantic design tokens (`--wpds-dimension-gap-xs`, `--wpds-dimension-padding-surface-xs`, etc.)
- Follows `@wordpress/ui` conventions: no docblock comments for imports, CSS modules with layer ordering
- All components support ref forwarding and render prop customization

## Testing

- Unit tests verify ref forwarding for all sub-components
- Storybook stories demonstrate default usage and custom visual variant
- Run tests:
```
npm run test:unit -- --testPathPattern="packages/ui/src/empty-state"
```

```
npm run test:unit -- --testPathPattern="packages/ui/src/empty-state"

> gutenberg@22.4.0-rc.1 test:unit
> wp-scripts test-unit-js --config test/unit/jest.config.js --testPathPattern=packages/ui/src/empty-state

 PASS  packages/ui/src/empty-state/test/icon.test.tsx
 PASS  packages/ui/src/empty-state/test/root.test.tsx
 PASS  packages/ui/src/empty-state/test/title.test.tsx
 PASS  packages/ui/src/empty-state/test/description.test.tsx
 PASS  packages/ui/src/empty-state/test/visual.test.tsx
 PASS  packages/ui/src/empty-state/test/actions.test.tsx

Test Suites: 6 passed, 6 total
Tests:       6 passed, 6 total
Snapshots:   0 total
Time:        2.411 s
Ran all test suites matching /packages\/ui\/src\/empty-state/i.
```
...
<img width="954" height="805" alt="image" src="https://github.com/user-attachments/assets/02f3c1f6-37c2-4e0d-bc2f-da9b35dc799b" />

## Issue comments

### github-actions on 2026-01-17T19:59:48Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3764274035
...
### github-actions on 2026-01-17T20:11:26Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3764287953
...
| `build/scripts/edit-site/index.min.js` | 252 kB |
| `build/scripts/edit-widgets/index.min.js` | 19.9 kB |
| `build/scripts/editor/index.min.js` | 356 kB |
...
| `build/scripts/sync/index.min.js` | 37.8 kB |
...
| `build/styles/edit-site/style-rtl.css` | 21 kB |
...
| `build/styles/editor/style-rtl.min.css` | 22.3 kB |
| `build/styles/editor/style.css` | 26.5 kB |
| `build/styles/editor/style.min.css` | 22.3 kB |
...
### github-actions on 2026-01-26T12:53:55Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3799468560

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 3a93b01cc41d903b81819c55b3dc679d35ed0409.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23353966154
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### aduth on 2026-01-30T18:27:40Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3825074773

@retrofox Can you check the lint errors on this one?

I'd be curious if we can demonstrate a use-case for this in Gutenberg. I feel like this is a perfect fit for `DataView`'s `empty` prop added in #70867 , where the discussion in that pull request showed several examples that look a lot like what's being proposed here. Maybe as a starting point we can update the [empty Storybook example](https://github.com/WordPress/gutenberg/blob/trunk/packages/dataviews/src/dataviews/stories/empty.tsx) to make use of this component?

### ciampo on 2026-02-04T17:11:08Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3848689716

@retrofox I took over this PR and pushed a couple of small commits (including lint fixes).

@aduth re. whether this component belongs to the `@wordpress/ui` package, I'm also torn. **This component is essentially a pattern of existing components**, and my instinct tells me it belongs to a higher level of abstraction.

Having said that, I believe there is a need for such component across wordpress-related UIs.

Maybe it could make sense to start moving it to dataviews and only abstract it if necessary?

cc @WordPress/gutenberg-components @WordPress/gutenberg-design for feedback on the matter.

### github-actions on 2026-02-04T17:11:20Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3848690619
...
### aduth on 2026-02-04T20:10:16Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3849511566

> @aduth re. whether this component belongs to the `@wordpress/ui` package, I'm also torn. **This component is essentially a pattern of existing components**, and my instinct tells me it belongs to a higher level of abstraction.

I'm a little less torn on it being part of `@wordpress/ui`, and rather in demonstrating a need for the components that are being introduced (like we have in DataViews). I don't personally feel components existing to compose other components is a problem in itself, since that would imply that something like a [`ConfirmDialog`](https://github.com/WordPress/gutenberg/issues/75205) to compose `Dialog` patterns would be a problem. There's certainly components that should exist outside `@wordpress/ui` at a higher level of abstraction, though I'd consider that more in terms of the domain of a package (i.e. is it reasonably within the scope of DataViews specifically). That being said, starting at higher-level and letting it "mature" into the lower-level packages once there's a demonstrated wider need could be a perfectly-reasonable, conservative, and future-compatible approach as well.

### ciampo on 2026-02-05T10:53:37Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3852775762
...
### jameskoster on 2026-02-05T15:17:46Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3854333010

Starting in DataViews seems reasonable to me. I'm struggling to think of a use case for `EmptyState` outside of that context, but perhaps @poligilad-auto has some ideas.

### poligilad-auto on 2026-02-05T15:54:01Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3854539946

I was just exploring it for the Analytics section, but it might not be needed actually. 
We're only really using it for dataviews at the moment.

### mirka on 2026-02-05T20:52:41Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3856137493

> Maybe it could make sense to start moving it to dataviews and only abstract it if necessary?

I'd prefer this too. "If can live somewhere else, it should" might be my rule of thumb for `@wordpress/ui` inclusion 🙂

### ciampo on 2026-02-06T15:53:29Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-3861171114

Let's make sure we have a clear understanding of the usages of `EmptyState` and all of its necessary permutations before making an informed decision on where should the component live and in what shape.

### retrofox on 2026-03-18T09:56:12Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4081189444

> I was just exploring it for the Analytics section, but it might not be needed actually.
> We're only really using it for dataviews at the moment.

No really, we are using this component in different contexts.


**Dashboard widget**

<img width="895" height="529" alt="image" src="https://github.com/user-attachments/assets/fa4653a2-7c91-4c6b-bbe7-d00917582d87" />


**Dashboards**

<img width="912" height="600" alt="image" src="https://github.com/user-attachments/assets/ba3fae1e-450d-4d45-aaea-4639a45ff1e2" />

### simison on 2026-03-18T10:40:58Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4081451789

If not in components, then [`admin-ui`](https://github.com/WordPress/gutenberg/tree/trunk/packages/admin-ui) might be another suitable package.

In that case, it might make sense to also add prop in `Page` which then would help position the empty state better in a middle of the page:

<img width="553" height="732" alt="image" src="https://github.com/user-attachments/assets/fc120815-bce7-4742-b569-fc51623b6981" />

### retrofox on 2026-03-18T10:47:02Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4081497151

> If not in components, then [`admin-ui`](https://github.com/WordPress/gutenberg/tree/trunk/packages/admin-ui) might be another suitable package.

`admin-ui` sgtm

> ... then would help position the empty state better in a middle of the page:

It was defined in the design, just in case.

### poligilad-auto on 2026-03-18T12:15:17Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4082081831

I don't think the empty state should be centered in the middle of the page. In all our designs, it's positioned closer to the top so it sits better with the header above.

<img width="1442" height="902" alt="image" src="https://github.com/user-attachments/assets/eaa0211b-c6b2-4886-aae1-286ca82cd37b" />
<img width="1442" height="902" alt="image" src="https://github.com/user-attachments/assets/84bd5952-ae2b-41d6-9c78-f5f65ed81232" />

### aduth on 2026-03-18T15:26:57Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4083438729

> If not in components, then [`admin-ui`](https://github.com/WordPress/gutenberg/tree/trunk/packages/admin-ui) might be another suitable package.

I think `@wordpress/admin-ui` makes sense for page-level concerns, and there's certainly an interpretation that this is a page-level concern in many of the use-cases that have been presented. But if I'm wanting to just use this in DataViews for representing an empty state, does reaching into the "page" grab-bag the right way to be thinking about this? At that point it's not necessarily a page-level concern if I'm just rendering a DataViews somewhere in my app. It's more general than that. Which, for me at least, has me returning to thinking it a reasonable fit for `@wordpress/ui`.

Separately, this does raise the need for more clarity on what `@wordpress/admin-ui` actually is. ["components to be used to build the Admin UI"](https://github.com/WordPress/gutenberg/blob/trunk/packages/admin-ui/README.md) is kinda what `@wordpress/ui` is? 😅 The way I've been thinking about `@wordpress/admin-ui` is as specifically focused on the parts of the surrounding page shell (sidebar, canvas, common page elements like title, breadcrumbs, etc.)

### jameskoster on 2026-03-18T15:37:21Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4083523136

Dashboards and Analytics seem like a valid use case outside of DataViews.

My feeling was that `admin-ui` was for these "patterns of existing components", and from that perspective this would be seem like a good fit. But if it's not obvious what the difference is between `ui` and `admin-ui` (and there's no benefit to them being separate) then maybe we consider consolidation, before `admin-ui` expands further?

### aduth on 2026-03-19T19:32:31Z

URL: https://github.com/WordPress/gutenberg/pull/74719#issuecomment-4092786583

Regarding discussion of `@wordress/admin-ui`, I'll follow-up separately to update the package description so we can get on the same page for what that package should contain. But that shouldn't be a blocker for this.

## Review threads

### Review thread 1 (resolved)

#### retrofox on 2026-02-04T19:44:02Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2765708584
Path: packages/ui/src/empty-state/icon.tsx
...
`_Icon` feels a bit awkward to me :-D

#### ciampo on 2026-02-05T10:46:03Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2768330451
Path: packages/ui/src/empty-state/icon.tsx
...
Yeah it's not the most usual. It is a pattern that we've been adopting in the `@wordpress/ui` package when importing Base UI primitives

### Review thread 2

#### aduth on 2026-03-19T17:11:18Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2961493453
Path: packages/ui/src/empty-state/icon.tsx
...
#### retrofox on 2026-03-19T18:12:08Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2961835550
Path: packages/ui/src/empty-state/icon.tsx
...
#### aduth on 2026-03-19T17:13:09Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2961505662
Path: packages/ui/src/empty-state/style.module.css
...
Per flagged errors, these need to be formatted without the spacing in the parentheses, [per CSS guidelines](https://developer.wordpress.org/coding-standards/wordpress-coding-standards/css/#values:~:text=Do%20not%20pad%20parentheses%20with%20spaces.).

### Review thread 4

#### jameskoster on 2026-03-20T12:22:53Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2965478526
Path: packages/ui/src/empty-state/style.module.css
...
I just want to note that if https://github.com/WordPress/gutenberg/pull/76548 is merged and `EmptyState` is rendered directly on the `Page` (as I assume is intended) then the icon background will effectively be invisible. We might want to consider using `--wpds-color-bg-surface-neutral-weak` instead, or drop the background altogether.

### Review thread 5

#### jameskoster on 2026-03-20T12:25:07Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2965488361
Path: packages/ui/src/empty-state/style.module.css
...
#### jameskoster on 2026-03-20T12:27:05Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2965499573
Path: packages/ui/src/empty-state/style.module.css
...
#### jameskoster on 2026-03-20T12:26:00Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2965493551
Path: packages/ui/src/empty-state/style.module.css
...
If we used [`Text`](https://wordpress.github.io/gutenberg/?path=/docs/design-system-components-text--docs) we could eliminate some of these styles. Same for the description.

#### retrofox on 2026-03-20T15:53:53Z

URL: https://github.com/WordPress/gutenberg/pull/74719#discussion_r2966574080
Path: packages/ui/src/empty-state/style.module.css
...
Nice suggestion [2d84ea9](https://github.com/WordPress/gutenberg/pull/74719/commits/2d84ea93ae5ad54bf41b617ea934f3c9d8cf225c)

## Changed files

- packages/ui/src/empty-state/actions.tsx (ADDED, +24/-0)
- packages/ui/src/empty-state/description.tsx (ADDED, +27/-0)
- packages/ui/src/empty-state/icon.tsx (ADDED, +24/-0)
- packages/ui/src/empty-state/index.ts (ADDED, +8/-0)
- packages/ui/src/empty-state/root.tsx (ADDED, +23/-0)
- packages/ui/src/empty-state/stories/index.story.tsx (ADDED, +64/-0)
- packages/ui/src/empty-state/style.module.css (ADDED, +53/-0)
- packages/ui/src/empty-state/test/actions.test.tsx (ADDED, +18/-0)
- packages/ui/src/empty-state/test/description.test.tsx (ADDED, +13/-0)
- packages/ui/src/empty-state/test/icon.test.tsx (ADDED, +13/-0)
- packages/ui/src/empty-state/test/root.test.tsx (ADDED, +13/-0)
- packages/ui/src/empty-state/test/title.test.tsx (ADDED, +13/-0)
- packages/ui/src/empty-state/test/visual.test.tsx (ADDED, +17/-0)
- packages/ui/src/empty-state/title.tsx (ADDED, +23/-0)
- packages/ui/src/empty-state/types.ts (ADDED, +45/-0)
- packages/ui/src/empty-state/visual.tsx (ADDED, +24/-0)
