...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/76329
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #76329: ui/CollapsibleCard: include card title in trigger's accessible label

- URL: https://github.com/WordPress/gutenberg/pull/76329
- Author: ciampo
- Created: 2026-03-09T16:33:15Z
- Updated: 2026-03-19T11:46:08Z
- Comments: 4 of 4
- Review threads: 7 of 7
- Changed files: 6 of 6
- Diff: ../pr-diffs/76329.diff
...
### github-actions on 2026-03-09T16:54:47Z

URL: https://github.com/WordPress/gutenberg/pull/76329#issuecomment-4025238463
...
| :--- | :---: | :---: |
| `build/scripts/edit-post/index.min.js` | 16.3 kB | -2 B (-0.01%) |
...
### github-actions on 2026-03-09T17:13:10Z

URL: https://github.com/WordPress/gutenberg/pull/76329#issuecomment-4025393601
...
### github-actions on 2026-03-09T17:34:16Z

URL: https://github.com/WordPress/gutenberg/pull/76329#issuecomment-4025543233

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 0628aefb6a7bd3e3afcb4fb01434b0b9fb2fcf3b.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22865379788
📝  Reported issues:
...
### ciampo on 2026-03-13T15:59:35Z

URL: https://github.com/WordPress/gutenberg/pull/76329#issuecomment-4056195810

Closing as superseded by https://github.com/WordPress/gutenberg/pull/76265

## Review threads

### Review thread 1 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T16:38:40Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906605551
...
`useLayoutEffect` here runs after every render (no dependency array). Because it returns a cleanup that calls `setTitleText( undefined )`, React will also call that cleanup before the next effect run, causing redundant state updates and potentially transiently clearing the header label. Consider moving the cleanup into a separate effect that only runs on unmount/context change, and keep the per-render text sync effect without a cleanup (or otherwise ensure the clear only happens on unmount).

### Review thread 2 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T16:38:40Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906605582
Path: packages/ui/src/card/index.ts
...
Exporting `TitleTextContext` from `card/index.ts` makes it part of the public `Card` namespace (`packages/ui/src/index.ts` re-exports `* as Card`). If this context is intended to be internal plumbing for `CollapsibleCard`, consider keeping it unexported from the public barrel and importing it via a module-internal path (e.g. `../card/context`) or marking it as unstable/private to avoid expanding the public API surface.

### Review thread 3 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T16:38:41Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906605604
...
This file imports both `* as Card` and `{ TitleTextContext }` from `../card`, but the named import is redundant because `TitleTextContext` is already included in the namespace import. Consider using `Card.TitleTextContext` (or, if you keep the context internal, importing it from `../card/context`) to avoid duplicate imports.

### Review thread 4 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T16:38:41Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906605630
Path: packages/ui/src/collapsible-card/header.tsx
...
The fallback `useLayoutEffect` only re-runs when `titleText` changes. If no `Card.Title` is used and the header children/text changes over time, `headerText` (and therefore `triggerLabel`) can become stale because the effect won't re-run. Consider including `children` (or another signal) in the dependency array, or otherwise re-syncing the fallback text whenever the header content changes while `titleText` is undefined.

### Review thread 5 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T16:50:48Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906678049
Path: packages/ui/src/card/title.tsx

```diff
@@ -8,13 +15,37 @@ import type { TitleProps } from './types';
  * prop to swap in a semantic heading element when appropriate.
  */
 export const Title = forwardRef< HTMLDivElement, TitleProps >(
-	function CardTitle( { render, ...props }, ref ) {
...
`Card.Title` now schedules a layout effect on every render and unconditionally reads `textContent`, even when it's not inside a `TitleTextContext.Provider` (i.e., the common non-`CollapsibleCard` case). This adds avoidable DOM work for all cards. Consider early-returning in the effect when `titleTextContext` is undefined (and only reading `textContent`/calling `setTitleText` when the context exists).
```suggestion
			// If there's no TitleTextContext, there's nothing to sync and we
			// can avoid the DOM read.
			if ( ! titleTextContext ) {
				return;
			}
		...
		// sync above to avoid transiently clearing the value between runs.
		useLayoutEffect( () => {
			// No context means there's nothing to clear on unmount.
			if ( ! titleTextContext ) {
				return;
			}
...
#### ciampo on 2026-03-09T17:00:58Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906741668
Path: packages/ui/src/card/title.tsx
...
Applied early return, moved logic to hooks to better encapsulate it

### Review thread 6 (resolved)

#### copilot-pull-request-reviewer on 2026-03-09T17:06:55Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2906784676
Path: packages/ui/src/card/title.tsx
...
`Card.Title` docs say the `render` prop can swap in a semantic heading, but the new `internalRef` is typed as `HTMLDivElement`. If `render` outputs a non-`div` element, this ref type becomes inaccurate (and could encourage div-specific assumptions later). Consider typing the internal ref (and `useSyncTitleText`’s ref param) to `HTMLElement` (or `Element`) instead of `HTMLDivElement`, while keeping the public forwarded ref type as-is if you don’t want a breaking type change.

### Review thread 7

#### ntsekouras on 2026-03-10T10:14:57Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2910708375
Path: packages/ui/src/collapsible-card/header.tsx
...
#### ciampo on 2026-03-10T10:47:43Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2910885869
Path: packages/ui/src/collapsible-card/header.tsx
...
#### ciampo on 2026-03-10T11:34:29Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2911117237
Path: packages/ui/src/collapsible-card/header.tsx
...
#### ntsekouras on 2026-03-10T16:57:33Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2913155985
Path: packages/ui/src/collapsible-card/header.tsx
...
#### ciampo on 2026-03-11T10:32:51Z

URL: https://github.com/WordPress/gutenberg/pull/76329#discussion_r2917415055
Path: packages/ui/src/collapsible-card/header.tsx
...
I'll do a bit more research into seeing if we can drop the tooltip, especially in the context of [a separate `Card` exploration](https://github.com/WordPress/gutenberg/pull/76265) where dropping the tooltip is also being discused. 
...
- packages/ui/src/card/context.ts (ADDED, +8/-0)
- packages/ui/src/card/title.tsx (MODIFIED, +52/-4)
- packages/ui/src/collapsible-card/header.tsx (MODIFIED, +56/-5)
- packages/ui/src/collapsible-card/stories/index.story.tsx (MODIFIED, +43/-0)
- packages/ui/src/collapsible-card/test/index.test.tsx (MODIFIED, +83/-16)
