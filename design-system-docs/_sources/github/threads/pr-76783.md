...
type: pull_request
state: MERGED
merged_at: 2026-04-03T16:52:27Z
url: https://github.com/WordPress/gutenberg/pull/76783
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #76783: @wordpress/ui: Add global CSS defense module

- URL: https://github.com/WordPress/gutenberg/pull/76783
- Author: mirka
- Created: 2026-03-24T13:21:20Z
- Updated: 2026-04-03T16:53:08Z
- Comments: 4 of 4
- Review threads: 9 of 9
- Changed files: 17 of 17
- Diff: ../pr-diffs/76783.diff
...
Adds a shared, unlayered CSS module (`global-css-defense.module.css`) that shields `@wordpress/ui` components from wp-admin's global stylesheets (`common.css`, `forms.css`), and wires it into all currently affected components.

## Why?

`@wordpress/ui` places all component styles in CSS cascade layers. Per the spec, unlayered styles always win over layered styles regardless of specificity. Since wp-admin's global stylesheets are unlayered, they unconditionally override component styles — breaking borders, outlines, colors, focus rings, typography, and more.

This is a blocker for using `@wordpress/ui` in Gutenberg. While the problem itself is better addressed [upstream](https://core.trac.wordpress.org/ticket/64939), waiting for these changes to land in a WP release is going to be too slow for plugin developers to start adoption of `@wordpress/ui`. These plugins usually want to support at least two latest versions of WP, so that means they won't be able to use `@wordpress/ui` for another 6–12 months.

The solution proposed in this PR is pretty much the only solution I could come up with that unblocks us immediately, is reasonably manageable, and is easy to remove once upstream improvements make the workarounds unnecessary.

## How?

The defense module provides per-element classes (`.input`, `.textarea`, `.button`, `.div`, `.p`, `.heading`, `.a`) that are applied directly to the elements that need protection. Each class is unlayered, so it competes with admin CSS on equal footing and wins by specificity.

Properties are declared via a **custom property bridge** pattern:
...
- The fallback value is what makes sense for `@wordpress/ui`, so most components need zero overrides.
- When a component needs to set a specific value, it defines the custom property in its layered CSS module. The layered definition propagates through to the unlayered declaration because no admin style competes for the custom property.

## Testing Instructions

1. Run Storybook (`npm run storybook:dev`) and enable the WordPress global CSS mode via the toolbar.
2. Navigate to the InputControl, Textarea, Button, Link, and Text stories.
3. Verify that components render correctly and are not visually broken by admin styles (borders, colors, focus rings, typography).
4. Compare with the WordPress global CSS mode disabled — the components should look the same in both modes.
...
### github-actions on 2026-03-24T13:33:34Z

URL: https://github.com/WordPress/gutenberg/pull/76783#issuecomment-4118357826

**Size Change:** +2.31 kB (+0.03%) 

**Total Size:** 7.74 MB

| Filename | Size | Change |
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 340 kB | +836 B (+0.25%) |
| `build/scripts/edit-site/index.min.js` | 261 kB | +707 B (+0.27%) |
| `build/scripts/editor/index.min.js` | 411 kB | +763 B (+0.19%) |
...
### github-actions on 2026-03-24T14:09:22Z

URL: https://github.com/WordPress/gutenberg/pull/76783#issuecomment-4118614008
...
### github-actions on 2026-03-24T14:33:02Z

URL: https://github.com/WordPress/gutenberg/pull/76783#issuecomment-4118781749

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 9fbaf303a85d26cf13c2b7797de631e66dc492a6.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23952899915
📝  Reported issues:
- #76515 in `/test/e2e/specs/editor/various/pattern-overrides.spec.js`
...
### mirka on 2026-04-01T19:28:16Z

URL: https://github.com/WordPress/gutenberg/pull/76783#issuecomment-4172475611
...
The "safety profile" is no different than it was before with `@wordpress/components`. We could potentially set up some kind of mechanism where we are alerted when the upstream styles change, but not yet sure if something like that will be worth the trouble, especially considering signal/noise ratio. It wasn't all that necessary during our several years looking after `@wordpress/components`.

## Review threads

### Review thread 1 (resolved)

#### mirka on 2026-03-24T14:07:43Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2981863781
...
+
+	&:is([type="text"], [type="password"], [type="color"], [type="date"], [type="datetime"], [type="datetime-local"], [type="email"], [type="month"], [type="number"], [type="search"], [type="tel"], [type="time"], [type="url"], [type="week"], :not([type])) {
...
+	}
+
+	&:is([type="text"], [type="password"], [type="date"], [type="datetime"], [type="datetime-local"], [type="email"], [type="month"], [type="number"], [type="search"], [type="tel"], [type="time"], [type="url"], [type="week"], :not([type])) {
...
+
+.div {
+	outline: var(--_gcd-div-outline, 0);
+}
+
...
Any thoughts on how to deal with `margin` on the `Text` component? Currently if it's rendered as a `p` or heading, it just keeps the margins set by the UA styles. Might be better to standardize (e.g. margin 0 by default)?

#### ciampo on 2026-04-03T08:40:41Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3032020716
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Resolving this as the conversation continues in https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994200153

### Review thread 2 (resolved)

#### ciampo on 2026-03-26T11:21:30Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994143462
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Does this remove the transition from the focus ring?

In case, we could change it to
...
and have `focus.module.css` set `--_gcd-a-transition` alongside the existing `--_gcd-a-outline`

#### mirka on 2026-04-01T19:23:34Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024162143
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Good catch 4c0ab85036cfd1ee2d1b15041e50939c8b5676e5

### Review thread 3 (resolved)

#### ciampo on 2026-03-26T11:26:37Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994179136
...
This change may introduce a subtle unwanted effect: when `--_gcd-a-outline` / `--_gcd-div-outline` is not set (idle state), the fallback is `0`, which resolves to `outline: 0`, ie. that is `outline-width: 0; outline-style: none; outline-color: initial`. This differs from the original `outline-style: solid; outline-color: transparent` and may affect forced-colors mode (as the comment above pointed out)

#### mirka on 2026-04-01T19:24:59Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024170032
Path: packages/ui/src/utils/css/focus.module.css
...
Fixed in 9112619176c0bda389b52d2d6059daf48d9829ab.

### Review thread 4 (resolved)

#### ciampo on 2026-03-26T11:27:57Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994189399
Path: packages/ui/src/utils/css/focus.module.css
...
Is the reason why we only have dedicated vars for `a` and `div` because global form styles only target those elements? What about `button`s ?

#### mirka on 2026-04-01T17:49:00Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3023648503
Path: packages/ui/src/utils/css/focus.module.css
...
Correct. There are no global outline styles for unscoped `button` elements.

### Review thread 5 (resolved)

#### ciampo on 2026-03-26T11:28:24Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994192688
...
Very nitty, but should this import be moved up next to other CSS utils?

#### mirka on 2026-04-01T19:25:12Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024171416
Path: packages/ui/src/form/primitives/fieldset/description.tsx
...
Moved in 90488962fcb4beefd9925098b2dbdc6bcab6d124.

### Review thread 6

#### ciampo on 2026-03-26T11:29:27Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994200153
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
#### mirka on 2026-03-26T16:40:23Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2996271841
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
So [would you say that](https://github.com/WordPress/gutenberg/pull/76783/changes#r2981863781) the `Text` component should set `margin: 0`? It currently doesn't, so will have UA margins when rendered as `p` or `h*` (existing behavior unrelated to this defense file).

`p` is a bit of a tough call because I can imagine a lot of multi-paragraph `Text`. Like maybe there _should_ be a default vertical margin. `.text + p.text` selectors for margin-top on consecutive paragraphs? Maybe even a similar setup for `.text + :is(h1, h2, h3, h4, h5, h6).text`. An alternative may be to nudge people towards `Stack`.

@WordPress/gutenberg-design Do we have any specs on how vertical margins should be in body copy, between headings and paragraphs? We don't cover them in the [typography tokens doc](https://wordpress.github.io/gutenberg/?path=/docs/tokens-typography--page).

#### ciampo on 2026-03-27T16:24:52Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3001953133
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
#### jameskoster on 2026-03-31T13:01:28Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3015762176
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
As a next layer, we _could_ introduce something like a `TextBlock` component to encode paragraph spacing. But then there's the question of headings + paragraphs... The gap between a `h1` and a paragraph might be different to the gap between a `h4` and a paragraph. A heading following a paragraph might benefit from a top margin. And then there's the question of how to space consecutive `TextBlock`s 😅
...
#### mirka on 2026-04-01T18:59:36Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024035222
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
#### mirka on 2026-04-01T19:23:08Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024160099
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Changed the margin fallback to `0` in c1b91dd7fc6e6735d6448ece63097e59e8ecb05d.

### Review thread 7 (resolved)

#### ciampo on 2026-03-26T11:31:50Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r2994218228
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Is the `:not([type])` selector there to target `Textarea` ? Regardless, maybe it deserves a little inline comment to explain what's going on?

#### mirka on 2026-04-01T19:26:03Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3024176264
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
Right. I attempted to improve clarity of intent, and also added a comment in 05e28fb24f17c61443e564b751a611dec669a8d2.

### Review thread 8

#### ciampo on 2026-04-03T08:41:53Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3032024275
...
#### ciampo on 2026-04-03T08:44:22Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3032032088
Path: packages/ui/src/utils/css/global-css-defense.module.css
...
#### mirka on 2026-04-03T16:09:47Z

URL: https://github.com/WordPress/gutenberg/pull/76783#discussion_r3033428528
Path: packages/ui/src/utils/css/global-css-defense.module.css

Added section in 9fbaf303a85d26cf13c2b7797de631e66dc492a6.

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +4/-0)
- packages/ui/CONTRIBUTING.md (MODIFIED, +25/-0)
- packages/ui/src/button/button.tsx (MODIFIED, +2/-0)
- packages/ui/src/button/style.module.css (MODIFIED, +4/-0)
- packages/ui/src/form/primitives/field/description.tsx (MODIFIED, +6/-1)
- packages/ui/src/form/primitives/fieldset/description.tsx (MODIFIED, +9/-1)
- packages/ui/src/form/primitives/input-layout/input-layout.tsx (MODIFIED, +2/-0)
- packages/ui/src/form/primitives/input/input.tsx (MODIFIED, +6/-1)
- packages/ui/src/form/primitives/input/style.module.css (MODIFIED, +4/-0)
- packages/ui/src/form/primitives/textarea/textarea.tsx (MODIFIED, +10/-1)
- packages/ui/src/link/link.tsx (MODIFIED, +2/-0)
- packages/ui/src/link/style.module.css (MODIFIED, +10/-0)
- packages/ui/src/text/style.module.css (MODIFIED, +22/-0)
- packages/ui/src/text/text.tsx (MODIFIED, +8/-1)
- packages/ui/src/utils/css/field.module.css (MODIFIED, +4/-1)
- packages/ui/src/utils/css/focus.module.css (MODIFIED, +7/-5)
- packages/ui/src/utils/css/global-css-defense.module.css (ADDED, +117/-0)
