...
type: pull_request
state: MERGED
merged_at: 2025-11-11T07:46:30Z
url: https://github.com/WordPress/gutenberg/pull/73097
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Performance","[Package] Theme"]
---

# PR #73097: Theme: use logarithms for contrast difference

- URL: https://github.com/WordPress/gutenberg/pull/73097
- Author: jsnajdr
- Created: 2025-11-07T17:07:08Z
- Updated: 2025-11-11T09:27:36Z
- Comments: 6 of 6
- Review threads: 0 of 0
- Changed files: 6 of 6
- Diff: ../pr-diffs/73097.diff

## Body

This is another little optimization of the color ramps algorithm that speeds up the "background ramps" unit test from 1900ms to 1370ms, almost 30% improvement.

Before this patch we most often reached the target color in the 4th iteration, and the entire background ramp test (360 ramps) did 23809 color iterations:

| 1it | 2it | 3it | 4it | 5it | 6it | 7it | 8it | 9it | 10it | total |
|-----|-----|-----|-----|-----|-----|-----|-----|-----|------|--------|
| 69 | 441 | 1230 | 2322 | 1796 | 1118 | 51 | 107 | 1 | 4 |  **23809** |

After this patch we are most often finished in the 2nd iteration, and the total is reduced by 18%:


| 1it | 2it | 3it | 4it | 5it | 6it | 7it | 8it | 9it | 10it | total |
|-----|-----|-----|-----|-----|-----|-----|-----|-----|------|--------|
| 390 | 3138 | 1532 | 1583 | 248 | 114 | 0 | 0 | 0 | 4 | **19558** |

I achieved this my merely taking a logarithm of the contrast values before computing their difference.

Why does it matter? Look at this plot where we're searching for a color that has a small contrast (1.05) against black (luminance zero). Then we are bisecting between luminance values of 0 and 1. The 0 luminance has contrast 1 against the reference (the same black color), and the 1 luminance has maximum contrast 20.

<img width="1098" height="742" alt="color-iterations" src="https://github.com/user-attachments/assets/d5d20b5e-0cb5-4c35-a727-010173dc2011" />

When we make a straight line between these two points, it crosses the L axis very close to zero, because the contrast value 20 is so high. It doesn't get better as we iterate, so we converge very slowly.

The luminance vs contrast relationship is not linear, it's more like an exponential. Taking a logarithm straightens the line out, so bisection starts to give much better estimates of the final value. The calculation converges much faster.

## Issue comments

### github-actions on 2025-11-07T17:36:28Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3503852271

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 40f24af1652de487fd446985ba65f15770027a0d.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/19175567684
📝  Reported issues:
- #71211 in `/test/e2e/specs/editor/various/block-hierarchy-navigation.spec.js`

### aduth on 2025-11-07T18:39:14Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3504194966

It looks like there are some changes to the prebuilt files that need to be committed. I'm assuming these changes are expected? The differences look quite small.

```
npm run --workspace @wordpress/theme build
```

### github-actions on 2025-11-07T18:39:24Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3504195580
...
### github-actions on 2025-11-08T09:46:31Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3506320218
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 79.6 kB | -2 B (0%) |
| `build/scripts/theme/index.min.js` | 21.5 kB | +26 B (+0.12%) |
...
### jsnajdr on 2025-11-10T10:39:58Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3510774155

> It looks like there are some changes to the prebuilt files that need to be committed.

That's right, I pushed a commit with the missing updates 👍

### jsnajdr on 2025-11-11T09:27:36Z

URL: https://github.com/WordPress/gutenberg/pull/73097#issuecomment-3515818423

> Am I correct in understanding that the slight differences in the colors produced here are because the changes impact the "jumps" we attempt when trying different candidate colors?

I traced the calculation for the `surface4` color for the background ramp. With the logarithm method the resulting color changed from `#eaeaea` to `#ebebeb`. Why?

The `surface4` color is supposed to have contrast 1.13 against the seed color, which is `#f8f8f8`. And we're aiming for contrast precision 0.004 (`CONTRAST_EPSILON`). Once we find color that has contrast between 1.126 and 1.134, that's the result.

The original (linear) algorithm needed 6 iterations to find lighness 93.790 (in OKLCH space). It has contrast 1.1299. Very precise, the difference is just 0.0001, way below the required precision. The 5th iteration still wasn't precise enough, it was 0.008 above the target, but then the last iteration nailed it very well.

Lightness 93.790 maps to RGB color `#eaeaea`. In OKLCH, this color's lightness is 93.705, close to our calculated value.

The new (logarithmic) algorithm finds a good solution in just one (!) iteration. The lightness is 93.903, and the contrast is 1.12604. That's very very close to the limit, but still good. Difference 0.00396.

Lightness 93.902 maps to RGB color `#ebebeb`. This color's lightness is 94.007.

One step in hex RGB (1/255) corresponds to a 0.3 difference in lightness. And our contrast precision (0.004) corresponds to a 0.2 difference in lightness. So it makes sense that the calculated colors often differ by one RGB step.
...
- packages/theme/src/color-ramps/lib/find-color-with-constraints.ts (MODIFIED, +22/-11)
- packages/theme/src/color-ramps/test/__snapshots__/index.test.ts.snap (MODIFIED, +2307/-2307)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +50/-50)
- packages/theme/src/prebuilt/json/figma.json (MODIFIED, +50/-50)
- packages/theme/tokens/color.json (MODIFIED, +50/-50)
