...
type: pull_request
state: MERGED
merged_at: 2023-09-12T10:30:13Z
url: https://github.com/WordPress/gutenberg/pull/54286
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #54286: Popover: remove custom frame scroll/resize listeners

- URL: https://github.com/WordPress/gutenberg/pull/54286
- Author: jsnajdr
- Created: 2023-09-08T07:47:16Z
- Updated: 2023-09-12T17:01:26Z
- Comments: 9 of 9
- Review threads: 1 of 1
- Changed files: 4 of 4
- Diff: ../pr-diffs/54286.diff
...
### github-actions on 2023-09-08T08:23:55Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1711275950

<!-- flaky-tests-report-comment -->
**Flaky tests detected in af40356c4cffd5c46b505e4337d066ccb45851cb.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/6119114278
📝  Reported issues:
- #53524 in `/test/e2e/specs/editor/various/multi-block-selection.spec.js`

### github-actions on 2023-09-11T10:56:58Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1713648498
...
| :--- | :---: | :---: |
| `build/components/index.min.js` | 255 kB | -218 B (0%) |
...
### jsnajdr on 2023-09-11T11:01:17Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1713654411

The `getOverflowAncestors` fix has been released as `@floating-ui/utils@0.1.2`, and this PR is amended to upgrade to the new version. Ready to review and merge.

**How to test:**
The most affected part of the UI is the zoom out mode in site editor:
1. Enable the zoomed-out mode experiment in Gutenberg settings
2. Go to the site editor and start editing some template
3. Click on the "Zoom-out View" button in toolbar
4. On the edited page, verify that the "Add pattern" inserter buttons scroll with the content

### jsnajdr on 2023-09-11T11:51:03Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1713723343

> I was just curious - should there be a reliable way to test this in storybook too?

Floating UI has "visual" tests, and for my patch I added only a unit test. A visual test where you can test that it really works in a real browser could be a useful follow-up.

In Gutenberg's storybook? I'm not sure, we currently don't have any stories for iframes explicitly, do we?

### tyxla on 2023-09-11T12:08:55Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1713749486

> In Gutenberg's storybook? I'm not sure, we currently don't have any stories for iframes explicitly, do we?

We seem to have at least one: https://wordpress.github.io/gutenberg/?path=/story/components-popover--with-slot-outside-iframe

### jsnajdr on 2023-09-12T09:28:46Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1715352400

> the popover is places correctly only when the toolbar and the sidebar are hidden. I'd be curious to know why that is still the case

Thanks for pointing this out, the popover is really quite broken in the storybook. I'll try to find out why.

### jsnajdr on 2023-09-12T10:29:56Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1715464642

It's happening because the iframe offset is taken into account only when calculating the client rect for the _reference_ (anchor) element, but not for the _floating_ (popover) element.

In Gutenberg only the anchor element is ever in an iframe, the popover is always in the top document. So the bug is not visible. But Storybook renders the entire story, with all elements, inside a "preview iframe". That iframe has an offset that is not applied to the popover element, only the anchor.

I'm now going to merge this PR because it's not really related to this situation. We should expand the iframe story to also include the case when the iframe's parent element is scrolling, but let's do it in a different PR, after the positioning gets fixed. If we did it now, we'd be testing something that's already broken.

### ciampo on 2023-09-12T10:34:26Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1715471111

Agreed 100% with your opinion and your suggested next steps. Thank you!

### ciampo on 2023-09-12T17:01:25Z

URL: https://github.com/WordPress/gutenberg/pull/54286#issuecomment-1716109561

Also noting that Tooltips (using ariakit, and therefore floating UI) are currently facing the same issue in Storybook

![Screenshot 2023-09-12 at 18 59 52](https://github.com/WordPress/gutenberg/assets/1083581/c9002a93-8b71-48d1-93fa-185bad55611f)

https://wordpress.github.io/gutenberg/?path=/story/components-tooltip--default

## Review threads

### Review thread 1 (resolved)

#### tyxla on 2023-09-11T11:13:16Z

URL: https://github.com/WordPress/gutenberg/pull/54286#discussion_r1321383863
...
Still testing/reviewing, but in the meantime, here's a minor fix to make the CHANGELOG check happy:

```suggestion
-   `Popover`: Remove `scroll` and `resize` listeners for iframe overflow parents and rely on recently added native Floating UI support ([#54286](https://github.com/WordPress/gutenberg/pull/54286)).
```

#### jsnajdr on 2023-09-11T11:19:14Z

URL: https://github.com/WordPress/gutenberg/pull/54286#discussion_r1321390466
Path: packages/components/CHANGELOG.md
...
- packages/components/src/popover/index.tsx (MODIFIED, +0/-43)
- packages/components/src/popover/utils.ts (MODIFIED, +17/-68)
