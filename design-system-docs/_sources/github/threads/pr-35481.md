...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/35481
matched_labels: ["[Package] Components"]
all_labels: ["[Package] Components"]
---

# PR #35481: Input Control: Set a color

- URL: https://github.com/WordPress/gutenberg/pull/35481
- Author: scruffian
- Created: 2021-10-08T20:15:19Z
- Updated: 2021-12-14T12:04:16Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 1 of 1
- Diff: ../pr-diffs/35481.diff
...
### github-actions on 2021-10-08T20:21:36Z

URL: https://github.com/WordPress/gutenberg/pull/35481#issuecomment-939094945
...
| `build/block-library/blocks/shortcode/editor-rtl.css` | 491 B | +17 B (+4%) |
| `build/block-library/blocks/shortcode/editor.css` | 491 B | +17 B (+4%) |
| `build/block-library/editor-rtl.css` | 9.91 kB | +11 B (0%) |
| `build/block-library/editor.css` | 9.9 kB | +11 B (0%) |
| `build/components/style-rtl.css` | 15.3 kB | +11 B (0%) |
| `build/components/style.css` | 15.3 kB | +10 B (0%) |
| `build/editor/style-rtl.css` | 3.77 kB | +7 B (0%) |
| `build/editor/style.css` | 3.76 kB | +7 B (0%) |
...
### mirka on 2021-11-01T23:14:39Z

URL: https://github.com/WordPress/gutenberg/pull/35481#issuecomment-956852673

> I'm not super keen on this solution as it feels very heavy handed.

I'm afraid I agree, this would be a breaking change on a rather widely-used mixin.

I don't know what the "videomaker theme" is so I can't talk about it specifically, but would it be unreasonable to expect theme authors to not add such broad rules (e.g. `input { color: white; }`) to their style-editor.css if they don't mean it? And what if they _do_ mean it, and want to change the input styles even inside block placeholders?

I kind of think the responsibility/flexibility is in the hands of the theme author. They can scope the `input` rule appropriately, or reset the color inside the `.components-placeholder` scope.

### scruffian on 2021-11-01T23:42:41Z

URL: https://github.com/WordPress/gutenberg/pull/35481#issuecomment-956906988

What scope should theme authors to target `input` elements if they want consistent styling across form elements in their theme?

### mirka on 2021-11-02T02:09:43Z

URL: https://github.com/WordPress/gutenberg/pull/35481#issuecomment-957034239

> What scope should theme authors to target `input` elements if they want consistent styling across form elements in their theme?

Until there is a [clear contract](https://github.com/WordPress/gutenberg/issues/29167#issuecomment-953005663) between block authors and theme authors, I don't think there's a foolproof way that will work as expected for every block. Especially when third-party blocks are involved, there's no way to tell programmatically whether a given `input` is safe to be themed in a certain way. (Not just color, but size, borders, etc.)

Under the current circumstances, I would scope it under specific `wp-block-*` classes that I have manually verified to work. Not saying that is ideal, but personally that is how I've been approaching it. I think that's the only way to ensure that your theming is appropriate/sufficient.

In your input color case specifically, another option might be for the theme to set `input { color: inherit; }`.

### scruffian on 2021-12-14T12:04:13Z

URL: https://github.com/WordPress/gutenberg/pull/35481#issuecomment-993472835

Closing in favour of https://github.com/WordPress/gutenberg/pull/33494
