...
type: pull_request
state: MERGED
merged_at: 2025-12-30T13:14:40Z
url: https://github.com/WordPress/gutenberg/pull/74226
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Tool] stylelint config","[Package] Theme"]
---

# PR #74226: Stylelint: Add design token linting

- URL: https://github.com/WordPress/gutenberg/pull/74226
- Author: mirka
- Created: 2025-12-25T21:07:30Z
- Updated: 2025-12-30T16:40:16Z
- Comments: 2 of 2
- Review threads: 5 of 5
- Changed files: 21 of 21
- Diff: ../pr-diffs/74226.diff

## Body

## What?

Adds custom linters for misuse of design tokens.

## Why?

Here we set up two new stylelint linters to:

- Catch references to non-existent design tokens.
- Prevent reassignments of design token variables.

It's quite easy to refer to non-existent design tokens in CSS by mistake, so a linter makes it much easier to catch these early. Manual reassignments of design token variables should never be done, so we catch these as well.

## How?

I think we should add these linters to the public [`@wordpress/stylelint-config`](https://www.npmjs.com/package/@wordpress/stylelint-config) package, rather than keep them an internal thing. They are as relevant to any WordPress ecosystem developer if they do stuff with tokens, and it shouldn't affect them at all if they don't.

### Testing Instructions

I've added unit tests, but you can also try it out on some stylesheets in the codebase.

## Issue comments

### github-actions on 2025-12-25T21:07:40Z

URL: https://github.com/WordPress/gutenberg/pull/74226#issuecomment-3691745554
...
### github-actions on 2025-12-25T21:20:23Z

URL: https://github.com/WordPress/gutenberg/pull/74226#issuecomment-3691753790
...
#### mirka on 2025-12-25T21:36:08Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2647276960
Path: packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.js
...
@ciampo What do you think about this wording? Too long? I felt the "(public)" might be a bit confusing without context.

```suggestion
		`The following CSS variables are not valid Design System tokens, or are private tokens that should not be used directly: ${ tokenNames }`,
```

#### ciampo on 2025-12-29T14:55:42Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651149245
Path: packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.js
...
I think in this context, longer and clearer is better 👍

#### mirka on 2025-12-29T22:18:44Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651865632
Path: packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.js
...
Wait, I was looking at the `@wordpress/theme` code, and I'm not seeing any private tokens being exposed anymore. Is that correct? Can we remove the mention of private tokens here?

#### ciampo on 2025-12-30T09:33:08Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2652603069
Path: packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.js
...
You're right! Probably no need to mention the private part. Probably a change that was applied while I was away?

### Review thread 2

#### mirka on 2025-12-25T22:10:43Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2647288929
Path: packages/theme/src/stylelint-plugins/test/utils/index.ts

Copied from the test utilities in the [`stylelint-plugins`](https://github.com/WordPress/gutenberg/blob/trunk/packages/stylelint-config/test/utils/index.js) package. Probably not worth setting up any code sharing?

#### ciampo on 2025-12-29T14:57:37Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651153055
Path: packages/theme/src/stylelint-plugins/test/utils/index.ts

Agreed. We could at most leave a code comment linking to the source of inspiration

### Review thread 3

#### mirka on 2025-12-25T22:15:39Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2647290494
...
+		"./stylelint-plugins/no-unknown-ds-tokens": "./src/stylelint-plugins/no-unknown-ds-tokens.js",
+		"./stylelint-plugins/no-setting-wpds-custom-properties": "./src/stylelint-plugins/no-setting-wpds-custom-properties.js",
```

I thought about simply having these plugins live in the `stylelint-config` package since the testing infrastructure is already there, but alas people need to be able to use these lint plugins regardless of whether they use `@wordpress/stylelint-config`.

### Review thread 4 (resolved)

#### SirLouen on 2025-12-26T20:13:36Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2648629437
Path: packages/theme/package.json
...
```suggestion
		"./stylelint-plugins/*": "./src/stylelint-plugins/*.js",
```

#### ciampo on 2025-12-29T15:04:13Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651167384
Path: packages/theme/package.json
...
From the [official docs](https://nodejs.org/api/packages.html#subpath-patterns):
...
#### SirLouen on 2025-12-29T15:53:32Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651259678
Path: packages/theme/package.json
...
Ok, in that case, maybe it should grow a bit before adding this wildcard.

#### mirka on 2025-12-29T22:29:47Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2651878496
Path: packages/theme/package.json
...
Hm right, I wasn't thinking too deeply about this, but yes, I guess we can combine if it grows. At the moment I don't expect there to be more than these two though.

### Review thread 5

#### aduth on 2025-12-30T13:40:39Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2653035390
Path: packages/theme/package.json
...
Something I'm noticing as I'm resolving conflicts in #74129: Should these have been written as `.ts` files and we reference the package exports from the `build` directory?

#### mirka on 2025-12-30T16:23:32Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2653346597
Path: packages/theme/package.json
...
Just saw [this](https://github.com/WordPress/gutenberg/pull/74129#issuecomment-3699538280). What would be your preference here? No strong opinion on my end. Could be simpler to just split out a separate package for the linters, even.

#### aduth on 2025-12-30T16:40:16Z

URL: https://github.com/WordPress/gutenberg/pull/74226#discussion_r2653398374
Path: packages/theme/package.json
...
I was able to unblock that work by reverting back to the `src/` file, but that basically just restores how you'd implemented this. But that's a good indication of the complications we'll encounter and need to address if we switch to using TypeScript.

## Changed files

- .stylelintignore (MODIFIED, +1/-0)
- package-lock.json (MODIFIED, +8/-1)
- packages/stylelint-config/CHANGELOG.md (MODIFIED, +4/-0)
- packages/stylelint-config/index.js (MODIFIED, +6/-0)
- packages/stylelint-config/package.json (MODIFIED, +1/-0)
- packages/theme/CHANGELOG.md (MODIFIED, +1/-0)
- packages/theme/README.md (MODIFIED, +54/-0)
- packages/theme/package.json (MODIFIED, +9/-1)
- packages/theme/src/stylelint-plugins/no-setting-wpds-custom-properties.js (ADDED, +45/-0)
- packages/theme/src/stylelint-plugins/no-unknown-ds-tokens.js (ADDED, +101/-0)
- packages/theme/src/stylelint-plugins/test/.stylelintignore (ADDED, +1/-0)
- packages/theme/src/stylelint-plugins/test/.stylelintrc.json (ADDED, +10/-0)
- packages/theme/src/stylelint-plugins/test/__snapshots__/no-setting-wpds-custom-properties.test.ts.snap (ADDED, +24/-0)
- packages/theme/src/stylelint-plugins/test/__snapshots__/no-unknown-ds-tokens.test.ts.snap (ADDED, +24/-0)
- packages/theme/src/stylelint-plugins/test/fixtures/no-setting-wpds-custom-properties-invalid.css (ADDED, +9/-0)
- packages/theme/src/stylelint-plugins/test/fixtures/no-setting-wpds-custom-properties-valid.css (ADDED, +12/-0)
- packages/theme/src/stylelint-plugins/test/fixtures/no-unknown-ds-tokens-invalid.css (ADDED, +9/-0)
- packages/theme/src/stylelint-plugins/test/fixtures/no-unknown-ds-tokens-valid.css (ADDED, +19/-0)
- packages/theme/src/stylelint-plugins/test/no-setting-wpds-custom-properties.test.ts (ADDED, +47/-0)
- packages/theme/src/stylelint-plugins/test/no-unknown-ds-tokens.test.ts (ADDED, +47/-0)
- packages/theme/src/stylelint-plugins/test/utils/index.ts (ADDED, +29/-0)
