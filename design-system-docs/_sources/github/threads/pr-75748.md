...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/75748
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Bug","[Package] Theme"]
---

# PR #75748: Theme: Fix TypeScript 5.9 type error in generate-primitive-tokens script

- URL: https://github.com/WordPress/gutenberg/pull/75748
- Author: jeryj
- Created: 2026-02-19T20:16:24Z
- Updated: 2026-02-24T14:04:28Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 2 of 2
- Diff: ../pr-diffs/75748.diff
...
### github-actions on 2026-02-19T20:16:36Z

URL: https://github.com/WordPress/gutenberg/pull/75748#issuecomment-3929719417
...
### github-actions on 2026-02-19T20:28:01Z

URL: https://github.com/WordPress/gutenberg/pull/75748#issuecomment-3929795194
...
### aduth on 2026-02-23T20:29:52Z

URL: https://github.com/WordPress/gutenberg/pull/75748#issuecomment-3947144649
...
Separately, I wonder how come we don't have any feedback to this issue in our existing CI setup? Should we be ensuring these types build clean via `tsc --build`, and how come that doesn't already happen in our existing tests?

Regarding "the theme package's declaration files (index.d.ts etc.) were never emitted", this had me concerned if it was included in the published package (a bigger issue), but they appear to be there ([source](https://unpkg.com/@wordpress/theme@0.7.0/build-types/index.d.ts)). I guess the next question would be: Why is it different for `tsc --build` vs. our normal build process, and should we care about the former if we don't actually use it for anything?

For what it's worth, I can't actually verify the original issue. `tsc --build` passes fine for me locally on `trunk` after running `npm run clean:package-types`. I also don't see any errors surfaced in my IDE, and `tokenValue` is typed as `string` without the type-casting.

### jsnajdr on 2026-02-24T08:36:31Z

URL: https://github.com/WordPress/gutenberg/pull/75748#issuecomment-3950055056

I cannot reproduce the original issue. We use TypeScript 5.9.3 and there is no type error on the `transformColorStringToDTCGValue` call, the `tokenValue` is a `string`. Neither the VSCode IDE nor CLI `tsc --build` report an error.

Can you provide more details about what we're actually fixing here? `tsc --build` is also a part of the `npm run build` command (the "📘 Building TypeScript types" step), so if there was really an obvious type error there, all builds would fail, both locally and in CI.

### jeryj on 2026-02-24T14:04:28Z

URL: https://github.com/WordPress/gutenberg/pull/75748#issuecomment-3952136567
...
- packages/theme/bin/generate-primitive-tokens/index.ts (MODIFIED, +3/-1)
