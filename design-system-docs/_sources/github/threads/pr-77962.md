...
type: pull_request
state: MERGED
merged_at: 2026-05-12T16:21:48Z
url: https://github.com/WordPress/gutenberg/pull/77962
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Focus] Accessibility (a11y)","[Package] UI","[Feature] Guidelines"]
---

# PR #77962: ui/CollapsibleCard: support rendering Header as a heading element

- URL: https://github.com/WordPress/gutenberg/pull/77962
- Author: t-hamano
- Created: 2026-05-05T14:22:35Z
- Updated: 2026-05-12T16:22:33Z
- Comments: 7 of 7
- Review threads: 3 of 3
- Changed files: 6 of 6
- Diff: ../pr-diffs/77962.diff
...
### github-actions on 2026-05-05T14:30:45Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4380259090
...
### github-actions on 2026-05-05T14:32:06Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4380269810
...
| :--- | :---: | :---: |
| `build/modules/content-types/index.min.js` | 136 kB | +101 B (+0.07%) |
| `build/scripts/block-editor/index.min.js` | 343 kB | +38 B (+0.01%) |
| `build/scripts/edit-site/index.min.js` | 265 kB | +64 B (+0.02%) |
| `build/scripts/editor/index.min.js` | 428 kB | +46 B (+0.01%) |
...
### github-actions on 2026-05-05T15:08:11Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4380554679

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 6d396f8cfbcf4549726e13ba5f42edfa69aa1d4a.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25745235412
📝  Reported issues:
- #77710 in `/test/e2e/specs/widgets/editing-widgets.spec.js`

### ciampo on 2026-05-06T15:52:15Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4389799411

@t-hamano @mirka I pushed changes with a new, cleaner, proposed approach:

- default-wrap the header in a `h3`
- different HTML elements can be specified direclty via `render` prop, instead of a new custom prop
- Global defense styles ship with the component directly

Let me know what you think

### t-hamano on 2026-05-07T09:36:05Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4395940272

@ciampo Thanks for the update!

> default-wrap the header in a `h3`

Consumers may not necessarily place this component after an `h2`. Defaulting to a specific level could lead to an improperly formed document outline. Similar to many other components, How about defaulting to a `div`?

### ciampo on 2026-05-07T14:01:00Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4397784395

@t-hamano fair point! I updated the PR , `CollapsibleCard.Header` now renders an outer `div` by default

### t-hamano on 2026-05-12T09:52:23Z

URL: https://github.com/WordPress/gutenberg/pull/77962#issuecomment-4429312737
...
#### ciampo on 2026-05-05T15:53:10Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3189814840
Path: packages/ui/src/collapsible-card/stories/index.story.tsx

```diff
@@ -159,6 +159,33 @@ export const Stacked: Story = {
 	),
 };
...
It looks like there are more `common.css` styles that need overriding:

<img width="928" height="434" alt="Image" src="https://github.com/user-attachments/assets/e7b4423f-94e6-44aa-99d2-157a7402ea69" />

We should probably re-specify `color`, `font-size`, and `font-weight` at least. Maybe we can get away with adding them to the internal `CollapsibleCard.Header` styles? Some of those rules (ie. `color`) are currently specified on `Card.Root`

Although I'm still not in love with the fact that we have to manually specify `margin: 0` , which makes me wonder if we should re-consider adding a dedicated prop instead. @mirka , what do you think?

#### mirka on 2026-05-05T19:49:52Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3191147422
Path: packages/ui/src/collapsible-card/stories/index.story.tsx
...
> we should re-consider adding a dedicated prop instead

Yes, we should look into that. This is getting a bit too weird.

#### ciampo on 2026-05-06T09:05:22Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3194202971
Path: packages/ui/src/collapsible-card/stories/index.story.tsx
...
@t-hamano do you mind trying out a dedicated prop — something like `headingLevel`?

It could default to undefined but, when a number is specified, it switches to wrapping the whole header in heading tag, with the correct style overrides?

#### t-hamano on 2026-05-06T11:36:23Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3195097838
Path: packages/ui/src/collapsible-card/stories/index.story.tsx
...
While researching this matter, I discovered that Base UI not only offers Collapsible but also [Accordion](https://base-ui.com/react/components/accordion). It seems to be the `Accordion.Header` component natively supports heading elements by default, which can be changed to any level via a render prop.

Furthermore, it includes exclusive control within groups and keyboard navigation between items, making it appear to be the perfect component for guideline pages.

I am interested in whether there are plans to add an Accordion component to @wordpress/ui 🤔

#### ciampo on 2026-05-06T13:39:14Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3195850456
Path: packages/ui/src/collapsible-card/stories/index.story.tsx
...
I can look into it, and understand if `Accordion` would be a better foundation for `CollapsibleCard` than `Collapsible`

#### ciampo on 2026-05-06T15:18:38Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3196443084
Path: packages/ui/src/collapsible-card/stories/index.story.tsx
...
I looked into it, and I don't think `Accordion` is a good fit. Its value sits mostly in coordinating groups of expanding/collapsing components, which doesn't apply to `CollapsibleCard`, since it's meant to be consumed and expanded/collapsed independently.

I will push some code changes in this branch to try out a more integrated approach, which also takes inspiration from how `Accordion` defaults to `h3`

### Review thread 2 (resolved)

#### aduth on 2026-05-11T17:29:46Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3220873321
Path: packages/ui/src/collapsible-card/header.tsx

```diff
@@ -15,13 +16,19 @@ import type { HeaderProps } from './types';
...
Do we want this change? It still renders a `div` ? 

Or is this needed to support additional props being passed if overridden to something other than a `div` ? In which case, would we need to revisit all other components to generalize the `forwardRef` generic?

#### aduth on 2026-05-11T17:35:41Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3220913135
Path: packages/ui/src/collapsible-card/header.tsx
...
>Or is this needed to support additional props being passed if overridden to something other than a `div` ?

Although in this case, wouldn't we want to encourage those props to be applied on the `render` element instead?

e.g.

```tsx
<CollapsibleCard.Header render={ <h2 align="left" /> } />
```

(ref: https://developer.mozilla.org/en-US/docs/Web/API/HTMLHeadingElement#instance_properties)

#### ciampo on 2026-05-11T18:40:03Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3221289879
Path: packages/ui/src/collapsible-card/header.tsx
...
Fair point, I think we can revert to `HTMLDivElement`

### Review thread 3 (resolved)

#### aduth on 2026-05-11T17:30:15Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3220876535
Path: packages/ui/src/collapsible-card/header.tsx
...
I don't see that we're doing anything special with `style` that we need to pick it out of the props, vs. just letting it apply through `restProps` ?

#### ciampo on 2026-05-11T18:40:15Z

URL: https://github.com/WordPress/gutenberg/pull/77962#discussion_r3221291669
Path: packages/ui/src/collapsible-card/header.tsx
...
Also fair point, it could be a leftover from a previous attempt in the  PR

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +2/-0)
- packages/ui/src/collapsible-card/header.tsx (MODIFIED, +55/-42)
- packages/ui/src/collapsible-card/stories/index.story.tsx (MODIFIED, +55/-0)
- packages/ui/src/collapsible-card/style.module.css (MODIFIED, +20/-0)
- packages/ui/src/collapsible-card/test/index.test.tsx (MODIFIED, +60/-1)
- routes/guidelines/components/guideline-accordion.tsx (MODIFIED, +1/-1)
