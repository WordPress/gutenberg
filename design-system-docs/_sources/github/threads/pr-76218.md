...
type: pull_request
state: MERGED
merged_at: 2026-03-17T15:57:38Z
url: https://github.com/WordPress/gutenberg/pull/76218
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Code Quality","[Package] UI"]
---

# PR #76218: UI: Use `--wpds-cursor-control` design token

- URL: https://github.com/WordPress/gutenberg/pull/76218
- Author: mirka
- Created: 2026-03-05T21:20:46Z
- Updated: 2026-03-17T15:58:27Z
- Comments: 6 of 6
- Review threads: 0 of 0
- Changed files: 5 of 5
- Diff: ../pr-diffs/76218.diff
...
Uses the `--wpds-cursor-control` design token for all non-link interactive controls in the `@wordpress/ui` package, and adds a stylelint rule there to prevent new `cursor: pointer` declarations from being introduced.

## Why?

The `--wpds-cursor-control` token (added in #75697) allows the cursor style for interactive non-link controls to be configured via the design system. This PR adopts it in the one applicable place in `@wordpress/ui` (the Tabs component), and adds a lint rule so future components use the token instead of hardcoding `cursor: pointer`.

This is the first step towards repo-wide adoption. Next will be `@wordpress/components`.

## How?

- Replaced `cursor: pointer` with `var(--wpds-cursor-control)` in `tabs/style.module.css`.
- Created `packages/ui/.stylelintrc.mjs` with a `declaration-property-value-disallowed-list` rule that flags `cursor: pointer`, using a custom message function to guide toward the token.
- Added a `stylelint-disable-next-line` comment in `button/style.module.css` for the `&[href]` selector, where `cursor: pointer` is intentional (links should always use pointer).

## Testing Instructions

1. Run `npm run storybook:dev` and check the Tabs stories (the token defaults to `default`).
2. Verify the lint rule works:
   ```
   npx stylelint --config packages/ui/.stylelintrc.mjs "packages/ui/src/**/*.css"
   ```
   Should pass with no errors. Adding a `cursor: pointer` to any `@wordpress/ui` CSS file should trigger the rule.

## Issue comments

### github-actions on 2026-03-05T21:33:54Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4007936933
...
| `build/scripts/editor/index.min.js` | 353 kB |
...
### github-actions on 2026-03-05T21:39:08Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4007964834
...
### github-actions on 2026-03-05T21:46:33Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4008010118

<!-- flaky-tests-report-comment -->
**Flaky tests detected in f46ca3f08e72bbdfc047d58bb48c51f774f67bdc.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22737290393
📝  Reported issues:
- #52687 in `/test/e2e/specs/editor/various/post-editor-template-mode.spec.js`

### im3dabasia on 2026-03-09T15:05:35Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4024432882

> This is the first step towards repo-wide adoption. Next will be @wordpress/components.

@mirka , Thanks for the work and discussion around this issue.

I was looking into it with the intention of contributing, but it seems that addressing it properly would require a fair amount of work, including significant testing and potentially updating existing tests to avoid regressions.

Since this relates to the `@wordpress/components` package, I wanted to ask whether it makes sense to invest that level of effort here, given that it’s expected to eventually be superceded by `@wordpress/ui`. If the change would still provide value in the interim or benefit other packages, I’d be happy to explore it further.

Just wanted to check on the direction before putting in the required time and effort.

### mirka on 2026-03-11T16:58:13Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4040677842

> Since this relates to the `@wordpress/components` package, I wanted to ask whether it makes sense to invest that level of effort here, given that it’s expected to eventually be superceded by `@wordpress/ui`.

@im3dabasia Thank you for volunteering, and great question. Yes, it's important that these cursor settings be consistent across the app, so the idea is to truly migrate everything to a single token. I'm still working on the early implications of this change, so I will handle it up to `@wordpress/components`, and then open the task up to more contributors for the other packages.

### mirka on 2026-03-11T17:02:46Z

URL: https://github.com/WordPress/gutenberg/pull/76218#issuecomment-4040708045
...
Lol I completely forgot about this part 💀 Added now.

I'll follow up with a `ThemeProvider` integration (#76410), but for now this snippet should work for quick Storybook testing (won't work across portaled popovers until the `ThemeProvider` integration is in place):

```diff
diff --git a/storybook/decorators/with-design-system-theme.tsx b/storybook/decorators/with-design-system-theme.tsx
index 36c7cf67aff..9a058b26e3f 100644
--- a/storybook/decorators/with-design-system-theme.tsx
+++ b/storybook/decorators/with-design-system-theme.tsx
@@ -56,7 +56,7 @@ export function WithDesignSystemTheme(
...
- packages/ui/.stylelintrc.mjs (ADDED, +18/-0)
- packages/ui/src/button/style.module.css (MODIFIED, +2/-0)
- packages/ui/src/tabs/style.module.css (MODIFIED, +1/-1)
- packages/ui/src/utils/css/item-popup.module.css (MODIFIED, +1/-0)
- packages/ui/src/utils/css/select-trigger.module.css (MODIFIED, +1/-0)
