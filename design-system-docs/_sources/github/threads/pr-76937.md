...
type: pull_request
state: MERGED
merged_at: 2026-04-08T11:16:28Z
url: https://github.com/WordPress/gutenberg/pull/76937
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #76937: ui/AlertDialog: better async confirm APIs, fully use base ui's `AlertDialog`

- URL: https://github.com/WordPress/gutenberg/pull/76937
- Author: ciampo
- Created: 2026-03-31T16:48:42Z
- Updated: 2026-04-08T11:17:07Z
- Comments: 3 of 3
- Review threads: 6 of 6
- Changed files: 9 of 9
- Diff: ../pr-diffs/76937.diff
...
### github-actions on 2026-03-31T17:02:14Z

URL: https://github.com/WordPress/gutenberg/pull/76937#issuecomment-4164096267
...
| `build/scripts/edit-site/index.min.js` | 262 kB | +1 B (0%) |
...
### github-actions on 2026-03-31T17:19:05Z

URL: https://github.com/WordPress/gutenberg/pull/76937#issuecomment-4164185949

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 6f9e59052d31d303c9c18b676dc4be24c2e67dbd.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24130521432
📝  Reported issues:
...
### github-actions on 2026-04-02T15:20:30Z

URL: https://github.com/WordPress/gutenberg/pull/76937#issuecomment-4178642247
...
#### ciampo on 2026-04-02T11:34:26Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3027513280
Path: packages/ui/src/text/style.module.css

Changes to `Text` should be moved to a separate PR, but I included them for now to improve the example spacing

#### aduth on 2026-04-02T21:08:36Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3030389041
Path: packages/ui/src/text/style.module.css

This is being handled in #76970.

#### ciampo on 2026-04-03T07:43:48Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3031851655
Path: packages/ui/src/text/style.module.css

With that PR merged, I rebased and removed `Text` changes from this PR

### Review thread 2 (resolved)

#### aduth on 2026-04-02T21:07:00Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3030381762
Path: packages/ui/src/alert-dialog/root.tsx

```diff
@@ -11,32 +27,148 @@ import type { RootProps } from './types';
...
+
+	// Generation counter to discard stale async completions. Incremented
+	// on every `confirm()` call and when the dialog finishes closing.
+	// A stale promise settling will see a mismatched id and be ignored.
...
+
+			// Discard if a newer confirm has started or the dialog was
+			// dismissed and reopened while we were awaiting.
...
If we just `console.error`'d here, couldn't we get rid of `handleConfirm` memoized callback in `Popup` and use `confirm` context directly?

#### ciampo on 2026-04-03T07:43:26Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3031850625
Path: packages/ui/src/alert-dialog/root.tsx
...
That's a good observation — I initialy started with throwing as it felt more correct, but really the only code catching is then shows a console error. I simplified the code as you suggested

### Review thread 3 (resolved)

#### aduth on 2026-04-02T21:13:17Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3030407961
Path: packages/ui/src/alert-dialog/root.tsx
...
Is this even possible to do? As a user I wasn't able to dismiss the modal at all after starting the async confirmation.

It also makes me wonder: What happens (or needs to happen) if the component is unmounted while the promise resolution is in progress? Do we need to increment the ref when the component unmounts, for example?

#### ciampo on 2026-04-03T07:46:56Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3031861251
Path: packages/ui/src/alert-dialog/root.tsx
...
I realized that the comment is a bit out of date, since at some point I experimented with a `allowDismissWhilePending` prop, but then realized it's not a UX choice I wanted to expose. I still think it makes sense to keep the check, but I updated the code comment to explain it's mainly a safety net for when the component is unmounted during pending state. Technically React 18 should not fire state updates on unmounted components, but I prefer the explicit code to make it intentional.

### Review thread 4 (resolved)

#### aduth on 2026-04-02T21:14:31Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3030413308
Path: packages/ui/src/alert-dialog/root.tsx
...
What's the point of the `'closing'` phase? I'm not sure I can follow how we're actually using it.

#### ciampo on 2026-04-03T07:49:59Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3031870052
Path: packages/ui/src/alert-dialog/root.tsx
...
It's used to prevent "confirm" from being submitted while the dialog is closing, both by guarding the `confirm()` function, and by making buttons disabled.

I updated the code comments to more explicitly explain all the different scenarios and associated variables of the state machine

### Review thread 5

#### aduth on 2026-04-03T19:00:35Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3034000317
Path: packages/ui/src/alert-dialog/stories/index.story.tsx

```diff
@@ -168,56 +190,101 @@ export const MenuTrigger: Story = {
 	},
 };
...
+ *
+ * When the task fails (promise rejects), the dialog stays open and returns
+ * to idle — the consumer can display an error message via `children`. The
+ * error resets when the dialog is reopened.
  */
...
#### ciampo on 2026-04-08T09:42:25Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3050490993
Path: packages/ui/src/alert-dialog/stories/index.story.tsx
...
- I tweaked `ConfirmResult` to also accept an `error` string prop alongside the `close` boolean prop; 
- when `error` is specified, the dialog will stay open and show that error next to the confirm button;
- instead of `role="status"`, we use `speak()` from `@wordpress/a11y`


![Screenshot 2026-04-08 at 11 41 39](https://github.com/user-attachments/assets/a4965c1c-f6c2-4f8c-b3f2-cf01c5600ab1)


Happy to iterate in follow-ups for any tweaks necessary to position and styling of the error message (cc @jameskoster )

#### ciampo on 2026-04-08T10:28:50Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3050726295
Path: packages/ui/src/alert-dialog/stories/index.story.tsx
...
Specifically on this part, I opted for _not_ making the error message part of the accessible description of the dialog, for a few reasons:

- the error already gets announced to an `aria-live` region via the `speak()` function, so we'd risk duplicating the announcement;
- the error message is easily discoverable by sighted users and assistive technology, just after the confirm button;
- `aria-describedby` is only "read" when the dialog is either explicitly focused, or when the dialog is somehow re-announced, which I don't think would happen often;
- we'd be having to work around base UI, since we're already using the `AlertDialog.Description` to pass an accessible description to the dialog

### Review thread 6

#### aduth on 2026-04-03T19:10:33Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3034040529
Path: packages/ui/src/alert-dialog/stories/index.story.tsx
...
<img width="599" height="257" alt="image" src="https://github.com/user-attachments/assets/64758288-f032-44e1-8034-d77b2004d138" />

This also makes me wonder again if this is the best API we expect people to use for customizing text color. Is this a standard way we'd expect people to show error texts? We had [weighed a `color` prop before already](https://github.com/WordPress/gutenberg/pull/73931#issuecomment-3656672419) but opted against it. Do you think an example like this is an example where a prop like that could be useful?

#### ciampo on 2026-04-08T09:59:45Z

URL: https://github.com/WordPress/gutenberg/pull/76937#discussion_r3050580481
Path: packages/ui/src/alert-dialog/stories/index.story.tsx
...
![Screenshot 2026-04-08 at 11 44 18](https://github.com/user-attachments/assets/56b74bad-0db1-4705-813b-b799c521884d)

We also have a separate fg color for the interactive text (resting state, active state):

![Screenshot 2026-04-08 at 11 44 50](https://github.com/user-attachments/assets/30b7a016-d439-408d-b045-6d1df0f10a11)
...
- Would this be something only for the `Text` component? If not, how do we design an API that scales well across the whole UI (and potentially beyond it)? For comparison, applying a CSS variable is a lower-level API than a direct prop, but is universal.
- What colors would we allow? Any `--wpds-color-fg-content` variables? Would we split the prop into `tone` `emphasis` etc or would we keep it as one string prop? Again, apart from the TS validation/autocompletion (which partially overlaps with linters anyway), the API wouldn't look too different from using a CSS var directly?

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +9/-0)
- packages/ui/src/alert-dialog/context.tsx (MODIFIED, +12/-4)
- packages/ui/src/alert-dialog/popup.tsx (MODIFIED, +91/-33)
- packages/ui/src/alert-dialog/root.tsx (MODIFIED, +191/-13)
- packages/ui/src/alert-dialog/stories/index.story.tsx (MODIFIED, +116/-65)
- packages/ui/src/alert-dialog/style.module.css (MODIFIED, +11/-0)
- packages/ui/src/alert-dialog/test/index.test.tsx (MODIFIED, +1265/-347)
- packages/ui/src/alert-dialog/trigger.tsx (MODIFIED, +2/-2)
- packages/ui/src/alert-dialog/types.ts (MODIFIED, +59/-28)
