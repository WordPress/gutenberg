...
type: pull_request
state: OPEN
url: https://github.com/WordPress/gutenberg/pull/76399
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components"]
---

# PR #76399: InputControl: Respect prop value if modified during onChange callback

- URL: https://github.com/WordPress/gutenberg/pull/76399
- Author: aduth
- Created: 2026-03-11T14:54:46Z
- Updated: 2026-03-11T20:58:36Z
- Comments: 8 of 8
- Review threads: 0 of 0
- Changed files: 5 of 5
- Diff: ../pr-diffs/76399.diff
...
### github-actions on 2026-03-11T14:54:58Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4039799923
...
URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4039799940
...
### github-actions on 2026-03-11T15:06:36Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4039881222
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 265 kB | +10 B (0%) |
...
| `build/scripts/core-data/index.min.js` | 29.4 kB |
...
| `build/scripts/editor/index.min.js` | 354 kB |
...
### aduth on 2026-03-11T15:16:50Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4039953478

I'm investigating the test failure. Based on [the test behavior](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/range-control/test/index.tsx#L105-L123), it makes sense that the invalid value would not be affected until loss of focus, but then this seems to me [the purpose of `isPressEnterToChange`](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/input-control/types.ts#L97-L103), which [`RangeControl` does not use](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/range-control/index.tsx#L327-L352).

### github-actions on 2026-03-11T15:19:46Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4039973693

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 489c4bd820c86b1d387ebdf5566efe2a88d2dcf6.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22971836094
📝  Reported issues:
...
### aduth on 2026-03-11T15:45:00Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4040152753

> I'm investigating the test failure. Based on [the test behavior](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/range-control/test/index.tsx#L105-L123), it makes sense that the invalid value would not be affected until loss of focus, but then this seems to me [the purpose of `isPressEnterToChange`](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/input-control/types.ts#L97-L103), which [`RangeControl` does not use](https://github.com/WordPress/gutenberg/blob/d68cf4c502a76a6068e2243b5f3032562529de5f/packages/components/src/range-control/index.tsx#L327-L352).

So it might be right that `RangeControl` ought to be using `isPressEnterToChange`, but the actual issue here is different, and has to do with types of how `props.value` and `draft.value` are represented. Previously, it would be comparing based on draft values (both the "current" and "last known" values). After these changes, we're comparing `props.value` and `draft.value`. These _should_ both be typed as `string | undefined` and therefore it's fine to compare, but in practice this isn't always the case.

For example, see how `NumberControl` bypasses type-checking and passes a number instead of a string:

https://github.com/WordPress/gutenberg/blob/0086dd2989d7bd96a8536fe1e789c3bed85e0acd/packages/components/src/number-control/index.tsx#L238-L239

This worked previously because it was always based on the draft value which [comes from `input.value`](https://github.com/WordPress/gutenberg/blob/421782ebb24606bd1c0671271171fa3035b9e890/packages/components/src/input-control/input-field.tsx#L97), a string. But after these changes, we now have a scenario where it tries to compare a prop value (numeric `-1.1`) with the draft value (string `'-1.1'`), which aren't the same.

Ultimately, this feels like it's an issue with how `NumberControl` passes its value as the wrong type.

### aduth on 2026-03-11T18:51:33Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4041416859

Okay, after testing this in the browser and seeing that the behavior asserted in the tests was the behavior I was seeing in the browser, I discovered that updating the test to use `userEvent` _passes_. I think it may be that because we were emitting raw change events with values that would never actually in a real browser (like, for example, being able to enter a value of `-1.1` if the `min` is `-1`).

Separately, it's still true that `NumberControl` should be passing a value in the correct type `string | undefined` and stop passing a number. I updated this in b7b4174.

The fact that `NumberControl` is misusing the `InputControl` `value` prop like this does have me concerned if there are other components or third-party usage that might be doing the same. We could maybe be a bit more defensive here and coerce the non-`undefined` prop value to a string in the same way we might expect it to be reflected in `draft.value` from `input.value`.

I'll mark this as ready for review, but it's not quite as urgent to land given that #76400 now includes a solution that doesn't directly rely on this fix.

### aduth on 2026-03-11T19:29:36Z

URL: https://github.com/WordPress/gutenberg/pull/76399#issuecomment-4041643428

The failing test in ColorPicker is another example of a knock-on effect 😕  Likely due to timing of [internal debouncing](https://github.com/WordPress/gutenberg/blob/a246a381af55f9e0bdeb2eb10d714f0e4b2b448b/packages/components/src/color-picker/component.tsx#L88). It might be back to the drawing board for how best to approach this problem in a way that retroactively handles all the components that depend on the current behavior.
...
- packages/components/src/input-control/test/index.js (MODIFIED, +30/-0)
- packages/components/src/input-control/utils.ts (MODIFIED, +2/-10)
- packages/components/src/number-control/index.tsx (MODIFIED, +1/-2)
- packages/components/src/range-control/test/index.tsx (MODIFIED, +6/-4)
