...
type: pull_request
state: MERGED
merged_at: 2025-11-18T09:06:19Z
url: https://github.com/WordPress/gutenberg/pull/73331
matched_labels: ["Design System","[Package] Theme"]
all_labels: ["[Type] Enhancement","Design System","[Package] Theme"]
---

# PR #73331: Theme: fix color ramp generation to avoid unmet contrasts

- URL: https://github.com/WordPress/gutenberg/pull/73331
- Author: jsnajdr
- Created: 2025-11-16T20:15:09Z
- Updated: 2025-11-26T15:38:35Z
- Comments: 4 of 4
- Review threads: 0 of 0
- Changed files: 12 of 12
- Diff: ../pr-diffs/73331.diff
...
<img width="387" height="80" alt="image" src="https://github.com/user-attachments/assets/255ba1e7-89e0-4bbd-9094-2725196f041b" />
...
I'm also making adjustments to the `stroke1` and `stroke2` target contrasts that @jameskoster proposed recently. They are quite unrelated, but not entirely: this PR changes the generated colors in three distinct ways, and I wanted to see the entire effect of all three changes together.

**Change number 1:** change `clampToGamut` to use the sRGB color space instead of P3. When we generate a color, we eventually store it in the RGB hex format, like `#c8ba80`. As part of this formatting, colors that are outside the sRGB gamut are automatically mapped to sRGB, otherwise they couldn't be expressed in the hex format. Here it can happen that two colors that previously had sufficient contrast in a wide-gamut space (like P3), have a slightly lower contrast after the sRGB conversion. That's one way how the contrast target could be missed. With `clampToGamut` converting to sRGB, we use sRGB colors during the actual color search and its contrast checks.

I'm also moving `clampToGamut` to the _color-utils.ts_ file, as that's a nicer place for the function.

**Change number 2:** Increase the "contrast topup" from 0.012 to 0.02. It's a safer value for edge cases when the lighter color is rounded down, to a slightly darker color, and the darker color is rounded up, to a slightly lighter color. Then the contrast can have as big as a 0.016 difference. This was another reason why target contrasts were missed sometimes.

**Change number 3**: Apply @jameskoster's suggestion to change contrast targets for `stroke1` and `stroke2`, so that the two colors are less prominent against the background surface. If these changes were applied alone, then for the default background ramp (`#f8f8f8` seed) the `stroke1` and `stroke2` colors were `#e1e1e1` and `#d8d8d8`. When all three tweaks in this PR are applied together, the colors are `#e0e0e0` and `#d8d8d8`. A minimal difference. This difference I what I wanted to see, and verify that the other two changes don't change the colors substantially.

**How to test:** In Storybook, look at the `/story/design-system-theme-provider-color-scales--default` story, and verify that the contrast mismatch warnings are gone.

## Issue comments

### github-actions on 2025-11-16T20:15:19Z

URL: https://github.com/WordPress/gutenberg/pull/73331#issuecomment-3539315127
...
### github-actions on 2025-11-16T20:27:34Z

URL: https://github.com/WordPress/gutenberg/pull/73331#issuecomment-3539324547
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 68.9 kB | -9 B (-0.01%) |
| `build/scripts/theme/index.min.js` | 20.8 kB | -6 B (-0.03%) |
...
| `build/scripts/abilities/index.min.js` | 43 kB |
...
| `build/scripts/workflow/index.min.js` | 18.8 kB |
...
| `build/styles/edit-site/style-rtl.css` | 15.5 kB |
| `build/styles/edit-site/style.css` | 15.5 kB |
| `build/styles/edit-widgets/style-rtl.css` | 4.65 kB |
| `build/styles/edit-widgets/style.css` | 4.66 kB |
| `build/styles/editor/style-rtl.css` | 18.2 kB |
...
| `build/styles/workflow/style-rtl.css` | 1.08 kB |
| `build/styles/workflow/style.css` | 1.08 kB |
...
### github-actions on 2025-11-16T20:44:25Z

URL: https://github.com/WordPress/gutenberg/pull/73331#issuecomment-3539337192

<!-- flaky-tests-report-comment -->
**Flaky tests detected in e8bce5dc006a7827c95dbcc50a1db066cdce5fef.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/19411403142
📝  Reported issues:
- #69470 in `/test/e2e/specs/site-editor/template-part-focus-mode.spec.js`

### jsnajdr on 2025-11-18T11:05:22Z

URL: https://github.com/WordPress/gutenberg/pull/73331#issuecomment-3546982865

> Couple side-observations

The two questions are related. Why are there warnings in the sample combinations story? One of the color ramps that fail is background ramp for WP Blue, `#096484`. In OKCHL this color is `oklch(0.4716 0.09084 229.62)`. If you display that color on the okchl.com picker, you'll see this chart for lightness:

<img width="409" height="325" alt="Screenshot 2025-11-18 at 11 40 05" src="https://github.com/user-attachments/assets/d7b010ff-cbbc-43a4-a527-8605d9673238" />

What does this chart show?
- if you move up and down, i.e., move the horizontal line labeled as C, you're changing the chroma value. The color is more gray, less gray.
- if you move left and right, the vertical line labeled as L, you're changing lightness. The color is darker or lighter.
- there are three nested triangles marked by white lines. The biggest one is the Rec2020 gamut, the middle one is the P3 gamut, and the smallest one is the sRGB gamut.
- our seed color is very close to the boundary of the sRGB gamut. You cannot increase the C or decrease the L very much.

Now, this seed color has insufficient contrast against white. It's just 6.63, while `fgSurface4` requires a bit over 7. The seed needs to be adjusted, i.e., made a bit darker. But we can't easily go darker! The lightness L is already at the bottom of the sRGB gamut. That's why the adjustment fails.

We could get a darker color by changing (lowering) the chroma C, by making the color less blue and more gray/black. That's what the `taperChroma` function is supposed to be doing, and it's also what the `toGamut` function in colorjs.io partially does. But our current implementations are currently not very good at this. Something to discuss in detail with @ciampo when he's back.

>  would these potential inaccuracies extend to any of the internal P3 usage?

The only other internal usage of P3 is in `taperChroma`. And yes, it should probably also use sRGB instead of P3 as the default. Also, the `taperChroma` and `clampToGamut` functions should be merged into one, because they both do basically the same job. This area needs further work.
...
- packages/theme/src/color-ramps/index.ts (MODIFIED, +16/-11)
- packages/theme/src/color-ramps/lib/color-utils.ts (MODIFIED, +11/-0)
- packages/theme/src/color-ramps/lib/constants.ts (MODIFIED, +3/-3)
- packages/theme/src/color-ramps/lib/default-ramps.ts (MODIFIED, +88/-88)
- packages/theme/src/color-ramps/lib/find-color-with-constraints.ts (MODIFIED, +2/-2)
- packages/theme/src/color-ramps/lib/index.ts (MODIFIED, +1/-2)
- packages/theme/src/color-ramps/lib/ramp-configs.ts (MODIFIED, +2/-2)
- packages/theme/src/color-ramps/lib/utils.ts (MODIFIED, +1/-8)
- packages/theme/src/color-ramps/stories/index.story.tsx (MODIFIED, +7/-4)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +51/-51)
- packages/theme/src/prebuilt/json/figma.json (MODIFIED, +51/-51)
- packages/theme/tokens/color.json (MODIFIED, +88/-88)
