...
type: pull_request
state: OPEN
url: https://github.com/WordPress/gutenberg/pull/78798
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components","[Package] Block editor","[Package] DataViews"]
---

# PR #78798: Code Quality: Pass boolean values to the `inert` attribute

- URL: https://github.com/WordPress/gutenberg/pull/78798
- Author: t-hamano
- Created: 2026-05-29T07:50:24Z
- Updated: 2026-06-01T11:57:24Z
- Comments: 10 of 10
- Review threads: 0 of 0
- Changed files: 11 of 11
- Diff: ../pr-diffs/78798.diff
...
### github-actions on 2026-05-29T08:08:52Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4572423647
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 51.7 kB | -7 B (-0.01%) |
| `build/modules/content-types/index.min.js` | 156 kB | -13 B (-0.01%) |
| `build/scripts/block-editor/index.min.js` | 379 kB | -2 B (0%) |
| `build/scripts/components/index.min.js` | 263 kB | -4 B (0%) |
| `build/scripts/edit-site/index.min.js` | 295 kB | -64 B (-0.02%) |
| `build/scripts/editor/index.min.js` | 462 kB | -53 B (-0.01%) |
| `build/scripts/media-utils/index.min.js` | 112 kB | -11 B (-0.01%) |
...
| `build/scripts/block-library/index.min.js` | 323 kB | 0 B |  |
...
### github-actions on 2026-05-29T08:26:43Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4572575628
...
### github-actions on 2026-05-29T08:33:00Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4572633561

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 9190c70564069e741ffce27f3f7fd5ad78aa2197.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26625644028
📝  Reported issues:
- #78706 in `/test/e2e/specs/editor/various/autosave.spec.js`

### t-hamano on 2026-05-29T09:26:17Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4573178099

Thanks for the review!

> I think we need to keep the `dataviews` package backward compatible with React 18.

It appears the dataviews package no longer supports React 18.

https://github.com/WordPress/gutenberg/blob/9190c70564069e741ffce27f3f7fd5ad78aa2197/packages/dataviews/CHANGELOG.md#L11-L13

https://github.com/WordPress/gutenberg/blob/9190c70564069e741ffce27f3f7fd5ad78aa2197/packages/dataviews/package.json#L85-L88

### Mamaduka on 2026-05-29T09:32:39Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4573249228

That might change https://github.com/WordPress/gutenberg/pull/78766#issuecomment-4572496112

### jsnajdr on 2026-05-29T10:01:42Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4573594667

> It appears the dataviews package no longer supports React 18.

If we decide to support both 18 and 19 in `ui` and `dataviews` (the bundled packages), then `inert` means trouble 🙁 The versions are incompatible. Not only the TypeScript types, but also runtime behavior when translating the React `inert` prop into a HTML `inert` attribute. I'm curious if Base UI or Ariakit work around this somehow.

### t-hamano on 2026-05-29T10:19:23Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4573814908

> I'm curious if Base UI or Ariakit work around this somehow.

It appears that Base UI internally checks `React.version` via a utility function. Can we utilize a similar approach for Gutenberg?

https://github.com/mui/base-ui/blob/b35d32224182337998097449d42ab39ed4fc2a4d/packages/utils/src/inertValue.ts#L3C17-L9

### tyxla on 2026-05-30T13:34:36Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4582971926
...
Since we'll have to support React 18 for the time being, I think we'll have to do something like this with the React version condition. It will make everything more complicated, but there's no other way until 7.2.

### t-hamano on 2026-05-30T14:05:33Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4583045316

Quoting from https://github.com/WordPress/gutenberg/pull/78766#issuecomment-4572496112:

> We'll probably need to continue supporting React 18 in some packages, but `block-editor` shouldn't be one of them. Only the bundled ones like `@wordpress/ui` or `@wordpress/dataviews`.

My understanding is that packages without a `wpScript: true` or `wpModuleExports: true` field might be intended for external use via npm, so we may need to support React 18 for all of them. This includes the following packages:
...
However, I don't think we need to support React 18 for all of these. For example, it's unlikely that consumers will use `@wordpress/global-styles-ui` or `@wordpress/image-cropper`. Perhaps we should support React 18 for the following packages:

- `@wordpress/dataviews`
- `@wordpress/icons`
- `@wordpress/ui`

### jsnajdr on 2026-06-01T07:50:14Z

URL: https://github.com/WordPress/gutenberg/pull/78798#issuecomment-4590568339

BaseUI has an `inertValue` helper:
```jsx
<div inert={ inertValue( false ) } />
```
where the helper internally checks `React.version` and the returns either a boolean or a string.

However, code that doesn't use the helper will break. If you use React 19 types then you use a boolean value:
```jsx
<div inert={ false } />
```
but in React 18 the `false` value will be serialized as `"false"` which is a truthy string.

If you use React 18 types, then you can pass a string:
```jsx
<div inert="" />
```
Here `""` is a truthy HTML attribute value, even though it's an empty string. In React 18 the `inert` attribute will be truthy, but in React 19 it will be interpreted as a boolean, and `""` is a falsy JS string. React will interpret is as such and will show a warning:
```
...
So, attribute written for one React version can be interpreted differently by the other version. For published code that takes only one version into account, switching between WordPress version will break it, and at this moment we don't offer any back-compat solution.

Ironically, this PR makes the compatibility worse. The `{ isInert ? 'true' : undefined }` syntax is compatible with both 18 and 19, but mere `{ isInert }` is not.
...
- packages/boot/src/components/canvas/index.tsx (MODIFIED, +1/-5)
...
- packages/dataviews/src/components/dataviews-footer/index.tsx (MODIFIED, +1/-5)
- packages/dataviews/src/components/dataviews-layouts/activity/index.tsx (MODIFIED, +2/-4)
- packages/dataviews/src/components/dataviews-layouts/grid/composite-grid.tsx (MODIFIED, +1/-1)
- packages/dataviews/src/components/dataviews-layouts/grid/index.tsx (MODIFIED, +1/-1)
- packages/dataviews/src/components/dataviews-layouts/list/index.tsx (MODIFIED, +1/-4)
- packages/dataviews/src/components/dataviews-layouts/table/index.tsx (MODIFIED, +1/-2)
