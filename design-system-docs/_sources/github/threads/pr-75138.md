...
type: pull_request
state: MERGED
merged_at: 2026-02-03T10:56:12Z
url: https://github.com/WordPress/gutenberg/pull/75138
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components"]
---

# PR #75138: ToggleGroupControl: add visual emphasis to selected item

- URL: https://github.com/WordPress/gutenberg/pull/75138
- Author: ciampo
- Created: 2026-02-02T14:50:44Z
- Updated: 2026-02-19T16:30:32Z
- Comments: 22 of 22
- Review threads: 0 of 0
- Changed files: 3 of 3
- Diff: ../pr-diffs/75138.diff
...
### github-actions on 2026-02-02T15:02:32Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3835661662
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 266 kB | -10 B (0%) |
...
| `build/styles/editor/style-rtl.css` | 26 kB |
...
### github-actions on 2026-02-02T16:35:37Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3836336292
...
### aduth on 2026-02-02T17:06:35Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3836531106

Is there any discussion or context to share with this? Noting since the design was the subject of much discussion in #74036.

Though personally, to my untrained eye this feels like an improvement, as the "Before" as introduced in #74036 was quite subtle and perhaps an overcorrection from the earlier design.

### ciampo on 2026-02-02T17:56:18Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3836781000
...
### jasmussen on 2026-02-03T07:58:51Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3839697996

The broad context is really in [this issue](https://github.com/WordPress/gutenberg/issues/64439) and specifically [this comment](https://github.com/WordPress/gutenberg/issues/64439#issuecomment-3659271652). And it is a rabbit hole: the borders are there to ensure contrast in icon only modes, and to match adjacent controls like inputs and so on. The broad agreement was that the old variant, with the black total inversion for active state, drew undue attention.

As far as this PR, it looks good to me. I think Gray 100 may be _slightly_ too much, but arguably the followup here is to refine those variables through the token engine. @jameskoster WDYT?

### t-hamano on 2026-02-03T08:02:21Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3839714498

Personally, I support this change for now. I suggested a similar change in [this comment](https://github.com/WordPress/gutenberg/pull/74036#issuecomment-3660731272).

### ciampo on 2026-02-03T10:48:38Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3840572352

> As far as this PR, it looks good to me. I think Gray 100 may be _slightly_ too much, but arguably the followup here is to refine those variables through the token engine. @jameskoster WDYT?

I went with gray-100 to avoid using a custom value, and stick to existing variables instead.

In terms of DS tokens, with Jay we observed that we'd likely use the `--wpds-color-bg-interactive-neutral-weak-active` token (the same used for the hover/focus state of [neutral outline Button](https://wordpress.github.io/gutenberg/?path=/story/design-system-components-button--neutral-outline)), which as of now is actually _slightly_ darker (Gray 100 is `#f0f0f0`, the DS token is`#eaeaea`).

As always, as we work on the `@wordpress/ui` equivalent, we can look into tweaking those values, although it's always tricky to balance all use cases and requirements (including color contrast vs resting background state and vs text color)

### ciampo on 2026-02-03T10:49:30Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3840576971

Also, if we're ok with merging this PR, I'd ask for someone to approve it

### jasmussen on 2026-02-03T10:50:12Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3840580986
...
### ciampo on 2026-02-03T10:55:13Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3840603618

> eaeaea would be too dark for this. I don't think that's a blocker, but it needs to be arguably lighter, a la fcfcfc. I recognize that needs to fit into the system so let's connect on details.

Absolutely 💯

### ciampo on 2026-02-09T16:24:39Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3872755482

Hey @jasmussen , I just spent some time pairing with @jameskoster on tweaking DS tokens (mostly in the context of https://github.com/WordPress/gutenberg/pull/75204).

We also looked at the token that we'd use for the background of the active item of `ToggleGroupControl` (ie. the `--wpds-color-bg-interactive-neutral-weak-active` token) to see if we could find ways to make it lighter, as per your earlier feedback.

Unfortunately, the current constraints of the DS don't allow for a much lighter color than `#eaeaea` (on the default neutral scale). That is because we are reserving lighter colors (such as `#fff`, `#f8f8f8` and `#f0f0f0`) for the "app surfaces", ie. page/card backgrounds:
- if we made the `--wpds-color-bg-interactive-neutral-weak-active` lighter, we would end up in a situation where it would have a very similar (if not the same) color as the page/card background—for example, in the sidebar;
- we also discussed "compressing" the page/card background colors range to allow for the `--wpds-color-bg-interactive-neutral-weak-active` to be lighter, but it would make the differences across the 3 different page/card background colors very subtle and, again, almost impossible to distinguish.

We feel that the best compromise would be to keep the value associated with the `--wpds-color-bg-interactive-neutral-weak-active` as-is, but we're also happy to discuss the topic more in detail if necessary.

### jasmussen on 2026-02-10T09:10:34Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3876326015
...
### jameskoster on 2026-02-10T11:10:58Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3876953605

The token for the background of interactive elements in their hover/active state needs to be visible against all three application surface tokens. There also needs to be meaningful contrast between those three surface tokens. Currently they look like this:

<img width="1440" height="960" alt="surfaces" src="https://github.com/user-attachments/assets/373a4032-38c9-4cde-ae8b-c91932fc44cd" />

If we place a minimal button on the darkest of these colors, here's how the hover state looks:

<img width="751" height="281" alt="button" src="https://github.com/user-attachments/assets/2530dcbb-81c7-408b-a960-45188a84ba69" />

Clearly there's not much (any) scope to make this color lighter. 

We could reduce the contrast between the surface backgrounds, which would allow us to also lighten the token in question, but then we lose meaningful contrast between the surfaces to the point where it becomes questionable whether it's worth having them at all:

<img width="751" height="281" alt="surfaces" src="https://github.com/user-attachments/assets/c1d08401-e7ef-4a63-99ff-6fcd82f1c6be" />

Even _with_ this change, the color output is ~`#EFEFEF`, which is still too dark for `ToggleGroupControl`...

<img width="987" height="100" alt="Screenshot 2026-02-10 at 10 59 55" src="https://github.com/user-attachments/assets/c6f9a8d0-ff7f-4865-8064-6062955d311c" />

Designing with a system always means a degree of back-and-forth and occasional compromise. At this point it’s worth stepping back and asking whether these knock-on changes are worth the cost. For me, system-level coherence typically outweighs the appearance of an individual primitive in a single state. Particularly when the change is so _incredibly_ subtle like this (the difference between `#ffffff` and `#fcfcfc` is almost imperceptible). 

If we're still not happy then I’d rather revisit the component design than start forcing changes that risk undermining the system as a whole.

### jasmussen on 2026-02-10T11:24:55Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3877013804

It's still a bit dark, but it's an improvement over what exists. And of course we would not want to undermine anything at all. I'm asking in part to explore the territory and for my own education: what options exist with the current design, within the system? Does the backdrop color of the active toggle control have to be the same token as the hover state shown in the example? 

Not suggesting the following, to be very very clear: but is there room to say: the backdrop of the active state is _item 100 on the grayscale_? I.e. it would invert that grayscale in darkmode, but otherwise not be tied to any surfaces at all.

And yes, the alternative is to revisit the design. Not revert, but see if there's room in the beta territory for small lifts. Here's a [togglecontrol playground file](https://www.figma.com/design/FDq7uk8PEq6ArfOrUWkryi/ToggleControl-design-playground?node-id=0-1&t=p7Otn8rH4BCuooak-1), and if we need to, we can go full circle:

<img width="248" height="260" alt="Group 2147226653" src="https://github.com/user-attachments/assets/0b81cd41-5388-402b-b424-53263cff1fa5" />
...
### jameskoster on 2026-02-10T14:18:32Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3877925980

> Does the backdrop color of the active toggle control have to be the same token as the hover state shown in the example?
...
There is certainly room for a 'Track' token. In fact I think we already have one. The issue is that all the hover/active background tokens are darker than white, so even the proposed design wouldn't be achievable.

### jasmussen on 2026-02-10T14:26:07Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3877968193

It does sound like the next step is to explore the territory in Figma then. Happy to huddle there if useful. I feel this is a healthy but important trial for the token system, to be able to account for this challenge. What are the creative ways, back and forth, that will let us improve the control? Because while improved in its reduced optical balance, as-is it's a bit bare compared to other players in the scene. What does [ShadCDN](https://ui.shadcn.com/docs/components/radix/toggle-group) do, tokenwise, for example?

### jameskoster on 2026-02-10T15:58:49Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3878857107

Yes I think the next step is probably to explore 'track based' elements and controls together. Things like scrollbars, sliders, progress bars, and (potentially) `ToggleGroupControl`. I'm not super optimistic it will be fruitful though, because I suspect we'll still want the strong blueberry foregrounds for things like sliders and progress bars.

I agree this is a good trial, but I also think we need to balance impact and effort. It's worth questioning whether _this_ control justifies the (potential) wider implications across the system.

### t-hamano on 2026-02-11T14:10:10Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3884673787

I realized that `#f0f0f0` is also the color used when a form element is disabled. Below is an artificially disabled letter spacing control. The selected and disabled states are both represented in the same color, which can be confusing for some users.

<img width="313" height="226" alt="image" src="https://github.com/user-attachments/assets/59011a41-b22b-46bc-99e5-41acde5ea41e" />

Additionally, what if we need to add a disabled state to the `ToggleGroupControl` component in the future? I remember seeing a discussion somewhere suggesting adding a `disabled` prop.
...
### jasmussen on 2026-02-11T15:58:48Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3885369366

Some options:

<img width="1464" height="675" alt="active state backdrop tokens, but with _neutral bg surface_ active state" src="https://github.com/user-attachments/assets/5259931f-7ed1-4634-9341-1846afb61df3" />

In context:

<img width="280" height="260" alt="1" src="https://github.com/user-attachments/assets/353025ae-166a-45dd-811b-1e5a539b6409" />

I think I'm using a non existing gray in the above, f0f0f0. Here's one with eaeaea:

<img width="1464" height="675" alt="active state backdrop tokens, neutral weak active" src="https://github.com/user-attachments/assets/863bd611-5e16-4f0c-9998-b5914d7dfe6c" />

In context:

<img width="280" height="260" alt="2" src="https://github.com/user-attachments/assets/c879ca8d-b588-436b-a796-57bdb0132dc5" />

I briefly tried with a track backdrop too, but it didn't connect with me:

<img width="956" height="540" alt="track backdrop" src="https://github.com/user-attachments/assets/65d58718-2590-4da9-b5a9-3a4ab44703e8" />

### ciampo on 2026-02-12T22:36:01Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3893761824
...
### jasmussen on 2026-02-13T09:12:15Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3895838537

> just to make sure — do you have a preference for any new styles to apply to ToggleGroupControl? are there any changes that you'd like to see applied immediately?

I will be mostly AFK today, but otherwise I feel mostly unblocked to help with a PR. But if you have capacity, I'd appreciate your help too. I don't think it has to happen before the beta, but I do think it'd be good to refine this even a bit. The following is improved, but I still get that "unstyled" or "barebones" vibe from it:

<img width="188" height="66" alt="image" src="https://github.com/user-attachments/assets/b07920c8-d1e3-4474-911d-e19d063e6540" />

Whereas by mainly finessing the color of inactive icons, especially in the above borderless context, I feel it helps to make it look slightly more intentional:

<img width="224" height="105" alt="image" src="https://github.com/user-attachments/assets/14573ed0-0f45-468b-8d46-a5b508f5dcc2" />

I know, this is detail territory, the main change is the color of the icons is now gray, like the text is in text-based segmented controls.

### ciampo on 2026-02-19T16:30:32Z

URL: https://github.com/WordPress/gutenberg/pull/75138#issuecomment-3928372628

> if you have capacity, I'd appreciate your help too

https://github.com/WordPress/gutenberg/pull/75737
