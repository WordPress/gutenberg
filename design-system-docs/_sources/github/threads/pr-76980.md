...
type: pull_request
state: MERGED
merged_at: 2026-04-06T19:43:02Z
url: https://github.com/WordPress/gutenberg/pull/76980
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components","[Feature] Real-time Collaboration","Backported to WP Core"]
---

# PR #76980: RTC: Fix inline inserter reset on update sync

- URL: https://github.com/WordPress/gutenberg/pull/76980
- Author: alecgeatches
- Created: 2026-04-01T22:41:36Z
- Updated: 2026-04-29T10:04:59Z
- Comments: 8 of 8
- Review threads: 3 of 3
- Changed files: 3 of 3
- Diff: ../pr-diffs/76980.diff
...
https://github.com/user-attachments/assets/11c92ee2-f9d3-481a-bbe0-cd0992382bff

## Why?

The root cause is `useLastDifferentValue` in the autocomplete component. This hook tracks whether the user has changed something by keeping a history of the last 2 rich-text record objects in a `Set`. Since `Set` uses object reference equality, when `RESET_BLOCKS` fires during RTC sync and rebuilds the block tree from scratch, the resulting new record objects (same text, different references) append to the history rather than it staying the same. The "previous" record ends up having the same `.text` as the current one, so `didUserInput` evaluates to `false` and the popover is removed.

## How?

This is a `RESET_BLOCKS`-related bug like https://github.com/WordPress/gutenberg/pull/76025, a class of bugs we've seen where the editor will reset local editor state on sync in a way that doesn't make sense in a collaborative model.

This PR changes `useLastDifferentValue` to compare by `.text` value instead of object reference. The new implementation uses two refs: one for the previous record and one for the last record that had a different `.text`. When a re-render produces a new record object with the same text content (as happens during `RESET_BLOCKS` in an RTC sync), the function correctly recognizes that no actual text change occurred and preserves the earlier record that had different text. This keeps `didUserInput` as `true` and the popover stays visible.
...
```
npm run test:unit -- --testPathPattern='packages/components/src/autocomplete'
```
...
3. In one session, begin continuously typing in a paragraph block. Here's a JS one-liner that can do that:

    ```js
    // First, click into block text, then paste:
    (function(){var s="Lorem ipsum dolor sit amet. ",i=0,doc=document.querySelector('.block-editor-iframe__scale-container iframe').contentDocument,el=doc.activeElement;window._typer=setInterval(function(){el.focus();doc.execCommand('insertText',false,s[i%s.length]);i++;},200);})();

    // To stop typing:
    // clearInterval(window._typer)
    ```

4. In the other session, click into a separate paragraph block and type `/`.
5. Verify the inline block inserter popover stays open while the other user is typing:

    https://github.com/user-attachments/assets/98a1a115-1c3a-4954-9633-85f0dc331c39

7. Select an option from the inserter and verify it inserts correctly:
...
### github-actions on 2026-04-01T22:41:46Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4173390958
...
### github-actions on 2026-04-01T22:53:40Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4173431748
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 265 kB | +50 B (+0.02%) |
...
### github-actions on 2026-04-02T17:04:10Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4179216839

<!-- flaky-tests-report-comment -->
**Flaky tests detected in de9c87dc28077bfe67025c308bd839a78c4ece23.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24045669688
📝  Reported issues:
...
### jsnajdr on 2026-04-03T14:46:42Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4183779174

> I think the general problem with `RESET_BLOCKS` might need more attention [...] Can we keep the same reference for blocks that aren't affected by RTC?

The `RESET_BLOCKS` actions is used mainly by the `useBlockSync` hook and the `useEntityBlockEditor` hook, whose return value is passed as input the `useBlockSync`. These hooks perform a two-way sync between an entity in the `core-data` store and block tree state in `block-editor` store.

Tt's used for two purposes:
1. Reset the block editor state when we start editing a certain entity, or when we switch from editing entity A to editing entity B.
2. Update the block editor state when the entity in `core-data` is updated, from an "external source".

Technically these two cases are almost the same (`getEditedEntityRecord` return value changes), but semantically they are very different.

And the second use case, updating the existing entity, although it has always been present, started to be seriously used only now, when RTC was implemented. Because the "updates from external sources" didn't really happen before.

This is why `RESET_BLOCKS` is suddenly a problematic action behind several bugs. For a long time we could get away with assuming that reset is done only when switching from editing entity A to editing entity B, but that doesn't work any more.

One of the typical recent fixes is #76025. Here `RESET_BLOCKS` was resetting the UI state of list view sidebars. That's a good idea when navigating to a different entity, but a bad idea when merely receiving RTC updates for an existing post.

The original version of #76025 added an `isExternal` flag to `RESET_BLOCKS`, maybe we'll eventually need to implement something like this, to distinguish between the two types of reset.

### github-actions on 2026-04-27T16:26:29Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4328675738
...
There was a conflict while trying to cherry-pick the commit to the wp/7.0 branch. Please resolve the conflict manually and create a PR to the wp/7.0 branch.
  ...
  git cherry-pick 0babbd92b2c05c80109af90bc117ccf40d4b167f
...
### alecgeatches on 2026-04-27T16:37:07Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4328742302

Cherry-pick conflict fixed in https://github.com/WordPress/gutenberg/pull/77706.

### github-actions on 2026-04-28T08:22:20Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4333525587
...
### t-hamano on 2026-04-29T10:04:44Z

URL: https://github.com/WordPress/gutenberg/pull/76980#issuecomment-4342712920

This PR was backported to `wp/7.0` in #77706.

## Review threads

### Review thread 1

#### Mamaduka on 2026-04-02T16:40:10Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3029135160
Path: packages/components/src/autocomplete/index.tsx

```diff
@@ -393,17 +393,48 @@ export function useAutocomplete( {
 	};
 }
...
#### alecgeatches on 2026-04-02T18:32:52Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3029666367
Path: packages/components/src/autocomplete/index.tsx
...
#### Mamaduka on 2026-04-03T07:58:30Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3031895798
Path: packages/components/src/autocomplete/index.tsx
...
#### alecgeatches on 2026-04-03T14:54:42Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3033173668
Path: packages/components/src/autocomplete/index.tsx
...
Removed most of the lines in https://github.com/WordPress/gutenberg/pull/76980/changes/7d47aa3821a32fa574592d7ddef2c7c504a635f7!

### Review thread 3

#### oandregal on 2026-04-08T08:16:42Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3050030572
...
The changelog here has git conflict markers. I'm removing them at https://github.com/WordPress/gutenberg/pull/77127

#### Mamaduka on 2026-04-08T08:28:33Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3050097338
Path: packages/components/CHANGELOG.md
...
#### alecgeatches on 2026-04-08T16:14:19Z

URL: https://github.com/WordPress/gutenberg/pull/76980#discussion_r3052684219
Path: packages/components/CHANGELOG.md
...
- packages/components/src/autocomplete/index.tsx (MODIFIED, +30/-6)
- packages/components/src/autocomplete/test/index.tsx (MODIFIED, +110/-1)
