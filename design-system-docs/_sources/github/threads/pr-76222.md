...
type: pull_request
state: MERGED
merged_at: 2026-03-13T15:04:59Z
url: https://github.com/WordPress/gutenberg/pull/76222
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Code Quality","[Package] Block editor","[Package] UI"]
---

# PR #76222: ESLint: Add `use-recommended-components` rule

- URL: https://github.com/WordPress/gutenberg/pull/76222
- Author: mirka
- Created: 2026-03-05T21:48:08Z
- Updated: 2026-03-13T15:05:46Z
- Comments: 4 of 4
- Review threads: 8 of 8
- Changed files: 16 of 16
- Diff: ../pr-diffs/76222.diff
...
Adds a new `use-recommended-components` ESLint rule to `@wordpress/eslint-plugin` and enables it in the repo.

## Why?

As `@wordpress/ui` components become ready for use, we need a way to communicate which ones are recommended and which aren't yet. On the other side, as components in older packages like `@wordpress/components` gain newer alternatives, we want to nudge consumers toward them.

This rule provides an evolving, up-to-date set of that guidance directly as lint feedback.
...
- **Allowlist**: For packages like `@wordpress/ui` where only a subset of components are ready for general use. Named imports not on the list are flagged.
- **Denylist**: For specific components in other packages that have a recommended alternative. Each entry has a custom message pointing to the replacement.

The rule is enabled as `error` in this repo's `.eslintrc.js` only, not in the shared `recommended` config, so third-party consumers of `@wordpress/eslint-plugin` can opt in if they want. At this early stage, it is mostly intended for the Gutenberg repo.
...
```
npx jest packages/eslint-plugin/rules/__tests__/use-recommended-components.js
```

## Issue comments

### mirka on 2026-03-05T22:01:07Z

URL: https://github.com/WordPress/gutenberg/pull/76222#issuecomment-4008092857

Currently figuring out what to do with the two violations in `routes/content-guidelines/components`, but otherwise ready for review.

### github-actions on 2026-03-05T22:02:03Z

URL: https://github.com/WordPress/gutenberg/pull/76222#issuecomment-4008097711
...
### github-actions on 2026-03-05T22:02:57Z

URL: https://github.com/WordPress/gutenberg/pull/76222#issuecomment-4008101720
...
### github-actions on 2026-03-05T22:14:29Z

URL: https://github.com/WordPress/gutenberg/pull/76222#issuecomment-4008156764

<!-- flaky-tests-report-comment -->
**Flaky tests detected in a7958f591d0a8f2f7d178577c56558e313e3eda6.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23055311398
📝  Reported issues:
...
#### mirka on 2026-03-05T22:41:13Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2892767188
...
#### ciampo on 2026-03-06T11:01:34Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2895162221
Path: packages/eslint-plugin/rules/__tests__/use-recommended-components.js

Should we somehow test the denylist too? Either by adding a "test" entry, or by mocking it for the purpose of the tests?

#### mirka on 2026-03-10T20:05:48Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2914156141
Path: packages/eslint-plugin/rules/__tests__/use-recommended-components.js

Added deny list entry in 47b5de8cbafc9e5816d402e5eb903f58ae1b1d50, added tests in 90fe6d0d385882b939b40d41fba5f03756ff5a32

### Review thread 3

#### ciampo on 2026-03-06T11:03:08Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2895169150
Path: packages/eslint-plugin/docs/rules/use-recommended-components.md

Let's also update the table in `packages/eslint-plugin/README.md`

#### mirka on 2026-03-10T20:04:57Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2914151903
Path: packages/eslint-plugin/docs/rules/use-recommended-components.md

Updated in 7568ac158950782f0ae5cfb7bda30561a60eb73f

### Review thread 4

#### ciampo on 2026-03-06T11:04:35Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2895175483
...
#### mirka on 2026-03-10T20:04:35Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2914150371
Path: packages/eslint-plugin/rules/use-recommended-components.js
...
Added tests in 90fe6d0d385882b939b40d41fba5f03756ff5a32

### Review thread 5

#### mirka on 2026-03-10T20:37:07Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2914332347
Path: routes/content-guidelines/components/block-guideline-modal.tsx
...
([Discussed](https://github.com/WordPress/gutenberg/pull/76155/changes#r2911631169) with the team in charge)

### Review thread 6

#### aduth on 2026-03-13T13:40:57Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2931270277
Path: packages/eslint-plugin/docs/rules/use-recommended-components.md
...
#### mirka on 2026-03-13T14:26:06Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2931531532
Path: packages/eslint-plugin/docs/rules/use-recommended-components.md
...
#### aduth on 2026-03-13T13:48:11Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2931310104
Path: packages/eslint-plugin/rules/use-recommended-components.js
...
Probably want `hasOwnProperty` here , lest we flag on `import { valueOf } from '@wordpress/components';` 🙃 (which would be separately flagged as a missing import)

```suggestion
					if ( denylistEntry && denylistEntry.hasOwnProperty( name ) ) {
```

#### aduth on 2026-03-13T13:55:10Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2931349723
Path: packages/eslint-plugin/rules/use-recommended-components.js
...
Optionally simplified further with optional chaining:
...
#### aduth on 2026-03-13T13:53:54Z

URL: https://github.com/WordPress/gutenberg/pull/76222#discussion_r2931342595
Path: packages/eslint-plugin/rules/use-recommended-components.js
...
If we're not actually doing anything special with `messageId`, should we just be sending `message` directly and skipping the templating?
...
(Tangential thought as I was exploring if we could not do the manual templating in `resolveMessage`, though I think how we have it there makes sense given the configurability and lack of direct access to ESLint's internal templating)

## Changed files

- .eslintrc.js (MODIFIED, +1/-0)
- packages/block-editor/src/components/global-styles/color-panel.js (MODIFIED, +3/-1)
- packages/block-editor/src/components/global-styles/filters-panel.js (MODIFIED, +2/-0)
- packages/eslint-plugin/CHANGELOG.md (MODIFIED, +4/-0)
- packages/eslint-plugin/README.md (MODIFIED, +1/-0)
- packages/eslint-plugin/docs/rules/use-recommended-components.md (ADDED, +25/-0)
- packages/eslint-plugin/rules/__tests__/use-recommended-components.js (ADDED, +96/-0)
- packages/eslint-plugin/rules/use-recommended-components.js (ADDED, +114/-0)
- packages/global-styles-ui/src/palette.tsx (MODIFIED, +2/-0)
- packages/ui/AGENTS.md (DELETED, +0/-9)
- packages/ui/CLAUDE.md (DELETED, +0/-1)
- routes/content-guidelines/components/actions-section.tsx (MODIFIED, +2/-0)
- routes/content-guidelines/components/block-guideline-modal.tsx (MODIFIED, +2/-0)
- routes/content-guidelines/components/block-guidelines.tsx (MODIFIED, +2/-0)
- routes/content-guidelines/components/guideline-accordion-form.tsx (MODIFIED, +2/-0)
- routes/content-guidelines/stage.tsx (MODIFIED, +2/-0)
