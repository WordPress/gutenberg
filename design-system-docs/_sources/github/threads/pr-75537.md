...
type: pull_request
state: MERGED
merged_at: 2026-02-16T18:12:32Z
url: https://github.com/WordPress/gutenberg/pull/75537
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components"]
---

# PR #75537: Tabs: Set explicit font-family on tab buttons

- URL: https://github.com/WordPress/gutenberg/pull/75537
- Author: mirka
- Created: 2026-02-13T19:43:21Z
- Updated: 2026-02-16T18:13:21Z
- Comments: 4 of 4
- Review threads: 0 of 0
- Changed files: 4 of 4
- Diff: ../pr-diffs/75537.diff

## Body

## What?

Sets an explicit `font-family` on the `Tabs` tab buttons in both `@wordpress/components` and `@wordpress/ui`.

## Why?

The tab buttons don't have a proper font-family specified, so they can end up rendering in unintended fonts (e.g. the browser default like Arial or a generic `sans-serif` like Helvetica).

The font inconsistency can be noticeable in actual UIs like this (here, rendered in Arial):

<img width="646" height="461" alt="Template parts page" src="https://github.com/user-attachments/assets/85fc2a39-d412-43d6-b1aa-ac46c3b9bd32" />


## How?

- `@wordpress/components`: Adds `font-family` via the existing `font( 'default.fontFamily' )` utility, matching other components like `BaseControl` and `TextareaControl`.
- `@wordpress/ui`: Replaces the hardcoded `font-family: sans-serif` and `font-size: 13px` with the design tokens `--wpds-font-family-body` and `--wpds-font-size-md`. Also removes the resolved TODO comment.

## Testing Instructions

See stories for `Tabs` in both `@wordpress/components` and `@wordpress/ui`.

## Issue comments

### github-actions on 2026-02-13T19:55:27Z

URL: https://github.com/WordPress/gutenberg/pull/75537#issuecomment-3899144176
...
### github-actions on 2026-02-13T20:07:01Z

URL: https://github.com/WordPress/gutenberg/pull/75537#issuecomment-3899204051
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 266 kB | +18 B (+0.01%) |
...
| `build/scripts/editor/index.min.js` | 346 kB |
...
| `build/scripts/upload-media/index.min.js` | 7.04 kB |
...
### aduth on 2026-02-16T16:10:16Z

URL: https://github.com/WordPress/gutenberg/pull/75537#issuecomment-3909314946

Do we document anywhere the expectation that components should provide their own basic styles like typography styles?

### mirka on 2026-02-16T17:30:24Z

URL: https://github.com/WordPress/gutenberg/pull/75537#issuecomment-3909711007

> Do we document anywhere the expectation that components should provide their own basic styles like typography styles?

With `@wordpress/ui` and design tokens, we might be at a stage where every component should just set their own fonts. It was a bit more loose with `@wordpress/components`, in that it would be fine if the component rendered properly, inheriting from the `body` font-family. Now with a CSS variable, I don't think there are any downsides to setting an explicit font-family everywhere.
...
- packages/ui/src/tabs/style.module.css (MODIFIED, +3/-3)
