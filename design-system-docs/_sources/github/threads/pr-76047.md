...
type: pull_request
state: MERGED
merged_at: 2026-03-13T13:23:30Z
url: https://github.com/WordPress/gutenberg/pull/76047
matched_labels: ["Design System","[Package] Theme"]
all_labels: ["[Type] Enhancement","Design System","[Package] Theme"]
---

# PR #76047: Theme package: Add surface width design tokens

- URL: https://github.com/WordPress/gutenberg/pull/76047
- Author: jameskoster
- Created: 2026-03-02T17:02:49Z
- Updated: 2026-03-13T13:24:16Z
- Comments: 13 of 13
- Review threads: 1 of 1
- Changed files: 8 of 8
- Diff: ../pr-diffs/76047.diff

## Body

## What?

Adds a new set of `surface-width` design tokens to `@wordpress/theme`:
...
Also adds a corresponding `SurfaceWidthSize` TypeScript type (`'xs' | 'sm' | 'md' | 'lg' | 'xl' | '2xl'`).

## Why?

Components like [`Dialog`](https://wordpress.github.io/gutenberg/?path=/docs/design-system-components-dialog--docs) currently hardcode their width constraints (e.g. `max-width: 384px`, `max-width: 512px`). These values are scattered across component stylesheets with no shared source of truth, making it difficult to maintain consistency.

Width tokens provide a standardized scale for component widths, similar to how `padding` and `gap` tokens standardize spacing.

**Examples of other components that could benefit from these tokens:**
...
- Added a `surface-width` group to the existing `dimension.json` token file, alongside `padding` and `gap`.
- Values are direct pixel values on the 4px base grid.
- Width tokens are static across density modes (no compact/comfortable overrides) for now.
- Added a `SurfaceWidthSize` type pattern to the Terrazzo config for TypeScript generation.
- Rebuilt tokens to regenerate CSS custom properties, fallbacks, type definitions, and documentation.

## Design decisions

**Naming: "surface-width"**
We considered "measure" (a typography term for line length, used by methodologies like Every Layout) for its writing-mode agnosticism and design-concept framing. Ultimately we chose "surface-width" for its simplicity and because it pairs cleanly with a potential future `surface-height` token family. It also aligns nicely with color tokens e.g. `-wpds-color-stroke-surface-warning`, `--wpds-color-bg-surface-neutral`.
...
**No density modes**
Unlike `padding` and `gap`, width tokens don't vary across compact/comfortable density modes. Component width constraints are about content layout, not information density. We can explore adding density-responsive width values later if use cases emerge.
...
Confirm the new `--wpds-dimension-width-*` variables appear in `packages/theme/src/prebuilt/css/design-tokens.css`.

## Issue comments

### github-actions on 2026-03-02T17:03:19Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-3985643531
...
### github-actions on 2026-03-02T17:18:54Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-3985756378
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 16.8 kB | +47 B (+0.28%) |
...
| `build/scripts/core-data/index.min.js` | 29.3 kB |
...
### mirka on 2026-03-04T17:32:34Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-3999058193

In relation to the naming, do you have plans to add "height" tokens, and if so, what would that be called?

### jameskoster on 2026-03-05T16:18:17Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4006226024
...
* **measure** → inline-size (width in horizontal writing modes)
* **extent** → block-size (height in horizontal writing modes)

Do you think we're too far in the weeds here? :)

### mirka on 2026-03-05T16:24:37Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4006281613

I'm fine with plain `width`/`height`, or `block-size`/`inline-size`, but I think `measure/extent` would be obscure even for our target audience? 😄

### jameskoster on 2026-03-06T15:12:25Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4012320188

I renamed to `width` :D

### github-actions on 2026-03-06T15:36:05Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4012455891

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 5ee8d84fe65a1121c78533769f2f856de29337f9.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22903749999
📝  Reported issues:
- #48558 in `/test/e2e/specs/editor/various/undo.spec.js`

### ciampo on 2026-03-06T16:50:05Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4012851703

What about much smaller widths, as the ones we use for icons / buttons / input controls? More in general, I wonder if we're missing smaller values in the scale.


Beyond that, this PR will require a lot of manual testing — @jameskoster , have you already given it a first pass?

### jameskoster on 2026-03-06T19:15:08Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4013569931

Good question. My feeling is they're a different category of token rather than an extension of the `width` scale.

The width tokens are about constraining _container_ widths; dialogs, cards, drawers, panels, etc. They're more about layout.

Icon sizes and button min-widths are about sizing individual elements and they're generally square or near-square. If they were part of the same scale there would be a big gap between the largest 'element size' (`40px`) and the smallest 'width' (`320px`) which further indicates they are separate concerns imo.

Perhaps that's where a `size` family of tokens comes into play:

| Token | Value | Use case |
|---|---|---|
| `--wpds-size-xs` | 16px | Small icons, checkboxes, radios |
| `--wpds-size-sm` | 24px | Default icons, 'small' controls/buttons |
| `--wpds-size-md` | 32px | 'Compact' button/control height |
| `--wpds-size-lg` | 40px | Large buttons/ controls |

WDYT?

### ciampo on 2026-03-06T21:39:48Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4014298496

So, two separate families of tokens. Then maybe `size` and `container-widths` to make the difference clearer? Namig is hard.

### jameskoster on 2026-03-09T14:57:25Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4024380834

Maybe `surface-width` to align with other token categories?

### ciampo on 2026-03-09T20:42:54Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4026783253

Yeah, I think that `size` + `surface-width` (and potentially `surface-height`) could work.

### jameskoster on 2026-03-10T12:55:55Z

URL: https://github.com/WordPress/gutenberg/pull/76047#issuecomment-4031192131

Based on discussion in https://github.com/WordPress/gutenberg/issues/76325 we might need to include a smaller value in the scale for certain popovers and menus. Something around `240px` might work well.

## Review threads

### Review thread 1 (resolved)

#### mirka on 2026-03-10T16:12:38Z

URL: https://github.com/WordPress/gutenberg/pull/76047#discussion_r2912886817
...
The values in the parentheses are stale (but also I don't think we necessarily need to write out the values in the changelog).

## Changed files

- packages/theme/CHANGELOG.md (MODIFIED, +1/-0)
- packages/theme/docs/tokens.md (MODIFIED, +23/-17)
- packages/theme/src/prebuilt/css/design-tokens.css (MODIFIED, +12/-0)
- packages/theme/src/prebuilt/js/design-token-fallbacks.mjs (MODIFIED, +6/-0)
- packages/theme/src/prebuilt/js/design-tokens.mjs (MODIFIED, +6/-0)
- packages/theme/src/prebuilt/ts/token-types.ts (MODIFIED, +5/-0)
- packages/theme/terrazzo.config.ts (MODIFIED, +5/-0)
- packages/theme/tokens/dimension.json (MODIFIED, +44/-0)
