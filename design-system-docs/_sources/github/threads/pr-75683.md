...
type: pull_request
state: MERGED
merged_at: 2026-02-18T14:28:33Z
url: https://github.com/WordPress/gutenberg/pull/75683
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components"]
---

# PR #75683: DateCalendar, DateRangeCalendar: use lighter gray for disabled dates

- URL: https://github.com/WordPress/gutenberg/pull/75683
- Author: ciampo
- Created: 2026-02-18T13:13:50Z
- Updated: 2026-02-24T14:10:17Z
- Comments: 6 of 6
- Review threads: 0 of 0
- Changed files: 2 of 2
- Diff: ../pr-diffs/75683.diff
...
### github-actions on 2026-02-18T13:14:00Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3920771037
...
### github-actions on 2026-02-18T13:31:08Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3920849759
...
| :--- | :---: | :---: |
| `build/styles/components/style-rtl.css` | 17.6 kB | +7 B (+0.04%) |
| `build/styles/components/style-rtl.min.css` | 14.4 kB | +6 B (+0.04%) |
| `build/styles/components/style.css` | 17.7 kB | +7 B (+0.04%) |
| `build/styles/components/style.min.css` | 14.4 kB | +6 B (+0.04%) |
...
| `build/styles/edit-widgets/style-rtl.css` | 5.19 kB |
| `build/styles/edit-widgets/style-rtl.min.css` | 3.79 kB |
| `build/styles/edit-widgets/style.css` | 5.18 kB |
...
### github-actions on 2026-02-18T13:42:02Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3920902112

<!-- flaky-tests-report-comment -->
**Flaky tests detected in b20f889e5c125a858eeead6a43bbb911b9eeeb78.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/22141382347
📝  Reported issues:
- #75667 in `/test/e2e/specs/site-editor/page-list.spec.js`

### fcoveram on 2026-02-23T15:50:56Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3945620762

If I understand correctly, the prev/next month chevrons and a selected but disabled date are visible in the screenshot with the new color for DateCalendar and DateRangeCalendar, right? If so, it looks odd that the unchanged elements remain with the previous color.

Not sure if there are follow ups for those so the whole calendar, with all the internal elements, look consistent on what interactive elements are enabled/disabled

### ciampo on 2026-02-24T11:11:26Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3950952567

> If so, it looks odd that the unchanged elements remain with the previous color.

@fcoveram I fully understand your feedback. I opted for keeping the color of disabled chevrons and disabled (but selected) date background unchanged for two reasons:

- consistency with other disabled UI across components (which use `gray-600`). The usage of `gray-400` in this PR is the real exception
- the new shade of gray introduced for the disabled (non-selected) date is lighter, and my concern is that the reduced contrast would make it harder for end-users to discern those pieces of UI. Using a lighter shade of gray is, IMO, ok for disabled (non selected) dates also thanks to the "visual context", ie. a sighted user will recognize a calendar layout and will correctly expext the dates to be in a given place on the screen, even if the contrast is lower.

In other words: I tried to pick the most sensible compromise, given that we wouldn't be able to achieve package-wide consistency unless we do a full overhaul of disabled colors. Hopefully switching to DS tokens will allow us to bring such consistency back.

### fcoveram on 2026-02-24T14:10:17Z

URL: https://github.com/WordPress/gutenberg/pull/75683#issuecomment-3952182783

Thanks for explaining the change.

> …given that we wouldn't be able to achieve package-wide consistency unless we do a full overhaul of disabled colors. Hopefully switching to DS tokens will allow us to bring such consistency back.

My comment didn't intend to block this progress, and the strategy described above makes sense to me.
...
- packages/components/src/calendar/style.scss (MODIFIED, +4/-2)
