...
type: pull_request
state: MERGED
merged_at: 2026-02-05T20:43:55Z
url: https://github.com/WordPress/gutenberg/pull/74986
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Task","[Package] UI"]
---

# PR #74986: UI: Remove Box component abstraction

- URL: https://github.com/WordPress/gutenberg/pull/74986
- Author: aduth
- Created: 2026-01-27T20:21:32Z
- Updated: 2026-02-05T20:44:50Z
- Comments: 6 of 6
- Review threads: 1 of 1
- Changed files: 11 of 11
- Diff: ../pr-diffs/74986.diff
...
Removes the `Box` component from `@wordpress/ui`.

## Why?

See rationale in #72784.

## How?

- Removed `Box` implementation
- Updated existing usage in `Badge` to use equivalent design token CSS property styles
   - Plus a bit of clean-up to simplify default rendering of `span`, now that it is directly responsible for rendering element
- Updated UI package README examples to use `Stack` component as replacement for `Box`

## Testing Instructions

Verify that `Badge` and `Stack` stories are unaffected:

1. `npm run storybook:dev`
2. Go to local [Badge](http://localhost:50240/?path=/story/design-system-components-badge--all-intents) and [Stack](http://localhost:50240/?path=/docs/design-system-components-stack--docs) stories
3. Observe visual appearance is identical to live Storybook for [Badge](https://wordpress.github.io/gutenberg/?path=/story/design-system-components-badge--all-intents) and [Stack](https://wordpress.github.io/gutenberg/?path=/docs/design-system-components-stack--docs)

## Issue comments

### github-actions on 2026-01-27T20:21:59Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3807358697
...
### aduth on 2026-02-04T20:46:20Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3849664168

Thanks @mirka , pushed a rebase.

Something to consider: In the rebase I nearly overlooked a token rename for border radiuses, and it wasn't flagged by the lint rule. The way we're using CSS properties in [the Badge implementation](https://github.com/WordPress/gutenberg/blob/trunk/packages/ui/src/badge/badge.tsx) means they're not checked by the lint rule you added in #74325, since they're not applied through the `style` prop. Do you think this is common enough to be concerned? 

It's also not entirely clear to me how we'd support those, other than checking every string (ouch), or maybe looking at variables whose names are `style`, or whose TypeScript types are `CSSProperties`.

### github-actions on 2026-02-04T20:55:12Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3849698568
...
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 328 kB | +39 B (+0.01%) |
| `build/scripts/edit-site/index.min.js` | 244 kB | +23 B (+0.01%) |
| `build/scripts/editor/index.min.js` | 323 kB | -17 B (-0.01%) |
...
### ciampo on 2026-02-05T10:30:10Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3852638509

I'm not sure that linting every string is a good idea for the performance hit it may cause.

Should we generally move away from inline styles and use `is-[intent]-intent` classnames instead, for a better lintability of the code and easier style composition?

### mirka on 2026-02-05T15:12:55Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3854307153

> Should we generally move away from inline styles and use `is-[intent]-intent` classnames instead, for a better lintability of the code and easier style composition?

Sounds reasonable, at least for first-party code.

### aduth on 2026-02-05T19:44:10Z

URL: https://github.com/WordPress/gutenberg/pull/74986#issuecomment-3855835551

Aha, that's a good point, and is probably a better general approach to the problem I highlighted. I recall we had discussed this already in https://github.com/WordPress/gutenberg/pull/73875#discussion_r2613998548 with the initial expectation we'd refactor the inline styles to `Box` & `Text`, but since we're moving away from that, it makes sense to introduce the dedicated stylesheet. Updated in 3e4cfef.

## Review threads

### Review thread 1

#### aduth on 2026-02-05T19:54:18Z

URL: https://github.com/WordPress/gutenberg/pull/74986#discussion_r2770888465
Path: packages/ui/src/badge/style.module.css
...
I considered a CSS property [like what we do in Button](https://github.com/WordPress/gutenberg/blob/db3b37a28f9e548043c345620cae425be3667249/packages/ui/src/button/style.module.css#L14), but ... seems overkill? And maybe encourages extensibility we don't want? This mirrors the existing implementation.

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +4/-0)
- packages/ui/README.md (MODIFIED, +13/-12)
- packages/ui/src/badge/badge.tsx (MODIFIED, +19/-78)
- packages/ui/src/badge/style.module.css (ADDED, +48/-0)
- packages/ui/src/box/box.tsx (DELETED, +0/-107)
- packages/ui/src/box/index.ts (DELETED, +0/-1)
- packages/ui/src/box/stories/index.story.tsx (DELETED, +0/-49)
- packages/ui/src/box/test/box.test.tsx (DELETED, +0/-29)
- packages/ui/src/box/types.ts (DELETED, +0/-61)
- packages/ui/src/index.ts (MODIFIED, +0/-1)
- packages/ui/src/stack/stories/index.story.tsx (MODIFIED, +2/-3)
