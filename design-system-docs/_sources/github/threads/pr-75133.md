...
type: pull_request
state: MERGED
merged_at: 2026-02-03T16:17:04Z
url: https://github.com/WordPress/gutenberg/pull/75133
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #75133: ui/`Button`: add min width

- URL: https://github.com/WordPress/gutenberg/pull/75133
- Author: ciampo
- Created: 2026-02-02T13:17:07Z
- Updated: 2026-02-03T16:26:42Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 3 of 3
- Diff: ../pr-diffs/75133.diff
...
### github-actions on 2026-02-02T13:21:18Z

URL: https://github.com/WordPress/gutenberg/pull/75133#issuecomment-3835089531
...
### github-actions on 2026-02-02T13:30:50Z

URL: https://github.com/WordPress/gutenberg/pull/75133#issuecomment-3835140042
...
### ciampo on 2026-02-02T15:58:49Z

URL: https://github.com/WordPress/gutenberg/pull/75133#issuecomment-3836083915

I guess one question is whether we want to apply similar changes to the `Button` component from `@wordpress/components`, although I'm always a bit wary of applying changes to that component 😅

### jasmussen on 2026-02-03T09:15:08Z

URL: https://github.com/WordPress/gutenberg/pull/75133#issuecomment-3840069630

I like the video, thanks for doing this. I'll defer to Andrew and Jay if he chimes in on technical pieces, though ask about this one:

```
--wp-ui-button-min-width: calc(4ch + 2 * var(--wp-ui-button-padding-inline));
```

Should this be a CSS variable? Is this how an implementer should change the min-width? Or should they not change the min-width from the default 4?

### ciampo on 2026-02-03T10:43:04Z

URL: https://github.com/WordPress/gutenberg/pull/75133#issuecomment-3840544224

The `--wp-ui-button-min-width` exists mostly as an internal variable to the UI package (for example, it's used by `IconButton` to unset it). I'm not sure if we have a documented official stance on `--wp-ui-*` variables, whether we advise consumers against reading/writing them or not.

Regardless, my preference would be to assume that the Button's min width is a DS decision that should not be advertised as tweakable for the consumers — after all, we're using a quite conservative `4ch` value. If we find that this min-width gets in the way of our consumers more than it helps them, then we may as well not have an internal min-width at all and expect consumers to add one if/when needed with custom styles?
...
- packages/ui/src/button/style.module.css (MODIFIED, +3/-0)
