...
type: pull_request
state: MERGED
merged_at: 2026-02-16T17:26:34Z
url: https://github.com/WordPress/gutenberg/pull/75568
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #75568: ui/Button: fix disabled styles and variable composition

- URL: https://github.com/WordPress/gutenberg/pull/75568
- Author: ciampo
- Created: 2026-02-16T11:20:34Z
- Updated: 2026-02-25T15:29:42Z
- Comments: 5 of 5
- Review threads: 1 of 1
- Changed files: 2 of 2
- Diff: ../pr-diffs/75568.diff
...
### github-actions on 2026-02-16T11:32:20Z

URL: https://github.com/WordPress/gutenberg/pull/75568#issuecomment-3907972309
...
### github-actions on 2026-02-16T13:09:10Z

URL: https://github.com/WordPress/gutenberg/pull/75568#issuecomment-3908418128
...
### ciampo on 2026-02-16T17:26:27Z

URL: https://github.com/WordPress/gutenberg/pull/75568#issuecomment-3909696451

Going to merge, but happy to reconsider which attribute(s) to use for disabled styles in a follow-up

> I wonder if there's any takeaways here for things to look out for in other components, maybe documenting rules. Things like states, use of CSS properties, cascade layers, etc. 

My main takeaway is to avoid re-defining variables for states, since it could easily break compoasability via overrides.

Not sure if there's a good place to document it?

### aduth on 2026-02-16T20:59:56Z

URL: https://github.com/WordPress/gutenberg/pull/75568#issuecomment-3910489992
...
Maybe we want to hazard this as a pitfall of [the documented CSS layers](https://github.com/WordPress/gutenberg/blob/trunk/packages/ui/CONTRIBUTING.md#css-layers) ? If I'm following, is the issue that higher layers will always take precedent regardless of selector specificity, so if we want specificity to "win" through in the lower layers, it should do so through by bypassing the CSS properties that are exposed for those higher layers?

### ciampo on 2026-02-25T15:29:42Z

URL: https://github.com/WordPress/gutenberg/pull/75568#issuecomment-3960098294

> Maybe we want to hazard this as a pitfall of [the documented CSS layers](https://github.com/WordPress/gutenberg/blob/trunk/packages/ui/CONTRIBUTING.md#css-layers) ? If I'm following, is the issue that higher layers will always take precedent regardless of selector specificity, so if we want specificity to "win" through in the lower layers, it should do so through by bypassing the CSS properties that are exposed for those higher layers?

Opened https://github.com/WordPress/gutenberg/pull/75912 to add a section to the contrubuting guidelines, as suggested.

## Review threads

### Review thread 1

#### aduth on 2026-02-16T15:54:50Z

URL: https://github.com/WordPress/gutenberg/pull/75568#discussion_r2813095143
...
Is the `aria-disabled` ➡️  `data-disabled` change relevant to what's being fixed here? I recall this coming up in an earlier pull request and personally recommending the `aria-` property to at least avoid some amount of vendor lock-in effect.

#### ciampo on 2026-02-16T16:25:05Z

URL: https://github.com/WordPress/gutenberg/pull/75568#discussion_r2813218687
Path: packages/ui/src/button/style.module.css
...
It's relevant because Base UI applies `disabled` or `aria-disabled="true"` based on the value of `focusableWhenDisabled`.

I guess an alternative could be to target both `disabled` and `aria-disabled="true"` in the CSS selectors, although [the Base UI docs](https://base-ui.com/react/components/button#:~:text=Description-,data%2Ddisabled,-Present%20when%20the) refer to `data-disabled` as the public attribute associated with a disabled button.

What do you think?

#### mirka on 2026-02-16T17:40:02Z

URL: https://github.com/WordPress/gutenberg/pull/75568#discussion_r2813476669
Path: packages/ui/src/button/style.module.css
...
I also eventually reached the conclusion that `data-disabled` was preferable for our styles. I was also concerned about the vendor lock-in point, but it's also an advantage in some ways, for example:

- If vendor behavior changes subtly (e.g. what was previously a `disabled` changed to a `aria-disabled`), we don't have to update our styles.
- Shared styles (e.g. between `Select` and `Autocomplete`) can be written simply as `data-disabled`, even if the underlying disable behavior differs across components.

Targeting both `disabled` and `aria-disabled="true"` for everything can get pretty verbose and hard to read, especially when it's needed in a bunch of `:not()` expressions as well.

#### aduth on 2026-02-16T21:01:50Z

URL: https://github.com/WordPress/gutenberg/pull/75568#discussion_r2814030753
Path: packages/ui/src/button/style.module.css
...
I don't really have strong feelings. In retrospect, the "vendor lock-in" point is a weak one if it's one that's coming from a place of avoiding breakage if we were to move away from it in the future, since ideally we'd have stronger guarantees of it being styled as expected than just relying on "standards-based" attributes. (e.g. visual regression testing)

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +2/-0)
- packages/ui/src/button/style.module.css (MODIFIED, +17/-9)
