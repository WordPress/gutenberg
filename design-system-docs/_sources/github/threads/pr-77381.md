...
type: pull_request
state: MERGED
merged_at: 2026-04-16T17:18:17Z
url: https://github.com/WordPress/gutenberg/pull/77381
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77381: UI: Fix focus-trap broken by ThemeProvider's display:contents

- URL: https://github.com/WordPress/gutenberg/pull/77381
- Author: ciampo
- Created: 2026-04-15T14:22:11Z
- Updated: 2026-04-16T17:18:58Z
- Comments: 11 of 11
- Review threads: 1 of 1
- Changed files: 6 of 6
- Diff: ../pr-diffs/77381.diff

## Body

## What?

Fix the focus-trap regression in `@wordpress/ui` overlay popups after the Base UI 1.4 update.

## Why?

PR #77308 updated `@base-ui/react` from 1.3.0 to 1.4.0. Base UI 1.4 replaced the external `tabbable` dependency with an internal implementation that uses `checkVisibility()` semantics for visibility detection.

`ThemeProvider` wraps popup elements and uses `display: contents`, which means it does not generate a layout box. That causes popup descendants to be treated as non-tabbable during the ancestor walk, breaking focus-trap cycling.

## How?

Temporarily override `display: contents` to `display: block` on the `ThemeProvider` wrapper when it directly wraps a popup element, using the CSS rule:
...
The rule is unlayered so it wins over the unlayered `display: contents` from `ThemeProvider`. The popup is `position: fixed` or positioned by a positioner, so the extra block wrapper is layout-inert for these overlay cases.
...
This keeps the fix local to the affected overlay styles without expanding the `ThemeProvider` API, while we reassess whether the underlying visibility detection should be addressed upstream in Base UI.
...
1. Open Storybook examples for `Dialog`, `AlertDialog`, `Popover`, and `Select`.
2. Verify the overlay opens and pressing `Tab` cycles focus through all interactive elements.
3. Verify `Shift+Tab` cycles focus in reverse.

## Verification

- `npm run lint:js -- packages/ui/src/alert-dialog/popup.tsx packages/ui/src/dialog/popup.tsx packages/ui/src/form/primitives/select/popup.tsx packages/ui/src/popover/popup.tsx packages/ui/src/tooltip/popup.tsx`
...
- `npm run test:unit -- packages/ui/src/dialog/test/index.test.tsx packages/ui/src/popover/test/index.test.tsx packages/ui/src/utils/test/use-deprioritized-initial-focus.test.tsx`

## Issue comments

### github-actions on 2026-04-15T14:22:54Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4252868659

The following accounts have interacted with this PR and/or linked issues. I will continue to update these lists as activity occurs. You can also manually ask me to refresh this list by adding the `props-bot` label.

## Unlinked Accounts

The following contributors have not linked their GitHub and WordPress.org accounts: @atomiks.
...
### github-actions on 2026-04-15T14:35:14Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4252960381
...
### github-actions on 2026-04-15T14:54:13Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4253104099

<!-- flaky-tests-report-comment -->
**Flaky tests detected in c718bb008ae85326b9042ffe4144883c70cf6c63.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24522131923
📝  Reported issues:
...
### aduth on 2026-04-15T15:19:33Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4253287332

> That causes popup descendants to be treated as non-tabbable during the ancestor walk

Is this a correct / expected behavior on BaseUI's part? 🤔 

I guess I'm wondering if this is something that seems reasonably expected behavior for box layout semantics that we'd want to expand the props API to cover this, especially when we didn't need it before. 

I don't know that either of these alternatives are great, but wondering:

- Could we support pass-through props on `ThemeProvider` that these instances could pass their own `className` or `style` props to override the `display: contents`. Downside being that this feels like overriding specific styles that we don't want downstream consumers to be aware of and/or overriding (in the same way we adopted CSS modules in the first place).
- Could `display: block` be the default? What downsides would we face in that approach?
- If it's not an expected behavior on BaseUI's part, what options do we have to intervene in BaseUI's handling?

### mirka on 2026-04-15T15:25:51Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4253328519

Like we see in #77191, `checkVisibility` is not a sufficient visibility check in many cases. So ideally Base UI hardens their detection logic so it covers cases like this, or allows an opt-in through e.g. a data attribute like we do in the `isElementVisible` case.

If we agree that we should try getting it addressed upstream, then any temporary fix here should perhaps not involve adding a new public prop to `ThemeProvider`.

### ciampo on 2026-04-15T17:52:17Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4254267927

If you look at the first commit in this PR, would that be a better temporary fix while we bring the concern to Base UI?

### mirka on 2026-04-15T18:02:42Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4254336955
...
### ciampo on 2026-04-15T23:11:40Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4256304336

Updates:

- Updated the PR to use the first approach
- Opened https://github.com/mui/base-ui/issues/4622 to report our findings and requests upstream


@aduth I would prefer to avoid exposing `className` and/or `style` on `ThemeProvider` because it kind of opens a very easy way for consumers to mess up with it. And I would also prefer to keep `display: contents` around, since `ThemeProvider` should really be layout-inert. Let's see what base ui folks say.

### atomiks on 2026-04-16T11:23:28Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4259635592

Sorry for the regression here. I didn't realize `checkVisibility` is `false` for `display: contents` which seems counterintuitive to me, though MDN clearly states it. 

We will harden this logic so `display: contents` ancestors don't break tabbable logic. Is there a need for us to do a patch release soon for you, or is this mostly fixed by this PR for the time being?

### ciampo on 2026-04-16T13:11:20Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4260312073

@atomiks Re. popup tabbability, I _think_ we should be able to hold off with this workaround until 1.5.

@mirka re. https://github.com/mui/base-ui/issues/4622,  do you think we can hold too?

### mirka on 2026-04-16T14:03:15Z

URL: https://github.com/WordPress/gutenberg/pull/77381#issuecomment-4260673876
...
#### mirka on 2026-04-16T09:22:00Z

URL: https://github.com/WordPress/gutenberg/pull/77381#discussion_r3092118689
...
#### ciampo on 2026-04-16T16:36:19Z

URL: https://github.com/WordPress/gutenberg/pull/77381#discussion_r3094811138
Path: packages/ui/src/popover/style.module.css
...
Good catch, fixed in c718bb008ae.
...
- packages/ui/src/dialog/style.module.css (MODIFIED, +12/-0)
- packages/ui/src/popover/popup.tsx (MODIFIED, +4/-1)
- packages/ui/src/popover/style.module.css (MODIFIED, +23/-1)
- packages/ui/src/tooltip/style.module.css (MODIFIED, +12/-0)
- packages/ui/src/utils/css/item-popup.module.css (MODIFIED, +12/-0)
