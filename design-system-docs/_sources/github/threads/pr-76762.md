...
type: pull_request
state: MERGED
merged_at: 2026-03-30T18:29:24Z
url: https://github.com/WordPress/gutenberg/pull/76762
matched_labels: ["[Package] Theme"]
all_labels: ["[Type] Enhancement","[Package] Theme"]
---

# PR #76762: Theme: Change default control cursor to `pointer`

- URL: https://github.com/WordPress/gutenberg/pull/76762
- Author: mirka
- Created: 2026-03-23T13:02:42Z
- Updated: 2026-03-30T18:30:12Z
- Comments: 3 of 3
- Review threads: 2 of 2
- Changed files: 5 of 5
- Diff: ../pr-diffs/76762.diff

## Body

## What?

Changes the default value of the `--wpds-cursor-control` design token from `default` to `pointer`.

## Why?

As `@wordpress/ui` components start appearing alongside `@wordpress/components` in the editor, using `default` cursors on the new components creates an inconsistent experience when legacy components still use `pointer` cursors. Additionally, many `@wordpress/components` lack sufficient hover states to compensate for losing the `pointer` affordance ([discussion](https://github.com/WordPress/gutenberg/pull/76282#discussion_r2961863380)).

Rather than rushing the shift to `default` cursors, the agreed approach is to keep `pointer` as the default while the cursor token continues to be propagated across packages. The global switch to `default` can happen once hover states on existing componentry are in place.

## Testing Instructions

1. Open Storybook and navigate to any `@wordpress/ui` component story that renders interactive controls (e.g. Button, Toggle).
2. Use the theming toolbar to switch between themes/configurations.
3. Verify that interactive controls use a `pointer` cursor by default across all theme settings.
4. Use the theming toolbar to set the cursor control to `default` and confirm the override applies correctly.
...
### github-actions on 2026-03-23T13:08:08Z

URL: https://github.com/WordPress/gutenberg/pull/76762#issuecomment-4110480509
...
### github-actions on 2026-03-23T13:19:23Z

URL: https://github.com/WordPress/gutenberg/pull/76762#issuecomment-4110549426
...
| `build/scripts/edit-site/index.min.js` | 260 kB | +2 B (0%) |
| `build/scripts/editor/index.min.js` | 410 kB | +1 B (0%) |
...
### github-actions on 2026-03-23T13:33:00Z

URL: https://github.com/WordPress/gutenberg/pull/76762#issuecomment-4110636577

<!-- flaky-tests-report-comment -->
**Flaky tests detected in e094c283e55dd97e28d100f22f96e57283a06313.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23759338995
📝  Reported issues:
...
#### aduth on 2026-03-24T17:48:35Z

URL: https://github.com/WordPress/gutenberg/pull/76762#discussion_r2983310112
...
<img width="1050" height="108" alt="image" src="https://github.com/user-attachments/assets/de676c4c-ad33-4a25-beed-7c1d6a1ac667" />

But it's already pretty broken on [the Developer Site copy of this document](https://developer.wordpress.org/block-editor/reference-guides/packages/packages-theme/):

<img width="870" height="129" alt="image" src="https://github.com/user-attachments/assets/22795388-6980-4543-8c89-354f6b6ccaf5" />

Maybe we should just revert it to a standard paragraph.

#### mirka on 2026-03-30T17:52:38Z

URL: https://github.com/WordPress/gutenberg/pull/76762#discussion_r3011324043
Path: packages/theme/README.md
...
Agreed, fixed in e094c283e55dd97e28d100f22f96e57283a06313

### Review thread 2

#### ciampo on 2026-03-26T11:05:16Z

URL: https://github.com/WordPress/gutenberg/pull/76762#discussion_r2994039792
Path: packages/theme/README.md

```diff
@@ -138,7 +137,11 @@ import { ThemeProvider } from '@wordpress/theme';
...
Does it make sense to add `cursor: pointer` to the example, now that it's the default value?

#### mirka on 2026-03-30T17:52:21Z

URL: https://github.com/WordPress/gutenberg/pull/76762#discussion_r3011322677
Path: packages/theme/README.md
...
Removed in 6b7d120ce593a6c15270a55e0b468a2582aab59f

## Changed files

- packages/theme/CHANGELOG.md (MODIFIED, +4/-0)
- packages/theme/README.md (MODIFIED, +18/-17)
...
- packages/theme/tokens/cursor.json (MODIFIED, +1/-1)
