...
type: pull_request
state: MERGED
merged_at: 2026-04-28T06:43:34Z
url: https://github.com/WordPress/gutenberg/pull/77552
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components","[Package] Block library","[Package] DataViews"]
---

# PR #77552: `FormTokenField`: Add `help` prop to render additional help text below the field

- URL: https://github.com/WordPress/gutenberg/pull/77552
- Author: ntsekouras
- Created: 2026-04-22T10:08:48Z
- Updated: 2026-04-28T06:44:12Z
- Comments: 3 of 3
- Review threads: 6 of 6
- Changed files: 13 of 13
- Diff: ../pr-diffs/77552.diff
...
### github-actions on 2026-04-22T10:22:02Z

URL: https://github.com/WordPress/gutenberg/pull/77552#issuecomment-4295409787
...
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 341 kB | -5 B (0%) |
| `build/scripts/block-library/index.min.js` | 319 kB | -21 B (-0.01%) |
| `build/scripts/components/index.min.js` | 266 kB | +205 B (+0.08%) |
| `build/scripts/edit-site/index.min.js` | 264 kB | -8 B (0%) |
| `build/scripts/editor/index.min.js` | 423 kB | -5 B (0%) |
| `build/scripts/media-utils/index.min.js` | 79.4 kB | -6 B (-0.01%) |
...
### github-actions on 2026-04-22T10:39:57Z

URL: https://github.com/WordPress/gutenberg/pull/77552#issuecomment-4295507207

<!-- flaky-tests-report-comment -->
**Flaky tests detected in aaa1eeef86e1f5b7efc70355d3686f6ff3bd64e6.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25036543967
📝  Reported issues:
- #77720 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### github-actions on 2026-04-22T19:18:38Z

URL: https://github.com/WordPress/gutenberg/pull/77552#issuecomment-4299267964
...
#### mirka on 2026-04-22T16:39:33Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3125502209
...
Let's keep the todo comment, it isn't resolved.

### Review thread 2

#### tyxla on 2026-04-22T19:16:25Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3126365146
...
`__experimentalShowHowTo` always felt odd to me as it didn't allow customization.

Somehow it feels like a missed opportunity that we're not just allowing the prop to customize the text - just like your new `help` prop would.  

Having both enabled feels odd right now, and is underlined by the fact that there are two `.components-form-token-field__help` elements:

<img width="529" height="130" alt="Image" src="https://github.com/user-attachments/assets/6023e20a-58dc-4478-8921-abe023135403" />

So should we display the `__experimentalShowHowTo` content only if `help` is not provided (and if `__experimentalShowHowTo` is `true` of course? 

Or is this time to rethink this entire `__experimentalShowHowTo` prop and remove it altogether, leaving the logic solely to the outsider and exposing a simple string `help` prop? How frequently is the `__experimentalShowHowTo` prop disabled vs. kept on (which is the default)?

#### ntsekouras on 2026-04-23T07:14:58Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3128969290
Path: packages/components/src/form-token-field/index.tsx
...
I agree with you that if both are provided it can be a bit odd visually, but they are kind of different help texts. `__experimentalShowHowTo` is a keyboard-interaction hint where the `help` could be anything related to the control. 

It's unfortunate that `__experimentalShowHowTo` defaults to `true` but I don't think the component should auto-hide one conditionally, as a consumer could want both of them. Maybe a small icon next to the label with a tooltip would be a good place to add the `__experimentalShowHowTo` message? 🤔 

Maybe another direction would be to combine both messages in a single help message? That could have some nuances though since `help` is a `ReactNode`.

Do you think we need to decide this on this PR or it can be a follow up?

#### mirka on 2026-04-23T14:46:46Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3131649762
Path: packages/components/src/form-token-field/index.tsx
...
I don't think we can push decision to follow up, because that could involve a breaking change. Better to decide now, rather than change behavior later.

> I don't think the component should auto-hide one conditionally, as a consumer could want both of them

I would say that they are free to add whatever message to the `help` text themselves, including the current "how to" text. What do you think about defaulting `help` to the current "how to"  message? Then we could perhaps deprecate the `__experimentalShowHowTo` prop, with some back compat logic to honor `__experimentalShowHowTo = false`.

> Maybe a small icon next to the label with a tooltip would be a good place to add the `__experimentalShowHowTo` message? 🤔

This type of thing is not going to be a supported pattern, due to scalability and accessibility reasons.

#### tyxla on 2026-04-23T20:40:21Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3133683181
Path: packages/components/src/form-token-field/index.tsx
...
I agree with Lena on the direction. And I'd love that `__experimentalShowHowTo` prop deprecated in favor of a more flexible `help` prop.

#### ntsekouras on 2026-04-24T06:20:32Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3135825276
Path: packages/components/src/form-token-field/index.tsx
...
Sounds good. See: https://github.com/WordPress/gutenberg/pull/77552/changes/704adf781e5b086fa3faf72ecfe8866a4d7a3b04.

My logic is:
1. If `help` is provided it takes precedence over `__experimentalShowHowTo`
2. By default `help` contains the previous `how to` message
3. If `__experimentalShowHowTo:false` and `help` is not passed, render nothing

### Review thread 3

#### ntsekouras on 2026-04-24T06:21:41Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3135830179
...
Should this be split in two entries for the same PR? 🤔 

I saw there are entries in the changelog with `Deprecations` sections.

#### mirka on 2026-04-24T12:46:59Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3137768384
Path: packages/components/CHANGELOG.md
...
Yes, a separate entry under a `Deprecations` heading may be good for scanning.

#### tyxla on 2026-04-24T15:32:37Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3138716654
Path: packages/components/CHANGELOG.md
...
Yeah, we've had multiple changelog entries per PR many times. This seems like a good case.

#### ntsekouras on 2026-04-27T05:24:31Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3145034644
Path: packages/components/CHANGELOG.md
...
I used two entries. Any wording suggestions are welcome.

### Review thread 4 (resolved)

#### mirka on 2026-04-24T12:49:41Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3137782915
Path: packages/components/src/form-token-field/index.tsx
...
This is going to log warnings at all the existing call sites in the codebase (I count 6 instances), so we usually migrate them before (or together with) starting logging.

### Review thread 5

#### mirka on 2026-04-24T12:56:09Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3137817198
...
Is this necessary by the way? I don't think I've ever needed it so far, I wonder why.

#### ntsekouras on 2026-04-27T05:48:11Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3145100586
Path: packages/components/src/form-token-field/test/index.tsx
...
Is need when we have more than one test case for deprecated logged messages. It's because of `deprecated` internal logic to log unique messages.

### Review thread 6

#### ntsekouras on 2026-04-27T05:50:56Z

URL: https://github.com/WordPress/gutenberg/pull/77552#discussion_r3145109441
Path: packages/dataviews/src/components/dataform-controls/array.tsx
...
I think it's fine not add a DataViews changelog entry for this change.
...
- packages/dataviews/src/components/dataform-controls/array.tsx (MODIFIED, +1/-1)
