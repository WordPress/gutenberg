...
type: pull_request
state: MERGED
merged_at: 2026-01-14T05:08:19Z
url: https://github.com/WordPress/gutenberg/pull/74560
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Build Tooling","[Package] Components","[Package] Sync"]
---

# PR #74560: Update TypeScript base config to use bundler module resolution

- URL: https://github.com/WordPress/gutenberg/pull/74560
- Author: aduth
- Created: 2026-01-12T19:29:24Z
- Updated: 2026-01-14T12:46:10Z
- Comments: 4 of 4
- Review threads: 4 of 4
- Changed files: 13 of 13
- Diff: ../pr-diffs/74560.diff

## Body

## What?

Extracted from #74396

Updates TypeScript configuration to use `'bundler'` option as the default for `'moduleResolution'`.

**Draft:** ~I was seeing some issues with how the `theme` package splits its configuration to `tsconfig.bin.json` and `tsconfig.src.json` and curious to see if it's a local issue or if it persists into CI.~

## Why?

- Because it is the most correct of [the available options](https://www.typescriptlang.org/tsconfig/#moduleResolution):
   - We build the project using a bundler (ESBuild), and specifically the `'bundler'` option is documented as unique in its behavior to not require file extensions on relative paths (like a bundler)
   - `'node'` is documented as "for Node.js versions older than v10" and "You probably won’t need to use [it] in modern code"
- Because some dependencies (e.g. Storybook v10 in #74396) require it
- Because its strictness can be considered a benefit (e.g. validates correctness of `exports`, [see example](https://github.com/WordPress/gutenberg/pull/74310#discussion_r2683351637))

## How?

- Updates `moduleResolution` option in base, shared TypeScript configuration
- Fixes existing issues (e.g. issues stemming from `exports`)
...
### github-actions on 2026-01-12T19:40:33Z

URL: https://github.com/WordPress/gutenberg/pull/74560#issuecomment-3740198786
...
| :--- | :---: | :---: |
| `build/scripts/components/index.min.js` | 267 kB | +173 B (+0.06%) |
...
### github-actions on 2026-01-12T19:50:00Z

URL: https://github.com/WordPress/gutenberg/pull/74560#issuecomment-3740233699
...
### aduth on 2026-01-12T19:51:33Z

URL: https://github.com/WordPress/gutenberg/pull/74560#issuecomment-3740239336

I had been seeing some build issues with the `theme` package locally but they don't seem to be happening in CI, so hopefully this should be good as-is. I'm not sure if it's a local cache issue, as it seemed rather intermittent and hard to pin down or fix consistently.

Marking as ready for review.

### aduth on 2026-01-13T20:01:41Z

URL: https://github.com/WordPress/gutenberg/pull/74560#issuecomment-3746330697

Thanks for the suggested improvements @manzoorwanijk . All of those should be reflected now in the branch.

## Review threads

### Review thread 1

#### aduth on 2026-01-12T19:52:43Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2683697312
...
See explanation at https://github.com/WordPress/gutenberg/pull/74396#discussion_r2666393853 :
...
#### aduth on 2026-01-12T19:53:01Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2683698483
...
See explanation at https://github.com/WordPress/gutenberg/pull/74396#discussion_r2666396893 :
...
#### aduth on 2026-01-12T19:54:32Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2683704038
...
This is the one package where I couldn't really figure out a great solution except to revert it back to `'node'` module resolution. It's one of those "type portability" problems with how an [internal vendored class implementation is extended](https://github.com/WordPress/gutenberg/blob/0e10309d7d92d4a524b62a19bc1e910e6ca03acc/packages/sync/src/y-utilities/y-multidoc-undomanager.js#L43). It's interesting that the `// @ts-nocheck` at the top of that file isn't enough to suppress the errors.

#### manzoorwanijk on 2026-01-13T07:57:47Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2685274522
Path: packages/sync/tsconfig.json
...
> It's interesting that the `// @ts-nocheck` at the top of that file isn't enough to suppress the errors.

Since it's imported by `undo-manager.ts`, it becomes a part of the build. Even if you add `checkJs: false`, it will still throw the error.

A simple solution could be to add a `src/y-utilities/y-multidoc-undomanager.d.ts` file and then exclude the corresponding `.js` file in tsconfig.

<details><summary>I asked Claude Code to create such a file</summary>
...
export class YMultiDocUndoManager extends Observable< string > {
	...
	constructor(
		...
		opts?: NonNullable< ConstructorParameters< typeof Y.UndoManager >[ 1 ] >
	);

	addToScope(
		ytypes: Array< Y.AbstractType< any > > | Y.AbstractType< any >
	): void;
...
#### aduth on 2026-01-13T20:00:51Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2687945736
Path: packages/sync/tsconfig.json
...
Thanks, that seems a lot better than reverting back to `'node'` module resolution for this package. It's nice that we can use this new default universally now! I had to make a revision to use `import { ... as ... }` in your snippet, but otherwise it works well! See 15b1050d7de6cb0121cae7ce54434d34ae4b2886

### Review thread 4

#### manzoorwanijk on 2026-01-13T07:22:38Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2685171623
...
A better and cleaner solution here would be to make `declarations.d.ts` the only source of truth.

- Move `src/declarations.d.ts` to `declarations.d.ts` and update the same in `tsconfig.json`.
- Remove `src` from here (`package.json` > `files`) and add `declarations.d.ts` instead of `index.d.ts`
- Use `declarations.d.ts` for `types` fields.
- Delete `index.d.ts`

That also makes the `build-types` directory useless for this package, and we can remove it from `files` here and add `"noEmit": true` to tsconfig here.


If you allow me, I can push those changes.

<details><summary>Here is the diff (Click to expand)</summary>

```diff
diff --git a/packages/jest-console/src/declarations.d.ts b/packages/jest-console/declarations.d.ts
similarity index 100%
rename from packages/jest-console/src/declarations.d.ts
rename to packages/jest-console/declarations.d.ts
diff --git a/packages/jest-console/index.d.ts b/packages/jest-console/index.d.ts
deleted file mode 100644
index a9100a5b0fd..00000000000
--- a/packages/jest-console/index.d.ts
+++ /dev/null
@@ -1,2 +0,0 @@
...
index 7d385cc2f76..c378b2b81dd 100644
...
index 45d5d4213b4..0e6abbff0f6 100644
...
#### aduth on 2026-01-13T20:01:18Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2687947491
Path: packages/jest-console/package.json
...
This feels much better 👍 Agree all this indirection wasn't helping things, and this is a simpler approach.

Applied it in ff0a1bef8d3cd429f65abd7e56c548e80da7671f

#### aduth on 2026-01-13T20:04:12Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2687958395
Path: packages/jest-console/package.json
...
I'm a bit hesitant about removing `src` in the `files` now that I notice it. I fundamentally agree that it probably shouldn't be included in the published package since everything we need should be defined in `exports` paths, but all the rest of the packages are including it. Maybe there's some scenarios where people are relying on direct imports to `/src/` in environments which allow bypassing `exports`. That feels like an edge case that we should want to discourage since it'll eventually break anyways. In which case I think I'd be okay to start omitting it and see if any disruption is reported.

#### manzoorwanijk on 2026-01-13T22:22:17Z

URL: https://github.com/WordPress/gutenberg/pull/74560#discussion_r2688314344
Path: packages/jest-console/package.json
...
I agree. Let us remove it for now. If needed, we can easily add it back.

## Changed files

- package-lock.json (MODIFIED, +11/-10)
- packages/components/package.json (MODIFIED, +1/-1)
- packages/components/src/context/context-system-provider.js (MODIFIED, +1/-0)
- packages/components/src/index.ts (MODIFIED, +6/-0)
- packages/eslint-plugin/tsconfig.json (MODIFIED, +0/-1)
- packages/jest-console/declarations.d.ts (RENAMED, +0/-0)
- packages/jest-console/index.d.ts (DELETED, +0/-2)
- packages/jest-console/package.json (MODIFIED, +3/-4)
- packages/jest-console/tsconfig.json (MODIFIED, +3/-2)
- packages/report-flaky-tests/tsconfig.json (MODIFIED, +0/-1)
- packages/sync/src/y-utilities/y-multidoc-undomanager.d.ts (ADDED, +32/-0)
- packages/sync/tsconfig.json (MODIFIED, +1/-0)
- tsconfig.base.json (MODIFIED, +2/-2)
