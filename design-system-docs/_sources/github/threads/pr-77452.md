...
type: pull_request
state: MERGED
merged_at: 2026-04-21T09:40:37Z
url: https://github.com/WordPress/gutenberg/pull/77452
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #77452: UI: Portal prop and Portal subcomponents for overlay Popups

- URL: https://github.com/WordPress/gutenberg/pull/77452
- Author: ciampo
- Created: 2026-04-17T12:31:35Z
- Updated: 2026-04-21T09:43:21Z
- Comments: 3 of 3
- Review threads: 12 of 12
- Changed files: 32 of 32
- Diff: ../pr-diffs/77452.diff
...
### github-actions on 2026-04-17T13:12:27Z

URL: https://github.com/WordPress/gutenberg/pull/77452#issuecomment-4268262801

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 27a552df906b120f8ea7a93c24fcc8da364705d4.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24713148780
📝  Reported issues:
- #76524 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`

### github-actions on 2026-04-17T15:48:33Z

URL: https://github.com/WordPress/gutenberg/pull/77452#issuecomment-4269425295
...
### github-actions on 2026-04-17T16:22:40Z

URL: https://github.com/WordPress/gutenberg/pull/77452#issuecomment-4269645425
...
#### copilot-pull-request-reviewer on 2026-04-17T14:17:15Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960476
...
This section is still labeled `describe( 'container' ...)` even though the API moved from `container` to the `portal` prop. Renaming the describe block/test names to `portal` would keep the tests consistent with the new public API.

#### ciampo on 2026-04-17T15:23:44Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101404347
Path: packages/ui/src/form/primitives/select/test/index.test.tsx
...
Addressed in an earlier push: the `describe` block is now `portal`, and the test title refers to the portal container. The DOM target is still configured via `Select.Portal`'s `container` prop (that name comes from Base UI).

### Review thread 2 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T14:17:15Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960516
...
The describe label/test naming here still refers to “container”, but the mechanism is now `portal={ <Popover.Portal container={ ... } /> }`. Consider updating the section name (e.g. “inline via portal container”) so the test terminology matches the new API.

#### ciampo on 2026-04-17T15:23:58Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101406611
Path: packages/ui/src/popover/test/index.test.tsx
...
Updated: the nested `describe` is now `inline via portal (custom container)` and the first test title mentions the portal container. The inline case still passes the mount target through `Popover.Portal`'s `container` prop.

### Review thread 3 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T14:17:16Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960537
Path: packages/ui/src/dialog/popup.tsx

```diff
@@ -59,8 +61,14 @@ const Popup = forwardRef< HTMLDivElement, PopupProps >( function DialogPopup(
...
The portal cloning/wrapping logic is duplicated across Dialog/AlertDialog/Popover/Tooltip/Select popups, but the PR description mentions a shared `render-with-portal` helper that doesn’t appear to exist in the codebase. Consider extracting this into a shared utility (and using it here) to avoid future divergence and keep behavior consistent across overlays.

#### ciampo on 2026-04-17T15:23:59Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101406853
Path: packages/ui/src/dialog/popup.tsx
...
Done in `580539a`: the duplicated `cloneElement` / `isValidElement` block is now `renderPortalChildren` in `packages/ui/src/utils/render-portal-children.ts`, and each overlay `Popup` calls it with `portal`, `<Portal />`, and the popup subtree. The PR description already describes the per-`Popup` pattern; this commit centralizes the merge logic only.

### Review thread 4 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T14:17:16Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960556
Path: packages/ui/src/popover/stories/index.story.tsx

```diff
@@ -443,7 +444,8 @@ export const OverlayPlacement: Story = {
...
+ * `<span>` with `display: contents` and pass it to
+ * `portal={ <Popover.Portal container={ ref } /> }`.
...
This story’s inline rendering docs are now inconsistent with the new API: it says “without a portal”, and later the description implies “no portal is used”, but the example explicitly uses `portal={ <Popover.Portal ... /> }` (still a portal, just targeting an inline container). Please reword the story text so it matches the actual behavior.

#### ciampo on 2026-04-17T15:24:01Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101407031
Path: packages/ui/src/popover/stories/index.story.tsx
...
Reworded earlier: the Inline story JSDoc and `Popover.Description` now explain that `portal={ <Popover.Portal container={…} /> }` still uses a portal, with the portal node mounted into the in-tree span instead of `document.body`.

### Review thread 5 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T14:17:16Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960569
Path: packages/ui/src/tooltip/test/index.test.tsx
...
This test section is still labeled `describe( 'container' ...)` even though the public API moved from `container` to the `portal` prop. Renaming the describe block/test titles to `portal` would keep terminology aligned with the new API and avoid confusion for future contributors.

#### ciampo on 2026-04-17T15:24:02Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101407237
Path: packages/ui/src/tooltip/test/index.test.tsx
...
Addressed earlier: this block uses `describe( 'portal' )` and test titles that refer to the portal / portal container.

### Review thread 6 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T14:17:16Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3100960591
Path: packages/ui/src/dialog/test/index.test.tsx
...
This block is still named `describe( 'container' ...)`, but the API under test is now `portal={ ... }`. Please rename the describe block/test names to use `portal` terminology to match the breaking API change.

#### ciampo on 2026-04-17T15:24:04Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101407432
Path: packages/ui/src/dialog/test/index.test.tsx
...
Same as the other overlay tests: `describe( 'portal' )` plus `it` titles that describe rendering through a custom portal container when `portal={ <Dialog.Portal … /> }` is passed.

### Review thread 7 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T15:35:12Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101477396
Path: packages/ui/src/utils/render-portal-children.ts
...
`renderPortalChildren` replaces the provided portal element’s `children` via `cloneElement(..., { children })`. If a caller passes a `<…Portal>` element that already has children, those children will be silently discarded. Consider either (a) making this explicit (e.g. dev-only warning when the passed portal has children) or (b) tightening the public `portal` prop typing to disallow `children` so consumers can’t accidentally provide content that will be ignored.

#### ciampo on 2026-04-17T16:12:40Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101696578
Path: packages/ui/src/utils/render-portal-children.ts
...
Addressed in the latest push: overlay `Popup` types now use `portal?: ReactElement<Omit<PortalProps, 'children'>>`, so consumers cannot pass portal elements with `children` that would be dropped. `renderPortalChildren` JSDoc notes that the injected subtree replaces any portal children.

### Review thread 8 (resolved)

#### copilot-pull-request-reviewer on 2026-04-17T15:35:12Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101477443
...
`portal?: ReactElement<PortalProps>` likely permits consumers to pass a portal element with its own `children`, but `Dialog.Popup` will always override portal children when injecting the popup/backdrop. Consider typing the `portal` prop as a ReactElement whose props omit `children` (and applying the same change across AlertDialog/Popover/Tooltip/Select) so TS prevents passing children that will be dropped at runtime.
```suggestion
	portal?: ReactElement< Omit< PortalProps, 'children' > >;
```

#### ciampo on 2026-04-17T16:12:42Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3101696720
Path: packages/ui/src/dialog/types.ts
...
Applied across Dialog, AlertDialog, Popover, Tooltip, and Select: `portal` is `ReactElement<Omit<PortalProps, 'children'>>`, matching the suggested pattern. JSDoc on each `portal` prop explains that popup content must not be passed as portal children.

### Review thread 9 (resolved)

#### mirka on 2026-04-20T17:22:42Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3112469438
Path: packages/ui/src/utils/render-portal-children.ts
...
Was this intended as a runtime check against `@ts-expect-error`-style misuse? Because the types already ensure that `rootPortal` is a valid element. I think I'd be comfortable with removing this conditional, given it's only used internally in `@wordpress/ui` (pretty safe against misuse).

#### ciampo on 2026-04-21T08:37:19Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3116123243
Path: packages/ui/src/utils/render-portal-children.ts
...
#### mirka on 2026-04-20T17:25:34Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3112485762
Path: packages/ui/src/utils/render-portal-children.ts
...
maybe more accurate? (nit)

#### ciampo on 2026-04-21T08:16:59Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3116011504
Path: packages/ui/src/utils/render-portal-children.ts
...
Renamed function and filename 👌

### Review thread 11 (resolved)

#### mirka on 2026-04-20T17:27:41Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3112500769
Path: packages/ui/src/tooltip/portal.tsx
...
#### ciampo on 2026-04-21T08:19:18Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3116023686
Path: packages/ui/src/tooltip/portal.tsx
...
Applied the import renaming to all Tootlip imports

### Review thread 12 (resolved)

#### mirka on 2026-04-20T17:45:21Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3112591364
Path: packages/ui/src/popover/stories/index.story.tsx

The "cross iframe" stories still mention the `container` prop.

#### ciampo on 2026-04-21T08:36:39Z

URL: https://github.com/WordPress/gutenberg/pull/77452#discussion_r3116119472
Path: packages/ui/src/popover/stories/index.story.tsx

Good catch — updated. The Cross-Iframe story docs now describe passing the parent mount target via `portal={ <Popover.Portal container={ ... } /> }` on `Popover.Popup` (instead of saying `container` on `Popover.Popup`).

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +4/-0)
- packages/ui/src/alert-dialog/index.ts (MODIFIED, +1/-0)
- packages/ui/src/alert-dialog/popup.tsx (MODIFIED, +8/-4)
- packages/ui/src/alert-dialog/portal.tsx (ADDED, +17/-0)
- packages/ui/src/alert-dialog/test/index.test.tsx (MODIFIED, +7/-3)
- packages/ui/src/alert-dialog/types.ts (MODIFIED, +13/-3)
- packages/ui/src/dialog/index.ts (MODIFIED, +12/-1)
- packages/ui/src/dialog/popup.tsx (MODIFIED, +11/-4)
- packages/ui/src/dialog/portal.tsx (ADDED, +18/-0)
- packages/ui/src/dialog/test/index.test.tsx (MODIFIED, +7/-3)
- packages/ui/src/dialog/types.ts (MODIFIED, +11/-3)
- packages/ui/src/form/primitives/select/index.ts (MODIFIED, +1/-0)
- packages/ui/src/form/primitives/select/popup.tsx (MODIFIED, +36/-34)
- packages/ui/src/form/primitives/select/portal.tsx (ADDED, +16/-0)
- packages/ui/src/form/primitives/select/test/index.test.tsx (MODIFIED, +7/-3)
- packages/ui/src/form/primitives/select/types.ts (MODIFIED, +9/-2)
- packages/ui/src/popover/index.ts (MODIFIED, +2/-1)
- packages/ui/src/popover/popup.tsx (MODIFIED, +11/-6)
- packages/ui/src/popover/portal.tsx (ADDED, +17/-0)
- packages/ui/src/popover/root.tsx (MODIFIED, +2/-2)
- packages/ui/src/popover/stories/index.story.tsx (MODIFIED, +32/-16)
- packages/ui/src/popover/test/index.test.tsx (MODIFIED, +7/-3)
- packages/ui/src/popover/types.ts (MODIFIED, +10/-5)
- packages/ui/src/tooltip/index.ts (MODIFIED, +2/-1)
- packages/ui/src/tooltip/popup.tsx (MODIFIED, +29/-27)
