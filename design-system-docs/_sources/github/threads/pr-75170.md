...
type: pull_request
state: MERGED
merged_at: 2026-03-19T10:41:09Z
url: https://github.com/WordPress/gutenberg/pull/75170
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #75170: Tabs: add runtime validation for tab/panel mismatches

- URL: https://github.com/WordPress/gutenberg/pull/75170
- Author: ciampo
- Created: 2026-02-03T09:53:31Z
- Updated: 2026-04-01T13:57:57Z
- Comments: 12 of 12
- Review threads: 3 of 3
- Changed files: 6 of 6
- Diff: ../pr-diffs/75170.diff

## Body

## What?

Adds development-mode runtime validation to the `Tabs` component in `@wordpress/ui` to help catch mismatches between the number of tabs and tabpanels.

## Why?

Given that the `Tabs` component implements the [Tabs pattern](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/), in order for it to be compliant and accessible, each `Tab` sub-component needs to have a corresponding `Panel`. Currently, misconfigurations fail silently or produce confusing behavior. 

## How?

- Created a validation context (`context.tsx`) that tracks the number of registered `Tab` and `Panel` components and throws an error when there is a mismatch between the two numbers
- Validation only runs in development mode (`process.env.NODE_ENV !== 'production'`). Separate dev and prod implementations to guardantee React Compiler compatibility and better tree-shakeability (as confirmed by the [bundle size report](https://github.com/WordPress/gutenberg/pull/75170#issuecomment-3840500258))

## Testing Instructions

- Check unit tests
- Make changes to the Storybook examples to introduce tab/tabpanel mismatches , and make sure that warnings/errors are triggered as expected in dev mode
- Enable production mode in Storybook (eg `NODE_ENV=production npm run storybook:dev`) and make sure that the same errors/warnings do not appear

## AI tools used

- Cursor, Claude Opus 4.6 (dev)
- GitHub Copilot (PR review)

## Issue comments

### github-actions on 2026-02-03T10:23:10Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-3840438516
...
### github-actions on 2026-02-03T10:34:44Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-3840500258
...
| `build/styles/edit-post/style-rtl.css` | 4.09 kB |
...
### ciampo on 2026-02-10T14:30:31Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-3877995821

@WordPress/gutenberg-components would you be able to take a look at this one?

### mirka on 2026-03-12T17:48:39Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4048753820

My initial motivation for adding this kind of runtime check was simply to stop consumers from using this as a faux tabs component. Basically, to prevent consumers from using it for tablist-like styling, and not passing tab panels at all. This is a much narrower scope than wanting to catch other kinds of misconfiguration errors, like value-level matching and duplicate detection.

In that light, I notice a lot of added complexity due to adding more rigorous validation than necessary to prevent the original problem that motivated a runtime check. To prevent the original problem, I feel like all we need is a counter for tabs and panels, and check that the number matches.

Do you think it's worth the complexity to add this level of validation? Has that been a problem, in your experience?

### ciampo on 2026-03-16T21:46:40Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4070788195

Over the years, we've definitely witnessed many misuses of `Tabs`, I guess because of two reasons:

- It was one of the first compound components, therefore more complex/fragmented/easier to only use partially
- it implements a functionality often needed (some sort of lightweight memory router), which folks often used even when there wasn't a need for 

As we add more compound components, given that we're trying to systematically prevent misuse (especially with extra lint rules), I think that the changes in this PR go in the same direction; in that sense, I think it's a good approach, _especially_ since most of the code is a no-op in production mode.

For example, we already implemented [a similar pattern](https://github.com/WordPress/gutenberg/blob/7b345ea9af8a295f3b1c1a7e35c45501b57dc585/packages/ui/src/dialog/context.tsx) to ensure that `Dialog` consumers render a `Dialog.Title`.


So, overall, despite the code definitely bringing complexity, I can also see the benefits, especially if we can manage to refactor the code to a shared utility (to limit the growing complexity) and apply it consistently. What do you think?

### mirka on 2026-03-17T15:12:05Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4075742367

I think there's a difference between misuse that is often unnoticeable from the developer (like accessibility issues), and misuse that they will notice as bugs.

Is the duplicate or mismatched panels case a noticeable problem or an unnoticeable problem? Like, does `Tabs` actually function with duplicate or mismatched panels? If it simply doesn't work, the developer will notice soon enough, without us having elaborate logic to catch them early.

I'm just playing around in the Storybook on trunk, but duplicate tab values crashes already, and mismatched tabs don't work. So if that's truly the case, it seems like we'd be adding complexity for slight developer convenience, not for quality assurance.

So my question is "are there actual abuse vectors that we can prevent by catching duplicate or mismatched tabs"? And by "abuse" I mean things that seem to work but are actually accessibility issues. If yes, I can agree it's worth the trouble.

### ciampo on 2026-03-17T15:39:28Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4075938058

I guess the main patterns with `Tabs` that we want to prevent are:

1. not rendering sub-components inside `Tabs.Root`
2. only rendering a `TabList` + individual `Tabs`, but then rendering custom UI instead of `TabPanel`s, basically using `Tabs` as a memory router

Base UI will throw an error for the missing `Tabs.Root`, but pattern no. 2 won't be flagged.

Do you think that scenario is enough of a reason?

### mirka on 2026-03-17T17:05:27Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4076569329

> 2\. only rendering a `TabList` + individual `Tabs`, but then rendering custom UI instead of `TabPanel`s, basically using `Tabs` as a memory router
>
> Do you think that scenario is enough of a reason?

Yes, this is the exact case I wanted to prevent in the first place, which I think can be caught by a [simpler strategy](https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4048753820) ("a counter for tabs and panels, and check that the number matches"). Wanting to catch duplicate/mismatched tab values on top of that is adding a good amount of complexity that doesn't seem warranted.

### github-actions on 2026-03-17T19:10:37Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4077382570

<!-- flaky-tests-report-comment -->
**Flaky tests detected in c77ecc2f7cba735ef9589172e26b86458a63192f.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23242388604
📝  Reported issues:
...
### ciampo on 2026-03-18T11:23:40Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4081697545

@mirka I updated (vastly simplified) the code to apply the check that you suggested. It's a good idea to start simple and add complexity only if needed.

I pushed changes, PR should be ready for a final round of review

### ciampo on 2026-03-19T09:35:22Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4088887549

@mirka I switched it back to throwing an error, and updated PR description and error message.

We can always tweak throwing or warning later, in case there are legitimate reasons to relax things around.

### ciampo on 2026-03-19T10:40:48Z

URL: https://github.com/WordPress/gutenberg/pull/75170#issuecomment-4089217738
...
#### copilot-pull-request-reviewer on 2026-02-04T09:26:09Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2763026356
Path: packages/ui/src/tabs/context.tsx
...
Tabs/panels are tracked in `Set`s, which can produce incorrect validation results when multiple `Tabs.Tab` or `Tabs.Panel` instances share the same `value`. Because `registerTab`/`registerPanel` returns an unregister that calls `Set.delete(value)`, unmounting one of the duplicates will remove the value even if another duplicate is still mounted, potentially triggering false mismatch warnings (or hiding real mismatches). Consider tracking counts per value (e.g., `Map<TabValue, number>` or a multiset) and only removing the key when the count reaches 0; this also enables a clearer warning for duplicate values if desired.

#### ciampo on 2026-02-04T14:15:47Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2764235444
Path: packages/ui/src/tabs/context.tsx
...
Switch from set to map in the latest commit

### Review thread 2 (resolved)

#### mirka on 2026-03-18T19:35:04Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2955776334
Path: packages/ui/src/tabs/test/index.test.tsx

```diff
@@ -2256,5 +2256,134 @@ describe( 'Tabs', () => {
 			);
 		} );
 	} );
+
...
It might be better to frame the "why" as an accessibility issue in the PR description and the warning message. That part is not clear right now, and people may ignore the warning if they don't understand the significance of it.

#### ciampo on 2026-03-19T09:31:54Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2958839597
Path: packages/ui/src/tabs/test/index.test.tsx
...
#### mirka on 2026-03-18T19:36:58Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2955784898
Path: packages/ui/src/tabs/context.tsx

Could maybe be simplified further in the future by extracting a reusable factory to set up the dev/prod scaffolding!

#### ciampo on 2026-03-19T09:29:20Z

URL: https://github.com/WordPress/gutenberg/pull/75170#discussion_r2958826908
Path: packages/ui/src/tabs/context.tsx
...
- packages/ui/src/tabs/context.tsx (ADDED, +170/-0)
- packages/ui/src/tabs/panel.tsx (MODIFIED, +3/-0)
- packages/ui/src/tabs/root.tsx (MODIFIED, +6/-1)
- packages/ui/src/tabs/tab.tsx (MODIFIED, +3/-0)
- packages/ui/src/tabs/test/index.test.tsx (MODIFIED, +162/-0)
