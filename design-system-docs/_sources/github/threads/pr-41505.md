...
type: pull_request
state: MERGED
merged_at: 2022-06-08T16:28:19Z
url: https://github.com/WordPress/gutenberg/pull/41505
...
# PR #41505: Components: refactor`Dropdown` to pass `exhaustive-deps`

- URL: https://github.com/WordPress/gutenberg/pull/41505
- Author: chad1008
- Created: 2022-06-02T12:29:36Z
- Updated: 2022-06-08T17:02:30Z
- Comments: 1 of 1
- Review threads: 1 of 1
- Changed files: 2 of 2
- Diff: ../pr-diffs/41505.diff

## Body

## What?
Updates the `Dropdown` component to satisfy the `exhaustive-deps` eslint rule

## Why?
Part of the effort in https://github.com/WordPress/gutenberg/pull/41166 to apply `exhuastive-deps` to the Components package

## How?
Adds `onToggle` to the `useEffect` dependency array. We don't expect `onToggle` to change, so this should satisfy `exhaustive-deps` while being equivalent to an empty dep array.

## Testing Instructions
1. From your local Gutenberg directory, run `npx eslint --rule 'react-hooks/exhaustive-deps: warn' packages/components/src/dropdown`
...
### github-actions on 2022-06-02T12:37:14Z

URL: https://github.com/WordPress/gutenberg/pull/41505#issuecomment-1144813960
...
#### mirka on 2022-06-02T15:22:07Z

URL: https://github.com/WordPress/gutenberg/pull/41505#discussion_r888077070
...
This effect seems weird to me, in that it will call `onToggle( false )` on cleanup even when no toggle has happened 🤔 So I checked the history, and the [pre-refactor logic](https://github.com/WordPress/gutenberg/pull/23142/files#diff-dacdfb5a25d27771d335dc7307b037ce12d6b122b40b24d1800bd9bd0cd045f7L34-L36) includes an `isOpen` condition, which makes sense. The "cleanup" is that _if the dropdown is open at unmount time_, it should be toggled closed. I have a feeling this is a refactoring error.

But since we haven't gotten any complaints in two years, I have to assume nobody is using the `onToggle` for non-idempotent things 😅

#### ciampo on 2022-06-02T15:32:14Z

URL: https://github.com/WordPress/gutenberg/pull/41505#discussion_r888087573
Path: packages/components/src/dropdown/index.js
...
Agreed. It feels like it should check `if (isOpen)` before calling `onToggle( false )`.

Although we may want to make that change in a separate PR (if we make it at all 😅 )

#### chad1008 on 2022-06-08T10:47:33Z

URL: https://github.com/WordPress/gutenberg/pull/41505#discussion_r892207989
Path: packages/components/src/dropdown/index.js
...
You make a good point. I can plan a followup to re-add that check! Thank you both!

#### chad1008 on 2022-06-08T17:02:29Z

URL: https://github.com/WordPress/gutenberg/pull/41505#discussion_r892643311
Path: packages/components/src/dropdown/index.js
...
- packages/components/src/dropdown/index.js (MODIFIED, +1/-1)
