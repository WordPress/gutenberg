...
type: pull_request
state: MERGED
merged_at: 2026-05-14T10:38:06Z
url: https://github.com/WordPress/gutenberg/pull/78095
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Enhancement","[Package] UI"]
---

# PR #78095: [ui] Tooltip: Default portal container to the wp compat overlay slot

- URL: https://github.com/WordPress/gutenberg/pull/78095
- Author: ciampo
- Created: 2026-05-08T11:00:34Z
- Updated: 2026-05-14T10:38:50Z
- Comments: 4 of 4
- Review threads: 7 of 7
- Changed files: 19 of 19
- Diff: ../pr-diffs/78095.diff

## Body

## What?

Wires `Tooltip.Portal`'s `container` to default to the wp compat overlay slot from #77851. End-to-end validator for the slot infrastructure: with this in place, `@wordpress/ui` Tooltips stack reliably above `@wordpress/components` overlays in mixed-library compositions, with no per-instance plumbing.

A caller-supplied `Tooltip.Portal` `container` continues to take precedence. No behavior change for `@wordpress/ui`-only consumers.
...
```sh
npx jest --config=test/unit/jest.config.js --testPathPattern='packages/ui/src/tooltip/test/index'
```

Expected: 8 passing (5 pre-existing + 3 new under `wp compat overlay slot`).
...
Open **Playground / WP Compat Overlay Slot / Tooltip inside @wordpress/components Modal**. Click **Open Modal**, hover **Hover me**: the `@wordpress/ui` Tooltip popup renders above the Modal, portaled into `[data-wp-compat-overlay-slot]`. Same for the Popover story.

In any other (un-decorated) story — e.g. **Design System / Components / Tooltip / Default** — confirm the dormant baseline: the popup uses the default portal container and `[data-wp-compat-overlay-slot]` is absent from the DOM.

### Editor (auto-detect, real consumer)

The slot auto-detects `window.wp.components`, which is on the global in the WordPress editor. To exercise the wiring end-to-end against a real editor consumer, swap an existing `@wordpress/components` `Tooltip` usage for a `@wordpress/ui` one.

<details>
<summary>Example patch — Inspector Controls tabs</summary>
...
@@ -175,14 +176,15 @@ export default function InspectorControlsTabs( {
 								{ tab.title }
 							</Tabs.Tab>
 						) : (
...
+									render={ <Tabs.Tab tabId={ tab.name } /> }
 								>
 									<Icon icon={ tab.icon } />
-								</Tabs.Tab>
-							</Tooltip>
...
Then `npm run dev`, open a post in the editor, select a block (for example, the Image block), and hover the icon-only tabs at the top of the Inspector Controls sidebar (Settings / Styles / List View). The tab tooltips are now `@wordpress/ui` Tooltips; in DevTools, confirm they portal into `[data-wp-compat-overlay-slot]` on `document.body`.

<details>
<summary>Files touched</summary>

- `packages/ui/src/tooltip/portal.tsx` — the wiring itself.
- `packages/ui/src/tooltip/test/index.test.tsx` — 3 new integration tests under a `wp compat overlay slot` describe block. Opt-in tests render a wrapper component that calls `useEnableWpCompatOverlaySlot()`, mirroring real-consumer usage rather than poking at the internal flag.
- `packages/ui/README.md` — documents `useEnableWpCompatOverlaySlot()` under a new "Mixing with `@wordpress/components`" subsection.
- `storybook/stories/playground/wp-compat-overlay-slot.story.jsx` — cross-library demo.
- `storybook/stories/playground/with-wp-compat-overlay-slot.tsx` — Storybook decorator that flips the gate during the playground story's lifetime. Kept private to the playground (not shared across per-component stories) because the opt-in flag is process-wide on `window`, and applying the decorator to per-component stories would leak the gate into sibling stories rendered on the same Storybook autodocs page.

</details>

## Next Steps

Wire the rest of `@wordpress/ui`'s overlays the same way in subsequent PRs. `.components-draggable__clone` migrates onto the slot in #78183 so an active drag stays above it.
...
### github-actions on 2026-05-08T11:57:33Z

URL: https://github.com/WordPress/gutenberg/pull/78095#issuecomment-4406218245

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 0b5ab9d49ca9e1567581198571a99f6f0062227a.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25807760468
📝  Reported issues:
- #48880 in `/test/e2e/specs/editor/various/undo.spec.js`

### github-actions on 2026-05-08T15:25:02Z

URL: https://github.com/WordPress/gutenberg/pull/78095#issuecomment-4407608061
...
| :--- | :---: | :---: |
| `build/modules/content-types/index.min.js` | 155 kB | +534 B (+0.35%) |
...
### github-actions on 2026-05-13T15:13:25Z

URL: https://github.com/WordPress/gutenberg/pull/78095#issuecomment-4442483540
...
### ciampo on 2026-05-14T09:41:27Z

URL: https://github.com/WordPress/gutenberg/pull/78095#issuecomment-4449542313
...
#### mirka on 2026-05-13T17:45:54Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236341695
...
+ * (when the runtime opts in via WordPress detection or the explicit window
+ * flag), so leaf Tooltips reliably stack above `@wordpress/components`
+ * overlays in mixed-library compositions. A caller-supplied `container`
+ * always wins — `{ ...props }` is spread *after* the default.
  */
 const Portal = forwardRef< HTMLDivElement, PortalProps >(
 	function TooltipPortal( props, ref ) {
-		return <_Tooltip.Portal ref={ ref } { ...props } />;
+		const defaultContainer = getWpCompatOverlaySlot() ?? undefined;
+		return (
+			<_Tooltip.Portal
...
This creates a container regardless of whether there's a `container` prop. Maybe something like this?

```diff
diff --git a/packages/ui/src/tooltip/portal.tsx b/packages/ui/src/tooltip/portal.tsx
index 91952119c2f..55c937c4e84 100644
--- a/packages/ui/src/tooltip/portal.tsx
+++ b/packages/ui/src/tooltip/portal.tsx
@@ -15,12 +15,11 @@ import { getWpCompatOverlaySlot } from '../utils/wp-compat-overlay-slot';
  * always wins — `{ ...props }` is spread *after* the default.
  */
 const Portal = forwardRef< HTMLDivElement, PortalProps >(
-	function TooltipPortal( props, ref ) {
...
-				container={ defaultContainer }
-				{ ...props }
+				container={ container ?? getWpCompatOverlaySlot() ?? undefined }
+				{ ...restProps }
 				ref={ ref }
 			/>
 		);
...
Also makes me wonder… Would it be easier downstream if the `getWpCompatOverlaySlot()` function itself returned `undefined` instead of `null`?

### Review thread 2 (resolved)

#### mirka on 2026-05-13T17:47:31Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236350067
...
These aria-labels seem meaningless to use here, and would set a bad example.
...
#### mirka on 2026-05-13T17:48:27Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236355036
Path: storybook/stories/playground/wp-compat-overlay-slot.story.jsx
...
Can we just write the render functions inline to avoid unnecessary indirection?

### Review thread 4 (resolved)

#### mirka on 2026-05-13T17:52:24Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236376177
Path: storybook/stories/playground/wp-compat-overlay-slot.story.jsx
...
I don't think this needs to be stated, especially in the publicly visible description.

### Review thread 5 (resolved)

#### mirka on 2026-05-13T17:52:43Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236378036
Path: storybook/stories/playground/wp-compat-overlay-slot.story.jsx
...
#### mirka on 2026-05-13T18:01:57Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236432624
Path: storybook/stories/playground/wp-compat-overlay-slot.story.jsx
...
I don't think this needs to be stated in a story file.

I'm observing that these code comments are starting to look like an unfiltered AI brain dump, rather than calling out important information that needs to be left in the code itself. Given the amount of time it takes for a human to review the walls of text for accuracy, as well as how a human reader in the codebase would probably gloss over any code comment this long, I think we should reconsider the level of detail that needs to be documented in the codebase, versus what should be fine to be documented (at whatever length!) in the PR description/comments.

### Review thread 7 (resolved)

#### mirka on 2026-05-13T18:10:55Z

URL: https://github.com/WordPress/gutenberg/pull/78095#discussion_r3236479702
Path: packages/ui/src/tooltip/portal.tsx
...
Given that this pattern is going to be used in many other Portal components, I'm not sure this is the level of detail we want to get into in each of the component descriptions. Also the last line reads like an internal code comment, rather than a public-facing one.

Something brief like "Defaults the `container` prop to the `@wordpress/ui` compat overlay slot.", with a link to a central Storybook doc that explains the concept, might be more manageable. (And the central Storybook doc can be added later when our decisions are a bit more solidified.)

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +1/-3)
- packages/ui/README.md (MODIFIED, +15/-0)
- packages/ui/src/alert-dialog/portal.tsx (MODIFIED, +1/-4)
- packages/ui/src/dialog/portal.tsx (MODIFIED, +1/-5)
- packages/ui/src/drawer/portal.tsx (MODIFIED, +1/-5)
- packages/ui/src/form/primitives/autocomplete/portal.tsx (MODIFIED, +1/-3)
- packages/ui/src/form/primitives/autocomplete/positioner.tsx (MODIFIED, +1/-4)
- packages/ui/src/form/primitives/select/portal.tsx (MODIFIED, +1/-3)
- packages/ui/src/form/primitives/select/positioner.tsx (MODIFIED, +1/-4)
- packages/ui/src/popover/portal.tsx (MODIFIED, +1/-4)
- packages/ui/src/popover/positioner.tsx (MODIFIED, +1/-4)
- packages/ui/src/tooltip/portal.tsx (MODIFIED, +11/-5)
- packages/ui/src/tooltip/positioner.tsx (MODIFIED, +1/-4)
- packages/ui/src/tooltip/test/index.test.tsx (MODIFIED, +117/-0)
- packages/ui/src/utils/test/use-enable-wp-compat-overlay-slot.test.tsx (MODIFIED, +5/-5)
- packages/ui/src/utils/test/wp-compat-overlay-slot.test.ts (MODIFIED, +22/-22)
- packages/ui/src/utils/wp-compat-overlay-slot.ts (MODIFIED, +8/-7)
- storybook/stories/playground/with-wp-compat-overlay-slot.tsx (ADDED, +30/-0)
- storybook/stories/playground/wp-compat-overlay-slot.story.jsx (ADDED, +98/-0)
