...
type: pull_request
state: MERGED
merged_at: 2024-02-16T11:07:43Z
url: https://github.com/WordPress/gutenberg/pull/59085
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components","[Package] Block editor"]
---

# PR #59085: Components: Use `Element.scrollIntoView()` instead of `dom-scroll-into-view`

- URL: https://github.com/WordPress/gutenberg/pull/59085
- Author: tyxla
- Created: 2024-02-15T15:34:10Z
- Updated: 2024-02-16T11:08:06Z
- Comments: 2 of 2
- Review threads: 0 of 0
- Changed files: 10 of 10
- Diff: ../pr-diffs/59085.diff

## Body

## What?
This PR suggests that we utilize the native `Element.scrollIntoView()` instead of the external package `dom-scroll-into-view`.

## Why?
All things being equal, using native functionality is always preferred to using external packages and libraries.

But also, the use case for `dom-scroll-into-view` is quite straightforward and goes way back to the initial implementation of `FormTokenField` in https://github.com/WordPress/gutenberg/pull/924 by @youknowriad. 

From what I understand, there's no good reason to use the external package. `Element.scrollIntoView()` has a solid browser support already and works really well in testing. 

## How?
* Replacing the `dom-scroll-into-view` syntax with `Element.scrollIntoView()` calls in both occurrences:
  * `URLInput`
  * `FormTokenField`
* Removing the unnecessary state that maintains whether we're currently scrolling - I couldn't spot a good reason to keep it, and I don't see a difference in testing.
* Removing the `dom-scroll-into-view` dependency, along with typings and a related lerna patch.

## Testing Instructions
* Make sure you have at least 10 pages that start with the same name ("Page 1", "Page 2", ... "Page 10")
* Make sure you have at least 10 tags that start with the same name ("Test 1", "Test 2", ... "Test 10"), and each of those tags is assigned to at least one post.
* Go to the post editor.
* Insert some text in a paragraph, select it, click on the anchor to make it a link, input "Page" in the search field, and verify that as you use the arrows, the suggestions are scrolled so the selected result is always visible.
* Head to the sidebar, and reveal the Tags box.
* In the tag field, input "Test" in the search field, and verify that as you use the arrows, the suggestions are scrolled so the selected result is always visible.
...
`URLInput:`
https://github.com/WordPress/gutenberg/assets/8436925/f941fc4e-43d8-415c-adc6-4fb5dddcfa40

`FormTokenField:`
https://github.com/WordPress/gutenberg/assets/8436925/b88a39a8-b4eb-4525-88c7-5dd5129bd745

## Issue comments

### github-actions on 2024-02-15T15:47:51Z

URL: https://github.com/WordPress/gutenberg/pull/59085#issuecomment-1946375032
...
| :--- | :---: | :---: |
| `build/block-editor/index.min.js` | 249 kB | -2.15 kB (-1%) |
| `build/components/index.min.js` | 223 kB | -2.16 kB (-1%) |
...
### github-actions on 2024-02-15T16:01:28Z

URL: https://github.com/WordPress/gutenberg/pull/59085#issuecomment-1946401430
...
- package-lock.json (MODIFIED, +0/-14)
- packages/block-editor/package.json (MODIFIED, +0/-1)
- packages/block-editor/src/components/url-input/index.js (MODIFIED, +6/-15)
- packages/components/CHANGELOG.md (MODIFIED, +2/-0)
- packages/components/package.json (MODIFIED, +0/-1)
- packages/components/src/form-token-field/suggestions-list.tsx (MODIFIED, +5/-17)
- packages/components/tsconfig.json (MODIFIED, +0/-1)
- patches/dom-scroll-into-view+1.2.1.patch (DELETED, +0/-25)
- test/unit/config/global-mocks.js (MODIFIED, +8/-0)
