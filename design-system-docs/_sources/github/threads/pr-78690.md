...
type: pull_request
state: OPEN
url: https://github.com/WordPress/gutenberg/pull/78690
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Compose","[Package] Components","[Package] Editor","[Package] Block library","[Package] Rich text","[Package] Block editor","[Package] Edit Post"]
---

# PR #78690: Compose: Deprecate `useRefEffect` in favor of native `useCallback` ref cleanups

- URL: https://github.com/WordPress/gutenberg/pull/78690
- Author: tyxla
- Created: 2026-05-26T15:45:17Z
- Updated: 2026-06-04T13:45:56Z
- Comments: 5 of 5
- Review threads: 1 of 1
- Changed files: 63 of 63
- Diff: ../pr-diffs/78690.diff
...
### github-actions on 2026-05-26T15:54:14Z

URL: https://github.com/WordPress/gutenberg/pull/78690#issuecomment-4545868322
...
| :--- | :---: | :---: |
| `build/scripts/block-editor/index.min.js` | 379 kB | +37 B (+0.01%) |
| `build/scripts/block-library/index.min.js` | 324 kB | -19 B (-0.01%) |
| `build/scripts/components/index.min.js` | 263 kB | -6 B (0%) |
| `build/scripts/compose/index.min.js` | 11.8 kB | +112 B (+0.96%) |
| `build/scripts/edit-post/index.min.js` | 52 kB | +7 B (+0.01%) |
| `build/scripts/editor/index.min.js` | 462 kB | -2 B (0%) |
| `build/scripts/rich-text/index.min.js` | 14.2 kB | +4 B (+0.03%) |
| `build/scripts/widgets/index.min.js` | 7.83 kB | -5 B (-0.06%) |
...
### github-actions on 2026-05-27T17:10:44Z

URL: https://github.com/WordPress/gutenberg/pull/78690#issuecomment-4556817864

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 349f1e49ea26ae3f2f5d5a728aa87d77804cfa66.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/26810722720
📝  Reported issues:
- #78706 in `/test/e2e/specs/editor/various/autosave.spec.js`

### tyxla on 2026-06-02T09:31:00Z

URL: https://github.com/WordPress/gutenberg/pull/78690#issuecomment-4601035368

I'm opening this for review.

### github-actions on 2026-06-02T09:31:16Z

URL: https://github.com/WordPress/gutenberg/pull/78690#issuecomment-4601037420
...
### tyxla on 2026-06-04T13:45:56Z

URL: https://github.com/WordPress/gutenberg/pull/78690#issuecomment-4622725924

Switching to draft for now due to the revert in #78940. Will get back to it once React 19 gets merged again with reasonable React 18 compat.

## Review threads

### Review thread 1

#### jsnajdr on 2026-05-27T13:16:56Z

URL: https://github.com/WordPress/gutenberg/pull/78690#discussion_r3310992888
...
If we really need these `! element` guards everywhere, we're probably doing something wrong, and maybe `useRefEffect` continues to have value.

`useRefEffect` guarantees that the callback is never called with a `null` node. It always has the new cleanup semantics, even if you don't return a cleanup function.

In a raw ref callback, the return value matters much more. Either the callback is in the old mode, receiving value or `null`, or in the cleanup mode, receiving only a value.

#### tyxla on 2026-05-27T13:39:04Z

URL: https://github.com/WordPress/gutenberg/pull/78690#discussion_r3311138821
Path: docs/reference-guides/block-api/block-api-versions/block-migration-for-iframe-editor-compatibility.md
...
Yes, unfortunately, we do need the guards. And it's the part that makes me doubt if `useRefEffect` should be deprecated in the end. I'm honestly a bit torn about it, because that change compounds when it's necessary in so many places, and it's yet another detail to consider. 

@Mamaduka what do you think?

#### Mamaduka on 2026-05-27T14:25:12Z

URL: https://github.com/WordPress/gutenberg/pull/78690#discussion_r3311454985
Path: docs/reference-guides/block-api/block-api-versions/block-migration-for-iframe-editor-compatibility.md
...
I'll have a better look tomorrow, but I don't see anything wrong with early bailout checks; it's a common pattern and often paired with cleanups in effects.

My preference would be using native APIs, even if it requires extra lines. I'm not sure if we'll ever get a native merge-refs API (see [fragment refs](https://react.dev/reference/react/Fragment#fragmentinstance)), but if we do, it will work with ref callbacks, not custom ones.

#### jsnajdr on 2026-05-27T16:23:57Z

URL: https://github.com/WordPress/gutenberg/pull/78690#discussion_r3312246277
Path: docs/reference-guides/block-api/block-api-versions/block-migration-for-iframe-editor-compatibility.md
...
> Yes, unfortunately, we do need the guards.

Do we really? If the ref callback returns a cleanup function, React (at least 19) guarantees to never pass `null` as argument, always the node.

#### tyxla on 2026-06-02T09:19:52Z

URL: https://github.com/WordPress/gutenberg/pull/78690#discussion_r3340068933
Path: docs/reference-guides/block-api/block-api-versions/block-migration-for-iframe-editor-compatibility.md
...
I think so - the reason I kept it is backwards compatibility. A block's `Edit` written against may still run on older WordPress versions that bundle React 18, where a returned cleanup from a ref callback is silently ignored and the callback is still invoked with `null` on detach. Without the guard, `const { ownerDocument } = element` (or `jQuery( element )`) will throw on unmount there. Since we're talking about React 18 compat in a few other places, I think this is a reasonable step.

## Changed files

- docs/reference-guides/block-api/block-api-versions/block-migration-for-iframe-editor-compatibility.md (MODIFIED, +18/-9)
- packages/block-editor/src/components/block-list/block.js (MODIFIED, +14/-7)
- packages/block-editor/src/components/block-list/use-block-props/index.js (MODIFIED, +8/-7)
- packages/block-editor/src/components/block-list/use-block-props/use-block-refs.js (MODIFIED, +10/-3)
- packages/block-editor/src/components/block-list/use-block-props/use-firefox-draggable-compatibility.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/block-list/use-block-props/use-focus-handler.js (MODIFIED, +6/-5)
- packages/block-editor/src/components/block-list/use-block-props/use-intersection-observer.js (MODIFIED, +5/-3)
- packages/block-editor/src/components/block-list/use-block-props/use-is-hovered.js (MODIFIED, +4/-6)
- packages/block-editor/src/components/block-list/use-block-props/use-scroll-into-view.js (MODIFIED, +6/-2)
- packages/block-editor/src/components/block-list/use-block-props/use-selected-block-event-handlers.js (MODIFIED, +3/-3)
- packages/block-editor/src/components/block-list/use-in-between-inserter.js (MODIFIED, +3/-4)
- packages/block-editor/src/components/block-popover/use-popover-scroll.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/block-preview/auto.js (MODIFIED, +6/-3)
- packages/block-editor/src/components/block-selection-clearer/index.js (MODIFIED, +3/-3)
- packages/block-editor/src/components/block-tools/use-block-toolbar-popover-props.js (MODIFIED, +4/-2)
- packages/block-editor/src/components/iframe/index.js (MODIFIED, +19/-5)
- packages/block-editor/src/components/list-view/use-clipboard-handler.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/observe-typing/index.js (MODIFIED, +8/-4)
- packages/block-editor/src/components/rich-text/event-listeners/index.js (MODIFIED, +8/-4)
- packages/block-editor/src/components/typewriter/index.js (MODIFIED, +3/-3)
- packages/block-editor/src/components/use-flash-editable-blocks/index.js (MODIFIED, +3/-3)
- packages/block-editor/src/components/writing-flow/index.js (MODIFIED, +6/-3)
- packages/block-editor/src/components/writing-flow/use-arrow-nav.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-click-selection.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-clipboard-handler.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-drag-selection.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-input.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-multi-selection.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-preview-mode-nav.js (MODIFIED, +3/-3)
- packages/block-editor/src/components/writing-flow/use-select-all.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-selection-observer.js (MODIFIED, +5/-2)
- packages/block-editor/src/components/writing-flow/use-tab-nav.js (MODIFIED, +6/-3)
- packages/block-library/src/button/edit.js (MODIFIED, +5/-2)
- packages/block-library/src/list-item/hooks/use-enter.js (MODIFIED, +6/-6)
- packages/block-library/src/list-item/hooks/use-space.js (MODIFIED, +6/-5)
- packages/block-library/src/paragraph/use-enter.js (MODIFIED, +6/-6)
- packages/block-library/src/utils/waveform-player.js (MODIFIED, +5/-6)
- packages/components/src/autocomplete/autocompleter-ui.tsx (MODIFIED, +5/-4)
- packages/components/src/autocomplete/index.tsx (MODIFIED, +5/-2)
- packages/components/src/checkbox-control/index.tsx (MODIFIED, +4/-4)
- packages/components/src/form-token-field/suggestions-list.tsx (MODIFIED, +6/-3)
- packages/components/src/higher-order/navigate-regions/index.tsx (MODIFIED, +7/-8)
- packages/compose/CHANGELOG.md (MODIFIED, +4/-0)
- packages/compose/README.md (MODIFIED, +6/-0)
- packages/compose/src/hooks/use-constrained-tabbing/index.js (MODIFIED, +12/-12)
- packages/compose/src/hooks/use-copy-to-clipboard/index.ts (MODIFIED, +5/-7)
- packages/compose/src/hooks/use-disabled/index.ts (MODIFIED, +8/-4)
- packages/compose/src/hooks/use-drop-zone/index.js (MODIFIED, +7/-3)
- packages/compose/src/hooks/use-focus-on-mount/index.ts (MODIFIED, +3/-4)
- packages/compose/src/hooks/use-focusable-iframe/index.ts (MODIFIED, +6/-3)
- packages/compose/src/hooks/use-ref-effect/index.ts (MODIFIED, +14/-3)
- packages/compose/src/hooks/use-ref-effect/test/index.tsx (ADDED, +34/-0)
- packages/e2e-tests/plugins/iframed-block/editor.js (MODIFIED, +6/-4)
- packages/e2e-tests/plugins/iframed-masonry-block/editor.js (MODIFIED, +7/-5)
- packages/edit-post/src/components/layout/index.js (MODIFIED, +4/-2)
- packages/editor/src/components/post-revisions-preview/diff-markers.js (MODIFIED, +5/-2)
- packages/editor/src/components/visual-editor/use-edit-content-only-section-exit.js (MODIFIED, +5/-2)
- packages/editor/src/components/visual-editor/use-padding-appender.js (MODIFIED, +5/-2)
- packages/editor/src/components/visual-editor/use-select-nearest-editable-block.js (MODIFIED, +3/-3)
- packages/editor/src/components/visual-editor/use-zoom-out-mode-exit.js (MODIFIED, +5/-2)
- packages/rich-text/src/hook/event-listeners/index.js (MODIFIED, +10/-3)
- packages/rich-text/src/hook/index.js (MODIFIED, +17/-6)
- packages/widgets/src/blocks/legacy-widget/edit/preview.js (MODIFIED, +5/-3)
