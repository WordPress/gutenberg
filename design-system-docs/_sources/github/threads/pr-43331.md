...
type: pull_request
state: MERGED
merged_at: 2022-08-26T11:00:17Z
url: https://github.com/WordPress/gutenberg/pull/43331
matched_labels: ["[Package] Components"]
all_labels: ["[Package] Components","[Package] Block editor","Needs User Documentation"]
---

# PR #43331: Upsize typography tools

- URL: https://github.com/WordPress/gutenberg/pull/43331
- Author: mirka
- Created: 2022-08-17T15:23:50Z
- Updated: 2022-09-20T21:55:17Z
- Comments: 1 of 1
- Review threads: 1 of 1
- Changed files: 12 of 12
- Diff: ../pr-diffs/43331.diff
...
### github-actions on 2022-08-17T15:35:17Z

URL: https://github.com/WordPress/gutenberg/pull/43331#issuecomment-1218172754
...
| :--- | :---: | :---: |
| `build/block-editor/index.min.js` | 159 kB | +38 B (0%) |
| `build/block-editor/style-rtl.css` | 15 kB | -42 B (0%) |
| `build/block-editor/style.css` | 15 kB | -37 B (0%) |
| `build/components/index.min.js` | 197 kB | +18 B (0%) |
...
#### mirka on 2022-08-17T15:28:48Z

URL: https://github.com/WordPress/gutenberg/pull/43331#discussion_r948105997
...
This is a tiny fix we need on the ToggleGroupControl label that causes a 1px height discrepancy with the other components, due to `block`/`inline` differences 😭 We might need to rethink the `BaseControl.VisualLabel` styles if this happens a lot.

https://user-images.githubusercontent.com/555336/185179405-b096ec83-95cf-4053-814a-5a3431376ef4.mp4

#### noisysocks on 2022-08-18T01:53:36Z

URL: https://github.com/WordPress/gutenberg/pull/43331#discussion_r948580907
Path: packages/components/src/toggle-group-control/toggle-group-control/styles.ts
...
Does it work if you remove `VisualLabelWrapper` altogether and put `VisualLabel` and `RadioGroup` into a `HStack`? I think that's what I did for `FontSizePicker`.

#### mirka on 2022-08-23T16:51:07Z

URL: https://github.com/WordPress/gutenberg/pull/43331#discussion_r952883001
Path: packages/components/src/toggle-group-control/toggle-group-control/styles.ts
...
In terms of the label height, yes, anything that makes it a flex child will do. In this case the main RadioGroup defaults to an inline-flex for some reason, so we'll have to look into that more before we can use a cleaner solution like the `HStack` you mention. (Ideally we just use the default BaseControl features for the label rendering, not this custom thing.)

## Changed files

- packages/block-editor/src/hooks/font-appearance.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/font-family.js (MODIFIED, +2/-0)
- packages/block-editor/src/hooks/font-size.js (MODIFIED, +2/-0)
- packages/block-editor/src/hooks/letter-spacing.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/line-height.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/text-decoration.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/text-transform.js (MODIFIED, +1/-0)
- packages/block-editor/src/hooks/typography.scss (MODIFIED, +0/-6)
- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/toggle-group-control/test/__snapshots__/index.tsx.snap (MODIFIED, +62/-44)
- packages/components/src/toggle-group-control/toggle-group-control/component.tsx (MODIFIED, +3/-2)
- packages/components/src/toggle-group-control/toggle-group-control/styles.ts (MODIFIED, +5/-0)
