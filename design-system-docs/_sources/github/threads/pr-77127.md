...
type: pull_request
state: MERGED
merged_at: 2026-04-09T16:15:28Z
url: https://github.com/WordPress/gutenberg/pull/77127
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Bug","[Package] Components"]
---

# PR #77127: RadioControl: add support for disabling radio group

- URL: https://github.com/WordPress/gutenberg/pull/77127
- Author: oandregal
- Created: 2026-04-08T08:12:10Z
- Updated: 2026-04-09T18:01:09Z
- Comments: 9 of 9
- Review threads: 3 of 3
- Changed files: 5 of 5
- Diff: ../pr-diffs/77127.diff
...
While the radiocontrol could be disabled, the styles didn't reflect that state. This was made evident at https://github.com/WordPress/gutenberg/pull/77090 where DataForm controls gained support for being disabled.

The control with disabled state:

Before | After
--- | ---
<img width="207" height="129" alt="Screenshot 2026-04-08 at 10 09 04" src="https://github.com/user-attachments/assets/f6fa77e5-c302-4971-ad88-9e2a8fb60a60" /> | <img width="207" height="129" alt="Screenshot 2026-04-08 at 10 09 21" src="https://github.com/user-attachments/assets/7c79db17-febb-4517-9813-9ff8c4c5dd39" /> | 

The control with disabled state in the DataForm story:

Before | After
--- | ---
<img width="1313" height="1076" alt="Screenshot 2026-04-08 at 10 08 11" src="https://github.com/user-attachments/assets/7b785d43-0dd2-479f-9d12-6cae86ea8d7d" /> | <img width="1313" height="1076" alt="Screenshot 2026-04-08 at 10 08 40" src="https://github.com/user-attachments/assets/90f54d6b-1cae-4efb-925a-d91d8c24fd98" />

                                                                                                                                                                      
## How?                                                      

  - Added a `disabled?: boolean` prop to RadioControlProps in types.ts.
  - Extracted disabled from props in the component and applied it to the <fieldset> element (which natively disables all child inputs). Also added an is-disabled CSS class for styling hooks.
  - Added disabled styles in style.scss matching the CheckboxControl pattern: light grey background ($gray-100), lighter border ($gray-300), and opacity: 1 on the input. Cursor changes from pointer to default on both the input and label when disabled.                                                                                                       
  - Added an explicit disabled boolean control in the Storybook argTypes so it appears in the controls panel for all stories.
...
The control story:
  ...
  2. Navigate to Components > Selection & Input > Common > RadioControl > Default.
  3. In the Controls panel, toggle the disabled control to true.                                                                                                                   
  4. Verify the radio inputs become visually disabled (grey background, lighter border, no pointer cursor).                                                                        
  5. Verify the fieldset label ("POST VISIBILITY") remains at full opacity and is unaffected.                                                                                      
  6. Verify clicking the radio inputs does not change the selection.                                                                                                               
  7. Switch to the With Option Descriptions story and repeat steps 3–6.                                                                                                            
  8. Toggle disabled back to false and verify normal styling and interaction is restored.                                                                                          
...
The DataForm story:
  ...
  3. In the Controls panel, toggle the disabled control to true.                                                                                                                   
  4. Verify the radio inputs become visually disabled (grey background, lighter border, no pointer cursor).                                                                        
  5. Verify the fieldset label remains at full opacity and is unaffected.                                                                                      
  6. Verify clicking the radio inputs does not change the selection.                                                                                                               
  7. Toggle disabled back to false and verify normal styling and interaction is restored.                                                                                          
...
### github-actions on 2026-04-08T08:12:21Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4204777280
...
### github-actions on 2026-04-08T08:24:22Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4204857783
...
| `build/styles/components/style.css` | 17.2 kB | +49 B (+0.29%) |
...
### github-actions on 2026-04-08T09:02:36Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4205077668

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 9965730c1adccd4e54b0e908516e0e07b559b2b1.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24198615112
📝  Reported issues:
...
### mirka on 2026-04-08T10:37:20Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4205630869

Unfortunately I don't think we want to update to these specific disabled styles at this point, because they don't match the new disabled styles we've defined in the design system. (See for example the new [`InputControl`](https://wordpress.github.io/gutenberg/?path=/docs/design-system-components-form-inputcontrol--docs).)

My rough plan here was to first update the `@wordpress/components` form component styles so they match the new design system styles, which ideally share styling code as much as possible with the `@wordpress/ui` equivalents. Otherwise we just add extremely temporary code right now that needs to be cleaned up immediately, with more effort.

Let's coordinate a bit here, since I'm noticing you're updating disabled styling for a bunch of components. Some may make sense to do right now, others may be better handled as part of the broader systematic updates. Do you have a list perhaps?

### oandregal on 2026-04-08T12:00:49Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4206071042

> Let's coordinate a bit here, since I'm noticing you're updating disabled styling for a bunch of components. Some may make sense to do right now, others may be better handled as part of the broader systematic updates. Do you have a list perhaps?

Yeah, I do have [a list](https://github.com/WordPress/gutenberg/issues/76045). That's every fix we need in DataForm and it is already implemented (it just took half the morning):

- https://github.com/WordPress/gutenberg/pull/77129
- https://github.com/WordPress/gutenberg/pull/77127
- https://github.com/WordPress/gutenberg/pull/77132
- https://github.com/WordPress/gutenberg/pull/77137
- https://github.com/WordPress/gutenberg/pull/77138
...
### ciampo on 2026-04-08T15:04:44Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4207256216

Could we use the DS tokens directly in `@wordpress/components`? @mirka , Is that what you were referring to with

> update the @wordpress/components form component styles so they match the new design system styles, which ideally share styling code as much as possible with the @wordpress/ui equivalents

?

### mirka on 2026-04-09T09:33:16Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4213119524

> Could we use the DS tokens directly in `@wordpress/components`? @mirka , Is that what you were referring to with
> 
> > update the @wordpress/components form component styles so they match the new design system styles, which ideally share styling code as much as possible with the @wordpress/ui equivalents

It would _involve_ using design tokens in `@wordpress/components`, but I haven't fully evaluated how and how much we should keep things in sync. For example, there's also a lot of stuff that lives in `@wordpress/base-styles` too. I'm leaning towards _not_ having design tokens in that package, as it generally runs counter to our current approach. But also, there may be a need for shared mixins across packages. So not sure.

I wouldn't be against doing quick temporary updates to get `disabled` styling available on the legacy components, but I think the visuals should at least move _towards_ our new design system styling, not decidedly away from it.

### oandregal on 2026-04-09T15:15:20Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4215348139

Thanks for pushing all those improvements, Lena!

### mirka on 2026-04-09T18:01:09Z

URL: https://github.com/WordPress/gutenberg/pull/77127#issuecomment-4216377402

For posterity, I ended up with this shade of gray for the selected state after doing #77132:

<img width="186" height="125" alt="Disabled radio group with darker checked state" src="https://github.com/user-attachments/assets/612b3ea6-0a5c-4077-a29e-e0efa72d7e1b" />

## Review threads

### Review thread 1 (resolved)

#### oandregal on 2026-04-08T08:15:29Z

URL: https://github.com/WordPress/gutenberg/pull/77127#discussion_r3050023514
...
I've noticed these git conflict marks and removed them at https://github.com/WordPress/gutenberg/pull/77127/changes/b9b22f163d9a9ee612fc30002e4e854b3bdfc879

I am not sure if the changelog needs to be tweaked?

#### oandregal on 2026-04-08T08:17:06Z

URL: https://github.com/WordPress/gutenberg/pull/77127#discussion_r3050033346
Path: packages/components/CHANGELOG.md
...
#### copilot-pull-request-reviewer on 2026-04-08T08:17:02Z

URL: https://github.com/WordPress/gutenberg/pull/77127#discussion_r3050032894
...
#### copilot-pull-request-reviewer on 2026-04-08T08:17:03Z

URL: https://github.com/WordPress/gutenberg/pull/77127#discussion_r3050032938
Path: packages/components/src/radio-control/index.tsx

```diff
@@ -88,7 +89,10 @@ export function RadioControl(
 		<fieldset
 			id={ id }
 			role="radiogroup"
...
- packages/components/src/radio-control/test/index.tsx (MODIFIED, +10/-0)
