...
type: pull_request
state: MERGED
merged_at: 2026-04-22T15:08:54Z
url: https://github.com/WordPress/gutenberg/pull/77520
matched_labels: ["[Package] UI"]
all_labels: ["[Type] Bug","[Package] UI"]
---

# PR #77520: UI: Update `@base-ui/react` from `1.4.0` to `1.4.1`

- URL: https://github.com/WordPress/gutenberg/pull/77520
- Author: ciampo
- Created: 2026-04-21T12:16:01Z
- Updated: 2026-04-22T15:10:34Z
- Comments: 3 of 3
- Review threads: 0 of 0
- Changed files: 8 of 8
- Diff: ../pr-diffs/77520.diff

## Body

Follow up to #77308, #77381.
Closes #77395.

## What?

Updates the `@base-ui/react` dependency of `@wordpress/ui` from [`1.4.0`](https://github.com/mui/base-ui/releases/tag/v1.4.0) to [`1.4.1`](https://github.com/mui/base-ui/releases/tag/v1.4.1), and takes advantage of two of that release's fixes:

1. **[Fix `display: contents` tabbability (`mui/base-ui#4642`)](https://github.com/mui/base-ui/pull/4642)** — the root cause of the focus-trap regression that shipped with the Base UI `1.4.0` bump in `@wordpress/ui@0.11.0`. With `1.4.1` in place, the temporary CSS workaround introduced in #77381 is no longer needed and is reverted here.
2. **[Mark `date-fns` peer dependencies as optional (`mui/base-ui#4639`)](https://github.com/mui/base-ui/pull/4639)** — the cause of the transitive peer dependency on `date-fns` / `@date-fns/tz` reported in #77395. Consumers of `@wordpress/ui` no longer have to install those packages to satisfy Base UI's peer dependency contract.

## Why?

-   Keeps `@wordpress/ui` on the latest Base UI patch release.
-   Lets us drop a nontrivial CSS hack (selector bypassing CSS layers + `.popup` / `.default` class split in `Popover`) in favor of the proper upstream fix.
-   Closes #77395 without forcing `@wordpress/ui` to bundle a specific `date-fns` version, which was undesirable (see [discussion](https://github.com/WordPress/gutenberg/issues/77395#issuecomment-4263303840)).

## How?

-   Bump `@base-ui/react` to `^1.4.1` in `packages/ui/package.json` and refresh `package-lock.json`.
-   Revert the workaround introduced in #77381:
    -   Remove the unlayered `[data-wpds-theme-provider-id]:has(> .popup) { display: block; }` rule from `dialog/style.module.css`, `popover/style.module.css`, `tooltip/style.module.css`, and `utils/css/item-popup.module.css`.
    -   Collapse the `Popover` popup's `.popup` (structural marker) / `.default` (visuals) split back into a single `.popup` class, and restore `popover/popup.tsx` to `className={ clsx( variant !== 'unstyled' && styles.popup ) }`.
-   Update `packages/ui/CHANGELOG.md` with a single `Internal` entry for the bump. Because #77381's workaround was Unreleased and is reverted here, the net user-facing delta between `0.11.0` and the next release is just the `1.4.0` → `1.4.1` bump, matching the `Internal`-category convention used for prior Base UI bumps (`0.9.0`, `0.11.0`).

Verified that `date-fns` / `@date-fns/tz` now appear under `peerDependenciesMeta` with `optional: true` in the refreshed lockfile:
...
Note: the original `display: contents` focus-trap regression is tricky to reproduce in JSDOM because it depends on browser visibility/focusability behavior, so manual Storybook smoke-testing is the primary signal.

1. Open Storybook (`npm run storybook:dev`).
2. For each of `Dialog`, `AlertDialog`, `Popover`, `Tooltip`, and `Select` (and any combobox-style `Popup` using `item-popup.module.css`):
    - Open an overlay.
    - Press `Tab` and confirm focus cycles through all focusable elements inside the popup.
    - Press `Shift+Tab` and confirm focus cycles in reverse.
    - Confirm visuals are unchanged from before (no extra wrapper behavior from the removed `display: block` override).
3. In a consumer project that imports `@wordpress/ui`, confirm `npm install` no longer reports missing `date-fns` / `@date-fns/tz` peer dependencies.

### Testing Instructions for Keyboard

Covered above — the main verification is keyboard focus cycling within overlays via `Tab` / `Shift+Tab`.

## Automated checks run locally

-   `npm run lint:js -- packages/ui/src/popover/popup.tsx`
-   `npm run lint:css -- packages/ui/src/dialog/style.module.css packages/ui/src/popover/style.module.css packages/ui/src/tooltip/style.module.css packages/ui/src/utils/css/item-popup.module.css`
-   `npm run test:unit -- packages/ui` (282 tests passing)

## Use of AI Tools

Authored with assistance from Cursor (Claude Opus).

## Issue comments

### github-actions on 2026-04-21T12:34:31Z

URL: https://github.com/WordPress/gutenberg/pull/77520#issuecomment-4288555765
...
| :--- | :---: | :---: |
| `build/modules/boot/index.min.js` | 17 kB | -27 B (-0.16%) |
| `build/scripts/block-editor/index.min.js` | 341 kB | -102 B (-0.03%) |
| `build/scripts/edit-site/index.min.js` | 263 kB | -74 B (-0.03%) |
| `build/scripts/editor/index.min.js` | 420 kB | -111 B (-0.03%) |
| `build/scripts/media-utils/index.min.js` | 79.2 kB | -22 B (-0.03%) |
...
### github-actions on 2026-04-21T12:36:39Z

URL: https://github.com/WordPress/gutenberg/pull/77520#issuecomment-4288568115

The following accounts have interacted with this PR and/or linked issues. I will continue to update these lists as activity occurs. You can also manually ask me to refresh this list by adding the `props-bot` label.

## Unlinked Accounts

The following contributors have not linked their GitHub and WordPress.org accounts: @Joseph-Mutua.
...
### github-actions on 2026-04-21T12:53:42Z

URL: https://github.com/WordPress/gutenberg/pull/77520#issuecomment-4288673754

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 326566ef9cef16db163387c65d8fafce9ccbb3fd.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/24783912118
📝  Reported issues:
...
- package-lock.json (MODIFIED, +14/-8)
- packages/ui/CHANGELOG.md (MODIFIED, +3/-1)
- packages/ui/package.json (MODIFIED, +1/-1)
- packages/ui/src/dialog/style.module.css (MODIFIED, +0/-12)
- packages/ui/src/popover/popup.tsx (MODIFIED, +1/-4)
- packages/ui/src/popover/style.module.css (MODIFIED, +1/-23)
- packages/ui/src/tooltip/style.module.css (MODIFIED, +0/-12)
- packages/ui/src/utils/css/item-popup.module.css (MODIFIED, +0/-12)
