...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/57459
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components"]
---

# PR #57459: Icon: Add `currentColor` prop

- URL: https://github.com/WordPress/gutenberg/pull/57459
- Author: mirka
- Created: 2023-12-29T16:15:31Z
- Updated: 2026-03-17T23:49:59Z
- Comments: 3 of 3
- Review threads: 2 of 2
- Changed files: 6 of 6
- Diff: ../pr-diffs/57459.diff
...
### github-actions on 2023-12-29T16:22:30Z

URL: https://github.com/WordPress/gutenberg/pull/57459#issuecomment-1872200341
...
| :--- | :---: | :---: |
| `build/components/index.min.js` | 235 kB | +96 B (0%) |
| `build/components/style-rtl.css` | 12.1 kB | +18 B (0%) |
| `build/components/style.css` | 12.1 kB | +15 B (0%) |
...
### DaniGuardiola on 2024-09-27T04:30:29Z

URL: https://github.com/WordPress/gutenberg/pull/57459#issuecomment-2378379220

How this isn't the default boggles my mind. I noticed recently as I encountered `fill` in a stylesheet for an icon, and manually setting the color too, which was... the same as `color`, set in the line right above 😆

I wonder if instead of it being a prop, we could just make it the default? Like, how many instances of icons are there with the current default color (that would break with this change)? Don't all users already set a fill color? If they don't, the `color` in the icon's context is likely good enough, probably just black too. Thoughts?

### mirka on 2024-09-27T09:37:43Z

URL: https://github.com/WordPress/gutenberg/pull/57459#issuecomment-2378869106

Yes that's partially why we put this on hold, so we can do a bit more investigation into whether we can just make it the default. Next steps would be to try it with lower CSS specificity, and then make a somewhat comprehensive list of scenarios where breakage could occur.

## Review threads

### Review thread 1

#### mirka on 2023-12-29T16:48:29Z

URL: https://github.com/WordPress/gutenberg/pull/57459#discussion_r1438333707
Path: packages/components/src/icon/with-currentcolor.tsx
...
This is a niche HOF designed to be an ergonomic way to feed `@wordpress/icons` arguments into `icon` props on components like RangeControl, where they pass that `icon` straight into an `<Icon>` but do not set any `currentColor` styles.

It should be rare to have to use this, because most components that pass an `icon` argument straight into an `<Icon>` already have `currentColor` styles set (e.g. Button).

### Review thread 2

#### mirka on 2023-12-29T17:06:42Z

URL: https://github.com/WordPress/gutenberg/pull/57459#discussion_r1438340135
Path: packages/components/src/icon/stories/index.story.tsx

```diff
@@ -32,42 +32,77 @@ Default.args = {
 	icon: wordpress,
 };
...
I improved the code snippets for the "with function" and "with component" stories to better demonstrate how the function/components need to be implemented in order to work as intended.

(Related to #56448, where part of the problem was that we weren't documenting how these size props need to be handled.)

## Changed files

- packages/components/src/icon/index.tsx (MODIFIED, +15/-3)
- packages/components/src/icon/stories/index.story.tsx (MODIFIED, +46/-11)
- packages/components/src/icon/style.scss (ADDED, +3/-0)
- packages/components/src/icon/with-currentcolor.tsx (ADDED, +30/-0)
