...
type: pull_request
state: MERGED
merged_at: 2026-03-17T14:06:45Z
url: https://github.com/WordPress/gutenberg/pull/76378
matched_labels: ["Design System","[Package] UI"]
all_labels: ["[Type] Enhancement","Design System","[Package] UI"]
---

# PR #76378: `CollapsibleCard`: add animations

- URL: https://github.com/WordPress/gutenberg/pull/76378
- Author: ciampo
- Created: 2026-03-10T21:50:31Z
- Updated: 2026-03-17T14:08:03Z
- Comments: 15 of 15
- Review threads: 1 of 1
- Changed files: 5 of 5
- Diff: ../pr-diffs/76378.diff
...
### github-actions on 2026-03-10T22:13:28Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4034800901
...
### github-actions on 2026-03-10T22:26:16Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4034866367

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 0de2140953544110099ff7df61be405712a7d7a8.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/23196476077
📝  Reported issues:
...
### github-actions on 2026-03-11T10:37:48Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038229944
...
### ciampo on 2026-03-11T10:41:12Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038250118
...
### jasmussen on 2026-03-11T10:58:56Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038382495

Nice work. I'd love @fcoveram input if he has capacity, he's been looking into adjacent behaviors for the command palette, seems like principles here could/should be the same.

The main thing that I'm personally stuck on is: the animation here is a a little bit of flair applied to the contents, which move and fade in. In fact to be true to the materials, if we imagined this was a physical card that was somehow folded (I know, physical cards don't behave like this), then the contents inside would not be animated at all: that would be printed on the paper itself, immutable. The thing that would animate is the _card itself_. So in that vein, this feels a bit upside down to me, I'd expect the text that gets revealed to always be visible, to not animate, to not fade, but for the card itself to disclose it with animation.

Can the card itself animate instead of the contents?

### ciampo on 2026-03-11T11:35:36Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038608200
...
### jasmussen on 2026-03-11T11:39:44Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038629656

> but since cards are often part of the document flow, animating the card itself would also animate the layout of the surrounding content

Valid and good point, though is there a way we can avoid that while still embracing the concept? Something like jump almost to the end of the animation and animate only the last 5% of the height? Perhaps even reserving the final height space already, so there's only a single jump in the content.

The alternative breaks principles as far as _what gets animated_, IMO. Not a strong opinion and will definitely defer to others, but I'd almost prefer no animation in this case.

### ciampo on 2026-03-11T11:52:12Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4038694786

> Something like jump almost to the end of the animation and animate only the last 5% of the height? Perhaps even reserving the final height space already, so there's only a single jump in the content.

This feels a bit difficult to implement / overengineered.

Another alternative solution to evaluate could be to:
- make the layout "jump" instantly, occupying with empty space the expanded card's height
- animate the card's height without causing animated layout reflows (since the initial jump already reserved the space)

It's still a non-trivial animation to implement. Let's hear other folks' thoughts before moving forward.

Happy also to implement a few variations (full height animations, or the most advanced behaviours discussed above) to get a better feeling for it

### jameskoster on 2026-03-11T14:38:00Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4039685453

I do agree that animating the root would be nice to try (like [`Collapsible`](https://base-ui.com/react/components/collapsible)/[`Accordion`](https://base-ui.com/react/components/accordion) in Base UI), but I don't think it needs to hold up this work which is a nice enhancement by itself. Seems fine to explore in a follow-up.

### fcoveram on 2026-03-12T16:17:00Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4048058671

> …if we imagined this was a physical card that was somehow folded (I know, physical cards don't behave like this), then the contents inside would not be animated at all: that would be printed on the paper itself, immutable. The thing that would animate is the *card itself*.
...
I’m not sure about this solution. There will be a small portion of time where the white space is visible.

In that vein, the base UI examples shared by Jay move the whole layout, and it doesn't feel distracting. I feel the outcome will be similar to the update in popover (#74082).

### ciampo on 2026-03-13T17:32:57Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4056794871

@fcoveram as shared before,  given how often `CollapsibleCard` is going to be rendered in content-heavy pages (eg settings pages with many cards stack up), I'm afraid it will lead to two main issues:

- cognitively, causing large portions of the page to all animate at once, which can feel overwhelming 
- potentially janky animations (every height change causes the browser to re-calculate the page layout and re-paint every frame, which can be quite expensive)

Which is why I discouraged animating the height like in the `Collapsible` base ui examples shared above.

Having said that, I went ahead and implemented it.

https://github.com/user-attachments/assets/af0e3d15-6337-48f0-bc9d-5a9e21e92adb



Happy to follow your guidance here on the next steps, including trying this animation and coming back to the drawing board if needed

### jameskoster on 2026-03-16T15:36:16Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4068574663
...
### fcoveram on 2026-03-16T15:52:59Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4068700574
...
The video shared above looks good to me. Nothing to add from the visual and interaction side.

### ciampo on 2026-03-16T16:04:19Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4068794718

Added a storybook example. Here's a video

https://github.com/user-attachments/assets/b0ca9a32-2503-45ea-a282-d042912151ff


It doesn't look too bad, but also, Storybook is not as complex as a real app, and my machine is quite fast.

Still, I believe we should ship this and keep an eye. Removing the animation wll be easy and non-breaking, anyway.

### jameskoster on 2026-03-17T12:42:53Z

URL: https://github.com/WordPress/gutenberg/pull/76378#issuecomment-4074684124
...
#### ciampo on 2026-03-13T18:21:25Z

URL: https://github.com/WordPress/gutenberg/pull/76378#discussion_r2933043674
...
The changes to DOM structure and styles actually now make adding `hiddenUntilFound` trivial  (cc @mirka  who suggested it in https://github.com/WordPress/gutenberg/pull/76280#pullrequestreview-3924130271 )

#### ciampo on 2026-03-13T18:23:27Z

URL: https://github.com/WordPress/gutenberg/pull/76378#discussion_r2933053668
Path: packages/ui/src/collapsible-card/style.module.css
...
https://github.com/WordPress/gutenberg/pull/76498

## Changed files

- packages/ui/CHANGELOG.md (MODIFIED, +1/-0)
- packages/ui/src/card/style.module.css (MODIFIED, +2/-1)
- packages/ui/src/collapsible-card/content.tsx (MODIFIED, +15/-3)
- packages/ui/src/collapsible-card/stories/index.story.tsx (MODIFIED, +51/-0)
- packages/ui/src/collapsible-card/style.module.css (MODIFIED, +28/-0)
