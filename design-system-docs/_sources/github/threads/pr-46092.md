...
type: pull_request
state: MERGED
merged_at: 2022-11-29T10:50:34Z
url: https://github.com/WordPress/gutenberg/pull/46092
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #46092: Tooltip: add readme and unit tests for `shortcut` prop

- URL: https://github.com/WordPress/gutenberg/pull/46092
- Author: t-hamano
- Created: 2022-11-27T14:32:45Z
- Updated: 2022-11-29T10:50:58Z
- Comments: 4 of 4
- Review threads: 3 of 3
- Changed files: 3 of 3
- Diff: ../pr-diffs/46092.diff
...
### codesandbox on 2022-11-27T14:32:46Z

URL: https://github.com/WordPress/gutenberg/pull/46092#issuecomment-1328259511

<img alt="CodeSandbox logo" src="https://raw.githubusercontent.com/codesandbox/open-in-codesandbox/main/assets/csb.light.svg#gh-dark-mode-only" /><img alt="CodeSandbox logo" src="https://raw.githubusercontent.com/codesandbox/open-in-codesandbox/main/assets/csb.svg#gh-light-mode-only" />&nbsp; Open in CodeSandbox <a href="https://codesandbox.io/p/github/WordPress/gutenberg/component/tooltip-shortcut?workspace=%7B%22gitSidebarPanel%22:%22PR%22,%22sidebarPanel%22:%22GIT%22%7D">Web Editor</a> | <a href="https://codesandbox.io/p/vscode?owner=WordPress&repo=gutenberg&branch=component/tooltip-shortcut">VS Code</a> | <a href="https://codesandbox.io/p/vscode?owner=WordPress&repo=gutenberg&branch=component/tooltip-shortcut&insiders=true">VS Code Insiders</a>

<!-- open-in-codesandbox:complete -->

### github-actions on 2022-11-27T14:48:05Z

URL: https://github.com/WordPress/gutenberg/pull/46092#issuecomment-1328262260
...
| :--- | :---: | :---: |
| `build/block-editor/index.min.js` | 179 kB | +7 B (0%) |
| `build/block-library/index.min.js` | 195 kB | -19 B (0%) |
| `build/blocks/index.min.js` | 50 kB | -8 B (0%) |
| `build/components/index.min.js` | 203 kB | +7 B (0%) |
| `build/core-data/index.min.js` | 15.6 kB | -1 B (0%) |
| `build/edit-post/index.min.js` | 34.5 kB | -2 B (0%) |
| `build/editor/index.min.js` | 44 kB | +23 B (0%) |
...
### t-hamano on 2022-11-27T15:03:08Z

URL: https://github.com/WordPress/gutenberg/pull/46092#issuecomment-1328265022

In the GitHub action, it appears that Unit Tests is failing, but this has occurred in other PRs and doesn't appear to be caused by this PR.

### t-hamano on 2022-11-27T16:22:30Z

URL: https://github.com/WordPress/gutenberg/pull/46092#issuecomment-1328283032

The unit test failure will be corrected by #46093.

## Review threads

### Review thread 1

#### ciampo on 2022-11-28T11:00:38Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033402107
...
We should try to avoid imperatively `focus`sing elements.

Is the over not enough to trigger the tooltip? If focus is necessary, then I'd suggest trying to use keyboard events (ie. pressing `tab`) or even clicking to focus the `button`.

cc @tyxla and @flootr who I believe recently worked on Tooltip's unit test and debated this aspect specifically

#### tyxla on 2022-11-28T11:19:36Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033419685
Path: packages/components/src/tooltip/test/index.js
...
What @ciampo said, essentially. The user can't just `focus()` - they can tab into the element, or click the element, but they can't just `.focus()`. Since we aim our tests to be as real representations of the user experience as possible, we should use actions that resemble user behavior when possible.

#### t-hamano on 2022-11-28T13:16:34Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033529460
Path: packages/components/src/tooltip/test/index.js
...
I used `focus` it because I don't need to wait for a delay, but I understood that it is important to reproduce user actions. As with many other test cases, I changed them to test that the tooltip doesn't appear immediately after hovering and that it appears after a `TOOLTIP_DELAY`.

### Review thread 2

#### tyxla on 2022-11-28T11:21:10Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033421074
Path: packages/components/src/tooltip/test/index.js
...
Noting that in my experience with tests, `.toBeVisible()` is always better than `.toBeInTheDocument()`, except when the element is actually not visible. So we should try to use the former when possible:

```suggestion
			expect( screen.getByText( 'shortcut text' ) ).toBeVisible();
```

#### t-hamano on 2022-11-28T13:18:08Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033531443
Path: packages/components/src/tooltip/test/index.js
...
In all tests, I replace `toBeInTheDocument` with `toBeVisible`.

### Review thread 3

#### tyxla on 2022-11-28T14:39:26Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1033626611
Path: packages/components/src/tooltip/test/index.js

```diff
@@ -276,5 +276,63 @@ describe( 'Tooltip', () => {
 			// Tooltip won't show, since the mouse has left the anchor
 			expect( screen.queryByText( 'Help text' ) ).not.toBeInTheDocument();
 		} );
+
...
I see this is part of prior art in this test file, however ideally this should be independent of the tooltip implementation. 
...
#### t-hamano on 2022-11-29T10:03:25Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1034541631
Path: packages/components/src/tooltip/test/index.js
...
Thanks for the advice! Your method seems to have worked.
However, it was converted to `findByXXX` according to eslint.

```
Prefer `findByText` query over using `waitFor` + `getByText`
```

#### tyxla on 2022-11-29T10:10:14Z

URL: https://github.com/WordPress/gutenberg/pull/46092#discussion_r1034551186
Path: packages/components/src/tooltip/test/index.js
...
- packages/components/src/tooltip/test/index.js (MODIFIED, +52/-10)
