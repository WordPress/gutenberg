...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/34128
matched_labels: ["[Package] Components"]
all_labels: ["[Package] Components"]
---

# PR #34128: Fix NumberControl validation

- URL: https://github.com/WordPress/gutenberg/pull/34128
- Author: stokesman
- Created: 2021-08-17T21:27:49Z
- Updated: 2022-03-07T16:56:12Z
- Comments: 5 of 5
- Review threads: 0 of 0
- Changed files: 7 of 7
- Diff: ../pr-diffs/34128.diff
...
### github-actions on 2021-08-17T21:33:53Z

URL: https://github.com/WordPress/gutenberg/pull/34128#issuecomment-900646442
...
| :--- | :---: | :---: |
| `build/components/index.min.js` | 218 kB | +202 B (0%) |
...
### ntsekouras on 2021-08-18T09:01:53Z

URL: https://github.com/WordPress/gutenberg/pull/34128#issuecomment-900946056

👋 - thanks for working on this! I haven't seen in depth all the changes in your PR so but I'd expect to see something like changing an `onChange` function to use some logic like `onPressEnter` which leads to something like `roundClamp( currentValue, min, max, step );` Have you explored something like that? It seems to involve way less changes (I think 😄 ), unless it doesn't solve the actual problem.

### stokesman on 2021-08-18T16:48:37Z

URL: https://github.com/WordPress/gutenberg/pull/34128#issuecomment-901269043
...
### stokesman on 2021-08-25T17:39:56Z

URL: https://github.com/WordPress/gutenberg/pull/34128#issuecomment-905740830

@ntsekouras, I got around to trying your suggestion. I might have known what the catch would be but had forgotten. Modification of the value on `change` events may interfere with input. A prime example is typing a multiple digit number whose first digit is a number less than `min` as it will be resolved to `min` as soon as the first digit is entered.

While that's probably not a very common scenario, it is not easy to deal with specifically. Even if it were, it would also involve skipping some calls to `onChange` which means it starts to look a lot like what is done in this PR.

I'm all ears in case I've overlooked something.

### stokesman on 2022-03-07T16:54:31Z

URL: https://github.com/WordPress/gutenberg/pull/34128#issuecomment-1060905133
...
- packages/components/src/input-control/index.tsx (MODIFIED, +3/-1)
- packages/components/src/input-control/input-field.tsx (MODIFIED, +35/-24)
- packages/components/src/input-control/reducer/reducer.ts (MODIFIED, +5/-2)
- packages/components/src/number-control/index.js (MODIFIED, +93/-52)
...
- packages/components/src/unit-control/index.tsx (MODIFIED, +21/-17)
