...
type: pull_request
state: MERGED
merged_at: 2022-07-12T00:51:23Z
url: https://github.com/WordPress/gutenberg/pull/42248
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Code Quality","[Package] Components"]
---

# PR #42248: Components: Add eslint to prevent SSR breakage

- URL: https://github.com/WordPress/gutenberg/pull/42248
- Author: mirka
- Created: 2022-07-07T19:30:02Z
- Updated: 2022-07-12T18:35:35Z
- Comments: 1 of 1
- Review threads: 1 of 1
- Changed files: 7 of 7
- Diff: ../pr-diffs/42248.diff

## Body

## What?

Adds [`eslint-plugin-ssr-friendly`](https://github.com/kopiro/eslint-plugin-ssr-friendly) to the Components package so we can do some basic checks for unsafe access of DOM globals.

## Why?

There have been some reports of breakage in SSR environments (#38400), and a sweep of the package codebase surfaced two more instances of unsafe `window` access.

## How?

The eslint plugin isn't perfect, but it will surface the more obvious breakage points, such as accessing DOM globals in module scope, or at the root level of a functional component (i.e. obviously not in a `useEffect`).
...
### github-actions on 2022-07-07T19:41:24Z

URL: https://github.com/WordPress/gutenberg/pull/42248#issuecomment-1178137404
...
| `build/edit-site/index.min.js` | 52.3 kB |
...
#### mirka on 2022-07-07T20:00:08Z

URL: https://github.com/WordPress/gutenberg/pull/42248#discussion_r916236507
...
Not that this way is incorrect, but it is harder to programmatically check for safety (plus the timeout functions become an `any` type in TS).

## Changed files

- .eslintrc.js (MODIFIED, +6/-0)
- package-lock.json (MODIFIED, +9/-0)
- package.json (MODIFIED, +1/-0)
- packages/components/CHANGELOG.md (MODIFIED, +1/-0)
- packages/components/src/focal-point-picker/grid.js (MODIFIED, +2/-5)
- packages/components/src/form-token-field/suggestions-list.tsx (MODIFIED, +2/-4)
- packages/components/src/resizable-box/resize-tooltip/utils.ts (MODIFIED, +2/-3)
