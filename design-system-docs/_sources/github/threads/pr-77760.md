...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/77760
matched_labels: ["[Package] Components"]
all_labels: ["[Package] Components","[Package] Edit Post","[Package] Edit Widgets","[Package] Edit Site","[Package] Base styles","[Package] Media Utils"]
---

# PR #77760: SnackbarList: Render inside the overlay legacy slot

- URL: https://github.com/WordPress/gutenberg/pull/77760
- Author: ciampo
- Created: 2026-04-28T16:48:14Z
- Updated: 2026-04-30T11:45:00Z
- Comments: 1 of 1
- Review threads: 0 of 0
- Changed files: 15 of 15
- Diff: ../pr-diffs/77760.diff
...
Move Gutenberg's `<SnackbarNotices>` calls into the overlay legacy slot via `createPortal(..., getOverlayLegacySlot())`, and consolidate the snackbar list's positioning styles onto `.components-snackbar-list` itself.

The previously shared `snackbar-container()` SCSS mixin in `@wordpress/base-styles/mixins` is removed; its rules are now baked into `.components-snackbar-list`. Shells' wrapper-class declarations that did nothing but include the mixin are deleted.

## Why?

Continuation of the legacy-slot migration. With `<SnackbarNotices>` portaled into the slot, snackbars share a stacking context with the rest of the legacy overlays. The list's `z-index` (100,000) is unchanged and now stacks relative to the slot.

Baking the positioning into `.components-snackbar-list` means the list self-positions whether portaled or rendered inline — useful for in-modal cases (`media-editor`, `media-utils`) that don't portal.

## How?

-   `packages/components/src/snackbar/style.scss`: `.components-snackbar-list` now has the full self-positioning rule (`position: fixed; bottom: 24px; left: 0; right: 0; padding-inline; display: flex; flex-direction: column; pointer-events: none;` plus the existing `z-index` and the `.components-snackbar { margin-inline: auto; }` rule).
-   `packages/base-styles/_mixins.scss`: remove the `snackbar-container()` mixin.
-   `packages/edit-post/src/components/layout/{index.js,style.scss}`, `packages/edit-site/src/components/layout/{index.js,style.scss}`, `packages/edit-widgets/src/components/notices/index.js`, `packages/boot/src/components/root/{index.tsx,single-page.tsx}`, `packages/boot/src/style.scss`: remove `@include snackbar-container()` (and the empty wrapper-class rules); wrap each `<SnackbarNotices>` in `createPortal(..., getOverlayLegacySlot())`.
-   `packages/edit-widgets/src/components/notices/style.scss`: deleted (only contained the empty wrapper rule).
-   `packages/media-editor/src/components/media-editor-modal/style.scss`, `packages/media-utils/src/components/media-upload-modal/style.scss`: drop `@include snackbar-container()` (positioning is now baked in) but keep elevated z-index / transform overrides for the in-modal case. These call sites are intentionally **not** migrated to the slot — their snackbars render in the modal context with elevated z-index.
-   `packages/components/CHANGELOG.md`: Internal entry plus a Breaking Changes entry for the mixin removal.

## Testing Instructions

1.  Open the post editor. Trigger a snackbar (e.g. publish a post, or use any flow that creates a snackbar notice). The snackbar list should be inside `<div class="wp-overlay-legacy">`.
2.  Repeat in the site editor and the widgets screen.
3.  In media-editor / media-upload modal flows: trigger a snackbar inside the modal. The snackbar should render inside the modal's content area (not portaled away) and appear above the modal overlay.
4.  Snackbar animations: verify enter / exit / layout-shift animations are unchanged (framer-motion).
5.  Run unit tests:

    ```sh
    npm run test:unit -- packages/components/src/snackbar
    ```

### Testing Instructions for Keyboard

Tab into a focusable snackbar action (e.g. an undo button when triggering an undoable action). Verify it is reachable, can be activated with `Enter`, and that the snackbar dismisses.
...
### github-actions on 2026-04-28T17:01:06Z

URL: https://github.com/WordPress/gutenberg/pull/77760#issuecomment-4337446871

**Size Change:** -921 B (-0.01%) 
...
| :--- | :---: | :---: | :---: |
| `build/modules/boot/index.min.js` | 18.5 kB | +10 B (+0.05%) |  |
| `build/scripts/edit-post/index.min.js` | 18.4 kB | +26 B (+0.14%) |  |
| `build/scripts/edit-site/index.min.js` | 264 kB | +26 B (+0.01%) |  |
| `build/scripts/edit-widgets/index.min.js` | 22 kB | +18 B (+0.08%) |  |
| `build/styles/components/style-rtl.css` | 17.4 kB | +27 B (+0.16%) |  |
| `build/styles/components/style-rtl.min.css` | 14.2 kB | +35 B (+0.25%) |  |
| `build/styles/components/style.css` | 17.4 kB | +28 B (+0.16%) |  |
| `build/styles/components/style.min.css` | 14.2 kB | +36 B (+0.25%) |  |
| `build/styles/edit-post/style-rtl.css` | 3.45 kB | -58 B (-1.65%) |  |
| `build/styles/edit-post/style-rtl.min.css` | 2.15 kB | -52 B (-2.36%) |  |
| `build/styles/edit-post/style.css` | 3.46 kB | -59 B (-1.68%) |  |
| `build/styles/edit-post/style.min.css` | 2.16 kB | -50 B (-2.26%) |  |
| `build/styles/edit-site/style-rtl.css` | 20.3 kB | -49 B (-0.24%) |  |
| `build/styles/edit-site/style-rtl.min.css` | 16.5 kB | -52 B (-0.31%) |  |
| `build/styles/edit-site/style.css` | 20.3 kB | -47 B (-0.23%) |  |
| `build/styles/edit-site/style.min.css` | 16.5 kB | -58 B (-0.35%) |  |
| `build/styles/edit-widgets/style-rtl.css` | 4.8 kB | -52 B (-1.07%) |  |
| `build/styles/edit-widgets/style-rtl.min.css` | 3.46 kB | -58 B (-1.65%) |  |
| `build/styles/edit-widgets/style.css` | 4.8 kB | -55 B (-1.13%) |  |
| `build/styles/edit-widgets/style.min.css` | 3.47 kB | -55 B (-1.56%) |  |
| `build/styles/editor/style-rtl.css` | 27.1 kB | -46 B (-0.17%) |  |
| `build/styles/editor/style-rtl.min.css` | 22.8 kB | -47 B (-0.21%) |  |
| `build/styles/editor/style.css` | 27.1 kB | -46 B (-0.17%) |  |
| `build/styles/editor/style.min.css` | 22.8 kB | -46 B (-0.2%) |  |
| `build/styles/media-utils/style-rtl.css` | 1.99 kB | -84 B (-4.04%) |  |
| `build/styles/media-utils/style-rtl.min.css` | 1.1 kB | -64 B (-5.48%) | ✅ |
| `build/styles/media-utils/style.css` | 1.99 kB | -83 B (-4%) |  |
| `build/styles/media-utils/style.min.css` | 1.1 kB | -66 B (-5.64%) | ✅ |
...
- packages/boot/src/components/root/index.tsx (MODIFIED, +9/-2)
- packages/boot/src/components/root/single-page.tsx (MODIFIED, +12/-2)
- packages/boot/src/style.scss (MODIFIED, +0/-3)
- packages/components/CHANGELOG.md (MODIFIED, +2/-0)
- packages/components/src/snackbar/style.scss (MODIFIED, +16/-1)
- packages/edit-post/src/components/layout/index.js (MODIFIED, +7/-2)
- packages/edit-post/src/components/layout/style.scss (MODIFIED, +0/-3)
- packages/edit-site/src/components/layout/index.js (MODIFIED, +9/-2)
- packages/edit-site/src/components/layout/style.scss (MODIFIED, +0/-3)
- packages/edit-widgets/src/components/notices/index.js (MODIFIED, +15/-1)
- packages/edit-widgets/src/components/notices/style.scss (DELETED, +0/-7)
- packages/edit-widgets/src/style.scss (MODIFIED, +0/-1)
- packages/media-editor/src/components/media-editor-modal/style.scss (MODIFIED, +4/-9)
- packages/media-utils/src/components/media-upload-modal/style.scss (MODIFIED, +2/-2)
