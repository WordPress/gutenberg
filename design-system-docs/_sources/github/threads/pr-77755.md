...
type: pull_request
state: CLOSED
url: https://github.com/WordPress/gutenberg/pull/77755
matched_labels: ["[Package] Components"]
all_labels: ["[Type] Enhancement","[Package] Components"]
---

# PR #77755: Components: Add overlay legacy slot infrastructure

- URL: https://github.com/WordPress/gutenberg/pull/77755
- Author: ciampo
- Created: 2026-04-28T16:08:53Z
- Updated: 2026-04-30T15:05:00Z
- Comments: 6 of 6
- Review threads: 4 of 4
- Changed files: 5 of 5
- Diff: ../pr-diffs/77755.diff

## Body

## What?

Adds a `getOverlayLegacySlot()` helper plus matching `.wp-overlay-legacy` styles to `@wordpress/components`. The helper lazily mounts a body-level container in the top-level document with `z-index: 99997` and `isolation: isolate`. No consumers are wired up yet; the slot exists but nothing portals to it.

## Why?

This is the foundation for a two-slot overlay architecture that will let `@wordpress/components` and `@wordpress/ui` overlays compose correctly when mixed:

-   **Overlay legacy slot** (`z-index: 99997`) — for `@wordpress/components` overlays (Modal, Popover, Tooltip, Snackbar, Draggable clone). Migrated in follow-ups.
-   **Overlay prime slot** (`z-index: 99998`) — for `@wordpress/ui` leaf overlays. Built in follow-ups.

Both slots sit below the WP admin bar (99,999). Within each slot, per-overlay z-indexes continue to control relative ordering, but those values now stack relative to the slot rather than the document body.
...
`packages/components/src/utils/overlay-legacy-slot.ts`:

-   `getOverlayLegacySlot()` returns the slot element, lazily creating `<div class=\"wp-overlay-legacy\">` on first call and caching the singleton.
-   Resolves the top-level document via `window.top?.document ?? window.document` so overlays portaling from inside an iframe land in the parent document's slot. Falls back to the current document when cross-origin restrictions block `window.top.document`.
-   If the cached element gets detached or the document changes, the helper recreates it.
-   If a `.wp-overlay-legacy` element already exists in the DOM (e.g., from a previous bundle), it is reused rather than duplicated.

`packages/components/src/utils/overlay-legacy-slot.scss`:

-   `.wp-overlay-legacy { position: fixed; top: 0; left: 0; z-index: 99997; isolation: isolate; }`
-   `position: fixed; top: 0; left: 0` (no width/height) makes the slot a zero-sized anchor at the viewport origin so it never intercepts pointer events.
-   `isolation: isolate` creates a stacking context regardless of the fixed positioning.
-   Declared outside any CSS layer for predictable plugin-override semantics.
...
-   #77756 — `Popover`: render the fallback container and Gutenberg's internal `Popover.Slot` declarations inside the legacy slot.
-   #77757 — `Modal`: portal modals into the slot; rewrite the aria-hide algorithm to handle the wrapper.
-   #77758 — `Tooltip`, `Menu`, `CustomSelectControl` v2: pass `portalElement={ getOverlayLegacySlot }` to each Ariakit-backed overlay.
-   #77760 — `SnackbarList`: portal Gutenberg shells' `<SnackbarNotices>` into the slot; bake the previously-mixin-only positioning into `.components-snackbar-list` and remove the `snackbar-container()` SCSS mixin.
-   #77761 — `Draggable`: append the drag clone to the slot instead of the document body.
...
```sh
npm run test:unit -- packages/components/src/utils/overlay-legacy-slot/test
```

The tests cover: lazy creation on first call, singleton caching across calls, reuse of a pre-existing `.wp-overlay-legacy` element in the DOM, and recreation when the cached element has been detached.

Once the follow-up PRs land, editor-level verification becomes meaningful.
...
### github-actions on 2026-04-28T16:17:15Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4337114264
...
### github-actions on 2026-04-28T16:25:25Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4337169107
...
| :--- | :---: | :---: |
| `build/styles/components/style-rtl.css` | 17.3 kB | +33 B (+0.19%) |
| `build/styles/components/style-rtl.min.css` | 14.2 kB | +30 B (+0.21%) |
| `build/styles/components/style.css` | 17.4 kB | +33 B (+0.19%) |
| `build/styles/components/style.min.css` | 14.2 kB | +31 B (+0.22%) |
...
| `build/modules/boot/index.min.js` | 18.5 kB |
...
### github-actions on 2026-04-28T16:42:10Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4337297186

<!-- flaky-tests-report-comment -->
**Flaky tests detected in 2f2483e7bb427cb4083fadb105611a2deaafcc5a.**
...
🔍  Workflow run URL: https://github.com/WordPress/gutenberg/actions/runs/25064244529
📝  Reported issues:
- #77720 in `/test/e2e/specs/editor/various/post-content-focus-mode.spec.js`
- #62862 in `/test/e2e/specs/admin/font-library.spec.js`

### ciampo on 2026-04-28T18:31:22Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4338063725

@mirka should we also enable this slot via a flag? Same flag that will enable the prime slot, or different one? If we don't document it (or we explicitly do as private), will we ever be able to remove it at a later point for the `@wordpress/components` package?

### ciampo on 2026-04-30T11:44:39Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4352166482

Closing since @mirka 's comments may have highlighted a blocker for the approach taken by this PR.

### ciampo on 2026-04-30T15:05:00Z

URL: https://github.com/WordPress/gutenberg/pull/77755#issuecomment-4353615912

Next attempt: https://github.com/WordPress/gutenberg/pull/77851

## Review threads

### Review thread 1

#### mirka on 2026-04-28T20:27:46Z

URL: https://github.com/WordPress/gutenberg/pull/77755#discussion_r3156981389
Path: packages/components/src/utils/overlay-legacy-slot/style.scss
...
#### mirka on 2026-04-28T20:28:08Z

URL: https://github.com/WordPress/gutenberg/pull/77755#discussion_r3156983344
Path: packages/components/src/utils/overlay-legacy-slot/style.scss
...
#### mirka on 2026-04-28T21:09:17Z

URL: https://github.com/WordPress/gutenberg/pull/77755#discussion_r3157213751
Path: packages/components/src/utils/overlay-legacy-slot/style.scss
...
https://github.com/WordPress/gutenberg/blob/ae954bf7bd988e8484b50f5895fc1567ac52fad5/packages/base-styles/_z-index.scss#L63-L71
...
#### ciampo on 2026-04-30T11:44:11Z

URL: https://github.com/WordPress/gutenberg/pull/77755#discussion_r3167646433
Path: packages/components/src/utils/overlay-legacy-slot/style.scss
...
#### mirka on 2026-04-28T21:09:48Z

URL: https://github.com/WordPress/gutenberg/pull/77755#discussion_r3157216845
Path: packages/components/src/utils/overlay-legacy-slot/index.ts
...
- packages/components/src/utils/overlay-legacy-slot/index.ts (ADDED, +67/-0)
- packages/components/src/utils/overlay-legacy-slot/style.scss (ADDED, +21/-0)
- packages/components/src/utils/overlay-legacy-slot/test/index.ts (ADDED, +57/-0)
