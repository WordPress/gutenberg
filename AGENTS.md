...
-   Block attributes and `theme.json` values are untrusted at render time: the declared type is not enforced for hand-edited or generated content. `render_block` / `render_block_data` filters and anything nested inside object-typed attributes such as `style` and `layout` receive raw values, so check with `is_string()` / `is_scalar()` before any string operation. An array reaching `preg_match()`, `explode()`, `trim()` or `esc_attr()` is a fatal `TypeError` on the front end. See [Coding Guidelines](docs/contributors/code/coding-guidelines.md#block-attributes-and-global-styles-values-are-untrusted-input).
