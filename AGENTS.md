...
-   Every code change in a package published to npm (not `"private": true`) must be accompanied by an entry in that package's `CHANGELOG.md` citing the pull request. Use the **Internal** heading for changes without consumer impact. Follow the formatting structure described in [Maintaining Changelogs](packages/README.md#maintaining-changelogs).
...
-   Treat block attributes and `theme.json` values as untrusted when their types haven't been validated. Raw values include `$block['attrs']` in render filters, unregistered attributes, and nested `style` or `layout` values. Guard them before strict string operations. Registered top-level attributes passed to render callbacks or block-support `apply` callbacks are already schema-validated.
