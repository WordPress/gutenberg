...
-   Code changes in a package that have an external impact to consumers should be accompanied by an entry in that package's `CHANGELOG.md`. Follow the formatting structure described in [Maintaining Changelogs](packages/README.md#maintaining-changelogs).
...
-   Block attributes and `theme.json` values are untrusted at render time. `WP_Block_Type::prepare_attributes_for_render()` validates top-level attributes against the registered block-type schema — including `style`, `layout`, `anchor` and `className`, which block supports add at `init` — so a malformed top-level value is dropped before a `render_callback` or a block-support `apply` callback. It does not reach values read from `$block['attrs']` in a `render_block` / `render_block_data` filter, unregistered attributes, or anything nested inside `style` and `layout`, which are registered as bare objects. Guard those with `is_string()` / `is_scalar()` before a strict string operation: on PHP 8, an array reaching `preg_match()`, `explode()`, `trim()` or `esc_url()` is a fatal `TypeError` on the front end.
