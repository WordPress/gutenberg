...
-   Block attributes and `theme.json` values are untrusted at render time; the declared type is not enforced for hand-edited or generated content. `WP_Block_Type::prepare_attributes_for_render()` only validates attributes that `block.json` declares, and no core block declares `style`, `layout`, `className` or `anchor`, so those always arrive raw — as does anything read from `$block['attrs']` in a `render_block` / `render_block_data` filter. Check with `is_string()` / `is_scalar()` before any string operation: on PHP 8, an array reaching `preg_match()`, `explode()`, `trim()` or `esc_url()` is a fatal `TypeError` on the front end.
