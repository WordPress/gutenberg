...
-   TypeScript configs are split per package: `tsconfig.build.json` (src only, emits `build-types`) and the default `tsconfig.json` (dev project: tests and stories, `noEmit`, jest types). Cross-package references always point at `../<pkg>/tsconfig.build.json`. `npm run build` never type checks dev files; use `npm run typecheck` for that, and never add jest types to a build project. See the TypeScript section in `packages/README.md`.
