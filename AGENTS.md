...
-   Never call `__`, `_x`, `_n` or `_nx` in a bundled package. Gettext calls are attributed to the script handle of the bundle they are compiled into, so a bundled package has no handle of its own for WordPress to load translations for and its strings stay in English. Put the strings in a companion message catalog package instead — `packages/ui-i18n` for `packages/ui`, `packages/dataviews-i18n` for `packages/dataviews` — and call the catalog entry (`i18n.CLEAR()`). See `packages/ui-i18n/README.md`. Only the gettext calls move: a catalog entry with placeholders returns the format string, and the bundled package keeps calling `sprintf` on it. `isRTL` reports locale direction rather than translating, so it also stays fine to call directly.
