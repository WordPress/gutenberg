...
describe( 'second review regressions', () => {
	test( 'leaves hunk payload lines that look like file headers alone', () => {
		const patch = [
			'diff --git a/node_modules/foo/i.js b/node_modules/foo/i.js',
			'--- a/node_modules/foo/i.js',
			'+++ b/node_modules/foo/i.js',
			'@@ -1,3 +1,3 @@',
			' context',
			'--- node_modules/foo/old-payload',
			'+++ node_modules/foo/new-payload',
			' tail',
		].join( '\n' );

		const result = retargetPatch( patch, 'foo', 'vendor/foo' ).split(
			'\n'
		);

		expect( result[ 5 ] ).toBe( '--- node_modules/foo/old-payload' );
		expect( result[ 6 ] ).toBe( '+++ node_modules/foo/new-payload' );
		expect( result[ 1 ] ).toBe( '--- a/vendor/foo/i.js' );
	} );

	test( 'still retargets the file after a hunk', () => {
		const patch = [
			'diff --git a/node_modules/foo/a.js b/node_modules/foo/a.js',
			'--- a/node_modules/foo/a.js',
			'+++ b/node_modules/foo/a.js',
			'@@ -1 +1 @@',
			'-one',
			'+two',
			'diff --git a/node_modules/foo/b.js b/node_modules/foo/b.js',
			'--- a/node_modules/foo/b.js',
			'+++ b/node_modules/foo/b.js',
		].join( '\n' );

		const result = retargetPatch( patch, 'foo', 'vendor/foo' );

		expect( result ).toContain( '--- a/vendor/foo/b.js' );
		expect( result ).toContain( '+++ b/vendor/foo/b.js' );
	} );

	test( 'treats a no-newline marker as outside the payload count', () => {
		const patch = [
			'diff --git a/node_modules/foo/a.js b/node_modules/foo/a.js',
			'--- a/node_modules/foo/a.js',
			'+++ b/node_modules/foo/a.js',
		...
		expect( retargetPatch( patch, 'foo', 'vendor/foo' ) ).toContain(
			'--- a/vendor/foo/b.js'
		);
	} );
	...
	test.each( [
		[ 'pkg', '1.2.3+4.5.6' ],
		[ '@scope/1.2.3', '4.5.6' ],
		[ '@scope/1.2.3-beta', '4.5.6' ],
		[ '@scope/name', '1.2.3' ],
	] )( 'round-trips the ambiguous name %s@%s', ( name, version ) => {
		expect( parsePatchFileName( patchFileName( name, version ) ) ).toEqual(
			{
				packageName: name,
				version,
			}
	...
	test( 'documents the update alias it accepts', () => {
		const lines = [];
		main( [ '--help' ], {
			rootDir: createRepository(),
			log: { ...silentLog, log: ( line ) => lines.push( line ) },
		} );

		expect( lines.join( '\n' ) ).toContain( 'patch update <package>' );
	} );

	test( 'accepts a package name after -- that starts with a dash', () => {
		const rootDir = createRepository();

		// Parsing must succeed; the package is simply not installed.
		expect(
			main( [ 'create', '--', '-foo' ], { rootDir, log: silentLog } )
		).toBe( 1 );
	} );
	...
	test( 'rejects an empty package name', () => {
		...
		expect( main( [ 'create', '' ], { rootDir, log: silentLog } ) ).toBe(
			1
		);
	} );
} );
