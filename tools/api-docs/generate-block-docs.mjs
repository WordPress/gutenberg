...
/**
 * Generate the token-delimited API reference content for a block (## level).
 *
 * @param {string} blockDir Directory name.
...
function writeBlockReadme( blockDir, apiSection ) {
	...
	if ( existing.includes( TOKEN_START ) ) {
		...
		fs.writeFileSync( readmePath, existing.replace( tokenPattern, apiSection ), {
			encoding: 'utf8',
		} );
		return;
	}
