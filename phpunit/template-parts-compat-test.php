...
class Template_Parts_Compat_Test extends WP_UnitTestCase {
	...
	public function test_get_block_templates_lists_customized_and_custom_parts() {
		...
		$sidebar_id = $this->create_customization( 'my-sidebar', '<!-- wp:paragraph --><p>Sidebar</p><!-- /wp:paragraph -->', 'My sidebar', 'footer' );
	 ...
	 */
	public function test_rest_create_makes_a_part_pattern_customization() {
		wp_set_current_user( self::$admin_id );
		...
		$this->assertSame( 'footer', $data['area'] );
		$this->assertSame( 'wp_block', get_post_type( $data['wp_id'] ) );
		$this->assertSame( $this->theme . '/part/promo', get_post_meta( $data['wp_id'], 'wp_pattern_slug', true ) );
	...
	public function test_template_part_post_saved_later_is_migrated() {
		...
		$copy = gutenberg_get_template_part_customization( $this->theme, 'late-part' );
		$this->assertInstanceOf( WP_Post::class, $copy );
		$this->assertSame( 'footer', get_post_meta( $copy->ID, 'wp_pattern_area', true ) );
		$this->assertStringContainsString( 'Late', get_block_template( $this->theme . '//late-part', 'wp_template_part' )->content );
	}
