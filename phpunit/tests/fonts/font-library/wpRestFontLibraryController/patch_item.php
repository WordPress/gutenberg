...
class Tests_Fonts_Font_Family_Controller_patch_item extends WP_REST_Font_Library_Controller_UnitTestCase {
	...
	/**
	 * Tests responses when sucessfully patching Font Families VIA POST CALL
	 *
	 * @dataProvider data_patch_font_family_via_post
	 *
	 * @param array $font_family     Font families to install in theme.json format.
	 * @param array $expected_response Expected response data.
	 */
	public function test_patch_font_family_success_via_post( $request, $update, $expected_response, $files=null ) {
		...
		// Patch the font family (via another call to post).
		$update_request  = new WP_REST_Request( 'POST', '/wp/v2/font-families' );
		if( array_key_exists( 'data', $update ) ){
			$update_request->set_param( 'data', $update['data'] );
		}
		if( $files ){
	...
	public function data_patch_font_family_via_post() {
		return array(
			'font family change name' => array(
				'request' => array(
					'data' => array(
