...
class Tests_Fonts_WPRESTFontLibraryController_RegisterRoutes extends WP_UnitTestCase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/wp/v2/wp_font_family/batch', $routes, 'Rest server has not the fonts path intialized.' );
		$this->assertCount( 2, $routes['/wp/v2/wp_font_family/batch'], 'Rest server has not the 2 fonts paths initialized.' );
		...
		$this->assertArrayHasKey( 'POST', $routes['/wp/v2/wp_font_family/batch'][0]['methods'], 'Rest server has not the POST method for fonts intialized.' );
		$this->assertArrayHasKey( 'DELETE', $routes['/wp/v2/wp_font_family/batch'][1]['methods'], 'Rest server has not the DELETE method for fonts intialized.' );
