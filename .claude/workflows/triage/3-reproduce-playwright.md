...
Get Playground URL from running instance and navigate in Playwright:

```
mcp__playwright__browser_navigate with url: <playground_url>
```
...
|--------------|-------------------|
| "Visit `/wp-admin/...`" | `browser_navigate` with url |
| "Enter `...` in the ... input" | `browser_type` with element, ref, text |
| "Click the Save button" | `browser_click` with element, ref |
| "Notice that ..." | Check for element presence in snapshot |

**Implementation flow:**
1. Use `browser_snapshot` to understand page structure (returns ref-based tree)
2. Identify target element by ref from snapshot
...
- **Console errors**:
  - Use `browser_console_messages` (filtered by level)
...
- **Network requests**:
  - Use `browser_network_requests` (focus on failed requests)
...
| INCONCLUSIVE | Could not complete steps, ambiguous results, or environment issues |
...
### Page Analysis
- `browser_snapshot` - Get accessibility tree with ref identifiers
- `browser_take_screenshot` - Capture visual evidence

### Interaction
- `browser_click` - Click element by ref (requires element description)
...
### Evidence Collection
- `browser_console_messages` - Get console logs (filtered by level)
- `browser_network_requests` - Get network activity

### Utilities
- `browser_wait_for` - Wait for text to appear/disappear
- `browser_handle_dialog` - Accept/dismiss popups
...
Use `browser_snapshot` to discover the actual structure - returns ref-based tree.
...
- Block controls appear on hover - use `browser_hover` first
...
| Unexpected dialog | Use `browser_handle_dialog` to dismiss |
